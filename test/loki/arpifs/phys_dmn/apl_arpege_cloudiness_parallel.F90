SUBROUTINE APL_ARPEGE_CLOUDINESS_PARALLEL (YDMF_PHYS_BASE_STATE, YDCPG_BNDS, YDCPG_OPTS, YDCPG_MISC, YDMF_PHYS, YDVARS,  &
& YDMODEL, LDREDPR, YD_PAIPCMT, YD_PBLH, YD_PDECRD, YD_PFLU_QSAT, YD_PMSC_QW, YD_PNEBC0, YD_PNEBCH, YD_PNEBS, YD_PNEBS0,  &
& YD_PNEB_CVPP, YD_PPFL_FPLCH, YD_PQI, YD_PQL, YD_PQLIS, YD_PQLIS0, YD_PQLI_CVP, YD_PQLI_CVPP, YD_PQV, YD_PUNEBH,  &
& YD_PCLCT_RAD, YDSTA)
...
#include "acnpart_openacc.intfb.h"
  ...
  CLASS(FIELD_3RB), POINTER :: YL_ZICEFR1 => NULL()
  REAL(KIND=JPRB), POINTER :: ZICEFR1(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZRHCRI => NULL()
  REAL(KIND=JPRB), POINTER :: ZRHCRI(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZQSATS => NULL()
  REAL(KIND=JPRB), POINTER :: ZQSATS(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZRH => NULL()
  REAL(KIND=JPRB), POINTER :: ZRH(:, :, :) => NULL()
  ...
  CALL FIELD_NEW(YL_ZICEFR1, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZQSATS, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZRH, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZRHCRI, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  ...
  IF (LPARALLELMETHOD('OPENMP', 'APL_ARPEGE_CLOUDINESS_PARALLEL:ACNEBCOND')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_CLOUDINESS_PARALLEL:ACNEBCOND:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
    ...
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_CLOUDINESS_PARALLEL:ACNEBCOND:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
...
!$OMP PARALLEL DO PRIVATE( JBLK, JLEV, JLON ) FIRSTPRIVATE( YLCPG_BNDS )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      ...
      CALL ACNEBCOND(YDMODEL%YRCST, YDMODEL%YRML_GCONF%YRRIP, YDMODEL%YRML_PHY_MF, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA,  &
      & YDCPG_OPTS%KLON, YDMODEL%YRML_PHY_MF%YRTOPH%NTPLUI, YDCPG_OPTS%KFLEVG, LDREDPR, YDSTA,  &
      & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF(:, :, JBLK),  &
      & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP(:, :, JBLK),  &
      & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP(:, :, JBLK),  &
      & PQV(:, :, JBLK), PQI(:, :, JBLK), PQL(:, :, JBLK), PMSC_QW(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_T(:, :, JBLK),  &
      & PNEBCH(:, :, JBLK), Z_YDVARS_GEOMETRY_GM_T0(:, JBLK), Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T(:, JBLK), PQLIS(:, :, JBLK),  &
      & PNEBS(:, :, JBLK), ZRHCRI(:, :, JBLK), ZRH(:, :, JBLK), ZQSATS(:, :, JBLK), ZICEFR1(:, :, JBLK), PQLIS0(:, :, JBLK),  &
      & PNEBS0(:, :, JBLK))
      ...
      IF (YDMODEL%YRML_PHY_MF%YRPHY%LNEBN) THEN
        ...
        & PFLU_QSAT(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_T(:, :, JBLK), PPFL_FPLCH(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(:, :, JBLK), PUNEBH(:, :, JBLK), PNEBS0(:, :, JBLK), PQLIS0(:, :, JBLK),  &
        & PQLI_CVP(:, :, JBLK), PNEB_CVPP(:, :, JBLK), PQLI_CVPP(:, :, JBLK), PAIPCMT(:, JBLK),  &
        & Z_YDCPG_MISC_NEB(:, :, JBLK), PNEBC0(:, :, JBLK), Z_YDCPG_MISC_QICE(:, :, JBLK), Z_YDCPG_MISC_QLI(:, :, JBLK),  &
        & YDSTA)
        DO JLEV=YDCPG_OPTS%KTDIA,YDCPG_OPTS%KFLEVG
          !DEC$ IVDEP
          DO JLON=YLCPG_BNDS%KIDIA,YLCPG_BNDS%KFDIA
            Z_YDCPG_MISC_NEB(JLON, JLEV, JBLK) = Z_YDCPG_MISC_NEB(JLON, JLEV, JBLK)*YDMODEL%YRML_PHY_MF%YRPHY0%GAEPS
          END DO
        END DO
      END IF
      ...
      & PDECRD(:, JBLK), Z_YDCPG_MISC_NEB(:, :, JBLK), Z_YDMF_PHYS_OUT_CLCH(:, JBLK), Z_YDMF_PHYS_OUT_CLCM(:, JBLK),  &
      & Z_YDMF_PHYS_OUT_CLCL(:, JBLK), Z_YDCPG_MISC_CLCT(:, JBLK), PCLCT_RAD(:, JBLK), PCLCC=YDMF_PHYS%OUT%CLCC,  &
      & PNEBC=PNEBC0, PTOPC=YDMF_PHYS%OUT%CTOP)
    ...
    IF (LSYNCHOST('APL_ARPEGE_CLOUDINESS_PARALLEL:ACNEBCOND')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_CLOUDINESS_PARALLEL:ACNEBCOND:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
      PAIPCMT => GET_HOST_DATA_RDWR(YD_PAIPCMT)
      PCLCT_RAD => GET_HOST_DATA_RDWR(YD_PCLCT_RAD)
      PDECRD => GET_HOST_DATA_RDWR(YD_PDECRD)
      PFLU_QSAT => GET_HOST_DATA_RDWR(YD_PFLU_QSAT)
      ...
      Z_YDMF_PHYS_OUT_CTOP => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_CTOP)
      Z_YDVARS_GEOMETRY_GM_T0 => GET_HOST_DATA_RDWR(YDVARS%GEOMETRY%GM%FT0)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_CLOUDINESS_PARALLEL:ACNEBCOND:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_CLOUDINESS_PARALLEL:ACNEBCOND:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
    PAIPCMT => NULL()
    PCLCT_RAD => NULL()
    PDECRD => NULL()
    PFLU_QSAT => NULL()
    ...
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_CLOUDINESS_PARALLEL:ACNEBCOND:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
  ELSE IF (LPARALLELMETHOD('OPENMPSINGLECOLUMN', 'APL_ARPEGE_CLOUDINESS_PARALLEL:ACNEBCOND')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_CLOUDINESS_PARALLEL:ACNEBCOND:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
    PAIPCMT => GET_HOST_DATA_RDWR(YD_PAIPCMT)
    PCLCT_RAD => GET_HOST_DATA_RDWR(YD_PCLCT_RAD)
    PDECRD => GET_HOST_DATA_RDWR(YD_PDECRD)
...
!$OMP PARALLEL DO PRIVATE( JBLK, JLEV, JLON, YLCPG_BNDS, YLSTACK )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        ...
        & YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDMODEL%YRML_PHY_MF%YRTOPH%NTPLUI, YDCPG_OPTS%KFLEVG, LDREDPR, YDSTA,  &
        & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP(:, :, JBLK),  &
        & PQV(:, :, JBLK), PQI(:, :, JBLK), PQL(:, :, JBLK), PMSC_QW(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_T(:, :, JBLK),  &
        & PNEBCH(:, :, JBLK), Z_YDVARS_GEOMETRY_GM_T0(:, JBLK), Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T(:, JBLK),  &
        & PQLIS(:, :, JBLK), PNEBS(:, :, JBLK), ZRHCRI(:, :, JBLK), ZRH(:, :, JBLK), ZQSATS(:, :, JBLK),  &
        & ZICEFR1(:, :, JBLK), PQLIS0(:, :, JBLK), PNEBS0(:, :, JBLK), YDSTACK=YLSTACK)
        
        IF (YDMODEL%YRML_PHY_MF%YRPHY%LNEBN) THEN
          CALL ACNEBN_OPENACC(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON,  &
          & YDMODEL%YRML_PHY_MF%YRTOPH%NTNEBU, YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP(:, :, JBLK), PQV(:, :, JBLK), PQL(:, :, JBLK), PQI(:, :, JBLK),  &
          & PFLU_QSAT(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_T(:, :, JBLK), PPFL_FPLCH(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP(:, :, JBLK),  &
        ...
        !         Diagnostique de nebulosite partielle.
        CALL ACNPART_OPENACC(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON,  &
        & YDMODEL%YRML_PHY_MF%YRTOPH%NTNEBU, YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(:, :, JBLK),  &
        & PDECRD(:, JBLK), Z_YDCPG_MISC_NEB(:, :, JBLK), Z_YDMF_PHYS_OUT_CLCH(:, JBLK), Z_YDMF_PHYS_OUT_CLCM(:, JBLK),  &
        & Z_YDMF_PHYS_OUT_CLCL(:, JBLK), Z_YDCPG_MISC_CLCT(:, JBLK), PCLCT_RAD(:, JBLK), PCLCC=YDMF_PHYS%OUT%CLCC,  &
        & PNEBC=PNEBC0, PTOPC=YDMF_PHYS%OUT%CTOP, YDSTACK=YLSTACK)
    ...
    Z_YDVARS_GEOMETRY_GM_T0 => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_CLOUDINESS_PARALLEL:ACNEBCOND:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_CLOUDINESS_PARALLEL:ACNEBCOND')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_CLOUDINESS_PARALLEL:ACNEBCOND:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
    PAIPCMT => GET_DEVICE_DATA_RDWR(YD_PAIPCMT)
    PCLCT_RAD => GET_DEVICE_DATA_RDWR(YD_PCLCT_RAD)
    PDECRD => GET_DEVICE_DATA_RDWR(YD_PDECRD)
    PFLU_QSAT => GET_DEVICE_DATA_RDWR(YD_PFLU_QSAT)
    PMSC_QW => GET_DEVICE_DATA_RDWR(YD_PMSC_QW)
    ...
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_CLOUDINESS_PARALLEL:ACNEBCOND:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_CLOUDINESS_PARALLEL:ACNEBCOND:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
!$ACC PARALLEL LOOP GANG PRESENT( PAIPCMT, PCLCT_RAD, PDECRD, PFLU_QSAT, PMSC_QW, PNEBC0, PNEBCH, PNEBS, PNEBS0, PNEB_CVPP,  &
!$ACC & PPFL_FPLCH, PQI, PQL, PQLIS, PQLIS0, PQLI_CVP, PQLI_CVPP, PQV, PUNEBH, YDCPG_OPTS, YDMODEL, YDSTA, YSTACK, ZICEFR1,  &
!$ACC & ZQSATS, ZRH, ZRHCRI, Z_YDCPG_MISC_CLCT, Z_YDCPG_MISC_NEB, Z_YDCPG_MISC_QICE, Z_YDCPG_MISC_QLI,  &
...
!$ACC & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP, Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T, Z_YDMF_PHYS_OUT_CLCC,  &
!$ACC & Z_YDMF_PHYS_OUT_CLCH, Z_YDMF_PHYS_OUT_CLCL, Z_YDMF_PHYS_OUT_CLCM, Z_YDMF_PHYS_OUT_CTOP, Z_YDVARS_GEOMETRY_GM_T0 )  &
