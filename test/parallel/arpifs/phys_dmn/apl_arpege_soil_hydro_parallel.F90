SUBROUTINE APL_ARPEGE_SOIL_HYDRO_PARALLEL (YDMF_PHYS_BASE_STATE, YDCPG_BNDS, YDCPG_OPTS, YDMF_PHYS, YDMF_PHYS_SURF, YDMODEL,  &
& YD_PCHROV, YD_PFLU_NEIJ, YD_PFLU_QSAT, YD_PFLU_QSATS, YD_PFLU_VEG, YD_PGWDCS, YD_PHQ, YD_PHTR, YD_PHU, YD_PQV, YD_PWFC,  &
& YD_PWLMX, YD_PWWILT)
...
!$OMP PARALLEL DO PRIVATE( JBLK ) FIRSTPRIVATE( YLCPG_BNDS )
      DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
        ...
        & YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_OUT_FRSO(:, :, :, JBLK), PQV(:, :, JBLK), PFLU_QSAT(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_T(:, :, JBLK), Z_YDMF_PHYS_SURF_GSD_VV_PD2(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VF_PLSM(:, JBLK),  &
        & Z_YDMF_PHYS_SURF_GSD_VV_PIVEG(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VV_PLAI(:, JBLK), PFLU_NEIJ(:, JBLK),  &
        & PFLU_VEG(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VV_PRSMIN(:, JBLK), PCHROV(:, JBLK), PGWDCS(:, JBLK), PWFC(:, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YGSP_RR_FC(:, JBLK), PWWILT(:, JBLK), Z_YDMF_PHYS_BASE_STATE_YGSP_SB_Q(:, 1, JBLK),  &
        & PFLU_QSATS(:, JBLK), PHQ(:, JBLK), PHTR(:, JBLK), PHU(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VV_PHV(:, JBLK),  &
        & PWLMX(:, JBLK))
    ...
    ELSE IF (LPARALLELMETHOD('OPENMPSINGLECOLUMN', 'APL_ARPEGE_SOIL_HYDRO_PARALLEL:ACVEG')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_SOIL_HYDRO_PARALLEL:ACVEG:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
      PCHROV => GET_HOST_DATA_RDWR(YD_PCHROV)
      PFLU_NEIJ => GET_HOST_DATA_RDWR(YD_PFLU_NEIJ)
      PFLU_QSAT => GET_HOST_DATA_RDWR(YD_PFLU_QSAT)
...
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
      DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
        DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
          ...
          CALL ACVEG_OPENACC(YDMODEL%YRML_PHY_MF%YRPHY, YDMODEL%YRML_PHY_MF%YRPHY1, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA,  &
          & YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_OUT_FRSO(:, :, :, JBLK), PQV(:, :, JBLK), PFLU_QSAT(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_T(:, :, JBLK), Z_YDMF_PHYS_SURF_GSD_VV_PD2(:, JBLK),  &
          & Z_YDMF_PHYS_SURF_GSD_VF_PLSM(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VV_PIVEG(:, JBLK),  &
          & Z_YDMF_PHYS_SURF_GSD_VV_PLAI(:, JBLK), PFLU_NEIJ(:, JBLK), PFLU_VEG(:, JBLK),  &
          & Z_YDMF_PHYS_SURF_GSD_VV_PRSMIN(:, JBLK), PCHROV(:, JBLK), PGWDCS(:, JBLK), PWFC(:, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YGSP_RR_FC(:, JBLK), PWWILT(:, JBLK), Z_YDMF_PHYS_BASE_STATE_YGSP_SB_Q(:, 1, JBLK),  &
          & PFLU_QSATS(:, JBLK), PHQ(:, JBLK), PHTR(:, JBLK), PHU(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VV_PHV(:, JBLK),  &
          & PWLMX(:, JBLK), YDSTACK=YLSTACK)
      ...
      Z_YDMF_PHYS_SURF_GSD_VV_PRSMIN => NULL()
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_SOIL_HYDRO_PARALLEL:ACVEG:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
    ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_SOIL_HYDRO_PARALLEL:ACVEG')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_SOIL_HYDRO_PARALLEL:ACVEG:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
      ...
      PFLU_VEG => GET_DEVICE_DATA_RDWR(YD_PFLU_VEG)
      PGWDCS => GET_DEVICE_DATA_RDWR(YD_PGWDCS)
      ...
      PWFC => GET_DEVICE_DATA_RDWR(YD_PWFC)
      PWLMX => GET_DEVICE_DATA_RDWR(YD_PWLMX)
      PWWILT => GET_DEVICE_DATA_RDWR(YD_PWWILT)
      Z_YDMF_PHYS_BASE_STATE_T => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_FC => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_FC)
      ...
      Z_YDMF_PHYS_SURF_GSD_VV_PRSMIN => GET_DEVICE_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_RSMIN)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_SOIL_HYDRO_PARALLEL:ACVEG:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_SOIL_HYDRO_PARALLEL:ACVEG:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
!$ACC PARALLEL LOOP GANG PRESENT( PCHROV, PFLU_NEIJ, PFLU_QSAT, PFLU_QSATS, PFLU_VEG, PGWDCS, PHQ, PHTR, PHU, PQV, PWFC,  &
!$ACC & PWLMX, PWWILT, YDCPG_OPTS, YDMODEL, YSTACK, Z_YDMF_PHYS_BASE_STATE_T, Z_YDMF_PHYS_BASE_STATE_YGSP_RR_FC,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YGSP_SB_Q, Z_YDMF_PHYS_OUT_FRSO, Z_YDMF_PHYS_SURF_GSD_VF_PLSM, Z_YDMF_PHYS_SURF_GSD_VV_PD2,  &
!$ACC & Z_YDMF_PHYS_SURF_GSD_VV_PHV, Z_YDMF_PHYS_SURF_GSD_VV_PIVEG, Z_YDMF_PHYS_SURF_GSD_VV_PLAI,  &
!$ACC & Z_YDMF_PHYS_SURF_GSD_VV_PRSMIN ) PRIVATE( JBLK ) VECTOR_LENGTH( YDCPG_OPTS%KLON )
      DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
