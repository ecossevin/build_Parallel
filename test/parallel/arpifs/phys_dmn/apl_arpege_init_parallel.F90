SUBROUTINE APL_ARPEGE_INIT_PARALLEL (YDCST, YDMF_PHYS_BASE_STATE, YDCPG_BNDS, YDCPG_OPTS, YDCPG_MISC, YDMF_PHYS,  &
& YDMF_PHYS_SURF, YDVARS, YDMODEL, YD_KMOC_CLPH, YD_KNLAB_CVPP, YD_PAER, YD_PAERINDS, YD_PAIPCMT, YD_PALBD, YD_PALBP,  &
& YD_PALPHA1, YD_PCEMTR, YD_PCFATH, YD_PCFAU, YD_PCFBTH, YD_PCFBU, YD_PCFBV, YD_PCOEFA, YD_PCONDCVPPI, YD_PCONDCVPPL,  &
& YD_PCTRSO, YD_PDECRD, YD_PDIFCVPPQ, YD_PDIFCVPPS, YD_PDIFCVPPU, YD_PDIFCVPPV, YD_PDIFWQ, YD_PDIFWS, YD_PEDMFQ, YD_PEDMFS,  &
& YD_PEDMFU, YD_PEDMFV, PEPS0, PEPSNEB, YD_PFPCOR, YD_PKQLROV, YD_PKQROV, YD_PKTROV, YD_PKUROV, YD_PLVT, YD_PMF_UP,  &
& YD_PMRIPP, YD_PNEBC0, YD_PNEBCH, YD_PNEBDIFF, YD_PNEBS, YD_PNEBS0, YD_PNEB_CVPP, YD_PPFL_FPLCH, YD_PPFL_FPLSH, YD_PPOID,  &
& YD_PPRODTH_CVPP, YD_PQC_DET_PCMT, YD_PQI, YD_PQIC, YD_PQICE, YD_PQL, YD_PQLC, YD_PQLIS, YD_PQLIS0, YD_PQLI_CVP,  &
& YD_PQLI_CVPP, YD_PQO3, YD_PQR, YD_PQS, YD_PQV, YD_PSC_FCLL, YD_PSC_FCLN, YD_PSC_FEVI, YD_PSC_FEVN, YD_PSEDIQI, YD_PSEDIQL,  &
& YD_PSFSWDIF, YD_PSFSWDIR, YD_PSUDU, YD_PTENHA, YD_PTENQVA, YD_PTENT, YD_PTRSOD, YD_PTRSODIF, YD_PTRSODIR, YD_PUNEBH,  &
& YD_PXDROV, YD_PXHROV, YD_PXQRO, YD_PXTRO, YD_PXTROV, YD_PXURO, YD_PXUROV)
...
#include "suozon_openacc.intfb.h"
  ...
  CLASS(FIELD_3RB), POINTER :: YL_ZIPOI => NULL()
  REAL(KIND=JPRB), POINTER :: ZIPOI(:, :, :) => NULL()
  REAL(KIND=JPRB) :: ZQV0
  CLASS(FIELD_3RB), POINTER :: YL_ZROZ => NULL()
  REAL(KIND=JPRB), POINTER :: ZROZ(:, :, :) => NULL()
  ...
  CALL FIELD_NEW(YL_ZIPOI, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZROZ, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
...
!$OMP PARALLEL DO PRIVATE( JBLK, JLON ) FIRSTPRIVATE( YLCPG_BNDS )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      ...
      IF (YDMODEL%YRML_PHY_MF%YRPHY0%RDECRD <= 0._JPRB .or. YDMODEL%YRML_PHY_MF%YRPHY%LRNUEXP) THEN
        DO JLON=YLCPG_BNDS%KIDIA,YLCPG_BNDS%KFDIA
          PDECRD(JLON, JBLK) = YDMODEL%YRML_PHY_MF%YRPHY0%RDECRD1 +  &
          & YDMODEL%YRML_PHY_MF%YRPHY0%RDECRD2*EXP(-((ASIN(Z_YDVARS_GEOMETRY_GEMU_T0(JLON, JBLK)) -  &
          & YDMODEL%YRML_PHY_MF%YRPHY0%RDECRD3*YDMODEL%YRML_GCONF%YRRIP%RDECLI) / YDMODEL%YRML_PHY_MF%YRPHY0%RDECRD4)**2)
        END DO
      END IF
  ...
  ELSE IF (LPARALLELMETHOD('OPENMPSINGLECOLUMN', 'APL_ARPEGE_INIT_PARALLEL:0')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:0:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
    PAIPCMT => GET_HOST_DATA_RDWR(YD_PAIPCMT)
    PDECRD => GET_HOST_DATA_RDWR(YD_PDECRD)
    Z_YDVARS_GEOMETRY_GEMU_T0 => GET_HOST_DATA_RDWR(YDVARS%GEOMETRY%GEMU%FT0)
...
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
    ...
    Z_YDVARS_GEOMETRY_GEMU_T0 => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:0:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_INIT_PARALLEL:0')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:0:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
    ...
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:0:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:0:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YSTACK,YDMODEL, PAIPCMT, PDECRD, Z_YDVARS_GEOMETRY_GEMU_T0 ) PRIVATE( JBLK )  &
...
!$OMP PARALLEL DO PRIVATE( JBLK, JLEV, JLON, ZEPSO3 ) FIRSTPRIVATE( YLCPG_BNDS )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      ...
      ELSE IF ( &
      & YDMODEL%YRML_PHY_MF%YRPHY%LO3FL .and. YDMODEL%YRML_PHY_RAD%YRERAD%NOZOCL == 1 .and. YDMODEL%YRML_PHY_MF%YRPHY%LRAYFM &
      & ) THEN
        IF (MOD(YDCPG_OPTS%NSTEP, YDMODEL%YRML_PHY_RAD%YRERAD%NRADFR) == 0) THEN
          CALL RADOZCMF(YDMODEL%YRCST, YDMODEL%YRML_GCONF%YRRIP%YREOZOC, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA,  &
          & YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD(:, :, JBLK),  &
          & Z_YDVARS_GEOMETRY_GEMU_T0(:, JBLK), ZROZ(:, :, JBLK))
          DO JLON=YLCPG_BNDS%KIDIA,YLCPG_BNDS%KFDIA
            PQO3(JLON, 0, JBLK) = 1.E-9_JPRB
          END DO
          DO JLEV=YDCPG_OPTS%KTDIA,YDCPG_OPTS%KFLEVG
            DO JLON=YLCPG_BNDS%KIDIA,YLCPG_BNDS%KFDIA
              PQO3(JLON, JLEV, JBLK) = ZROZ(JLON, JLEV, JBLK)*Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP(JLON, JLEV, JBLK)
            END DO
          END DO
        END IF
        
      ELSE
        CALL SUOZON(YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, PQO3(:, :, JBLK), .false.,  &
        & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP(:, :, JBLK),  &
        & YDMODEL%YRML_PHY_MF%YRPHY%LO3ABC, Z_YDMF_PHYS_SURF_GSD_VC_PA(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VC_PB(:, JBLK),  &
        & Z_YDMF_PHYS_SURF_GSD_VC_PC(:, JBLK))
      END IF
  ...
  ELSE IF (LPARALLELMETHOD('OPENMPSINGLECOLUMN', 'APL_ARPEGE_INIT_PARALLEL:1')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:1:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
    PQO3 => GET_HOST_DATA_RDWR(YD_PQO3)
    ZROZ => GET_HOST_DATA_RDWR(YL_ZROZ)
    Z_YDCPG_MISC_KOZO => GET_HOST_DATA_RDWR(YDCPG_MISC%F_KOZO)
    ...
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:1:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:1:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK, JLEV, ZEPSO3 )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        ...
        ELSE IF (YDMODEL%YRML_PHY_MF%YRPHY%LO3FL .and. YDMODEL%YRML_PHY_RAD%YRERAD%NOZOCL == 1 .and.  &
        & YDMODEL%YRML_PHY_MF%YRPHY%LRAYFM) THEN
          IF (MOD(YDCPG_OPTS%NSTEP, YDMODEL%YRML_PHY_RAD%YRERAD%NRADFR) == 0) THEN
            CALL RADOZCMF_OPENACC(YDMODEL%YRCST, YDMODEL%YRML_GCONF%YRRIP%YREOZOC, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA,  &
            & YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD(:, :, JBLK),  &
            & Z_YDVARS_GEOMETRY_GEMU_T0(:, JBLK), ZROZ(:, :, JBLK), YDSTACK=YLSTACK)
            PQO3(JLON, 0, JBLK) = 1.E-9_JPRB
            DO JLEV=YDCPG_OPTS%KTDIA,YDCPG_OPTS%KFLEVG
              PQO3(JLON, JLEV, JBLK) = ZROZ(JLON, JLEV, JBLK)*Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP(JLON, JLEV, JBLK)
            END DO
        ...
        ELSE
          CALL SUOZON_OPENACC(YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, PQO3(:, :, JBLK),  &
          & .false., Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP(:, :, JBLK), YDMODEL%YRML_PHY_MF%YRPHY%LO3ABC,  &
          & Z_YDMF_PHYS_SURF_GSD_VC_PA(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VC_PB(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VC_PC(:, JBLK),  &
          & YDSTACK=YLSTACK)
        END IF
    ...
    Z_YDVARS_GEOMETRY_GEMU_T0 => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:1:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_INIT_PARALLEL:1')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:1:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
    PQO3 => GET_DEVICE_DATA_RDWR(YD_PQO3)
    ZROZ => GET_DEVICE_DATA_RDWR(YL_ZROZ)
    Z_YDCPG_MISC_KOZO => GET_DEVICE_DATA_RDWR(YDCPG_MISC%F_KOZO)
    ...
    Z_YDMF_PHYS_SURF_GSD_VC_PA => GET_DEVICE_DATA_RDWR(YDMF_PHYS_SURF%GSD_VC%F_A)
    Z_YDMF_PHYS_SURF_GSD_VC_PB => GET_DEVICE_DATA_RDWR(YDMF_PHYS_SURF%GSD_VC%F_B)
    Z_YDMF_PHYS_SURF_GSD_VC_PC => GET_DEVICE_DATA_RDWR(YDMF_PHYS_SURF%GSD_VC%F_C)
    Z_YDVARS_GEOMETRY_GEMU_T0 => GET_DEVICE_DATA_RDWR(YDVARS%GEOMETRY%GEMU%FT0)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:1:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:1:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YSTACK,YDMODEL, PQO3, ZROZ, Z_YDCPG_MISC_KOZO,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP, Z_YDMF_PHYS_SURF_GSD_VC_PA,  &
!$ACC & Z_YDMF_PHYS_SURF_GSD_VC_PB, Z_YDMF_PHYS_SURF_GSD_VC_PC, Z_YDVARS_GEOMETRY_GEMU_T0 ) PRIVATE( JBLK ) VECTOR_LENGTH(  &
!$ACC & YDCPG_OPTS%KLON )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK, JLEV, ZEPSO3 )
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
...
!$OMP PARALLEL DO PRIVATE( JBLK, JLEV, JLON, ZGDT, ZGDTI ) FIRSTPRIVATE( YLCPG_BNDS )
  ...
  ELSE IF (LPARALLELMETHOD('OPENMPSINGLECOLUMN', 'APL_ARPEGE_INIT_PARALLEL:2')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:2:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
    PPOID => GET_HOST_DATA_RDWR(YD_PPOID)
    PPRODTH_CVPP => GET_HOST_DATA_RDWR(YD_PPRODTH_CVPP)
    ZIPOI => GET_HOST_DATA_RDWR(YL_ZIPOI)
    ...
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:2:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:2:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK, JLEV, ZGDT, ZGDTI )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
    ...
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:2:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_INIT_PARALLEL:2')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:2:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
    ...
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:2:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:2:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YSTACK,YDCST, YDMODEL, PPOID, PPRODTH_CVPP, ZIPOI,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP ) PRIVATE( JBLK ) VECTOR_LENGTH(  &
!$ACC & YDCPG_OPTS%KLON )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK, JLEV, ZGDT, ZGDTI )
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
...
!$OMP PARALLEL DO PRIVATE( JBLK ) FIRSTPRIVATE( YLCPG_BNDS )
  ...
  ELSE IF (LPARALLELMETHOD('OPENMPSINGLECOLUMN', 'APL_ARPEGE_INIT_PARALLEL:3')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:3:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
    KNLAB_CVPP => GET_HOST_DATA_RDWR(YD_KNLAB_CVPP)
    PALPHA1 => GET_HOST_DATA_RDWR(YD_PALPHA1)
    PCFATH => GET_HOST_DATA_RDWR(YD_PCFATH)
...
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
    ...
    PXUROV => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:3:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_INIT_PARALLEL:3')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:3:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
    ...
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:3:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:3:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YSTACK,KNLAB_CVPP, PALPHA1, PCFATH, PCFAU, PCFBTH, PCFBU, PCFBV, PCOEFA,  &
!$ACC & PCONDCVPPI, PCONDCVPPL, PDIFCVPPQ, PDIFCVPPS, PDIFCVPPU, PDIFCVPPV, PDIFWQ, PDIFWS, PEDMFQ, PEDMFS, PEDMFU, PEDMFV,  &
!$ACC & PFPCOR, PKQLROV, PKQROV, PKTROV, PKUROV, PLVT, PMF_UP, PMRIPP, PNEBC0, PNEBCH, PNEBDIFF, PNEBS, PNEBS0, PNEB_CVPP,  &
!$ACC & PQC_DET_PCMT, PQIC, PQICE, PQLC, PQLIS, PQLIS0, PQLI_CVP, PQLI_CVPP, PSC_FCLL, PSC_FCLN, PSC_FEVI, PSC_FEVN,  &
!$ACC & PSEDIQI, PSEDIQL, PTENHA, PTENQVA, PTENT, PUNEBH, PXQRO, PXTRO, PXTROV, PXURO, PXUROV ) PRIVATE( JBLK )  &
!$ACC & VECTOR_LENGTH( YDCPG_OPTS%KLON )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK )
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
...
!$OMP PARALLEL DO PRIVATE( JBLK, JLEV, JLON, ZDQC, ZDQI, ZDQL, ZDQR, ZDQS, ZDQV, ZQV0 ) FIRSTPRIVATE( YLCPG_BNDS )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      ...
      DO JLEV=1,YDCPG_OPTS%KFLEVG
        DO JLON=YLCPG_BNDS%KIDIA,YLCPG_BNDS%KFDIA
          ...
          ZQV0 = Z_YDMF_PHYS_BASE_STATE_Q(JLON, JLEV, JBLK) - ZIPOI(JLON, JLEV, JBLK)*(0.0_JPRB - Z_YDMF_PHYS_OUT_FCQNG(JLON, &
          &  JLEV - 1, JBLK) - Z_YDMF_PHYS_OUT_FCQNNG(JLON, JLEV - 1, JBLK) - Z_YDMF_PHYS_OUT_FCQLNG(JLON, JLEV - 1, JBLK) -  &
          & Z_YDMF_PHYS_OUT_FCQRNG(JLON, JLEV - 1, JBLK) - Z_YDMF_PHYS_OUT_FCQSNG(JLON, JLEV - 1, JBLK))
          PQV(JLON, JLEV, JBLK) = MAX(0.0_JPRB, ZQV0 - ZDQC)
          ZDQV = MAX(0.0_JPRB, ZQV0 - ZDQC) - Z_YDMF_PHYS_BASE_STATE_Q(JLON, JLEV, JBLK)
          
          Z_YDMF_PHYS_OUT_FCQNG(JLON, JLEV, JBLK) =  &
          & Z_YDMF_PHYS_OUT_FCQNG(JLON, JLEV - 1, JBLK) - ZDQV*PPOID(JLON, JLEV, JBLK)
          Z_YDMF_PHYS_OUT_FCQNNG(JLON, JLEV, JBLK) =  &
          & Z_YDMF_PHYS_OUT_FCQNNG(JLON, JLEV - 1, JBLK) - ZDQI*PPOID(JLON, JLEV, JBLK)
          Z_YDMF_PHYS_OUT_FCQLNG(JLON, JLEV, JBLK) =  &
          & Z_YDMF_PHYS_OUT_FCQLNG(JLON, JLEV - 1, JBLK) - ZDQL*PPOID(JLON, JLEV, JBLK)
          Z_YDMF_PHYS_OUT_FCQRNG(JLON, JLEV, JBLK) =  &
          & Z_YDMF_PHYS_OUT_FCQRNG(JLON, JLEV - 1, JBLK) - ZDQR*PPOID(JLON, JLEV, JBLK)
          Z_YDMF_PHYS_OUT_FCQSNG(JLON, JLEV, JBLK) =  &
          & Z_YDMF_PHYS_OUT_FCQSNG(JLON, JLEV - 1, JBLK) - ZDQS*PPOID(JLON, JLEV, JBLK)
        END DO
      END DO
  ...
  ELSE IF (LPARALLELMETHOD('OPENMPSINGLECOLUMN', 'APL_ARPEGE_INIT_PARALLEL:4')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:4:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
    PPOID => GET_HOST_DATA_RDWR(YD_PPOID)
    PQI => GET_HOST_DATA_RDWR(YD_PQI)
    PQL => GET_HOST_DATA_RDWR(YD_PQL)
    ...
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:4:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:4:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK, JLEV, ZDQC, ZDQI, ZDQL, ZDQR, ZDQS, ZDQV, ZQV0 )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
    ...
    Z_YDMF_PHYS_OUT_FCQSNG => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:4:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_INIT_PARALLEL:4')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:4:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
    PPOID => GET_DEVICE_DATA_RDWR(YD_PPOID)
    ...
    Z_YDMF_PHYS_BASE_STATE_R => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_R)
    Z_YDMF_PHYS_BASE_STATE_S => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_S)
    ...
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:4:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:4:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YSTACK,PPOID, PQI, PQL, PQR, PQS, PQV, ZIPOI, Z_YDMF_PHYS_BASE_STATE_I,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_L, Z_YDMF_PHYS_BASE_STATE_Q, Z_YDMF_PHYS_BASE_STATE_R, Z_YDMF_PHYS_BASE_STATE_S,  &
!$ACC & Z_YDMF_PHYS_OUT_FCQLNG, Z_YDMF_PHYS_OUT_FCQNG, Z_YDMF_PHYS_OUT_FCQNNG, Z_YDMF_PHYS_OUT_FCQRNG,  &
!$ACC & Z_YDMF_PHYS_OUT_FCQSNG ) PRIVATE( JBLK ) VECTOR_LENGTH( YDCPG_OPTS%KLON )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK, JLEV, ZDQC, ZDQI, ZDQL, ZDQR, ZDQS, ZDQV, ZQV0 )
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
...
!$OMP PARALLEL DO PRIVATE( JBLK, JCHA, JLEV, JLON, JSG ) FIRSTPRIVATE( YLCPG_BNDS )
  ...
  ELSE IF (LPARALLELMETHOD('OPENMPSINGLECOLUMN', 'APL_ARPEGE_INIT_PARALLEL:5')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:5:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
    KMOC_CLPH => GET_HOST_DATA_RDWR(YD_KMOC_CLPH)
    PAER => GET_HOST_DATA_RDWR(YD_PAER)
    PAERINDS => GET_HOST_DATA_RDWR(YD_PAERINDS)
    ...
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:5:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:5:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK, JCHA, JLEV, JSG )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
    ...
    PXHROV => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:5:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_INIT_PARALLEL:5')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:5:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
    ...
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:5:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:5:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YSTACK,YDMODEL, KMOC_CLPH, PAER, PAERINDS, PALBD, PALBP, PCEMTR, PCTRSO,  &
!$ACC & PSFSWDIF, PSFSWDIR, PSUDU, PTRSOD, PTRSODIF, PTRSODIR, PXDROV, PXHROV ) PRIVATE( JBLK ) VECTOR_LENGTH(  &
!$ACC & YDCPG_OPTS%KLON )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK, JCHA, JLEV, JSG )
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
...
!$OMP PARALLEL DO PRIVATE( JBLK, JLEV, JLON ) FIRSTPRIVATE( YLCPG_BNDS )
  ...
  ELSE IF (LPARALLELMETHOD('OPENMPSINGLECOLUMN', 'APL_ARPEGE_INIT_PARALLEL:6')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:6:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
    PPFL_FPLCH => GET_HOST_DATA_RDWR(YD_PPFL_FPLCH)
    PPFL_FPLSH => GET_HOST_DATA_RDWR(YD_PPFL_FPLSH)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:6:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
...
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK, JLEV )
    ...
    PPFL_FPLSH => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:6:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_INIT_PARALLEL:6')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:6:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
    PPFL_FPLCH => GET_DEVICE_DATA_RDWR(YD_PPFL_FPLCH)
    PPFL_FPLSH => GET_DEVICE_DATA_RDWR(YD_PPFL_FPLSH)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:6:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_INIT_PARALLEL:6:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YSTACK,YDMODEL, PPFL_FPLCH, PPFL_FPLSH ) PRIVATE( JBLK ) VECTOR_LENGTH(  &
!$ACC & YDCPG_OPTS%KLON )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
