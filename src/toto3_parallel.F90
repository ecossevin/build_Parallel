...
!$OMP PARALLEL DO PRIVATE( JBLK ) FIRSTPRIVATE( YLCPG_BNDS )
      DO JBLK=1,YDCPG_OPTS%KGPBLKS
        ...
        IF (YDMODEL%YRML_PHY_MF%YRPHY%LSOLV) THEN
          ...
          & Z_YDMF_PHYS_SURF_GSD_VV_PIVEG(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VV_PLAI(:, JBLK), YDMF_PHYS_BASE_STATE%YGSP_SG%A(:, 1),  &
          & YDMF_PHYS_BASE_STATE%YGSP_SG%R(:, 1), Z_YDMF_PHYS_SURF_GSD_VV_PSAB(:, JBLK), YDMF_PHYS_BASE_STATE%YGSP_SG%F(:, 1),  &
          & Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VF_PVEG(:, JBLK),  &
          & YDMF_PHYS_BASE_STATE%YGSP_SB%Q(:, 1), YDMF_PHYS_BASE_STATE%YGSP_SB%TL(:, 1),  &
        ...
        ELSE
          ...
          DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
            ...
            & YDMODEL%YRML_PHY_MF%YRPHY1%HSOLIWR*(YDMF_PHYS_BASE_STATE%YGSP_RR%W(JLON, JBLK, JBLK) +  &
            & YDMF_PHYS_BASE_STATE%YGSP_SB%Q(JLON, 1)) / (YDMODEL%YRML_PHY_MF%YRPHY1%WSMX + YDMODEL%YRML_PHY_MF%YRPHY1%WPMX) &
            & *EXP(-0.5_JPRB*(YDMODEL%YRML_PHY_MF%YRPHY1%HSOLIT0*(YDMF_PHYS_BASE_STATE%YGSP_RR%T(JLON, JBLK, JBLK) -  &
            & YDMODEL%YRCST%RTT))**2))
          END DO
        END IF
      END DO
      ...
      DO JBLK=1,YDCPG_OPTS%KGPBLKS
        IF (YDMODEL%YRML_PHY_MF%YRPHY%LSOLV) THEN
          ...
          DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
            ...
            & Z_YDMF_PHYS_SURF_GSD_VV_PSAB(:, JBLK), YDMF_PHYS_BASE_STATE%YGSP_SG%F(:, 1),  &
            & Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VF_PVEG(:, JBLK),  &
            & YDMF_PHYS_BASE_STATE%YGSP_SB%Q(:, 1), YDMF_PHYS_BASE_STATE%YGSP_SB%TL(:, 1),  &
            & Z_YDMF_PHYS_BASE_STATE_YGSP_RR_W(:, JBLK), Z_YDMF_PHYS_BASE_STATE_YGSP_RR_IC(:, JBLK), LLHMT, YL_ZDSA_C1(:, JBLK),  &
            & YL_ZDSA_C2(:, JBLK), YL_ZC3(:, JBLK), YL_ZCG(:, JBLK), YL_ZCN(:, JBLK), Z_YDMF_PHYS_OUT_CT(:, JBLK),  &
