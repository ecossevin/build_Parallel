...
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
  ...
  IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE:CPPHINP')) THEN
    ...
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE:CPPHINP:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
    CALL YLCPG_BNDS%INIT(YDCPG_OPTS)
!$ACC PARALLEL LOOP GANG PRESENT( Z_YDCPG_DYN0_CTY_EVEL,Z_YDVARS_CVGQ_DL,Z_YDVARS_CVGQ_DM,  &
!$ACC & Z_YDVARS_GEOMETRY_GELAM_T0,ZRDG_MU0(:,JBLK ) ZRDG_MU0LU( :,JBLK ) Z_YDVARS_Q_T0 Z_YDVARS_Q_DM Z_YDCPG_PHY0_XYB_RDELP  &
!$ACC & Z_YDVARS_GEOMETRY_GEMU_T0 Z_YDVARS_CVGQ_T0 ZRDG_MU0N( :,JBLK ) Z_YDVARS_Q_DL ZRDG_MU0M( :,JBLK ) Z_YDVARS_V_T0  &
!$ACC & ZRDG_CVGQ( :,:,JBLK ) Z_YDVARS_U_T0 PRIVATE( JBLK ) VECTOR_LENGTH( YDCPG_OPTS%KLON )
    DO JBLK=1,YDCPG_OPTS%KGPBLKS
...
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
  ...
  IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE:ZRDG_LCVQ')) THEN
    ...
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE:ZRDG_LCVQ:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
    CALL YLCPG_BNDS%INIT(YDCPG_OPTS)
!$ACC PARALLEL LOOP GANG PRESENT( ZRDG_LCVQ(:,:,JBLK ) ZRDG_CVGQ( :,:,JBLK ) ZFLU_QSATS( :,JBLK ) PRIVATE( JBLK ) VECTOR_LENGTH(  &
!$ACC & YDCPG_OPTS%KLON )
    DO JBLK=1,YDCPG_OPTS%KGPBLKS
!$ACC LOOP VECTOR PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
