...
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
  ...
  IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE:CPPHINP')) THEN
    ...
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE:CPPHINP:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
    CALL YLCPG_BNDS%INIT(YDCPG_OPTS)
!$ACC PARALLEL LOOP GANG PRESENT( Z_YDVARS_Q_T0,Z_YDVARS_GEOMETRY_GEMU_T0,ZRDG_MU0,  &
!$ACC & Z_YDVARS_CVGQ_DM,Z_YDVARS_GEOMETRY_GELAM_T0,Z_YDVARS_CVGQ_T0, ZRDG_MU0M,ZRDG_MU0N,Z_YDVARS_CVGQ_DL,  &
!$ACC & Z_YDVARS_U_T0,Z_YDCPG_DYN0_CTY_EVEL,Z_YDVARS_V_T0, Z_YDVARS_Q_DM,Z_YDCPG_PHY0_XYB_RDELP,Z_YDVARS_Q_DL,  &
!$ACC & ZRDG_CVGQ,ZRDG_MU0LU ) PRIVATE( JBLK ) VECTOR_LENGTH( YDCPG_OPTS%KLON )
    DO JBLK=1,YDCPG_OPTS%KGPBLKS
...
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
  ...
  IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE:ZRDG_LCVQ')) THEN
    ...
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE:ZRDG_LCVQ:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
    CALL YLCPG_BNDS%INIT(YDCPG_OPTS)
!$ACC PARALLEL LOOP GANG PRESENT( ZFLU_QSATS,ZRDG_CVGQ,ZRDG_LCVQ ) PRIVATE( JBLK ) VECTOR_LENGTH( YDCPG_OPTS%KLON )
    DO JBLK=1,YDCPG_OPTS%KGPBLKS
