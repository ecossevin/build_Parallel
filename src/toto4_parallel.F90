...
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
  ...
  IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE:CPPHINP')) THEN
    ...
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE:CPPHINP:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
    CALL YLCPG_BNDS%INIT(YDCPG_OPTS)
!$ACC PARALLEL LOOP GANG PRESENT( ZRDG_MU0M,Z_YDVARS_GEOMETRY_GELAM_T0,ZRDG_MU0LU, Z_YDVARS_CVGQ_T0,Z_YDVARS_V_T0,Z_YDVARS_Q_T0,  &
!$ACC & Z_YDVARS_Q_DM,Z_YDVARS_GEOMETRY_GEMU_T0,Z_YDVARS_CVGQ_DL, Z_YDCPG_PHY0_XYB_RDELP,ZRDG_MU0N,Z_YDVARS_U_T0,  &
!$ACC & Z_YDVARS_Q_DL,ZRDG_CVGQ,ZRDG_MU0, Z_YDCPG_DYN0_CTY_EVEL,Z_YDVARS_CVGQ_DM ) PRIVATE( JBLK ) VECTOR_LENGTH(  &
!$ACC & YDCPG_OPTS%KLON )
    DO JBLK=1,YDCPG_OPTS%KGPBLKS
