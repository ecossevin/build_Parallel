...
!$ACC PARALLEL LOOP GANG
    !$ACC&PRESENT(ZFLU_QSATS,ZRDG_CVGQ,ZRDG_LCVQ) &
    !$ACC&PRIVATE (JBLK) &
...
!$OMP PARALLEL DO PRIVATE( JBLK ) FIRSTPRIVATE( YLCPG_BNDS )
    DO JBLK=1,YDCPG_OPTS%KGPBLKS
      ...
      DO JLEV=YDCPG_OPTS%KTDIA,YDCPG_OPTS%KFLEVG
        DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
          YDMF_PHYS%OUT%DIFTQ(JLON, JLEV, JBLK) = YDMF_PHYS%OUT%DIFTQ(JLON, JLEV, JBLK) + ZDIFCVPPQ(JLON, JLEV, JBLK)
          YDMF_PHYS%OUT%DIFTS(JLON, JLEV, JBLK) = YDMF_PHYS%OUT%DIFTS(JLON, JLEV, JBLK) + ZDIFCVPPS(JLON, JLEV, JBLK)
          YDMF_PHYS%OUT%STRTU(JLON, JLEV, JBLK) = YDMF_PHYS%OUT%STRTU(JLON, JLEV, JBLK) + ZDIFCVPPU(JLON, JLEV, JBLK)
          YDMF_PHYS%OUT%STRTV(JLON, JLEV, JBLK) = YDMF_PHYS%OUT%STRTV(JLON, JLEV, JBLK) + ZDIFCVPPV(JLON, JLEV, JBLK)
        END DO
      END DO
    END DO
...
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
  ...
  IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE:DIFTQ')) THEN
    ...
    CALL YLCPG_BNDS%INIT(YDCPG_OPTS)
!$ACC PARALLEL LOOP GANG
    !$ACC&PRESENT(ZDIFCVPPV,ZDIFCVPPS,Z_YDMF_PHYS_OUT_DIFTQ, &
    !$ACC&        ZDIFCVPPQ,Z_YDMF_PHYS_OUT_STRTV,Z_YDMF_PHYS_OUT_DIFTS, &
    !$ACC&        Z_YDMF_PHYS_OUT_STRTU,ZDIFCVPPU) &
    !$ACC&PRIVATE (JBLK) &
    !$ACC&VECTOR_LENGTH (YDCPG_OPTS%KLON)
    DO JBLK=1,YDCPG_OPTS%KGPBLKS
...
!$OMP PARALLEL DO PRIVATE( JBLK ) FIRSTPRIVATE( YLCPG_BNDS )
    DO JBLK=1,YDCPG_OPTS%KGPBLKS
      ...
      !DEC$ IVDEP
      DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
        IF (.not.YDMODEL%YRML_PHY_MF%YRARPHY%LMSE) YDMF_PHYS%OUT%FRTHDS(JLON, JBLK) = YDMF_PHYS%OUT%FRTH(JLON, YDCPG_OPTS%KFLEVG, 1, JBLK) / ZFLU_EMIS(JLON, JBLK) +  &
        & YDMODEL%YRCST%RSIGMA*YDMF_PHYS_BASE_STATE%YGSP_RR%T(JLON, JBLK)**4
        IF (ZRDG_MU0(JLON, JBLK) <= 0.0_JPRB) THEN
          YDMF_PHYS%OUT%FRSOPT(JLON, JBLK) = 0.0_JPRB
        ELSE
          YDMF_PHYS%OUT%FRSOPT(JLON, JBLK) = YDMODEL%YRML_PHY_MF%YRPHY3%RII0*ZRDG_MU0(JLON, JBLK)
        END IF
      END DO
    END DO
...
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
  ...
  IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE:FRSOPT')) THEN
    ...
    CALL YLCPG_BNDS%INIT(YDCPG_OPTS)
!$ACC PARALLEL LOOP GANG
    !$ACC&PRESENT(Z_YDMF_PHYS_OUT_FRSOPT,ZRDG_MU0,Z_YDMF_PHYS_OUT_FRTH, &
    !$ACC&        Z_YDMF_PHYS_OUT_FRTHDS,ZFLU_EMIS,Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T) &
    !$ACC&PRIVATE (JBLK) &
    !$ACC&VECTOR_LENGTH (YDCPG_OPTS%KLON)
    DO JBLK=1,YDCPG_OPTS%KGPBLKS
