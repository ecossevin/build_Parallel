SUBROUTINE APL_ARPEGE_PARALLEL (YDMF_PHYS_BASE_STATE, YDMF_PHYS_NEXT_STATE, YDGEOMETRY, YDCPG_BNDS, YDCPG_OPTS, YDCPG_MISC,  &
& YDCPG_GPAR, YDCPG_PHY0, YDMF_PHYS, YDCPG_DYN0, YDMF_PHYS_SURF, YDCPG_SL2, YDVARS, YDCFU, YDMODEL, YDDDH, YDPHYSMWAVE)
  
  !**** *APL_ARPEGE*  - Call ARPEGE physics
  
  !     Author.
  !     -------
  !      Philippe Marguinaud *METEO-FRANCE*
  !      Original : 28-04-2021
  
  !     Modifications.
  !     --------------
  !     13-Jul-2022 R. El Khatib Fix initialization of ZQGM
  !     R. El Khatib 26-Apr-2023 Loop pushing in ppwetpoint
  !     04-Oct-2023 A. Alias initialization of ZFCHQ added
  ! End Modifications
  !-------------------------------------------------------------------------------
  
  USE ACPY_MOD
  USE STACK_MOD
  USE YOMPARALLELMETHOD
  USE FIELD_ACCESS_MODULE
  USE FIELD_FACTORY_MODULE
  USE FIELD_MODULE
#include "stack.h"
  USE GEOMETRY_MOD, ONLY: GEOMETRY
  USE MF_PHYS_TYPE_MOD, ONLY: MF_PHYS_TYPE
  USE CPG_TYPE_MOD, ONLY: CPG_MISC_TYPE, CPG_DYN_TYPE, CPG_GPAR_TYPE, CPG_PHY_TYPE
  USE CPG_SL2_TYPE_MOD, ONLY: CPG_SL2_TYPE
  USE CPG_OPTS_TYPE_MOD, ONLY: CPG_BNDS_TYPE, CPG_OPTS_TYPE
  USE MF_PHYS_SURFACE_TYPE_MOD, ONLY: MF_PHYS_SURF_TYPE
  USE FIELD_VARIABLES_MOD, ONLY: FIELD_VARIABLES
  USE MF_PHYS_BASE_STATE_TYPE_MOD, ONLY: MF_PHYS_BASE_STATE_TYPE
  USE MF_PHYS_NEXT_STATE_TYPE_MOD, ONLY: MF_PHYS_NEXT_STATE_TYPE
  USE TYPE_MODEL, ONLY: MODEL
  
  USE PARKIND1, ONLY: JPIM, JPRB
  USE YOMHOOK, ONLY: LHOOK, DR_HOOK, JPHOOK
  USE DDH_MIX, ONLY: TYP_DDH
  
  USE YOE_PHYS_MWAVE, ONLY: TEPHYSMWAVE
  USE YOMCFU, ONLY: TCFU
  
  IMPLICIT NONE
  
  TYPE(MF_PHYS_BASE_STATE_TYPE), INTENT(IN) :: YDMF_PHYS_BASE_STATE
  TYPE(MF_PHYS_NEXT_STATE_TYPE), INTENT(INOUT) :: YDMF_PHYS_NEXT_STATE
  TYPE(GEOMETRY), INTENT(IN) :: YDGEOMETRY
  TYPE(CPG_BNDS_TYPE), INTENT(IN) :: YDCPG_BNDS
  TYPE(CPG_OPTS_TYPE), INTENT(IN) :: YDCPG_OPTS
  TYPE(CPG_MISC_TYPE), INTENT(INOUT) :: YDCPG_MISC
  TYPE(CPG_GPAR_TYPE), INTENT(INOUT) :: YDCPG_GPAR
  TYPE(CPG_PHY_TYPE), INTENT(IN) :: YDCPG_PHY0
  TYPE(MF_PHYS_TYPE), INTENT(INOUT) :: YDMF_PHYS
  TYPE(CPG_DYN_TYPE), INTENT(IN) :: YDCPG_DYN0
  TYPE(MF_PHYS_SURF_TYPE), INTENT(INOUT) :: YDMF_PHYS_SURF
  TYPE(CPG_SL2_TYPE), INTENT(INOUT) :: YDCPG_SL2
  TYPE(FIELD_VARIABLES), INTENT(INOUT) :: YDVARS
  TYPE(TCFU), INTENT(IN) :: YDCFU
  TYPE(MODEL), INTENT(IN) :: YDMODEL
  TYPE(TYP_DDH), INTENT(INOUT) :: YDDDH
  TYPE(TEPHYSMWAVE), INTENT(INOUT) :: YDPHYSMWAVE
  
#include "accldia.intfb.h"
#include "accldia_openacc.intfb.h"
#include "acclph.intfb.h"
#include "acclph_openacc.intfb.h"
#include "acdnshf.intfb.h"
#include "acdnshf_openacc.intfb.h"
#include "acdrag.intfb.h"
#include "acdrag_openacc.intfb.h"
#include "acdrme.intfb.h"
#include "acdrme_openacc.intfb.h"
#include "acevadcape.intfb.h"
#include "acevadcape_openacc.intfb.h"
#include "achmt.intfb.h"
#include "achmt_openacc.intfb.h"
#include "achmtls.intfb.h"
#include "achmtls_openacc.intfb.h"
#include "acpluis.intfb.h"
#include "acpluis_openacc.intfb.h"
#include "acsol.intfb.h"
#include "acsol_openacc.intfb.h"
#include "actqsat.intfb.h"
#include "actqsat_openacc.intfb.h"
#include "acvisih.intfb.h"
#include "acvisih_openacc.intfb.h"
#include "aplpar_init.intfb.h"
#include "aplpar_init_openacc.intfb.h"
#include "checkmv.intfb.h"
#include "cpphinp.intfb.h"
#include "cpphinp_openacc.intfb.h"
#include "cpqsol.intfb.h"
#include "mf_phys_bayrad.intfb.h"
#include "mf_phys_bayrad_openacc.intfb.h"
#include "mf_phys_corwat.intfb.h"
#include "mf_phys_cvv.intfb.h"
#include "mf_phys_fpl_part1_parallel.intfb.h"
#include "mf_phys_fpl_part2_parallel.intfb.h"
#include "mf_phys_mocon_parallel.intfb.h"
#include "mf_phys_precips.intfb.h"
#include "mf_phys_precips_openacc.intfb.h"
#include "mf_phys_save_phsurf_part1_parallel.intfb.h"
#include "mf_phys_save_phsurf_part2_parallel.intfb.h"
#include "mf_phys_transfer_parallel.intfb.h"
#include "ppwetpoint.intfb.h"
#include "ppwetpoint_openacc.intfb.h"
#include "qngcor.intfb.h"
#include "qngcor_openacc.intfb.h"
#include "apl_arpege_oceanic_fluxes_parallel.intfb.h"
#include "apl_wind_gust_parallel.intfb.h"
#include "apl_arpege_shallow_convection_and_turbulence_parallel.intfb.h"
#include "apl_arpege_albedo_computation_parallel.intfb.h"
#include "apl_arpege_aerosols_for_radiation_parallel.intfb.h"
#include "apl_arpege_cloudiness_parallel.intfb.h"
#include "apl_arpege_radiation_parallel.intfb.h"
#include "apl_arpege_soil_hydro_parallel.intfb.h"
#include "apl_arpege_surface_parallel.intfb.h"
#include "apl_arpege_deep_convection_parallel.intfb.h"
#include "apl_arpege_turbulence_part2.intfb.h"
#include "apl_arpege_turbulence_part2_openacc.intfb.h"
#include "apl_arpege_precipitation_parallel.intfb.h"
#include "apl_arpege_hydro_budget_parallel.intfb.h"
#include "apl_arpege_surface_update_parallel.intfb.h"
#include "apl_arpege_atmosphere_update_parallel.intfb.h"
#include "apl_arpege_init_parallel.intfb.h"
#include "apl_arpege_init_surfex_parallel.intfb.h"
#include "apl_arpege_dprecips_parallel.intfb.h"
#include "apl_arpege_mwave_parallel.intfb.h"
  
  
  CLASS(FIELD_4RB), POINTER :: YL_ZDIFEXT => NULL()
  REAL(KIND=JPRB), POINTER :: ZDIFEXT(:, :, :, :) => NULL()
  CLASS(FIELD_3IM), POINTER :: YL_INLAB_CVPP => NULL()
  INTEGER(KIND=JPIM), POINTER :: INLAB_CVPP(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZXTROV => NULL()
  REAL(KIND=JPRB), POINTER :: ZXTROV(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZXUROV => NULL()
  REAL(KIND=JPRB), POINTER :: ZXUROV(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZMRIPP => NULL()
  REAL(KIND=JPRB), POINTER :: ZMRIPP(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZKTROV => NULL()
  REAL(KIND=JPRB), POINTER :: ZKTROV(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZNBVNO => NULL()
  REAL(KIND=JPRB), POINTER :: ZNBVNO(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZKUROV => NULL()
  REAL(KIND=JPRB), POINTER :: ZKUROV(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZKUROV2 => NULL()
  REAL(KIND=JPRB), POINTER :: ZKUROV2(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZKQROV => NULL()
  REAL(KIND=JPRB), POINTER :: ZKQROV(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZKQLROV => NULL()
  REAL(KIND=JPRB), POINTER :: ZKQLROV(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZNEBS => NULL()
  REAL(KIND=JPRB), POINTER :: ZNEBS(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZQLIS => NULL()
  REAL(KIND=JPRB), POINTER :: ZQLIS(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZNEBS0 => NULL()
  REAL(KIND=JPRB), POINTER :: ZNEBS0(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZQLIS0 => NULL()
  REAL(KIND=JPRB), POINTER :: ZQLIS0(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZNEBC0 => NULL()
  REAL(KIND=JPRB), POINTER :: ZNEBC0(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZNEBDIFF => NULL()
  REAL(KIND=JPRB), POINTER :: ZNEBDIFF(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZNEBCH => NULL()
  REAL(KIND=JPRB), POINTER :: ZNEBCH(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZUNEBH => NULL()
  REAL(KIND=JPRB), POINTER :: ZUNEBH(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZFPCOR => NULL()
  REAL(KIND=JPRB), POINTER :: ZFPCOR(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZPOID => NULL()
  REAL(KIND=JPRB), POINTER :: ZPOID(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZQV => NULL()
  REAL(KIND=JPRB), POINTER :: ZQV(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZQI => NULL()
  REAL(KIND=JPRB), POINTER :: ZQI(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZQL => NULL()
  REAL(KIND=JPRB), POINTER :: ZQL(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZQR => NULL()
  REAL(KIND=JPRB), POINTER :: ZQR(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZQS => NULL()
  REAL(KIND=JPRB), POINTER :: ZQS(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZTENHA => NULL()
  REAL(KIND=JPRB), POINTER :: ZTENHA(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZTENQVA => NULL()
  REAL(KIND=JPRB), POINTER :: ZTENQVA(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZCP => NULL()
  REAL(KIND=JPRB), POINTER :: ZCP(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZFPLSL => NULL()
  REAL(KIND=JPRB), POINTER :: ZFPLSL(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZFPLSN => NULL()
  REAL(KIND=JPRB), POINTER :: ZFPLSN(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZSEDIQL => NULL()
  REAL(KIND=JPRB), POINTER :: ZSEDIQL(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZSEDIQI => NULL()
  REAL(KIND=JPRB), POINTER :: ZSEDIQI(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZDIFCVPPQ => NULL()
  REAL(KIND=JPRB), POINTER :: ZDIFCVPPQ(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZDIFCVPPS => NULL()
  REAL(KIND=JPRB), POINTER :: ZDIFCVPPS(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZDIFCVPPU => NULL()
  REAL(KIND=JPRB), POINTER :: ZDIFCVPPU(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZDIFCVPPV => NULL()
  REAL(KIND=JPRB), POINTER :: ZDIFCVPPV(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZEDMFQ => NULL()
  REAL(KIND=JPRB), POINTER :: ZEDMFQ(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZEDMFS => NULL()
  REAL(KIND=JPRB), POINTER :: ZEDMFS(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZEDMFU => NULL()
  REAL(KIND=JPRB), POINTER :: ZEDMFU(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZEDMFV => NULL()
  REAL(KIND=JPRB), POINTER :: ZEDMFV(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZMF_UP => NULL()
  REAL(KIND=JPRB), POINTER :: ZMF_UP(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZCONDCVPPL => NULL()
  REAL(KIND=JPRB), POINTER :: ZCONDCVPPL(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZCONDCVPPI => NULL()
  REAL(KIND=JPRB), POINTER :: ZCONDCVPPI(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZPRODTH_CVPP => NULL()
  REAL(KIND=JPRB), POINTER :: ZPRODTH_CVPP(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZPRODTH => NULL()
  REAL(KIND=JPRB), POINTER :: ZPRODTH(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZDE2MR => NULL()
  REAL(KIND=JPRB), POINTER :: ZDE2MR(:, :, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZXDROV => NULL()
  REAL(KIND=JPRB), POINTER :: ZXDROV(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZXHROV => NULL()
  REAL(KIND=JPRB), POINTER :: ZXHROV(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZUGST => NULL()
  REAL(KIND=JPRB), POINTER :: ZUGST(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZVGST => NULL()
  REAL(KIND=JPRB), POINTER :: ZVGST(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZCDROV => NULL()
  REAL(KIND=JPRB), POINTER :: ZCDROV(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZCHROV => NULL()
  REAL(KIND=JPRB), POINTER :: ZCHROV(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZDQSTS => NULL()
  REAL(KIND=JPRB), POINTER :: ZDQSTS(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZGWDCS => NULL()
  REAL(KIND=JPRB), POINTER :: ZGWDCS(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZHQ => NULL()
  REAL(KIND=JPRB), POINTER :: ZHQ(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZHU => NULL()
  REAL(KIND=JPRB), POINTER :: ZHU(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZHTR => NULL()
  REAL(KIND=JPRB), POINTER :: ZHTR(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZRTI => NULL()
  REAL(KIND=JPRB), POINTER :: ZRTI(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZDPHI => NULL()
  REAL(KIND=JPRB), POINTER :: ZDPHI(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZPRS => NULL()
  REAL(KIND=JPRB), POINTER :: ZPRS(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZSTAB => NULL()
  REAL(KIND=JPRB), POINTER :: ZSTAB(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZWFC => NULL()
  REAL(KIND=JPRB), POINTER :: ZWFC(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZWPMX => NULL()
  REAL(KIND=JPRB), POINTER :: ZWPMX(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZWLMX => NULL()
  REAL(KIND=JPRB), POINTER :: ZWLMX(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZWSEQ => NULL()
  REAL(KIND=JPRB), POINTER :: ZWSEQ(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZWSMX => NULL()
  REAL(KIND=JPRB), POINTER :: ZWSMX(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZWWILT => NULL()
  REAL(KIND=JPRB), POINTER :: ZWWILT(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZC3 => NULL()
  REAL(KIND=JPRB), POINTER :: ZC3(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZCG => NULL()
  REAL(KIND=JPRB), POINTER :: ZCG(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZCN => NULL()
  REAL(KIND=JPRB), POINTER :: ZCN(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZNEIJG => NULL()
  REAL(KIND=JPRB), POINTER :: ZNEIJG(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZNEIJV => NULL()
  REAL(KIND=JPRB), POINTER :: ZNEIJV(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZPCLS => NULL()
  REAL(KIND=JPRB), POINTER :: ZPCLS(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZBLH => NULL()
  REAL(KIND=JPRB), POINTER :: ZBLH(:, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZQO3 => NULL()
  REAL(KIND=JPRB), POINTER :: ZQO3(:, :, :) => NULL()
  CLASS(FIELD_4RB), POINTER :: YL_ZAER => NULL()
  REAL(KIND=JPRB), POINTER :: ZAER(:, :, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZAERINDS => NULL()
  REAL(KIND=JPRB), POINTER :: ZAERINDS(:, :, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZDPHIV => NULL()
  REAL(KIND=JPRB), POINTER :: ZDPHIV(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZDPHIT => NULL()
  REAL(KIND=JPRB), POINTER :: ZDPHIT(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZTRSOD => NULL()
  REAL(KIND=JPRB), POINTER :: ZTRSOD(:, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZCEMTR => NULL()
  REAL(KIND=JPRB), POINTER :: ZCEMTR(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZCTRSO => NULL()
  REAL(KIND=JPRB), POINTER :: ZCTRSO(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZALBD => NULL()
  REAL(KIND=JPRB), POINTER :: ZALBD(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZALBP => NULL()
  REAL(KIND=JPRB), POINTER :: ZALBP(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZSFSWDIR => NULL()
  REAL(KIND=JPRB), POINTER :: ZSFSWDIR(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZSFSWDIF => NULL()
  REAL(KIND=JPRB), POINTER :: ZSFSWDIF(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZTRSODIR => NULL()
  REAL(KIND=JPRB), POINTER :: ZTRSODIR(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZTRSODIF => NULL()
  REAL(KIND=JPRB), POINTER :: ZTRSODIF(:, :, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZSUDU => NULL()
  REAL(KIND=JPRB), POINTER :: ZSUDU(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZTHETAVS => NULL()
  REAL(KIND=JPRB), POINTER :: ZTHETAVS(:, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZGEOSLC => NULL()
  REAL(KIND=JPRB), POINTER :: ZGEOSLC(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZTENT => NULL()
  REAL(KIND=JPRB), POINTER :: ZTENT(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZTHETAV => NULL()
  REAL(KIND=JPRB), POINTER :: ZTHETAV(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZCOEFN => NULL()
  REAL(KIND=JPRB), POINTER :: ZCOEFN(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZQCO2 => NULL()
  REAL(KIND=JPRB), POINTER :: ZQCO2(:, :, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZALBDIR => NULL()
  REAL(KIND=JPRB), POINTER :: ZALBDIR(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZCLCT_RAD => NULL()
  REAL(KIND=JPRB), POINTER :: ZCLCT_RAD(:, :) => NULL()
  
  ! LOCAL ARRAYS FOR ACVPPKF
  CLASS(FIELD_3RB), POINTER :: YL_ZQLI_CVPP => NULL()
  REAL(KIND=JPRB), POINTER :: ZQLI_CVPP(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZNEB_CVPP => NULL()
  REAL(KIND=JPRB), POINTER :: ZNEB_CVPP(:, :, :) => NULL()
  
  ! Implicit coupling coefficients
  CLASS(FIELD_3RB), POINTER :: YL_ZCFBTH => NULL()
  REAL(KIND=JPRB), POINTER :: ZCFBTH(:, :, :) => NULL()
  REAL(KIND=JPRB) :: ZDTMSE
  REAL(KIND=JPRB) :: ZRHGMT
  REAL(KIND=JPRB) :: ZSTATI
  CLASS(FIELD_3RB), POINTER :: YL_ZCFATH => NULL()
  REAL(KIND=JPRB), POINTER :: ZCFATH(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZCFAU => NULL()
  REAL(KIND=JPRB), POINTER :: ZCFAU(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZCFBU => NULL()
  REAL(KIND=JPRB), POINTER :: ZCFBU(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZCFBV => NULL()
  REAL(KIND=JPRB), POINTER :: ZCFBV(:, :, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZSRAIN => NULL()
  REAL(KIND=JPRB), POINTER :: ZSRAIN(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZSSNOW => NULL()
  REAL(KIND=JPRB), POINTER :: ZSSNOW(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZSGROUPEL => NULL()
  REAL(KIND=JPRB), POINTER :: ZSGROUPEL(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZTSN => NULL()
  REAL(KIND=JPRB), POINTER :: ZTSN(:, :) => NULL()
  
  !         ACFLUSO (ECUME) local variable
  REAL(KIND=JPRB) :: ZCE(YDCPG_OPTS%KLON)
  CLASS(FIELD_2RB), POINTER :: YL_ZCEROV => NULL()
  REAL(KIND=JPRB), POINTER :: ZCEROV(:, :) => NULL()
  REAL(KIND=JPRB) :: ZCRTI(YDCPG_OPTS%KLON)
  
  !        New ACDIFV1 local variable
  CLASS(FIELD_3RB), POINTER :: YL_ZXURO => NULL()
  REAL(KIND=JPRB), POINTER :: ZXURO(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZXQRO => NULL()
  REAL(KIND=JPRB), POINTER :: ZXQRO(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZXTRO => NULL()
  REAL(KIND=JPRB), POINTER :: ZXTRO(:, :, :) => NULL()
  
  !        New ACNORGWD local variables
  CLASS(FIELD_3RB), POINTER :: YL_ZFLX_LOTT_GWU => NULL()
  REAL(KIND=JPRB), POINTER :: ZFLX_LOTT_GWU(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZFLX_LOTT_GWV => NULL()
  REAL(KIND=JPRB), POINTER :: ZFLX_LOTT_GWV(:, :, :) => NULL()
  
  !    TKE+ for ACCLDIA
  CLASS(FIELD_3RB), POINTER :: YL_ZTKE1 => NULL()
  REAL(KIND=JPRB), POINTER :: ZTKE1(:, :, :) => NULL()
  
  !   For ACVISIH
  CLASS(FIELD_3RB), POINTER :: YL_ZQGM => NULL()
  REAL(KIND=JPRB), POINTER :: ZQGM(:, :, :) => NULL()
  
  
  !        New ARP_GROUND_PARAM local variable
  CLASS(FIELD_3RB), POINTER :: YL_ZALPHA1 => NULL()
  REAL(KIND=JPRB), POINTER :: ZALPHA1(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZCOEFA => NULL()
  REAL(KIND=JPRB), POINTER :: ZCOEFA(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZLVT => NULL()
  REAL(KIND=JPRB), POINTER :: ZLVT(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZQICE => NULL()
  REAL(KIND=JPRB), POINTER :: ZQICE(:, :, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZDIFWQ => NULL()
  REAL(KIND=JPRB), POINTER :: ZDIFWQ(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZDIFWS => NULL()
  REAL(KIND=JPRB), POINTER :: ZDIFWS(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZSC_FEVI => NULL()
  REAL(KIND=JPRB), POINTER :: ZSC_FEVI(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZSC_FEVN => NULL()
  REAL(KIND=JPRB), POINTER :: ZSC_FEVN(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZSC_FCLL => NULL()
  REAL(KIND=JPRB), POINTER :: ZSC_FCLL(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZSC_FCLN => NULL()
  REAL(KIND=JPRB), POINTER :: ZSC_FCLN(:, :) => NULL()
  
  !           TRAJECTORY (For diffusion !) local VARIABLES
  CLASS(FIELD_3RB), POINTER :: YL_ZTRAJGWD => NULL()
  REAL(KIND=JPRB), POINTER :: ZTRAJGWD(:, :, :) => NULL()
  
  CLASS(FIELD_2RB), POINTER :: YL_ZAIPCMT => NULL()
  REAL(KIND=JPRB), POINTER :: ZAIPCMT(:, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZQIC => NULL()
  REAL(KIND=JPRB), POINTER :: ZQIC(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZQLC => NULL()
  REAL(KIND=JPRB), POINTER :: ZQLC(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZQLI_CVP => NULL()
  REAL(KIND=JPRB), POINTER :: ZQLI_CVP(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZQC_DET_PCMT => NULL()
  REAL(KIND=JPRB), POINTER :: ZQC_DET_PCMT(:, :, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZDCAPE => NULL()
  REAL(KIND=JPRB), POINTER :: ZDCAPE(:, :) => NULL()
  
  ! ACRANEB/ACRANEB2 local variables
  CLASS(FIELD_2RB), POINTER :: YL_ZDECRD => NULL()
  REAL(KIND=JPRB), POINTER :: ZDECRD(:, :) => NULL()
  
  ! Precipitation type diagnostics
  CLASS(FIELD_2IM), POINTER :: YL_IMOC_CLPH => NULL()
  INTEGER(KIND=JPIM), POINTER :: IMOC_CLPH(:, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZBAY_QRCONV => NULL()
  REAL(KIND=JPRB), POINTER :: ZBAY_QRCONV(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZBAY_QSCONV => NULL()
  REAL(KIND=JPRB), POINTER :: ZBAY_QSCONV(:, :, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZDSA_C1 => NULL()
  REAL(KIND=JPRB), POINTER :: ZDSA_C1(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZDSA_C2 => NULL()
  REAL(KIND=JPRB), POINTER :: ZDSA_C2(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZDSA_CPS => NULL()
  REAL(KIND=JPRB), POINTER :: ZDSA_CPS(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZDSA_LHS => NULL()
  REAL(KIND=JPRB), POINTER :: ZDSA_LHS(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZDSA_RS => NULL()
  REAL(KIND=JPRB), POINTER :: ZDSA_RS(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZFLU_CDN => NULL()
  REAL(KIND=JPRB), POINTER :: ZFLU_CDN(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZFLU_CD => NULL()
  REAL(KIND=JPRB), POINTER :: ZFLU_CD(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZFLU_CHN => NULL()
  REAL(KIND=JPRB), POINTER :: ZFLU_CHN(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZFLU_CH => NULL()
  REAL(KIND=JPRB), POINTER :: ZFLU_CH(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZFLU_EMIS => NULL()
  REAL(KIND=JPRB), POINTER :: ZFLU_EMIS(:, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZFLU_FEVI => NULL()
  REAL(KIND=JPRB), POINTER :: ZFLU_FEVI(:, :, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZFLU_NEIJ => NULL()
  REAL(KIND=JPRB), POINTER :: ZFLU_NEIJ(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZFLU_QSATS => NULL()
  REAL(KIND=JPRB), POINTER :: ZFLU_QSATS(:, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZFLU_QSAT => NULL()
  REAL(KIND=JPRB), POINTER :: ZFLU_QSAT(:, :, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZFLU_VEG => NULL()
  REAL(KIND=JPRB), POINTER :: ZFLU_VEG(:, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZMSC_FRMQ => NULL()
  REAL(KIND=JPRB), POINTER :: ZMSC_FRMQ(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZFCHQ => NULL()
  REAL(KIND=JPRB), POINTER :: ZFCHQ(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZMSC_LH => NULL()
  REAL(KIND=JPRB), POINTER :: ZMSC_LH(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZMSC_LSCPE => NULL()
  REAL(KIND=JPRB), POINTER :: ZMSC_LSCPE(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZMSC_QW => NULL()
  REAL(KIND=JPRB), POINTER :: ZMSC_QW(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZMSC_TW => NULL()
  REAL(KIND=JPRB), POINTER :: ZMSC_TW(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZPFL_FEFB1 => NULL()
  REAL(KIND=JPRB), POINTER :: ZPFL_FEFB1(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZPFL_FEFB2 => NULL()
  REAL(KIND=JPRB), POINTER :: ZPFL_FEFB2(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZPFL_FEFB3 => NULL()
  REAL(KIND=JPRB), POINTER :: ZPFL_FEFB3(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZPFL_FPLCH => NULL()
  REAL(KIND=JPRB), POINTER :: ZPFL_FPLCH(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZPFL_FPLSH => NULL()
  REAL(KIND=JPRB), POINTER :: ZPFL_FPLSH(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZPFL_FTKEI => NULL()
  REAL(KIND=JPRB), POINTER :: ZPFL_FTKEI(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZPFL_FTKE => NULL()
  REAL(KIND=JPRB), POINTER :: ZPFL_FTKE(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZPRC_DPRECIPS2 => NULL()
  REAL(KIND=JPRB), POINTER :: ZPRC_DPRECIPS2(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZPRC_DPRECIPS => NULL()
  REAL(KIND=JPRB), POINTER :: ZPRC_DPRECIPS(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZRDG_CVGQ => NULL()
  REAL(KIND=JPRB), POINTER :: ZRDG_CVGQ(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZRDG_CVGT => NULL()
  REAL(KIND=JPRB), POINTER :: ZRDG_CVGT(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZRDG_LCVQ => NULL()
  REAL(KIND=JPRB), POINTER :: ZRDG_LCVQ(:, :, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZRDG_MU0LU => NULL()
  REAL(KIND=JPRB), POINTER :: ZRDG_MU0LU(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZRDG_MU0M => NULL()
  REAL(KIND=JPRB), POINTER :: ZRDG_MU0M(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZRDG_MU0N => NULL()
  REAL(KIND=JPRB), POINTER :: ZRDG_MU0N(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZRDG_MU0 => NULL()
  REAL(KIND=JPRB), POINTER :: ZRDG_MU0(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZRDG_SOLO => NULL()
  REAL(KIND=JPRB), POINTER :: ZRDG_SOLO(:, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZSAV_DDAL => NULL()
  REAL(KIND=JPRB), POINTER :: ZSAV_DDAL(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZSAV_DDOM => NULL()
  REAL(KIND=JPRB), POINTER :: ZSAV_DDOM(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZSAV_ENTCH => NULL()
  REAL(KIND=JPRB), POINTER :: ZSAV_ENTCH(:, :, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZSAV_FHPS => NULL()
  REAL(KIND=JPRB), POINTER :: ZSAV_FHPS(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZSAV_GZ0F => NULL()
  REAL(KIND=JPRB), POINTER :: ZSAV_GZ0F(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZSAV_GZ0HF => NULL()
  REAL(KIND=JPRB), POINTER :: ZSAV_GZ0HF(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZSAV_HV => NULL()
  REAL(KIND=JPRB), POINTER :: ZSAV_HV(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZSAV_PBLH => NULL()
  REAL(KIND=JPRB), POINTER :: ZSAV_PBLH(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZSAV_QSH => NULL()
  REAL(KIND=JPRB), POINTER :: ZSAV_QSH(:, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZSAV_UDAL => NULL()
  REAL(KIND=JPRB), POINTER :: ZSAV_UDAL(:, :, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZSAV_UDGRO => NULL()
  REAL(KIND=JPRB), POINTER :: ZSAV_UDGRO(:, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZSAV_UDOM => NULL()
  REAL(KIND=JPRB), POINTER :: ZSAV_UDOM(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZSAV_UNEBH => NULL()
  REAL(KIND=JPRB), POINTER :: ZSAV_UNEBH(:, :, :) => NULL()
  
  INTEGER(KIND=JPIM) :: INSTEP_DEB
  INTEGER(KIND=JPIM) :: INSTEP_FIN
  INTEGER(KIND=JPIM) :: JLEV
  INTEGER(KIND=JPIM) :: JLON
  INTEGER(KIND=JPIM) :: JSPP
  LOGICAL :: LLCLS
  LOGICAL :: LLHMT
  LOGICAL :: LLREDPR
  REAL(KIND=JPRB) :: ZEPS0
  REAL(KIND=JPRB) :: ZEPSNEB
  REAL(KIND=JPRB) :: ZRVMD
  
  ! Dissipation length for actke1 et actke2
  CLASS(FIELD_3RB), POINTER :: YL_ZUSLE => NULL()
  REAL(KIND=JPRB), POINTER :: ZUSLE(:, :, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZKCLS => NULL()
  REAL(KIND=JPRB), POINTER :: ZKCLS(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZECTCLS => NULL()
  REAL(KIND=JPRB), POINTER :: ZECTCLS(:, :) => NULL()
  
  REAL(KIND=JPHOOK) :: ZHOOK_HANDLE
  TYPE(CPG_BNDS_TYPE) :: YLCPG_BNDS
  TYPE(STACK) :: YLSTACK
  INTEGER(KIND=JPIM) :: JBLK
  REAL(KIND=JPHOOK) :: ZHOOK_HANDLE_FIELD_API
  REAL(KIND=JPHOOK) :: ZHOOK_HANDLE_PARALLEL
  REAL(KIND=JPHOOK) :: ZHOOK_HANDLE_COMPUTE
  REAL(KIND=JPRB), POINTER :: Z_YDCPG_DYN0_CTY_EVEL(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDVARS_CVGQ_DM(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDVARS_GEOMETRY_GEMU_T0(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDVARS_CVGQ_DL(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDCPG_PHY0_XYB_RDELP(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDVARS_T_DL(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDVARS_Q_DL(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDVARS_GEOMETRY_GELAM_T0(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDVARS_CVGQ_T0(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDVARS_Q_T0(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDVARS_U_T0(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDVARS_Q_DM(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDVARS_V_T0(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDVARS_T_DM(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_SURF_GSD_VF_PVEG(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_SURF_GSD_VF_PLSM(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_BASE_STATE_Q(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_BASE_STATE_T(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDCPG_MISC_RH(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_SURF_GSD_VV_PIVEG(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_BASE_STATE_YGSP_SG_R(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_SURF_GSD_VV_PARG(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_SURF_GSD_VV_PSAB(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_BASE_STATE_YGSP_RR_W(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_BASE_STATE_YGSP_RR_IC(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_BASE_STATE_YGSP_SG_F(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_CT(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_BASE_STATE_YGSP_SB_TL(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_SURF_GSD_VV_PD2(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_SURF_GSD_VF_PZ0RLF(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_BASE_STATE_YGSP_SB_Q(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_SURF_GSD_VF_PZ0F(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_BASE_STATE_YGSP_SG_A(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_SURF_GSD_VV_PLAI(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_SURF_GSD_VV_PZ0H(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDCPG_MISC_QS(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_GZ0H(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_BASE_STATE_U(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_GZ0(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_BASE_STATE_V(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_UCLS(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_QCLS(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_NVCLS(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_VGST(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_VCLS(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_SURF_GSD_VV_PHV(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_NUCLS(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_UGST(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_RHCLS(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_TCLS(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_VEIN(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_CLPH(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_DIFTQ(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_STRTU(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_DIFTS(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_STRTV(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_FRTH(:, :, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_FRSOPT(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_FRTHDS(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_FPEVPSL(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_SURF_GSD_VF_PVRLAN(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_SURF_GSD_VF_PVRLDI(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDVARS_GEOMETRY_RCORI_T0(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_STRDU(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_DRNSHF(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_FCSQL(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_FPEVPSN(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_SURF_GSD_VF_PGETRL(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_FPLSL(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_STRDV(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_FPFPSN(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_FPFPSL(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_FPLSN(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_FCSQN(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_STRCV(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_STRCU(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_DIFCS(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_BASE_STATE_TKE(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_EDR(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_DIFCQ(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_DIFTQN(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_FPEVPCL(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_DIFTQL(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_DIFCQN(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_FPEVPCN(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_FPLSG(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_FCCQN(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_FPFPCL(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_DIFCQL(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_FPFPCN(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_FCCQL(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_FCQNG(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDVARS_A_T1(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_FRMH(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_STRMU(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_STRMV(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDCPG_MISC_QLI(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDVARS_LRAD_T1(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDVARS_IRAD_T1(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDCPG_MISC_QICE(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDCPG_MISC_NEB(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_FPLCL(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_CAPE(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_FPLCN(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDVARS_GEOMETRY_OROG_T0(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_VISICLD(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_MXCLWC(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_VISIHYD(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_BASE_STATE_I(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_BASE_STATE_L(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_TPWCLS(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_FEVN(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_OUT_FEVL(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_SURF_GSD_VH_PPCN(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_SURF_GSD_VH_PPSL(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_SURF_GSD_VH_PPSN(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_SURF_GSD_VH_PEVA(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_SURF_GSD_VH_PPCL(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDCPG_PHY0_PREHYD(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_SURF_GSP_RR_PT_T0(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDCPG_MISC_QSOL(:, :)
  REAL(KIND=JPRB), POINTER :: Z_YDVARS_CVV_T0(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDVARS_CVV_T1(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDVARS_SCONV_T1(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDVARS_RCONV_T1(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_SURF_GSD_DI_PXEDR(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_SURF_GSD_XP_PPRECIP(:, :, :)
  REAL(KIND=JPRB), POINTER :: Z_YDMF_PHYS_SURF_GSD_XP2_PPRECIP2(:, :, :)
  
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE', 0, ZHOOK_HANDLE)
PRINT *,"apl_arpege_parallel_dbg ::: 1"
  IF (LHOOK) CALL DR_HOOK('CREATE_TEMPORARIES', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 2"
  
  CALL FIELD_NEW(YL_ZRDG_MU0, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZRDG_CVGQ, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZRDG_MU0M, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZRDG_MU0LU, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZRDG_MU0N, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZRDG_CVGT, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZRDG_SOLO, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZFLU_QSATS, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZRDG_LCVQ, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZQCO2, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZDSA_CPS, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZDSA_C2, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZDSA_LHS, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZFLU_EMIS, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZPFL_FEFB1, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZFLU_QSAT, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZFLU_CH, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZFLU_VEG, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZPFL_FEFB3, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZDSA_C1, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZMSC_QW, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZPFL_FTKEI, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZFLU_NEIJ, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZMSC_LH, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZDSA_RS, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZMSC_FRMQ, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZMSC_LSCPE, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZMSC_TW, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZFLU_CHN, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZPFL_FTKE, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZFLU_FEVI, UBOUNDS=(/ YDCPG_OPTS%KLON, YDMODEL%YRML_PHY_G%YRDPHY%NTSSG + 1, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZFLU_CDN, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZPFL_FEFB2, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZFLU_CD, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_IMOC_CLPH, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZFCHQ, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZQV, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZGEOSLC, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZC3, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZWSEQ, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZCG, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZWFC, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZNEIJV, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZWWILT, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZNEIJG, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZCN, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZWSMX, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZWPMX, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZDPHIV, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZDPHIT, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZGWDCS, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZRTI, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZNBVNO, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZDPHI, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZPRS, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZPCLS, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZMRIPP, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZHQ, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZDQSTS, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZCHROV, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZSTAB, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZHU, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZXDROV, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZXHROV, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZCDROV, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZPFL_FPLSH, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZPFL_FPLCH, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZUGST, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZCP, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZBLH, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZTHETAVS, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZTHETAV, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZVGST, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZEDMFQ, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZEDMFU, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZDIFCVPPQ, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZDIFCVPPV, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZEDMFV, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZDIFCVPPS, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZDIFCVPPU, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZEDMFS, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZQLIS, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZTRAJGWD, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZNEBS, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZECTCLS, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZKCLS, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZKUROV2, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZTKE1, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZUSLE, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZPRODTH, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZQI, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZSEDIQI, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZFPCOR, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZFLX_LOTT_GWV, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZQL, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZSEDIQL, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZFPLSN, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZFLX_LOTT_GWU, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZFPLSL, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZDCAPE, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZQR, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZQS, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZQGM, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZBAY_QSCONV, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZBAY_QRCONV, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZDE2MR, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZPRC_DPRECIPS, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%NDTPREC, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZPRC_DPRECIPS2, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%NDTPREC2, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZSAV_QSH, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZSAV_DDAL, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZSAV_UDAL, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZSAV_ENTCH, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZSAV_FHPS, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZSAV_HV, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZSAV_PBLH, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZSAV_GZ0HF, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZSAV_DDOM, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZSAV_UDOM, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZSAV_GZ0F, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZSAV_UNEBH, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZSAV_UDGRO, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZNEB_CVPP, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZXUROV, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZCFBTH, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZKUROV, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZSC_FEVI, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_INLAB_CVPP, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZKTROV, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZCFBV, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZNEBCH, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZQC_DET_PCMT, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZQO3, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZXTROV, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZPRODTH_CVPP, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZQLI_CVP, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZXQRO, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZTENQVA, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZTENHA, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZKQROV, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZTRSODIF, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KSW, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZNEBS0, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZQICE, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZXTRO, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZTRSODIR, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KSW, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZQLIS0, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZSFSWDIF, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KSW, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZAERINDS, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZXURO, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZNEBDIFF, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZCFATH, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZCFBU, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZDIFWQ, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZAER, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, 6, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZPOID, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZMF_UP, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZCONDCVPPI, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZSC_FCLN, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZCOEFA, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZQIC, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZQLC, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZSFSWDIR, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KSW, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZUNEBH, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZCTRSO, UBOUNDS=(/ YDCPG_OPTS%KLON, 1, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZCFAU, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZAIPCMT, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZSC_FCLL, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZDIFWS, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZSUDU, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZALPHA1, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZTRSOD, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZALBD, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KSW, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZNEBC0, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZCEMTR, UBOUNDS=(/ YDCPG_OPTS%KLON, 1, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZALBP, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KSW, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZLVT, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZQLI_CVPP, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZTENT, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZKQLROV, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZSC_FEVN, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZCONDCVPPL, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZDECRD, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZSGROUPEL, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZTSN, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZSSNOW, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZSRAIN, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZCEROV, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZCOEFN, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZALBDIR, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZCLCT_RAD, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZWLMX, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZHTR, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZDIFEXT,  &
  & UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDMODEL%YRML_GCONF%YGFL%NGFL_EXT, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  IF (LHOOK) CALL DR_HOOK('CREATE_TEMPORARIES', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 3"
  
  
  
  
  
  
  
  INSTEP_DEB = 1
  INSTEP_FIN = 1
  
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CPPHINP', 0, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 4"
  IF (LPARALLELMETHOD('OPENMP', 'APL_ARPEGE_PARALLEL:CPPHINP')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CPPHINP:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 5"
    ZRDG_MU0 => GET_HOST_DATA_RDWR(YL_ZRDG_MU0)
    ZRDG_CVGQ => GET_HOST_DATA_RDWR(YL_ZRDG_CVGQ)
    ZRDG_MU0M => GET_HOST_DATA_RDWR(YL_ZRDG_MU0M)
    ZRDG_MU0LU => GET_HOST_DATA_RDWR(YL_ZRDG_MU0LU)
    ZRDG_MU0N => GET_HOST_DATA_RDWR(YL_ZRDG_MU0N)
    ZRDG_CVGT => GET_HOST_DATA_RDWR(YL_ZRDG_CVGT)
    ZRDG_SOLO => GET_HOST_DATA_RDWR(YL_ZRDG_SOLO)
    Z_YDCPG_DYN0_CTY_EVEL => GET_HOST_DATA_RDWR(YDCPG_DYN0%CTY%F_EVEL)
    Z_YDVARS_CVGQ_DM => GET_HOST_DATA_RDWR(YDVARS%CVGQ%FDM)
    Z_YDVARS_GEOMETRY_GEMU_T0 => GET_HOST_DATA_RDWR(YDVARS%GEOMETRY%GEMU%FT0)
    Z_YDVARS_CVGQ_DL => GET_HOST_DATA_RDWR(YDVARS%CVGQ%FDL)
    Z_YDCPG_PHY0_XYB_RDELP => GET_HOST_DATA_RDWR(YDCPG_PHY0%XYB%F_RDELP)
    Z_YDVARS_T_DL => GET_HOST_DATA_RDWR(YDVARS%T%FDL)
    Z_YDVARS_Q_DL => GET_HOST_DATA_RDWR(YDVARS%Q%FDL)
    Z_YDVARS_GEOMETRY_GELAM_T0 => GET_HOST_DATA_RDWR(YDVARS%GEOMETRY%GELAM%FT0)
    Z_YDVARS_CVGQ_T0 => GET_HOST_DATA_RDWR(YDVARS%CVGQ%FT0)
    Z_YDVARS_Q_T0 => GET_HOST_DATA_RDWR(YDVARS%Q%FT0)
    Z_YDVARS_U_T0 => GET_HOST_DATA_RDWR(YDVARS%U%FT0)
    Z_YDVARS_Q_DM => GET_HOST_DATA_RDWR(YDVARS%Q%FDM)
    Z_YDVARS_V_T0 => GET_HOST_DATA_RDWR(YDVARS%V%FT0)
    Z_YDVARS_T_DM => GET_HOST_DATA_RDWR(YDVARS%T%FDM)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CPPHINP:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 6"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CPPHINP:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 7"
    CALL YLCPG_BNDS%INIT(YDCPG_OPTS)
!$OMP PARALLEL DO PRIVATE( JBLK ) FIRSTPRIVATE( YLCPG_BNDS )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      CALL YLCPG_BNDS%UPDATE(JBLK)
      
      CALL CPPHINP(YDGEOMETRY, YDMODEL, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, Z_YDVARS_GEOMETRY_GEMU_T0(:, JBLK),  &
      & Z_YDVARS_GEOMETRY_GELAM_T0(:, JBLK), Z_YDVARS_U_T0(:, :, JBLK), Z_YDVARS_V_T0(:, :, JBLK),  &
      & Z_YDVARS_T_DL(:, :, JBLK), Z_YDVARS_T_DM(:, :, JBLK), Z_YDVARS_Q_T0(:, :, JBLK), Z_YDVARS_Q_DL(:, :, JBLK),  &
      & Z_YDVARS_Q_DM(:, :, JBLK), Z_YDVARS_CVGQ_DL(:, :, JBLK), Z_YDVARS_CVGQ_DM(:, :, JBLK),  &
      & Z_YDCPG_PHY0_XYB_RDELP(:, :, JBLK), Z_YDCPG_DYN0_CTY_EVEL(:, :, JBLK), Z_YDVARS_CVGQ_T0(:, :, JBLK),  &
      & ZRDG_MU0(:, JBLK), ZRDG_SOLO(:, JBLK), ZRDG_MU0LU(:, JBLK), ZRDG_MU0M(:, JBLK), ZRDG_MU0N(:, JBLK),  &
      & ZRDG_CVGQ(:, :, JBLK), ZRDG_CVGT(:, :, JBLK))
      
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CPPHINP:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 8"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:CPPHINP')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CPPHINP:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 9"
      ZRDG_MU0 => GET_HOST_DATA_RDWR(YL_ZRDG_MU0)
      ZRDG_CVGQ => GET_HOST_DATA_RDWR(YL_ZRDG_CVGQ)
      ZRDG_MU0M => GET_HOST_DATA_RDWR(YL_ZRDG_MU0M)
      ZRDG_MU0LU => GET_HOST_DATA_RDWR(YL_ZRDG_MU0LU)
      ZRDG_MU0N => GET_HOST_DATA_RDWR(YL_ZRDG_MU0N)
      ZRDG_CVGT => GET_HOST_DATA_RDWR(YL_ZRDG_CVGT)
      ZRDG_SOLO => GET_HOST_DATA_RDWR(YL_ZRDG_SOLO)
      Z_YDCPG_DYN0_CTY_EVEL => GET_HOST_DATA_RDWR(YDCPG_DYN0%CTY%F_EVEL)
      Z_YDVARS_CVGQ_DM => GET_HOST_DATA_RDWR(YDVARS%CVGQ%FDM)
      Z_YDVARS_GEOMETRY_GEMU_T0 => GET_HOST_DATA_RDWR(YDVARS%GEOMETRY%GEMU%FT0)
      Z_YDVARS_CVGQ_DL => GET_HOST_DATA_RDWR(YDVARS%CVGQ%FDL)
      Z_YDCPG_PHY0_XYB_RDELP => GET_HOST_DATA_RDWR(YDCPG_PHY0%XYB%F_RDELP)
      Z_YDVARS_T_DL => GET_HOST_DATA_RDWR(YDVARS%T%FDL)
      Z_YDVARS_Q_DL => GET_HOST_DATA_RDWR(YDVARS%Q%FDL)
      Z_YDVARS_GEOMETRY_GELAM_T0 => GET_HOST_DATA_RDWR(YDVARS%GEOMETRY%GELAM%FT0)
      Z_YDVARS_CVGQ_T0 => GET_HOST_DATA_RDWR(YDVARS%CVGQ%FT0)
      Z_YDVARS_Q_T0 => GET_HOST_DATA_RDWR(YDVARS%Q%FT0)
      Z_YDVARS_U_T0 => GET_HOST_DATA_RDWR(YDVARS%U%FT0)
      Z_YDVARS_Q_DM => GET_HOST_DATA_RDWR(YDVARS%Q%FDM)
      Z_YDVARS_V_T0 => GET_HOST_DATA_RDWR(YDVARS%V%FT0)
      Z_YDVARS_T_DM => GET_HOST_DATA_RDWR(YDVARS%T%FDM)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CPPHINP:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 10"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CPPHINP:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 11"
    ZRDG_MU0 => NULL()
    ZRDG_CVGQ => NULL()
    ZRDG_MU0M => NULL()
    ZRDG_MU0LU => NULL()
    ZRDG_MU0N => NULL()
    ZRDG_CVGT => NULL()
    ZRDG_SOLO => NULL()
    Z_YDCPG_DYN0_CTY_EVEL => NULL()
    Z_YDVARS_CVGQ_DM => NULL()
    Z_YDVARS_GEOMETRY_GEMU_T0 => NULL()
    Z_YDVARS_CVGQ_DL => NULL()
    Z_YDCPG_PHY0_XYB_RDELP => NULL()
    Z_YDVARS_T_DL => NULL()
    Z_YDVARS_Q_DL => NULL()
    Z_YDVARS_GEOMETRY_GELAM_T0 => NULL()
    Z_YDVARS_CVGQ_T0 => NULL()
    Z_YDVARS_Q_T0 => NULL()
    Z_YDVARS_U_T0 => NULL()
    Z_YDVARS_Q_DM => NULL()
    Z_YDVARS_V_T0 => NULL()
    Z_YDVARS_T_DM => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CPPHINP:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 12"
  ELSE IF (LPARALLELMETHOD('OPENMPSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:CPPHINP')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CPPHINP:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 13"
    ZRDG_MU0 => GET_HOST_DATA_RDWR(YL_ZRDG_MU0)
    ZRDG_CVGQ => GET_HOST_DATA_RDWR(YL_ZRDG_CVGQ)
    ZRDG_MU0M => GET_HOST_DATA_RDWR(YL_ZRDG_MU0M)
    ZRDG_MU0LU => GET_HOST_DATA_RDWR(YL_ZRDG_MU0LU)
    ZRDG_MU0N => GET_HOST_DATA_RDWR(YL_ZRDG_MU0N)
    ZRDG_CVGT => GET_HOST_DATA_RDWR(YL_ZRDG_CVGT)
    ZRDG_SOLO => GET_HOST_DATA_RDWR(YL_ZRDG_SOLO)
    Z_YDCPG_DYN0_CTY_EVEL => GET_HOST_DATA_RDWR(YDCPG_DYN0%CTY%F_EVEL)
    Z_YDVARS_CVGQ_DM => GET_HOST_DATA_RDWR(YDVARS%CVGQ%FDM)
    Z_YDVARS_GEOMETRY_GEMU_T0 => GET_HOST_DATA_RDWR(YDVARS%GEOMETRY%GEMU%FT0)
    Z_YDVARS_CVGQ_DL => GET_HOST_DATA_RDWR(YDVARS%CVGQ%FDL)
    Z_YDCPG_PHY0_XYB_RDELP => GET_HOST_DATA_RDWR(YDCPG_PHY0%XYB%F_RDELP)
    Z_YDVARS_T_DL => GET_HOST_DATA_RDWR(YDVARS%T%FDL)
    Z_YDVARS_Q_DL => GET_HOST_DATA_RDWR(YDVARS%Q%FDL)
    Z_YDVARS_GEOMETRY_GELAM_T0 => GET_HOST_DATA_RDWR(YDVARS%GEOMETRY%GELAM%FT0)
    Z_YDVARS_CVGQ_T0 => GET_HOST_DATA_RDWR(YDVARS%CVGQ%FT0)
    Z_YDVARS_Q_T0 => GET_HOST_DATA_RDWR(YDVARS%Q%FT0)
    Z_YDVARS_U_T0 => GET_HOST_DATA_RDWR(YDVARS%U%FT0)
    Z_YDVARS_Q_DM => GET_HOST_DATA_RDWR(YDVARS%Q%FDM)
    Z_YDVARS_V_T0 => GET_HOST_DATA_RDWR(YDVARS%V%FT0)
    Z_YDVARS_T_DM => GET_HOST_DATA_RDWR(YDVARS%T%FDM)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CPPHINP:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 14"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CPPHINP:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 15"
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        CALL CPPHINP_OPENACC(YDGEOMETRY, YDMODEL, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, Z_YDVARS_GEOMETRY_GEMU_T0(:, JBLK),  &
        & Z_YDVARS_GEOMETRY_GELAM_T0(:, JBLK), Z_YDVARS_U_T0(:, :, JBLK), Z_YDVARS_V_T0(:, :, JBLK),  &
        & Z_YDVARS_T_DL(:, :, JBLK), Z_YDVARS_T_DM(:, :, JBLK), Z_YDVARS_Q_T0(:, :, JBLK), Z_YDVARS_Q_DL(:, :, JBLK),  &
        & Z_YDVARS_Q_DM(:, :, JBLK), Z_YDVARS_CVGQ_DL(:, :, JBLK), Z_YDVARS_CVGQ_DM(:, :, JBLK),  &
        & Z_YDCPG_PHY0_XYB_RDELP(:, :, JBLK), Z_YDCPG_DYN0_CTY_EVEL(:, :, JBLK), Z_YDVARS_CVGQ_T0(:, :, JBLK),  &
        & ZRDG_MU0(:, JBLK), ZRDG_SOLO(:, JBLK), ZRDG_MU0LU(:, JBLK), ZRDG_MU0M(:, JBLK), ZRDG_MU0N(:, JBLK),  &
        & ZRDG_CVGQ(:, :, JBLK), ZRDG_CVGT(:, :, JBLK), YDSTACK=YLSTACK)
        
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CPPHINP:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 16"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:CPPHINP')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CPPHINP:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 17"
      ZRDG_MU0 => GET_HOST_DATA_RDWR(YL_ZRDG_MU0)
      ZRDG_CVGQ => GET_HOST_DATA_RDWR(YL_ZRDG_CVGQ)
      ZRDG_MU0M => GET_HOST_DATA_RDWR(YL_ZRDG_MU0M)
      ZRDG_MU0LU => GET_HOST_DATA_RDWR(YL_ZRDG_MU0LU)
      ZRDG_MU0N => GET_HOST_DATA_RDWR(YL_ZRDG_MU0N)
      ZRDG_CVGT => GET_HOST_DATA_RDWR(YL_ZRDG_CVGT)
      ZRDG_SOLO => GET_HOST_DATA_RDWR(YL_ZRDG_SOLO)
      Z_YDCPG_DYN0_CTY_EVEL => GET_HOST_DATA_RDWR(YDCPG_DYN0%CTY%F_EVEL)
      Z_YDVARS_CVGQ_DM => GET_HOST_DATA_RDWR(YDVARS%CVGQ%FDM)
      Z_YDVARS_GEOMETRY_GEMU_T0 => GET_HOST_DATA_RDWR(YDVARS%GEOMETRY%GEMU%FT0)
      Z_YDVARS_CVGQ_DL => GET_HOST_DATA_RDWR(YDVARS%CVGQ%FDL)
      Z_YDCPG_PHY0_XYB_RDELP => GET_HOST_DATA_RDWR(YDCPG_PHY0%XYB%F_RDELP)
      Z_YDVARS_T_DL => GET_HOST_DATA_RDWR(YDVARS%T%FDL)
      Z_YDVARS_Q_DL => GET_HOST_DATA_RDWR(YDVARS%Q%FDL)
      Z_YDVARS_GEOMETRY_GELAM_T0 => GET_HOST_DATA_RDWR(YDVARS%GEOMETRY%GELAM%FT0)
      Z_YDVARS_CVGQ_T0 => GET_HOST_DATA_RDWR(YDVARS%CVGQ%FT0)
      Z_YDVARS_Q_T0 => GET_HOST_DATA_RDWR(YDVARS%Q%FT0)
      Z_YDVARS_U_T0 => GET_HOST_DATA_RDWR(YDVARS%U%FT0)
      Z_YDVARS_Q_DM => GET_HOST_DATA_RDWR(YDVARS%Q%FDM)
      Z_YDVARS_V_T0 => GET_HOST_DATA_RDWR(YDVARS%V%FT0)
      Z_YDVARS_T_DM => GET_HOST_DATA_RDWR(YDVARS%T%FDM)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CPPHINP:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 18"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CPPHINP:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 19"
    ZRDG_MU0 => NULL()
    ZRDG_CVGQ => NULL()
    ZRDG_MU0M => NULL()
    ZRDG_MU0LU => NULL()
    ZRDG_MU0N => NULL()
    ZRDG_CVGT => NULL()
    ZRDG_SOLO => NULL()
    Z_YDCPG_DYN0_CTY_EVEL => NULL()
    Z_YDVARS_CVGQ_DM => NULL()
    Z_YDVARS_GEOMETRY_GEMU_T0 => NULL()
    Z_YDVARS_CVGQ_DL => NULL()
    Z_YDCPG_PHY0_XYB_RDELP => NULL()
    Z_YDVARS_T_DL => NULL()
    Z_YDVARS_Q_DL => NULL()
    Z_YDVARS_GEOMETRY_GELAM_T0 => NULL()
    Z_YDVARS_CVGQ_T0 => NULL()
    Z_YDVARS_Q_T0 => NULL()
    Z_YDVARS_U_T0 => NULL()
    Z_YDVARS_Q_DM => NULL()
    Z_YDVARS_V_T0 => NULL()
    Z_YDVARS_T_DM => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CPPHINP:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 20"
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:CPPHINP')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CPPHINP:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 21"
    ZRDG_MU0 => GET_DEVICE_DATA_RDWR(YL_ZRDG_MU0)
    ZRDG_CVGQ => GET_DEVICE_DATA_RDWR(YL_ZRDG_CVGQ)
    ZRDG_MU0M => GET_DEVICE_DATA_RDWR(YL_ZRDG_MU0M)
    ZRDG_MU0LU => GET_DEVICE_DATA_RDWR(YL_ZRDG_MU0LU)
    ZRDG_MU0N => GET_DEVICE_DATA_RDWR(YL_ZRDG_MU0N)
    ZRDG_CVGT => GET_DEVICE_DATA_RDWR(YL_ZRDG_CVGT)
    ZRDG_SOLO => GET_DEVICE_DATA_RDWR(YL_ZRDG_SOLO)
    Z_YDCPG_DYN0_CTY_EVEL => GET_DEVICE_DATA_RDWR(YDCPG_DYN0%CTY%F_EVEL)
    Z_YDVARS_CVGQ_DM => GET_DEVICE_DATA_RDWR(YDVARS%CVGQ%FDM)
    Z_YDVARS_GEOMETRY_GEMU_T0 => GET_DEVICE_DATA_RDWR(YDVARS%GEOMETRY%GEMU%FT0)
    Z_YDVARS_CVGQ_DL => GET_DEVICE_DATA_RDWR(YDVARS%CVGQ%FDL)
    Z_YDCPG_PHY0_XYB_RDELP => GET_DEVICE_DATA_RDWR(YDCPG_PHY0%XYB%F_RDELP)
    Z_YDVARS_T_DL => GET_DEVICE_DATA_RDWR(YDVARS%T%FDL)
    Z_YDVARS_Q_DL => GET_DEVICE_DATA_RDWR(YDVARS%Q%FDL)
    Z_YDVARS_GEOMETRY_GELAM_T0 => GET_DEVICE_DATA_RDWR(YDVARS%GEOMETRY%GELAM%FT0)
    Z_YDVARS_CVGQ_T0 => GET_DEVICE_DATA_RDWR(YDVARS%CVGQ%FT0)
    Z_YDVARS_Q_T0 => GET_DEVICE_DATA_RDWR(YDVARS%Q%FT0)
    Z_YDVARS_U_T0 => GET_DEVICE_DATA_RDWR(YDVARS%U%FT0)
    Z_YDVARS_Q_DM => GET_DEVICE_DATA_RDWR(YDVARS%Q%FDM)
    Z_YDVARS_V_T0 => GET_DEVICE_DATA_RDWR(YDVARS%V%FT0)
    Z_YDVARS_T_DM => GET_DEVICE_DATA_RDWR(YDVARS%T%FDM)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CPPHINP:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 22"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CPPHINP:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 23"
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, YDGEOMETRY, ZRDG_MU0, ZRDG_CVGQ, ZRDG_MU0M, ZRDG_MU0LU,  &
!$ACC & ZRDG_MU0N, ZRDG_CVGT, ZRDG_SOLO, Z_YDCPG_DYN0_CTY_EVEL, Z_YDVARS_CVGQ_DM, Z_YDVARS_GEOMETRY_GEMU_T0,  &
!$ACC & Z_YDVARS_CVGQ_DL, Z_YDCPG_PHY0_XYB_RDELP, Z_YDVARS_T_DL, Z_YDVARS_Q_DL, Z_YDVARS_GEOMETRY_GELAM_T0,  &
!$ACC & Z_YDVARS_CVGQ_T0, Z_YDVARS_Q_T0, Z_YDVARS_U_T0, Z_YDVARS_Q_DM, Z_YDVARS_V_T0, Z_YDVARS_T_DM ) PRIVATE( JBLK )  &
!$ACC & VECTOR_LENGTH( YDCPG_OPTS%KLON )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK )
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        CALL CPPHINP_OPENACC(YDGEOMETRY, YDMODEL, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, Z_YDVARS_GEOMETRY_GEMU_T0(:, JBLK),  &
        & Z_YDVARS_GEOMETRY_GELAM_T0(:, JBLK), Z_YDVARS_U_T0(:, :, JBLK), Z_YDVARS_V_T0(:, :, JBLK),  &
        & Z_YDVARS_T_DL(:, :, JBLK), Z_YDVARS_T_DM(:, :, JBLK), Z_YDVARS_Q_T0(:, :, JBLK), Z_YDVARS_Q_DL(:, :, JBLK),  &
        & Z_YDVARS_Q_DM(:, :, JBLK), Z_YDVARS_CVGQ_DL(:, :, JBLK), Z_YDVARS_CVGQ_DM(:, :, JBLK),  &
        & Z_YDCPG_PHY0_XYB_RDELP(:, :, JBLK), Z_YDCPG_DYN0_CTY_EVEL(:, :, JBLK), Z_YDVARS_CVGQ_T0(:, :, JBLK),  &
        & ZRDG_MU0(:, JBLK), ZRDG_SOLO(:, JBLK), ZRDG_MU0LU(:, JBLK), ZRDG_MU0M(:, JBLK), ZRDG_MU0N(:, JBLK),  &
        & ZRDG_CVGQ(:, :, JBLK), ZRDG_CVGT(:, :, JBLK), YDSTACK=YLSTACK)
        
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CPPHINP:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 24"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:CPPHINP')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CPPHINP:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 25"
      ZRDG_MU0 => GET_HOST_DATA_RDWR(YL_ZRDG_MU0)
      ZRDG_CVGQ => GET_HOST_DATA_RDWR(YL_ZRDG_CVGQ)
      ZRDG_MU0M => GET_HOST_DATA_RDWR(YL_ZRDG_MU0M)
      ZRDG_MU0LU => GET_HOST_DATA_RDWR(YL_ZRDG_MU0LU)
      ZRDG_MU0N => GET_HOST_DATA_RDWR(YL_ZRDG_MU0N)
      ZRDG_CVGT => GET_HOST_DATA_RDWR(YL_ZRDG_CVGT)
      ZRDG_SOLO => GET_HOST_DATA_RDWR(YL_ZRDG_SOLO)
      Z_YDCPG_DYN0_CTY_EVEL => GET_HOST_DATA_RDWR(YDCPG_DYN0%CTY%F_EVEL)
      Z_YDVARS_CVGQ_DM => GET_HOST_DATA_RDWR(YDVARS%CVGQ%FDM)
      Z_YDVARS_GEOMETRY_GEMU_T0 => GET_HOST_DATA_RDWR(YDVARS%GEOMETRY%GEMU%FT0)
      Z_YDVARS_CVGQ_DL => GET_HOST_DATA_RDWR(YDVARS%CVGQ%FDL)
      Z_YDCPG_PHY0_XYB_RDELP => GET_HOST_DATA_RDWR(YDCPG_PHY0%XYB%F_RDELP)
      Z_YDVARS_T_DL => GET_HOST_DATA_RDWR(YDVARS%T%FDL)
      Z_YDVARS_Q_DL => GET_HOST_DATA_RDWR(YDVARS%Q%FDL)
      Z_YDVARS_GEOMETRY_GELAM_T0 => GET_HOST_DATA_RDWR(YDVARS%GEOMETRY%GELAM%FT0)
      Z_YDVARS_CVGQ_T0 => GET_HOST_DATA_RDWR(YDVARS%CVGQ%FT0)
      Z_YDVARS_Q_T0 => GET_HOST_DATA_RDWR(YDVARS%Q%FT0)
      Z_YDVARS_U_T0 => GET_HOST_DATA_RDWR(YDVARS%U%FT0)
      Z_YDVARS_Q_DM => GET_HOST_DATA_RDWR(YDVARS%Q%FDM)
      Z_YDVARS_V_T0 => GET_HOST_DATA_RDWR(YDVARS%V%FT0)
      Z_YDVARS_T_DM => GET_HOST_DATA_RDWR(YDVARS%T%FDM)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CPPHINP:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 26"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CPPHINP:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 27"
    ZRDG_MU0 => NULL()
    ZRDG_CVGQ => NULL()
    ZRDG_MU0M => NULL()
    ZRDG_MU0LU => NULL()
    ZRDG_MU0N => NULL()
    ZRDG_CVGT => NULL()
    ZRDG_SOLO => NULL()
    Z_YDCPG_DYN0_CTY_EVEL => NULL()
    Z_YDVARS_CVGQ_DM => NULL()
    Z_YDVARS_GEOMETRY_GEMU_T0 => NULL()
    Z_YDVARS_CVGQ_DL => NULL()
    Z_YDCPG_PHY0_XYB_RDELP => NULL()
    Z_YDVARS_T_DL => NULL()
    Z_YDVARS_Q_DL => NULL()
    Z_YDVARS_GEOMETRY_GELAM_T0 => NULL()
    Z_YDVARS_CVGQ_T0 => NULL()
    Z_YDVARS_Q_T0 => NULL()
    Z_YDVARS_U_T0 => NULL()
    Z_YDVARS_Q_DM => NULL()
    Z_YDVARS_V_T0 => NULL()
    Z_YDVARS_T_DM => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CPPHINP:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 28"
  END IF
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CPPHINP', 1, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 29"
  
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZRDG_LCVQ', 0, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 30"
  IF (LPARALLELMETHOD('OPENMP', 'APL_ARPEGE_PARALLEL:ZRDG_LCVQ')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZRDG_LCVQ:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 31"
    ZFLU_QSATS => GET_HOST_DATA_RDWR(YL_ZFLU_QSATS)
    ZRDG_CVGQ => GET_HOST_DATA_RDWR(YL_ZRDG_CVGQ)
    ZRDG_LCVQ => GET_HOST_DATA_RDWR(YL_ZRDG_LCVQ)
    ZQCO2 => GET_HOST_DATA_RDWR(YL_ZQCO2)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZRDG_LCVQ:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 32"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZRDG_LCVQ:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 33"
    CALL YLCPG_BNDS%INIT(YDCPG_OPTS)
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, JLEV ) FIRSTPRIVATE( YLCPG_BNDS )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      CALL YLCPG_BNDS%UPDATE(JBLK)
      
      DO JLEV=1,YDCPG_OPTS%KFLEVG
        DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
          ZRDG_LCVQ(JLON, JLEV, JBLK) = ZRDG_CVGQ(JLON, JLEV, JBLK)
        END DO
      END DO
      
      DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
        ZFLU_QSATS(JLON, JBLK) = 0.0_JPRB
      END DO
      
      DO JLEV=YDCPG_OPTS%KTDIA,YDCPG_OPTS%KFLEVG
        DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
          ZQCO2(JLON, JLEV, JBLK) = YDMODEL%YRML_PHY_MF%YRPHY3%QCO2
        END DO
      END DO
      
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZRDG_LCVQ:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 34"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ZRDG_LCVQ')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZRDG_LCVQ:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 35"
      ZFLU_QSATS => GET_HOST_DATA_RDWR(YL_ZFLU_QSATS)
      ZRDG_CVGQ => GET_HOST_DATA_RDWR(YL_ZRDG_CVGQ)
      ZRDG_LCVQ => GET_HOST_DATA_RDWR(YL_ZRDG_LCVQ)
      ZQCO2 => GET_HOST_DATA_RDWR(YL_ZQCO2)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZRDG_LCVQ:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 36"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZRDG_LCVQ:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 37"
    ZFLU_QSATS => NULL()
    ZRDG_CVGQ => NULL()
    ZRDG_LCVQ => NULL()
    ZQCO2 => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZRDG_LCVQ:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 38"
  ELSE IF (LPARALLELMETHOD('OPENMPSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ZRDG_LCVQ')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZRDG_LCVQ:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 39"
    ZFLU_QSATS => GET_HOST_DATA_RDWR(YL_ZFLU_QSATS)
    ZRDG_CVGQ => GET_HOST_DATA_RDWR(YL_ZRDG_CVGQ)
    ZRDG_LCVQ => GET_HOST_DATA_RDWR(YL_ZRDG_LCVQ)
    ZQCO2 => GET_HOST_DATA_RDWR(YL_ZQCO2)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZRDG_LCVQ:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 40"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZRDG_LCVQ:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 41"
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK, JLEV )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        DO JLEV=1,YDCPG_OPTS%KFLEVG
          ZRDG_LCVQ(JLON, JLEV, JBLK) = ZRDG_CVGQ(JLON, JLEV, JBLK)
        END DO
        
        ZFLU_QSATS(JLON, JBLK) = 0.0_JPRB
        
        DO JLEV=YDCPG_OPTS%KTDIA,YDCPG_OPTS%KFLEVG
          ZQCO2(JLON, JLEV, JBLK) = YDMODEL%YRML_PHY_MF%YRPHY3%QCO2
        END DO
        
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZRDG_LCVQ:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 42"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ZRDG_LCVQ')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZRDG_LCVQ:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 43"
      ZFLU_QSATS => GET_HOST_DATA_RDWR(YL_ZFLU_QSATS)
      ZRDG_CVGQ => GET_HOST_DATA_RDWR(YL_ZRDG_CVGQ)
      ZRDG_LCVQ => GET_HOST_DATA_RDWR(YL_ZRDG_LCVQ)
      ZQCO2 => GET_HOST_DATA_RDWR(YL_ZQCO2)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZRDG_LCVQ:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 44"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZRDG_LCVQ:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 45"
    ZFLU_QSATS => NULL()
    ZRDG_CVGQ => NULL()
    ZRDG_LCVQ => NULL()
    ZQCO2 => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZRDG_LCVQ:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 46"
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ZRDG_LCVQ')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZRDG_LCVQ:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 47"
    ZFLU_QSATS => GET_DEVICE_DATA_RDWR(YL_ZFLU_QSATS)
    ZRDG_CVGQ => GET_DEVICE_DATA_RDWR(YL_ZRDG_CVGQ)
    ZRDG_LCVQ => GET_DEVICE_DATA_RDWR(YL_ZRDG_LCVQ)
    ZQCO2 => GET_DEVICE_DATA_RDWR(YL_ZQCO2)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZRDG_LCVQ:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 48"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZRDG_LCVQ:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 49"
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZFLU_QSATS, ZRDG_CVGQ, ZRDG_LCVQ, ZQCO2 ) PRIVATE( JBLK )  &
!$ACC & VECTOR_LENGTH( YDCPG_OPTS%KLON )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK, JLEV )
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        DO JLEV=1,YDCPG_OPTS%KFLEVG
          ZRDG_LCVQ(JLON, JLEV, JBLK) = ZRDG_CVGQ(JLON, JLEV, JBLK)
        END DO
        
        ZFLU_QSATS(JLON, JBLK) = 0.0_JPRB
        
        DO JLEV=YDCPG_OPTS%KTDIA,YDCPG_OPTS%KFLEVG
          ZQCO2(JLON, JLEV, JBLK) = YDMODEL%YRML_PHY_MF%YRPHY3%QCO2
        END DO
        
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZRDG_LCVQ:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 50"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ZRDG_LCVQ')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZRDG_LCVQ:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 51"
      ZFLU_QSATS => GET_HOST_DATA_RDWR(YL_ZFLU_QSATS)
      ZRDG_CVGQ => GET_HOST_DATA_RDWR(YL_ZRDG_CVGQ)
      ZRDG_LCVQ => GET_HOST_DATA_RDWR(YL_ZRDG_LCVQ)
      ZQCO2 => GET_HOST_DATA_RDWR(YL_ZQCO2)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZRDG_LCVQ:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 52"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZRDG_LCVQ:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 53"
    ZFLU_QSATS => NULL()
    ZRDG_CVGQ => NULL()
    ZRDG_LCVQ => NULL()
    ZQCO2 => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZRDG_LCVQ:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 54"
  END IF
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZRDG_LCVQ', 1, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 55"
  
  CALL MF_PHYS_FPL_PART1_PARALLEL(YDCPG_BNDS, YDCPG_OPTS, YL_ZPFL_FPLCH, YL_ZPFL_FPLSH, YDVARS%CPF%FT0, YDVARS%SPF%FT0,  &
  & YDMODEL)
  
  IF (YDCPG_OPTS%LCONFX) THEN
    
    CALL MF_PHYS_SAVE_PHSURF_PART1_PARALLEL(YDCPG_BNDS, YDCPG_OPTS, YL_ZSAV_DDAL, YL_ZSAV_DDOM, YL_ZSAV_ENTCH, YL_ZSAV_FHPS,  &
    & YL_ZSAV_GZ0F, YL_ZSAV_GZ0HF, YL_ZSAV_HV, YL_ZSAV_PBLH, YL_ZSAV_QSH, YL_ZSAV_UDAL, YL_ZSAV_UDGRO, YL_ZSAV_UDOM,  &
    & YL_ZSAV_UNEBH, YDMF_PHYS_SURF%GSD_VF%F_Z0F, YDMF_PHYS_SURF%GSD_VH%F_PBLH, YDMF_PHYS_SURF%GSD_VH%F_QSH,  &
    & YDMF_PHYS_SURF%GSD_VH%F_SPSH, YDMF_PHYS_SURF%GSD_VK%F_UDGRO, YDMF_PHYS_SURF%GSD_VV%F_HV, YDMF_PHYS_SURF%GSD_VV%F_Z0H,  &
    & YDVARS%DAL%FT0, YDVARS%DOM%FT0, YDVARS%UAL%FT0, YDVARS%UEN%FT0, YDVARS%UNEBH%FT0, YDVARS%UOM%FT0, YDMODEL)
    
  END IF
  
  
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:APLPAR_INIT', 0, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 56"
  IF (LPARALLELMETHOD('OPENMP', 'APL_ARPEGE_PARALLEL:APLPAR_INIT')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:APLPAR_INIT:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 57"
    ZDSA_CPS => GET_HOST_DATA_RDWR(YL_ZDSA_CPS)
    ZDSA_C2 => GET_HOST_DATA_RDWR(YL_ZDSA_C2)
    ZDSA_LHS => GET_HOST_DATA_RDWR(YL_ZDSA_LHS)
    ZFLU_EMIS => GET_HOST_DATA_RDWR(YL_ZFLU_EMIS)
    ZPFL_FEFB1 => GET_HOST_DATA_RDWR(YL_ZPFL_FEFB1)
    ZFLU_QSAT => GET_HOST_DATA_RDWR(YL_ZFLU_QSAT)
    ZFLU_CH => GET_HOST_DATA_RDWR(YL_ZFLU_CH)
    ZFLU_VEG => GET_HOST_DATA_RDWR(YL_ZFLU_VEG)
    ZPFL_FEFB3 => GET_HOST_DATA_RDWR(YL_ZPFL_FEFB3)
    ZDSA_C1 => GET_HOST_DATA_RDWR(YL_ZDSA_C1)
    ZMSC_QW => GET_HOST_DATA_RDWR(YL_ZMSC_QW)
    ZPFL_FTKEI => GET_HOST_DATA_RDWR(YL_ZPFL_FTKEI)
    ZFLU_NEIJ => GET_HOST_DATA_RDWR(YL_ZFLU_NEIJ)
    ZMSC_LH => GET_HOST_DATA_RDWR(YL_ZMSC_LH)
    ZDSA_RS => GET_HOST_DATA_RDWR(YL_ZDSA_RS)
    ZMSC_FRMQ => GET_HOST_DATA_RDWR(YL_ZMSC_FRMQ)
    ZMSC_LSCPE => GET_HOST_DATA_RDWR(YL_ZMSC_LSCPE)
    ZMSC_TW => GET_HOST_DATA_RDWR(YL_ZMSC_TW)
    ZFLU_CHN => GET_HOST_DATA_RDWR(YL_ZFLU_CHN)
    ZPFL_FTKE => GET_HOST_DATA_RDWR(YL_ZPFL_FTKE)
    ZFLU_FEVI => GET_HOST_DATA_RDWR(YL_ZFLU_FEVI)
    ZFLU_CDN => GET_HOST_DATA_RDWR(YL_ZFLU_CDN)
    ZPFL_FEFB2 => GET_HOST_DATA_RDWR(YL_ZPFL_FEFB2)
    ZFLU_CD => GET_HOST_DATA_RDWR(YL_ZFLU_CD)
    IMOC_CLPH => GET_HOST_DATA_RDWR(YL_IMOC_CLPH)
    ZFCHQ => GET_HOST_DATA_RDWR(YL_ZFCHQ)
    ZFLU_QSATS => GET_HOST_DATA_RDWR(YL_ZFLU_QSATS)
    Z_YDMF_PHYS_SURF_GSD_VF_PVEG => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_VEG)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:APLPAR_INIT:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 58"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:APLPAR_INIT:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 59"
    CALL YLCPG_BNDS%INIT(YDCPG_OPTS)
!$OMP PARALLEL DO PRIVATE( JBLK ) FIRSTPRIVATE( YLCPG_BNDS )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      CALL YLCPG_BNDS%UPDATE(JBLK)
      
      CALL APLPAR_INIT(YDCPG_OPTS%LAROME, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG,  &
      & YDMODEL%YRML_PHY_G%YRDPHY%NTSSG, YDCPG_OPTS%YRSURF_DIMS%YSP_SBD%NLEVS, Z_YDMF_PHYS_SURF_GSD_VF_PVEG(:, JBLK),  &
      & ZMSC_FRMQ(:, :, JBLK), ZFCHQ(:, :, JBLK), ZDSA_CPS(:, JBLK), ZDSA_LHS(:, JBLK), ZDSA_RS(:, JBLK),  &
      & ZMSC_LH(:, :, JBLK), ZMSC_LSCPE(:, :, JBLK), ZFLU_QSAT(:, :, JBLK), ZMSC_QW(:, :, JBLK), ZMSC_TW(:, :, JBLK),  &
      & ZFLU_CD(:, JBLK), ZFLU_CDN(:, JBLK), ZFLU_CH(:, JBLK), ZFLU_CHN(:, JBLK), ZDSA_C1(:, JBLK), ZDSA_C2(:, JBLK),  &
      & ZFLU_EMIS(:, JBLK), ZFLU_FEVI(:, :, JBLK), ZPFL_FTKE(:, :, JBLK), ZPFL_FTKEI(:, :, JBLK), ZPFL_FEFB1(:, :, JBLK),  &
      & ZPFL_FEFB2(:, :, JBLK), ZPFL_FEFB3(:, :, JBLK), ZFLU_NEIJ(:, JBLK), ZFLU_VEG(:, JBLK), ZFLU_QSATS(:, JBLK),  &
      & IMOC_CLPH(:, JBLK))
      
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:APLPAR_INIT:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 60"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:APLPAR_INIT')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:APLPAR_INIT:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 61"
      ZDSA_CPS => GET_HOST_DATA_RDWR(YL_ZDSA_CPS)
      ZDSA_C2 => GET_HOST_DATA_RDWR(YL_ZDSA_C2)
      ZDSA_LHS => GET_HOST_DATA_RDWR(YL_ZDSA_LHS)
      ZFLU_EMIS => GET_HOST_DATA_RDWR(YL_ZFLU_EMIS)
      ZPFL_FEFB1 => GET_HOST_DATA_RDWR(YL_ZPFL_FEFB1)
      ZFLU_QSAT => GET_HOST_DATA_RDWR(YL_ZFLU_QSAT)
      ZFLU_CH => GET_HOST_DATA_RDWR(YL_ZFLU_CH)
      ZFLU_VEG => GET_HOST_DATA_RDWR(YL_ZFLU_VEG)
      ZPFL_FEFB3 => GET_HOST_DATA_RDWR(YL_ZPFL_FEFB3)
      ZDSA_C1 => GET_HOST_DATA_RDWR(YL_ZDSA_C1)
      ZMSC_QW => GET_HOST_DATA_RDWR(YL_ZMSC_QW)
      ZPFL_FTKEI => GET_HOST_DATA_RDWR(YL_ZPFL_FTKEI)
      ZFLU_NEIJ => GET_HOST_DATA_RDWR(YL_ZFLU_NEIJ)
      ZMSC_LH => GET_HOST_DATA_RDWR(YL_ZMSC_LH)
      ZDSA_RS => GET_HOST_DATA_RDWR(YL_ZDSA_RS)
      ZMSC_FRMQ => GET_HOST_DATA_RDWR(YL_ZMSC_FRMQ)
      ZMSC_LSCPE => GET_HOST_DATA_RDWR(YL_ZMSC_LSCPE)
      ZMSC_TW => GET_HOST_DATA_RDWR(YL_ZMSC_TW)
      ZFLU_CHN => GET_HOST_DATA_RDWR(YL_ZFLU_CHN)
      ZPFL_FTKE => GET_HOST_DATA_RDWR(YL_ZPFL_FTKE)
      ZFLU_FEVI => GET_HOST_DATA_RDWR(YL_ZFLU_FEVI)
      ZFLU_CDN => GET_HOST_DATA_RDWR(YL_ZFLU_CDN)
      ZPFL_FEFB2 => GET_HOST_DATA_RDWR(YL_ZPFL_FEFB2)
      ZFLU_CD => GET_HOST_DATA_RDWR(YL_ZFLU_CD)
      IMOC_CLPH => GET_HOST_DATA_RDWR(YL_IMOC_CLPH)
      ZFCHQ => GET_HOST_DATA_RDWR(YL_ZFCHQ)
      ZFLU_QSATS => GET_HOST_DATA_RDWR(YL_ZFLU_QSATS)
      Z_YDMF_PHYS_SURF_GSD_VF_PVEG => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_VEG)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:APLPAR_INIT:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 62"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:APLPAR_INIT:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 63"
    ZDSA_CPS => NULL()
    ZDSA_C2 => NULL()
    ZDSA_LHS => NULL()
    ZFLU_EMIS => NULL()
    ZPFL_FEFB1 => NULL()
    ZFLU_QSAT => NULL()
    ZFLU_CH => NULL()
    ZFLU_VEG => NULL()
    ZPFL_FEFB3 => NULL()
    ZDSA_C1 => NULL()
    ZMSC_QW => NULL()
    ZPFL_FTKEI => NULL()
    ZFLU_NEIJ => NULL()
    ZMSC_LH => NULL()
    ZDSA_RS => NULL()
    ZMSC_FRMQ => NULL()
    ZMSC_LSCPE => NULL()
    ZMSC_TW => NULL()
    ZFLU_CHN => NULL()
    ZPFL_FTKE => NULL()
    ZFLU_FEVI => NULL()
    ZFLU_CDN => NULL()
    ZPFL_FEFB2 => NULL()
    ZFLU_CD => NULL()
    IMOC_CLPH => NULL()
    ZFCHQ => NULL()
    ZFLU_QSATS => NULL()
    Z_YDMF_PHYS_SURF_GSD_VF_PVEG => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:APLPAR_INIT:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 64"
  ELSE IF (LPARALLELMETHOD('OPENMPSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:APLPAR_INIT')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:APLPAR_INIT:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 65"
    ZDSA_CPS => GET_HOST_DATA_RDWR(YL_ZDSA_CPS)
    ZDSA_C2 => GET_HOST_DATA_RDWR(YL_ZDSA_C2)
    ZDSA_LHS => GET_HOST_DATA_RDWR(YL_ZDSA_LHS)
    ZFLU_EMIS => GET_HOST_DATA_RDWR(YL_ZFLU_EMIS)
    ZPFL_FEFB1 => GET_HOST_DATA_RDWR(YL_ZPFL_FEFB1)
    ZFLU_QSAT => GET_HOST_DATA_RDWR(YL_ZFLU_QSAT)
    ZFLU_CH => GET_HOST_DATA_RDWR(YL_ZFLU_CH)
    ZFLU_VEG => GET_HOST_DATA_RDWR(YL_ZFLU_VEG)
    ZPFL_FEFB3 => GET_HOST_DATA_RDWR(YL_ZPFL_FEFB3)
    ZDSA_C1 => GET_HOST_DATA_RDWR(YL_ZDSA_C1)
    ZMSC_QW => GET_HOST_DATA_RDWR(YL_ZMSC_QW)
    ZPFL_FTKEI => GET_HOST_DATA_RDWR(YL_ZPFL_FTKEI)
    ZFLU_NEIJ => GET_HOST_DATA_RDWR(YL_ZFLU_NEIJ)
    ZMSC_LH => GET_HOST_DATA_RDWR(YL_ZMSC_LH)
    ZDSA_RS => GET_HOST_DATA_RDWR(YL_ZDSA_RS)
    ZMSC_FRMQ => GET_HOST_DATA_RDWR(YL_ZMSC_FRMQ)
    ZMSC_LSCPE => GET_HOST_DATA_RDWR(YL_ZMSC_LSCPE)
    ZMSC_TW => GET_HOST_DATA_RDWR(YL_ZMSC_TW)
    ZFLU_CHN => GET_HOST_DATA_RDWR(YL_ZFLU_CHN)
    ZPFL_FTKE => GET_HOST_DATA_RDWR(YL_ZPFL_FTKE)
    ZFLU_FEVI => GET_HOST_DATA_RDWR(YL_ZFLU_FEVI)
    ZFLU_CDN => GET_HOST_DATA_RDWR(YL_ZFLU_CDN)
    ZPFL_FEFB2 => GET_HOST_DATA_RDWR(YL_ZPFL_FEFB2)
    ZFLU_CD => GET_HOST_DATA_RDWR(YL_ZFLU_CD)
    IMOC_CLPH => GET_HOST_DATA_RDWR(YL_IMOC_CLPH)
    ZFCHQ => GET_HOST_DATA_RDWR(YL_ZFCHQ)
    ZFLU_QSATS => GET_HOST_DATA_RDWR(YL_ZFLU_QSATS)
    Z_YDMF_PHYS_SURF_GSD_VF_PVEG => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_VEG)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:APLPAR_INIT:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 66"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:APLPAR_INIT:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 67"
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        CALL APLPAR_INIT_OPENACC(YDCPG_OPTS%LAROME, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG,  &
        & YDMODEL%YRML_PHY_G%YRDPHY%NTSSG, YDCPG_OPTS%YRSURF_DIMS%YSP_SBD%NLEVS, Z_YDMF_PHYS_SURF_GSD_VF_PVEG(:, JBLK),  &
        & ZMSC_FRMQ(:, :, JBLK), ZFCHQ(:, :, JBLK), ZDSA_CPS(:, JBLK), ZDSA_LHS(:, JBLK), ZDSA_RS(:, JBLK),  &
        & ZMSC_LH(:, :, JBLK), ZMSC_LSCPE(:, :, JBLK), ZFLU_QSAT(:, :, JBLK), ZMSC_QW(:, :, JBLK), ZMSC_TW(:, :, JBLK),  &
        & ZFLU_CD(:, JBLK), ZFLU_CDN(:, JBLK), ZFLU_CH(:, JBLK), ZFLU_CHN(:, JBLK), ZDSA_C1(:, JBLK), ZDSA_C2(:, JBLK),  &
        & ZFLU_EMIS(:, JBLK), ZFLU_FEVI(:, :, JBLK), ZPFL_FTKE(:, :, JBLK), ZPFL_FTKEI(:, :, JBLK), ZPFL_FEFB1(:, :, JBLK),  &
        & ZPFL_FEFB2(:, :, JBLK), ZPFL_FEFB3(:, :, JBLK), ZFLU_NEIJ(:, JBLK), ZFLU_VEG(:, JBLK), ZFLU_QSATS(:, JBLK),  &
        & IMOC_CLPH(:, JBLK), YDSTACK=YLSTACK)
        
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:APLPAR_INIT:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 68"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:APLPAR_INIT')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:APLPAR_INIT:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 69"
      ZDSA_CPS => GET_HOST_DATA_RDWR(YL_ZDSA_CPS)
      ZDSA_C2 => GET_HOST_DATA_RDWR(YL_ZDSA_C2)
      ZDSA_LHS => GET_HOST_DATA_RDWR(YL_ZDSA_LHS)
      ZFLU_EMIS => GET_HOST_DATA_RDWR(YL_ZFLU_EMIS)
      ZPFL_FEFB1 => GET_HOST_DATA_RDWR(YL_ZPFL_FEFB1)
      ZFLU_QSAT => GET_HOST_DATA_RDWR(YL_ZFLU_QSAT)
      ZFLU_CH => GET_HOST_DATA_RDWR(YL_ZFLU_CH)
      ZFLU_VEG => GET_HOST_DATA_RDWR(YL_ZFLU_VEG)
      ZPFL_FEFB3 => GET_HOST_DATA_RDWR(YL_ZPFL_FEFB3)
      ZDSA_C1 => GET_HOST_DATA_RDWR(YL_ZDSA_C1)
      ZMSC_QW => GET_HOST_DATA_RDWR(YL_ZMSC_QW)
      ZPFL_FTKEI => GET_HOST_DATA_RDWR(YL_ZPFL_FTKEI)
      ZFLU_NEIJ => GET_HOST_DATA_RDWR(YL_ZFLU_NEIJ)
      ZMSC_LH => GET_HOST_DATA_RDWR(YL_ZMSC_LH)
      ZDSA_RS => GET_HOST_DATA_RDWR(YL_ZDSA_RS)
      ZMSC_FRMQ => GET_HOST_DATA_RDWR(YL_ZMSC_FRMQ)
      ZMSC_LSCPE => GET_HOST_DATA_RDWR(YL_ZMSC_LSCPE)
      ZMSC_TW => GET_HOST_DATA_RDWR(YL_ZMSC_TW)
      ZFLU_CHN => GET_HOST_DATA_RDWR(YL_ZFLU_CHN)
      ZPFL_FTKE => GET_HOST_DATA_RDWR(YL_ZPFL_FTKE)
      ZFLU_FEVI => GET_HOST_DATA_RDWR(YL_ZFLU_FEVI)
      ZFLU_CDN => GET_HOST_DATA_RDWR(YL_ZFLU_CDN)
      ZPFL_FEFB2 => GET_HOST_DATA_RDWR(YL_ZPFL_FEFB2)
      ZFLU_CD => GET_HOST_DATA_RDWR(YL_ZFLU_CD)
      IMOC_CLPH => GET_HOST_DATA_RDWR(YL_IMOC_CLPH)
      ZFCHQ => GET_HOST_DATA_RDWR(YL_ZFCHQ)
      ZFLU_QSATS => GET_HOST_DATA_RDWR(YL_ZFLU_QSATS)
      Z_YDMF_PHYS_SURF_GSD_VF_PVEG => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_VEG)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:APLPAR_INIT:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 70"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:APLPAR_INIT:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 71"
    ZDSA_CPS => NULL()
    ZDSA_C2 => NULL()
    ZDSA_LHS => NULL()
    ZFLU_EMIS => NULL()
    ZPFL_FEFB1 => NULL()
    ZFLU_QSAT => NULL()
    ZFLU_CH => NULL()
    ZFLU_VEG => NULL()
    ZPFL_FEFB3 => NULL()
    ZDSA_C1 => NULL()
    ZMSC_QW => NULL()
    ZPFL_FTKEI => NULL()
    ZFLU_NEIJ => NULL()
    ZMSC_LH => NULL()
    ZDSA_RS => NULL()
    ZMSC_FRMQ => NULL()
    ZMSC_LSCPE => NULL()
    ZMSC_TW => NULL()
    ZFLU_CHN => NULL()
    ZPFL_FTKE => NULL()
    ZFLU_FEVI => NULL()
    ZFLU_CDN => NULL()
    ZPFL_FEFB2 => NULL()
    ZFLU_CD => NULL()
    IMOC_CLPH => NULL()
    ZFCHQ => NULL()
    ZFLU_QSATS => NULL()
    Z_YDMF_PHYS_SURF_GSD_VF_PVEG => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:APLPAR_INIT:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 72"
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:APLPAR_INIT')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:APLPAR_INIT:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 73"
    ZDSA_CPS => GET_DEVICE_DATA_RDWR(YL_ZDSA_CPS)
    ZDSA_C2 => GET_DEVICE_DATA_RDWR(YL_ZDSA_C2)
    ZDSA_LHS => GET_DEVICE_DATA_RDWR(YL_ZDSA_LHS)
    ZFLU_EMIS => GET_DEVICE_DATA_RDWR(YL_ZFLU_EMIS)
    ZPFL_FEFB1 => GET_DEVICE_DATA_RDWR(YL_ZPFL_FEFB1)
    ZFLU_QSAT => GET_DEVICE_DATA_RDWR(YL_ZFLU_QSAT)
    ZFLU_CH => GET_DEVICE_DATA_RDWR(YL_ZFLU_CH)
    ZFLU_VEG => GET_DEVICE_DATA_RDWR(YL_ZFLU_VEG)
    ZPFL_FEFB3 => GET_DEVICE_DATA_RDWR(YL_ZPFL_FEFB3)
    ZDSA_C1 => GET_DEVICE_DATA_RDWR(YL_ZDSA_C1)
    ZMSC_QW => GET_DEVICE_DATA_RDWR(YL_ZMSC_QW)
    ZPFL_FTKEI => GET_DEVICE_DATA_RDWR(YL_ZPFL_FTKEI)
    ZFLU_NEIJ => GET_DEVICE_DATA_RDWR(YL_ZFLU_NEIJ)
    ZMSC_LH => GET_DEVICE_DATA_RDWR(YL_ZMSC_LH)
    ZDSA_RS => GET_DEVICE_DATA_RDWR(YL_ZDSA_RS)
    ZMSC_FRMQ => GET_DEVICE_DATA_RDWR(YL_ZMSC_FRMQ)
    ZMSC_LSCPE => GET_DEVICE_DATA_RDWR(YL_ZMSC_LSCPE)
    ZMSC_TW => GET_DEVICE_DATA_RDWR(YL_ZMSC_TW)
    ZFLU_CHN => GET_DEVICE_DATA_RDWR(YL_ZFLU_CHN)
    ZPFL_FTKE => GET_DEVICE_DATA_RDWR(YL_ZPFL_FTKE)
    ZFLU_FEVI => GET_DEVICE_DATA_RDWR(YL_ZFLU_FEVI)
    ZFLU_CDN => GET_DEVICE_DATA_RDWR(YL_ZFLU_CDN)
    ZPFL_FEFB2 => GET_DEVICE_DATA_RDWR(YL_ZPFL_FEFB2)
    ZFLU_CD => GET_DEVICE_DATA_RDWR(YL_ZFLU_CD)
    IMOC_CLPH => GET_DEVICE_DATA_RDWR(YL_IMOC_CLPH)
    ZFCHQ => GET_DEVICE_DATA_RDWR(YL_ZFCHQ)
    ZFLU_QSATS => GET_DEVICE_DATA_RDWR(YL_ZFLU_QSATS)
    Z_YDMF_PHYS_SURF_GSD_VF_PVEG => GET_DEVICE_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_VEG)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:APLPAR_INIT:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 74"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:APLPAR_INIT:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 75"
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZDSA_CPS, ZDSA_C2, ZDSA_LHS, ZFLU_EMIS, ZPFL_FEFB1,  &
!$ACC & ZFLU_QSAT, ZFLU_CH, ZFLU_VEG, ZPFL_FEFB3, ZDSA_C1, ZMSC_QW, ZPFL_FTKEI, ZFLU_NEIJ, ZMSC_LH, ZDSA_RS, ZMSC_FRMQ,  &
!$ACC & ZMSC_LSCPE, ZMSC_TW, ZFLU_CHN, ZFLU_FEVI, ZFLU_CDN, ZPFL_FEFB2, IMOC_CLPH, ZFCHQ, ZFLU_QSATS,  &
!$ACC & Z_YDMF_PHYS_SURF_GSD_VF_PVEG ) PRIVATE( JBLK ) VECTOR_LENGTH( YDCPG_OPTS%KLON )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK )
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        CALL APLPAR_INIT_OPENACC(YDCPG_OPTS%LAROME, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG,  &
        & YDMODEL%YRML_PHY_G%YRDPHY%NTSSG, YDCPG_OPTS%YRSURF_DIMS%YSP_SBD%NLEVS, Z_YDMF_PHYS_SURF_GSD_VF_PVEG(:, JBLK),  &
        & ZMSC_FRMQ(:, :, JBLK), ZFCHQ(:, :, JBLK), ZDSA_CPS(:, JBLK), ZDSA_LHS(:, JBLK), ZDSA_RS(:, JBLK),  &
        & ZMSC_LH(:, :, JBLK), ZMSC_LSCPE(:, :, JBLK), ZFLU_QSAT(:, :, JBLK), ZMSC_QW(:, :, JBLK), ZMSC_TW(:, :, JBLK),  &
        & ZFLU_CD(:, JBLK), ZFLU_CDN(:, JBLK), ZFLU_CH(:, JBLK), ZFLU_CHN(:, JBLK), ZDSA_C1(:, JBLK), ZDSA_C2(:, JBLK),  &
        & ZFLU_EMIS(:, JBLK), ZFLU_FEVI(:, :, JBLK), ZPFL_FTKE(:, :, JBLK), ZPFL_FTKEI(:, :, JBLK), ZPFL_FEFB1(:, :, JBLK),  &
        & ZPFL_FEFB2(:, :, JBLK), ZPFL_FEFB3(:, :, JBLK), ZFLU_NEIJ(:, JBLK), ZFLU_VEG(:, JBLK), ZFLU_QSATS(:, JBLK),  &
        & IMOC_CLPH(:, JBLK), YDSTACK=YLSTACK)
        
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:APLPAR_INIT:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 76"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:APLPAR_INIT')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:APLPAR_INIT:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 77"
      ZDSA_CPS => GET_HOST_DATA_RDWR(YL_ZDSA_CPS)
      ZDSA_C2 => GET_HOST_DATA_RDWR(YL_ZDSA_C2)
      ZDSA_LHS => GET_HOST_DATA_RDWR(YL_ZDSA_LHS)
      ZFLU_EMIS => GET_HOST_DATA_RDWR(YL_ZFLU_EMIS)
      ZPFL_FEFB1 => GET_HOST_DATA_RDWR(YL_ZPFL_FEFB1)
      ZFLU_QSAT => GET_HOST_DATA_RDWR(YL_ZFLU_QSAT)
      ZFLU_CH => GET_HOST_DATA_RDWR(YL_ZFLU_CH)
      ZFLU_VEG => GET_HOST_DATA_RDWR(YL_ZFLU_VEG)
      ZPFL_FEFB3 => GET_HOST_DATA_RDWR(YL_ZPFL_FEFB3)
      ZDSA_C1 => GET_HOST_DATA_RDWR(YL_ZDSA_C1)
      ZMSC_QW => GET_HOST_DATA_RDWR(YL_ZMSC_QW)
      ZPFL_FTKEI => GET_HOST_DATA_RDWR(YL_ZPFL_FTKEI)
      ZFLU_NEIJ => GET_HOST_DATA_RDWR(YL_ZFLU_NEIJ)
      ZMSC_LH => GET_HOST_DATA_RDWR(YL_ZMSC_LH)
      ZDSA_RS => GET_HOST_DATA_RDWR(YL_ZDSA_RS)
      ZMSC_FRMQ => GET_HOST_DATA_RDWR(YL_ZMSC_FRMQ)
      ZMSC_LSCPE => GET_HOST_DATA_RDWR(YL_ZMSC_LSCPE)
      ZMSC_TW => GET_HOST_DATA_RDWR(YL_ZMSC_TW)
      ZFLU_CHN => GET_HOST_DATA_RDWR(YL_ZFLU_CHN)
      ZPFL_FTKE => GET_HOST_DATA_RDWR(YL_ZPFL_FTKE)
      ZFLU_FEVI => GET_HOST_DATA_RDWR(YL_ZFLU_FEVI)
      ZFLU_CDN => GET_HOST_DATA_RDWR(YL_ZFLU_CDN)
      ZPFL_FEFB2 => GET_HOST_DATA_RDWR(YL_ZPFL_FEFB2)
      ZFLU_CD => GET_HOST_DATA_RDWR(YL_ZFLU_CD)
      IMOC_CLPH => GET_HOST_DATA_RDWR(YL_IMOC_CLPH)
      ZFCHQ => GET_HOST_DATA_RDWR(YL_ZFCHQ)
      ZFLU_QSATS => GET_HOST_DATA_RDWR(YL_ZFLU_QSATS)
      Z_YDMF_PHYS_SURF_GSD_VF_PVEG => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_VEG)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:APLPAR_INIT:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 78"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:APLPAR_INIT:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 79"
    ZDSA_CPS => NULL()
    ZDSA_C2 => NULL()
    ZDSA_LHS => NULL()
    ZFLU_EMIS => NULL()
    ZPFL_FEFB1 => NULL()
    ZFLU_QSAT => NULL()
    ZFLU_CH => NULL()
    ZFLU_VEG => NULL()
    ZPFL_FEFB3 => NULL()
    ZDSA_C1 => NULL()
    ZMSC_QW => NULL()
    ZPFL_FTKEI => NULL()
    ZFLU_NEIJ => NULL()
    ZMSC_LH => NULL()
    ZDSA_RS => NULL()
    ZMSC_FRMQ => NULL()
    ZMSC_LSCPE => NULL()
    ZMSC_TW => NULL()
    ZFLU_CHN => NULL()
    ZPFL_FTKE => NULL()
    ZFLU_FEVI => NULL()
    ZFLU_CDN => NULL()
    ZPFL_FEFB2 => NULL()
    ZFLU_CD => NULL()
    IMOC_CLPH => NULL()
    ZFCHQ => NULL()
    ZFLU_QSATS => NULL()
    Z_YDMF_PHYS_SURF_GSD_VF_PVEG => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:APLPAR_INIT:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 80"
  END IF
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:APLPAR_INIT', 1, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 81"
  
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PLSM', 0, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 82"
  IF (LPARALLELMETHOD('OPENMP', 'APL_ARPEGE_PARALLEL:PLSM')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PLSM:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 83"
    Z_YDMF_PHYS_SURF_GSD_VF_PLSM => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_LSM)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PLSM:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 84"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PLSM:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 85"
    CALL YLCPG_BNDS%INIT(YDCPG_OPTS)
!$OMP PARALLEL DO PRIVATE( JBLK, JLON ) FIRSTPRIVATE( YLCPG_BNDS )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      CALL YLCPG_BNDS%UPDATE(JBLK)
      
      DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
        Z_YDMF_PHYS_SURF_GSD_VF_PLSM(JLON, JBLK) = REAL(NINT(Z_YDMF_PHYS_SURF_GSD_VF_PLSM(JLON, JBLK)), kind=JPRB)
      END DO
      
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PLSM:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 86"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:PLSM')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PLSM:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 87"
      Z_YDMF_PHYS_SURF_GSD_VF_PLSM => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_LSM)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PLSM:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 88"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PLSM:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 89"
    Z_YDMF_PHYS_SURF_GSD_VF_PLSM => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PLSM:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 90"
  ELSE IF (LPARALLELMETHOD('OPENMPSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:PLSM')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PLSM:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 91"
    Z_YDMF_PHYS_SURF_GSD_VF_PLSM => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_LSM)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PLSM:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 92"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PLSM:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 93"
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        Z_YDMF_PHYS_SURF_GSD_VF_PLSM(JLON, JBLK) = REAL(NINT(Z_YDMF_PHYS_SURF_GSD_VF_PLSM(JLON, JBLK)), kind=JPRB)
        
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PLSM:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 94"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:PLSM')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PLSM:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 95"
      Z_YDMF_PHYS_SURF_GSD_VF_PLSM => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_LSM)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PLSM:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 96"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PLSM:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 97"
    Z_YDMF_PHYS_SURF_GSD_VF_PLSM => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PLSM:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 98"
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:PLSM')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PLSM:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 99"
    Z_YDMF_PHYS_SURF_GSD_VF_PLSM => GET_DEVICE_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_LSM)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PLSM:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 100"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PLSM:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 101"
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, Z_YDMF_PHYS_SURF_GSD_VF_PLSM ) PRIVATE( JBLK ) VECTOR_LENGTH(  &
!$ACC & YDCPG_OPTS%KLON )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK )
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        Z_YDMF_PHYS_SURF_GSD_VF_PLSM(JLON, JBLK) = REAL(NINT(Z_YDMF_PHYS_SURF_GSD_VF_PLSM(JLON, JBLK)), kind=JPRB)
        
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PLSM:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 102"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:PLSM')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PLSM:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 103"
      Z_YDMF_PHYS_SURF_GSD_VF_PLSM => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_LSM)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PLSM:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 104"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PLSM:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 105"
    Z_YDMF_PHYS_SURF_GSD_VF_PLSM => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PLSM:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 106"
  END IF
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PLSM', 1, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 107"
  
  ! Check magnitude of model variables.
  
  
  IF (YDMODEL%YRML_PHY_MF%YRPHY%LGCHECKMV) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CHECKMV', 0, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 108"
    IF (LPARALLELMETHOD('OPENMP', 'APL_ARPEGE_PARALLEL:CHECKMV')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CHECKMV:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 109"
      ZRDG_MU0 => GET_HOST_DATA_RDWR(YL_ZRDG_MU0)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
      Z_YDVARS_GEOMETRY_GEMU_T0 => GET_HOST_DATA_RDWR(YDVARS%GEOMETRY%GEMU%FT0)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
      Z_YDVARS_GEOMETRY_GELAM_T0 => GET_HOST_DATA_RDWR(YDVARS%GEOMETRY%GELAM%FT0)
      Z_YDMF_PHYS_SURF_GSD_VF_PLSM => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_LSM)
      Z_YDMF_PHYS_BASE_STATE_Q => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_Q)
      Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CHECKMV:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 110"
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CHECKMV:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 111"
      CALL YLCPG_BNDS%INIT(YDCPG_OPTS)
!$OMP PARALLEL DO PRIVATE( JBLK ) FIRSTPRIVATE( YLCPG_BNDS )
      DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
        CALL YLCPG_BNDS%UPDATE(JBLK)
        CALL CHECKMV(YDCPG_OPTS%NINDAT, YDMODEL%YRCST, YDMODEL%YRML_GCONF%YRRIP, YDMODEL%YRML_PHY_MF%YRPHY0,  &
        & YDMODEL%YRML_PHY_MF%YRPHY2, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG,  &
        & YDCPG_OPTS%NSTEP, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(:, :, JBLK), Z_YDVARS_GEOMETRY_GELAM_T0(:, JBLK),  &
        & Z_YDVARS_GEOMETRY_GEMU_T0(:, JBLK), ZRDG_MU0(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VF_PLSM(:, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_T(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_Q(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T(:, JBLK))
      END DO
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CHECKMV:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 112"
      IF (LSYNCHOST('APL_ARPEGE_PARALLEL:CHECKMV')) THEN
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CHECKMV:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 113"
        ZRDG_MU0 => GET_HOST_DATA_RDWR(YL_ZRDG_MU0)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
        Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
        Z_YDVARS_GEOMETRY_GEMU_T0 => GET_HOST_DATA_RDWR(YDVARS%GEOMETRY%GEMU%FT0)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
        Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
        Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
        Z_YDVARS_GEOMETRY_GELAM_T0 => GET_HOST_DATA_RDWR(YDVARS%GEOMETRY%GELAM%FT0)
        Z_YDMF_PHYS_SURF_GSD_VF_PLSM => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_LSM)
        Z_YDMF_PHYS_BASE_STATE_Q => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_Q)
        Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CHECKMV:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 114"
      END IF
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CHECKMV:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 115"
      ZRDG_MU0 => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => NULL()
      Z_YDVARS_GEOMETRY_GEMU_T0 => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => NULL()
      Z_YDVARS_GEOMETRY_GELAM_T0 => NULL()
      Z_YDMF_PHYS_SURF_GSD_VF_PLSM => NULL()
      Z_YDMF_PHYS_BASE_STATE_Q => NULL()
      Z_YDMF_PHYS_BASE_STATE_T => NULL()
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CHECKMV:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 116"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CHECKMV', 1, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 117"
  END IF
  
  
  
  LLREDPR = .false.
  ZRVMD = YDMODEL%YRCST%RV - YDMODEL%YRCST%RD
  
  IF (YDCPG_OPTS%LCONFX) THEN
    ZDTMSE = 0.01_JPRB
    ZSTATI = YDMODEL%YRML_GCONF%YRRIP%RSTATI + (ZDTMSE - YDMODEL%YRML_GCONF%YRRIP%TSTEP)*0.5_JPRB
  ELSE
    ZDTMSE = YDMODEL%YRML_PHY_MF%YRPHY2%TSPHY
    ZSTATI = YDMODEL%YRML_GCONF%YRRIP%RSTATI
  END IF
  ZRHGMT = REAL(YDMODEL%YRML_GCONF%YRRIP%RHGMT, kind=JPRB)
  
  
  CALL APL_ARPEGE_INIT_PARALLEL(YDMODEL%YRCST, YDMF_PHYS_BASE_STATE, YDCPG_BNDS, YDCPG_OPTS, YDCPG_MISC, YDMF_PHYS,  &
  & YDMF_PHYS_SURF, YDVARS, YDMODEL, YL_IMOC_CLPH, YL_INLAB_CVPP, YL_ZAER, YL_ZAERINDS, YL_ZAIPCMT, YL_ZALBD, YL_ZALBP,  &
  & YL_ZALPHA1, YL_ZCEMTR, YL_ZCFATH, YL_ZCFAU, YL_ZCFBTH, YL_ZCFBU, YL_ZCFBV, YL_ZCOEFA, YL_ZCONDCVPPI, YL_ZCONDCVPPL,  &
  & YL_ZCTRSO, YL_ZDECRD, YL_ZDIFCVPPQ, YL_ZDIFCVPPS, YL_ZDIFCVPPU, YL_ZDIFCVPPV, YL_ZDIFWQ, YL_ZDIFWS, YL_ZEDMFQ,  &
  & YL_ZEDMFS, YL_ZEDMFU, YL_ZEDMFV, ZEPS0, ZEPSNEB, YL_ZFPCOR, YL_ZKQLROV, YL_ZKQROV, YL_ZKTROV, YL_ZKUROV, YL_ZLVT,  &
  & YL_ZMF_UP, YL_ZMRIPP, YL_ZNEBC0, YL_ZNEBCH, YL_ZNEBDIFF, YL_ZNEBS, YL_ZNEBS0, YL_ZNEB_CVPP, YL_ZPFL_FPLCH,  &
  & YL_ZPFL_FPLSH, YL_ZPOID, YL_ZPRODTH_CVPP, YL_ZQC_DET_PCMT, YL_ZQI, YL_ZQIC, YL_ZQICE, YL_ZQL, YL_ZQLC, YL_ZQLIS,  &
  & YL_ZQLIS0, YL_ZQLI_CVP, YL_ZQLI_CVPP, YL_ZQO3, YL_ZQR, YL_ZQS, YL_ZQV, YL_ZSC_FCLL, YL_ZSC_FCLN, YL_ZSC_FEVI,  &
  & YL_ZSC_FEVN, YL_ZSEDIQI, YL_ZSEDIQL, YL_ZSFSWDIF, YL_ZSFSWDIR, YL_ZSUDU, YL_ZTENHA, YL_ZTENQVA, YL_ZTENT, YL_ZTRSOD,  &
  & YL_ZTRSODIF, YL_ZTRSODIR, YL_ZUNEBH, YL_ZXDROV, YL_ZXHROV, YL_ZXQRO, YL_ZXTRO, YL_ZXTROV, YL_ZXURO, YL_ZXUROV)
  
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACTQSAT', 0, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 118"
  IF (LPARALLELMETHOD('OPENMP', 'APL_ARPEGE_PARALLEL:ACTQSAT')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACTQSAT:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 119"
    ZFLU_QSAT => GET_HOST_DATA_RDWR(YL_ZFLU_QSAT)
    ZQV => GET_HOST_DATA_RDWR(YL_ZQV)
    ZGEOSLC => GET_HOST_DATA_RDWR(YL_ZGEOSLC)
    ZMSC_QW => GET_HOST_DATA_RDWR(YL_ZMSC_QW)
    ZMSC_LH => GET_HOST_DATA_RDWR(YL_ZMSC_LH)
    ZMSC_LSCPE => GET_HOST_DATA_RDWR(YL_ZMSC_LSCPE)
    ZMSC_TW => GET_HOST_DATA_RDWR(YL_ZMSC_TW)
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
    Z_YDCPG_MISC_RH => GET_HOST_DATA_RDWR(YDCPG_MISC%F_RH)
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP)
    Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACTQSAT:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 120"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACTQSAT:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 121"
    CALL YLCPG_BNDS%INIT(YDCPG_OPTS)
!$OMP PARALLEL DO PRIVATE( JBLK ) FIRSTPRIVATE( YLCPG_BNDS )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      CALL YLCPG_BNDS%UPDATE(JBLK)
      
      CALL ACTQSAT(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF%YRPHY, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON,  &
      & YDMODEL%YRML_PHY_MF%YRTOPH%NTQSAT, YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(:, :, JBLK),  &
      & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP(:, :, JBLK), ZQV(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_T(:, :, JBLK),  &
      & ZGEOSLC(:, :, JBLK), ZMSC_LH(:, :, JBLK), ZMSC_LSCPE(:, :, JBLK), ZFLU_QSAT(:, :, JBLK), ZMSC_QW(:, :, JBLK),  &
      & Z_YDCPG_MISC_RH(:, :, JBLK), ZMSC_TW(:, :, JBLK))
      
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACTQSAT:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 122"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ACTQSAT')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACTQSAT:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 123"
      ZFLU_QSAT => GET_HOST_DATA_RDWR(YL_ZFLU_QSAT)
      ZQV => GET_HOST_DATA_RDWR(YL_ZQV)
      ZGEOSLC => GET_HOST_DATA_RDWR(YL_ZGEOSLC)
      ZMSC_QW => GET_HOST_DATA_RDWR(YL_ZMSC_QW)
      ZMSC_LH => GET_HOST_DATA_RDWR(YL_ZMSC_LH)
      ZMSC_LSCPE => GET_HOST_DATA_RDWR(YL_ZMSC_LSCPE)
      ZMSC_TW => GET_HOST_DATA_RDWR(YL_ZMSC_TW)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
      Z_YDCPG_MISC_RH => GET_HOST_DATA_RDWR(YDCPG_MISC%F_RH)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP)
      Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACTQSAT:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 124"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACTQSAT:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 125"
    ZFLU_QSAT => NULL()
    ZQV => NULL()
    ZGEOSLC => NULL()
    ZMSC_QW => NULL()
    ZMSC_LH => NULL()
    ZMSC_LSCPE => NULL()
    ZMSC_TW => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => NULL()
    Z_YDCPG_MISC_RH => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => NULL()
    Z_YDMF_PHYS_BASE_STATE_T => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACTQSAT:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 126"
  ELSE IF (LPARALLELMETHOD('OPENMPSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ACTQSAT')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACTQSAT:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 127"
    ZFLU_QSAT => GET_HOST_DATA_RDWR(YL_ZFLU_QSAT)
    ZQV => GET_HOST_DATA_RDWR(YL_ZQV)
    ZGEOSLC => GET_HOST_DATA_RDWR(YL_ZGEOSLC)
    ZMSC_QW => GET_HOST_DATA_RDWR(YL_ZMSC_QW)
    ZMSC_LH => GET_HOST_DATA_RDWR(YL_ZMSC_LH)
    ZMSC_LSCPE => GET_HOST_DATA_RDWR(YL_ZMSC_LSCPE)
    ZMSC_TW => GET_HOST_DATA_RDWR(YL_ZMSC_TW)
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
    Z_YDCPG_MISC_RH => GET_HOST_DATA_RDWR(YDCPG_MISC%F_RH)
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP)
    Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACTQSAT:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 128"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACTQSAT:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 129"
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        CALL ACTQSAT_OPENACC(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF%YRPHY, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON,  &
        & YDMODEL%YRML_PHY_MF%YRTOPH%NTQSAT, YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP(:, :, JBLK), ZQV(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_T(:, :, JBLK),  &
        & ZGEOSLC(:, :, JBLK), ZMSC_LH(:, :, JBLK), ZMSC_LSCPE(:, :, JBLK), ZFLU_QSAT(:, :, JBLK), ZMSC_QW(:, :, JBLK),  &
        & Z_YDCPG_MISC_RH(:, :, JBLK), ZMSC_TW(:, :, JBLK), YDSTACK=YLSTACK)
        
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACTQSAT:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 130"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ACTQSAT')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACTQSAT:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 131"
      ZFLU_QSAT => GET_HOST_DATA_RDWR(YL_ZFLU_QSAT)
      ZQV => GET_HOST_DATA_RDWR(YL_ZQV)
      ZGEOSLC => GET_HOST_DATA_RDWR(YL_ZGEOSLC)
      ZMSC_QW => GET_HOST_DATA_RDWR(YL_ZMSC_QW)
      ZMSC_LH => GET_HOST_DATA_RDWR(YL_ZMSC_LH)
      ZMSC_LSCPE => GET_HOST_DATA_RDWR(YL_ZMSC_LSCPE)
      ZMSC_TW => GET_HOST_DATA_RDWR(YL_ZMSC_TW)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
      Z_YDCPG_MISC_RH => GET_HOST_DATA_RDWR(YDCPG_MISC%F_RH)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP)
      Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACTQSAT:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 132"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACTQSAT:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 133"
    ZFLU_QSAT => NULL()
    ZQV => NULL()
    ZGEOSLC => NULL()
    ZMSC_QW => NULL()
    ZMSC_LH => NULL()
    ZMSC_LSCPE => NULL()
    ZMSC_TW => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => NULL()
    Z_YDCPG_MISC_RH => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => NULL()
    Z_YDMF_PHYS_BASE_STATE_T => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACTQSAT:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 134"
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ACTQSAT')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACTQSAT:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 135"
    ZFLU_QSAT => GET_DEVICE_DATA_RDWR(YL_ZFLU_QSAT)
    ZQV => GET_DEVICE_DATA_RDWR(YL_ZQV)
    ZGEOSLC => GET_DEVICE_DATA_RDWR(YL_ZGEOSLC)
    ZMSC_QW => GET_DEVICE_DATA_RDWR(YL_ZMSC_QW)
    ZMSC_LH => GET_DEVICE_DATA_RDWR(YL_ZMSC_LH)
    ZMSC_LSCPE => GET_DEVICE_DATA_RDWR(YL_ZMSC_LSCPE)
    ZMSC_TW => GET_DEVICE_DATA_RDWR(YL_ZMSC_TW)
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
    Z_YDCPG_MISC_RH => GET_DEVICE_DATA_RDWR(YDCPG_MISC%F_RH)
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP)
    Z_YDMF_PHYS_BASE_STATE_T => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACTQSAT:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 136"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACTQSAT:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 137"
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZFLU_QSAT, ZQV, ZGEOSLC, ZMSC_QW, ZMSC_LH, ZMSC_LSCPE,  &
!$ACC & ZMSC_TW, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF, Z_YDCPG_MISC_RH, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_T ) PRIVATE( JBLK ) VECTOR_LENGTH( YDCPG_OPTS%KLON )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK )
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        CALL ACTQSAT_OPENACC(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF%YRPHY, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON,  &
        & YDMODEL%YRML_PHY_MF%YRTOPH%NTQSAT, YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP(:, :, JBLK), ZQV(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_T(:, :, JBLK),  &
        & ZGEOSLC(:, :, JBLK), ZMSC_LH(:, :, JBLK), ZMSC_LSCPE(:, :, JBLK), ZFLU_QSAT(:, :, JBLK), ZMSC_QW(:, :, JBLK),  &
        & Z_YDCPG_MISC_RH(:, :, JBLK), ZMSC_TW(:, :, JBLK), YDSTACK=YLSTACK)
        
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACTQSAT:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 138"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ACTQSAT')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACTQSAT:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 139"
      ZFLU_QSAT => GET_HOST_DATA_RDWR(YL_ZFLU_QSAT)
      ZQV => GET_HOST_DATA_RDWR(YL_ZQV)
      ZGEOSLC => GET_HOST_DATA_RDWR(YL_ZGEOSLC)
      ZMSC_QW => GET_HOST_DATA_RDWR(YL_ZMSC_QW)
      ZMSC_LH => GET_HOST_DATA_RDWR(YL_ZMSC_LH)
      ZMSC_LSCPE => GET_HOST_DATA_RDWR(YL_ZMSC_LSCPE)
      ZMSC_TW => GET_HOST_DATA_RDWR(YL_ZMSC_TW)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
      Z_YDCPG_MISC_RH => GET_HOST_DATA_RDWR(YDCPG_MISC%F_RH)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP)
      Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACTQSAT:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 140"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACTQSAT:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 141"
    ZFLU_QSAT => NULL()
    ZQV => NULL()
    ZGEOSLC => NULL()
    ZMSC_QW => NULL()
    ZMSC_LH => NULL()
    ZMSC_LSCPE => NULL()
    ZMSC_TW => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => NULL()
    Z_YDCPG_MISC_RH => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => NULL()
    Z_YDMF_PHYS_BASE_STATE_T => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACTQSAT:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 142"
  END IF
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACTQSAT', 1, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 143"
  
  
  IF (.not.YDMODEL%YRML_PHY_MF%YRARPHY%LMSE) THEN
    
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACSOL', 0, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 144"
    IF (LPARALLELMETHOD('OPENMP', 'APL_ARPEGE_PARALLEL:ACSOL')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACSOL:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 145"
      ZDSA_C2 => GET_HOST_DATA_RDWR(YL_ZDSA_C2)
      ZC3 => GET_HOST_DATA_RDWR(YL_ZC3)
      ZWSEQ => GET_HOST_DATA_RDWR(YL_ZWSEQ)
      ZCG => GET_HOST_DATA_RDWR(YL_ZCG)
      ZDSA_C1 => GET_HOST_DATA_RDWR(YL_ZDSA_C1)
      ZWFC => GET_HOST_DATA_RDWR(YL_ZWFC)
      ZNEIJV => GET_HOST_DATA_RDWR(YL_ZNEIJV)
      ZWWILT => GET_HOST_DATA_RDWR(YL_ZWWILT)
      ZNEIJG => GET_HOST_DATA_RDWR(YL_ZNEIJG)
      ZCN => GET_HOST_DATA_RDWR(YL_ZCN)
      ZWSMX => GET_HOST_DATA_RDWR(YL_ZWSMX)
      ZWPMX => GET_HOST_DATA_RDWR(YL_ZWPMX)
      Z_YDMF_PHYS_SURF_GSD_VV_PIVEG => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_IVEG)
      Z_YDMF_PHYS_BASE_STATE_YGSP_SG_R => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_SG%F_R)
      Z_YDMF_PHYS_SURF_GSD_VV_PARG => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_ARG)
      Z_YDMF_PHYS_SURF_GSD_VV_PSAB => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_SAB)
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_W => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_W)
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_IC => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_IC)
      Z_YDMF_PHYS_BASE_STATE_YGSP_SG_F => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_SG%F_F)
      Z_YDMF_PHYS_SURF_GSD_VF_PLSM => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_LSM)
      Z_YDMF_PHYS_SURF_GSD_VF_PVEG => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_VEG)
      Z_YDMF_PHYS_OUT_CT => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_CT)
      Z_YDMF_PHYS_BASE_STATE_YGSP_SB_TL => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_SB%F_TL)
      Z_YDMF_PHYS_SURF_GSD_VV_PD2 => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_D2)
      Z_YDMF_PHYS_SURF_GSD_VF_PZ0RLF => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_Z0RLF)
      Z_YDMF_PHYS_BASE_STATE_YGSP_SB_Q => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_SB%F_Q)
      Z_YDMF_PHYS_SURF_GSD_VF_PZ0F => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_Z0F)
      Z_YDMF_PHYS_BASE_STATE_YGSP_SG_A => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_SG%F_A)
      Z_YDMF_PHYS_SURF_GSD_VV_PLAI => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_LAI)
      Z_YDMF_PHYS_SURF_GSD_VV_PZ0H => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_Z0H)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACSOL:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 146"
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACSOL:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 147"
      CALL YLCPG_BNDS%INIT(YDCPG_OPTS)
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, LLHMT ) FIRSTPRIVATE( YLCPG_BNDS )
      DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
        CALL YLCPG_BNDS%UPDATE(JBLK)
        
        IF (YDMODEL%YRML_PHY_MF%YRPHY%LSOLV) THEN
          LLHMT = .false.
          CALL ACSOL(YDCPG_OPTS%YRCLI, YDMODEL%YRCST, YDMODEL%YRML_PHY_MF%YRPHY, YDMODEL%YRML_PHY_MF%YRPHY1,  &
          & YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, Z_YDMF_PHYS_SURF_GSD_VV_PARG(:, JBLK),  &
          & Z_YDMF_PHYS_SURF_GSD_VV_PD2(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VF_PZ0F(:, JBLK),  &
          & Z_YDMF_PHYS_SURF_GSD_VV_PZ0H(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VF_PZ0RLF(:, JBLK),  &
          & Z_YDMF_PHYS_SURF_GSD_VF_PLSM(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VV_PIVEG(:, JBLK),  &
          & Z_YDMF_PHYS_SURF_GSD_VV_PLAI(:, JBLK), Z_YDMF_PHYS_BASE_STATE_YGSP_SG_A(:, 1, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YGSP_SG_R(:, 1, JBLK), Z_YDMF_PHYS_SURF_GSD_VV_PSAB(:, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YGSP_SG_F(:, 1, JBLK), Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T(:, JBLK),  &
          & Z_YDMF_PHYS_SURF_GSD_VF_PVEG(:, JBLK), Z_YDMF_PHYS_BASE_STATE_YGSP_SB_Q(:, 1, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YGSP_SB_TL(:, 1, JBLK), Z_YDMF_PHYS_BASE_STATE_YGSP_RR_W(:, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YGSP_RR_IC(:, JBLK), LLHMT, ZDSA_C1(:, JBLK), ZDSA_C2(:, JBLK), ZC3(:, JBLK),  &
          & ZCG(:, JBLK), ZCN(:, JBLK), Z_YDMF_PHYS_OUT_CT(:, JBLK), ZNEIJG(:, JBLK), ZNEIJV(:, JBLK), ZWFC(:, JBLK),  &
          & ZWPMX(:, JBLK), ZWSEQ(:, JBLK), ZWSMX(:, JBLK), ZWWILT(:, JBLK))
        ELSE
          
          !DEC$ IVDEP
          DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
            Z_YDMF_PHYS_OUT_CT(JLON, JBLK) = YDMODEL%YRML_PHY_MF%YRPHY1%HSOL / (1.0_JPRB +  &
            & YDMODEL%YRML_PHY_MF%YRPHY1%HSOLIWR*(Z_YDMF_PHYS_BASE_STATE_YGSP_RR_W(JLON, JBLK) +  &
            & Z_YDMF_PHYS_BASE_STATE_YGSP_SB_Q(JLON, 1, JBLK)) / (YDMODEL%YRML_PHY_MF%YRPHY1%WSMX +  &
            & YDMODEL%YRML_PHY_MF%YRPHY1%WPMX) &
            & *EXP(-0.5_JPRB*(YDMODEL%YRML_PHY_MF%YRPHY1%HSOLIT0*(Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T(JLON, JBLK) -  &
            & YDMODEL%YRCST%RTT))**2))
          END DO
        END IF
        
      END DO
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACSOL:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 148"
      IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ACSOL')) THEN
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACSOL:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 149"
        ZDSA_C2 => GET_HOST_DATA_RDWR(YL_ZDSA_C2)
        ZC3 => GET_HOST_DATA_RDWR(YL_ZC3)
        ZWSEQ => GET_HOST_DATA_RDWR(YL_ZWSEQ)
        ZCG => GET_HOST_DATA_RDWR(YL_ZCG)
        ZDSA_C1 => GET_HOST_DATA_RDWR(YL_ZDSA_C1)
        ZWFC => GET_HOST_DATA_RDWR(YL_ZWFC)
        ZNEIJV => GET_HOST_DATA_RDWR(YL_ZNEIJV)
        ZWWILT => GET_HOST_DATA_RDWR(YL_ZWWILT)
        ZNEIJG => GET_HOST_DATA_RDWR(YL_ZNEIJG)
        ZCN => GET_HOST_DATA_RDWR(YL_ZCN)
        ZWSMX => GET_HOST_DATA_RDWR(YL_ZWSMX)
        ZWPMX => GET_HOST_DATA_RDWR(YL_ZWPMX)
        Z_YDMF_PHYS_SURF_GSD_VV_PIVEG => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_IVEG)
        Z_YDMF_PHYS_BASE_STATE_YGSP_SG_R => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_SG%F_R)
        Z_YDMF_PHYS_SURF_GSD_VV_PARG => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_ARG)
        Z_YDMF_PHYS_SURF_GSD_VV_PSAB => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_SAB)
        Z_YDMF_PHYS_BASE_STATE_YGSP_RR_W => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_W)
        Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
        Z_YDMF_PHYS_BASE_STATE_YGSP_RR_IC => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_IC)
        Z_YDMF_PHYS_BASE_STATE_YGSP_SG_F => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_SG%F_F)
        Z_YDMF_PHYS_SURF_GSD_VF_PLSM => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_LSM)
        Z_YDMF_PHYS_SURF_GSD_VF_PVEG => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_VEG)
        Z_YDMF_PHYS_OUT_CT => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_CT)
        Z_YDMF_PHYS_BASE_STATE_YGSP_SB_TL => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_SB%F_TL)
        Z_YDMF_PHYS_SURF_GSD_VV_PD2 => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_D2)
        Z_YDMF_PHYS_SURF_GSD_VF_PZ0RLF => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_Z0RLF)
        Z_YDMF_PHYS_BASE_STATE_YGSP_SB_Q => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_SB%F_Q)
        Z_YDMF_PHYS_SURF_GSD_VF_PZ0F => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_Z0F)
        Z_YDMF_PHYS_BASE_STATE_YGSP_SG_A => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_SG%F_A)
        Z_YDMF_PHYS_SURF_GSD_VV_PLAI => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_LAI)
        Z_YDMF_PHYS_SURF_GSD_VV_PZ0H => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_Z0H)
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACSOL:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 150"
      END IF
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACSOL:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 151"
      ZDSA_C2 => NULL()
      ZC3 => NULL()
      ZWSEQ => NULL()
      ZCG => NULL()
      ZDSA_C1 => NULL()
      ZWFC => NULL()
      ZNEIJV => NULL()
      ZWWILT => NULL()
      ZNEIJG => NULL()
      ZCN => NULL()
      ZWSMX => NULL()
      ZWPMX => NULL()
      Z_YDMF_PHYS_SURF_GSD_VV_PIVEG => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_SG_R => NULL()
      Z_YDMF_PHYS_SURF_GSD_VV_PARG => NULL()
      Z_YDMF_PHYS_SURF_GSD_VV_PSAB => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_W => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_IC => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_SG_F => NULL()
      Z_YDMF_PHYS_SURF_GSD_VF_PLSM => NULL()
      Z_YDMF_PHYS_SURF_GSD_VF_PVEG => NULL()
      Z_YDMF_PHYS_OUT_CT => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_SB_TL => NULL()
      Z_YDMF_PHYS_SURF_GSD_VV_PD2 => NULL()
      Z_YDMF_PHYS_SURF_GSD_VF_PZ0RLF => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_SB_Q => NULL()
      Z_YDMF_PHYS_SURF_GSD_VF_PZ0F => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_SG_A => NULL()
      Z_YDMF_PHYS_SURF_GSD_VV_PLAI => NULL()
      Z_YDMF_PHYS_SURF_GSD_VV_PZ0H => NULL()
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACSOL:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 152"
    ELSE IF (LPARALLELMETHOD('OPENMPSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ACSOL')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACSOL:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 153"
      ZDSA_C2 => GET_HOST_DATA_RDWR(YL_ZDSA_C2)
      ZC3 => GET_HOST_DATA_RDWR(YL_ZC3)
      ZWSEQ => GET_HOST_DATA_RDWR(YL_ZWSEQ)
      ZCG => GET_HOST_DATA_RDWR(YL_ZCG)
      ZDSA_C1 => GET_HOST_DATA_RDWR(YL_ZDSA_C1)
      ZWFC => GET_HOST_DATA_RDWR(YL_ZWFC)
      ZNEIJV => GET_HOST_DATA_RDWR(YL_ZNEIJV)
      ZWWILT => GET_HOST_DATA_RDWR(YL_ZWWILT)
      ZNEIJG => GET_HOST_DATA_RDWR(YL_ZNEIJG)
      ZCN => GET_HOST_DATA_RDWR(YL_ZCN)
      ZWSMX => GET_HOST_DATA_RDWR(YL_ZWSMX)
      ZWPMX => GET_HOST_DATA_RDWR(YL_ZWPMX)
      Z_YDMF_PHYS_SURF_GSD_VV_PIVEG => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_IVEG)
      Z_YDMF_PHYS_BASE_STATE_YGSP_SG_R => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_SG%F_R)
      Z_YDMF_PHYS_SURF_GSD_VV_PARG => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_ARG)
      Z_YDMF_PHYS_SURF_GSD_VV_PSAB => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_SAB)
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_W => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_W)
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_IC => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_IC)
      Z_YDMF_PHYS_BASE_STATE_YGSP_SG_F => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_SG%F_F)
      Z_YDMF_PHYS_SURF_GSD_VF_PLSM => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_LSM)
      Z_YDMF_PHYS_SURF_GSD_VF_PVEG => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_VEG)
      Z_YDMF_PHYS_OUT_CT => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_CT)
      Z_YDMF_PHYS_BASE_STATE_YGSP_SB_TL => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_SB%F_TL)
      Z_YDMF_PHYS_SURF_GSD_VV_PD2 => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_D2)
      Z_YDMF_PHYS_SURF_GSD_VF_PZ0RLF => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_Z0RLF)
      Z_YDMF_PHYS_BASE_STATE_YGSP_SB_Q => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_SB%F_Q)
      Z_YDMF_PHYS_SURF_GSD_VF_PZ0F => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_Z0F)
      Z_YDMF_PHYS_BASE_STATE_YGSP_SG_A => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_SG%F_A)
      Z_YDMF_PHYS_SURF_GSD_VV_PLAI => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_LAI)
      Z_YDMF_PHYS_SURF_GSD_VV_PZ0H => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_Z0H)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACSOL:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 154"
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACSOL:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 155"
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK, LLHMT )
      DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
        DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
          YLCPG_BNDS%KIDIA = JLON
          YLCPG_BNDS%KFDIA = JLON
          YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          
          IF (YDMODEL%YRML_PHY_MF%YRPHY%LSOLV) THEN
            LLHMT = .false.
            CALL ACSOL_OPENACC(YDCPG_OPTS%YRCLI, YDMODEL%YRCST, YDMODEL%YRML_PHY_MF%YRPHY, YDMODEL%YRML_PHY_MF%YRPHY1,  &
            & YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, Z_YDMF_PHYS_SURF_GSD_VV_PARG(:, JBLK),  &
            & Z_YDMF_PHYS_SURF_GSD_VV_PD2(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VF_PZ0F(:, JBLK),  &
            & Z_YDMF_PHYS_SURF_GSD_VV_PZ0H(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VF_PZ0RLF(:, JBLK),  &
            & Z_YDMF_PHYS_SURF_GSD_VF_PLSM(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VV_PIVEG(:, JBLK),  &
            & Z_YDMF_PHYS_SURF_GSD_VV_PLAI(:, JBLK), Z_YDMF_PHYS_BASE_STATE_YGSP_SG_A(:, 1, JBLK),  &
            & Z_YDMF_PHYS_BASE_STATE_YGSP_SG_R(:, 1, JBLK), Z_YDMF_PHYS_SURF_GSD_VV_PSAB(:, JBLK),  &
            & Z_YDMF_PHYS_BASE_STATE_YGSP_SG_F(:, 1, JBLK), Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T(:, JBLK),  &
            & Z_YDMF_PHYS_SURF_GSD_VF_PVEG(:, JBLK), Z_YDMF_PHYS_BASE_STATE_YGSP_SB_Q(:, 1, JBLK),  &
            & Z_YDMF_PHYS_BASE_STATE_YGSP_SB_TL(:, 1, JBLK), Z_YDMF_PHYS_BASE_STATE_YGSP_RR_W(:, JBLK),  &
            & Z_YDMF_PHYS_BASE_STATE_YGSP_RR_IC(:, JBLK), LLHMT, ZDSA_C1(:, JBLK), ZDSA_C2(:, JBLK), ZC3(:, JBLK),  &
            & ZCG(:, JBLK), ZCN(:, JBLK), Z_YDMF_PHYS_OUT_CT(:, JBLK), ZNEIJG(:, JBLK), ZNEIJV(:, JBLK), ZWFC(:, JBLK),  &
            & ZWPMX(:, JBLK), ZWSEQ(:, JBLK), ZWSMX(:, JBLK), ZWWILT(:, JBLK), YDSTACK=YLSTACK)
          ELSE
            
            !DEC$ IVDEP
            Z_YDMF_PHYS_OUT_CT(JLON, JBLK) = YDMODEL%YRML_PHY_MF%YRPHY1%HSOL / (1.0_JPRB +  &
            & YDMODEL%YRML_PHY_MF%YRPHY1%HSOLIWR*(Z_YDMF_PHYS_BASE_STATE_YGSP_RR_W(JLON, JBLK) +  &
            & Z_YDMF_PHYS_BASE_STATE_YGSP_SB_Q(JLON, 1, JBLK)) / (YDMODEL%YRML_PHY_MF%YRPHY1%WSMX +  &
            & YDMODEL%YRML_PHY_MF%YRPHY1%WPMX) &
            & *EXP(-0.5_JPRB*(YDMODEL%YRML_PHY_MF%YRPHY1%HSOLIT0*(Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T(JLON, JBLK) -  &
            & YDMODEL%YRCST%RTT))**2))
          END IF
          
        END DO
      END DO
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACSOL:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 156"
      IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ACSOL')) THEN
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACSOL:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 157"
        ZDSA_C2 => GET_HOST_DATA_RDWR(YL_ZDSA_C2)
        ZC3 => GET_HOST_DATA_RDWR(YL_ZC3)
        ZWSEQ => GET_HOST_DATA_RDWR(YL_ZWSEQ)
        ZCG => GET_HOST_DATA_RDWR(YL_ZCG)
        ZDSA_C1 => GET_HOST_DATA_RDWR(YL_ZDSA_C1)
        ZWFC => GET_HOST_DATA_RDWR(YL_ZWFC)
        ZNEIJV => GET_HOST_DATA_RDWR(YL_ZNEIJV)
        ZWWILT => GET_HOST_DATA_RDWR(YL_ZWWILT)
        ZNEIJG => GET_HOST_DATA_RDWR(YL_ZNEIJG)
        ZCN => GET_HOST_DATA_RDWR(YL_ZCN)
        ZWSMX => GET_HOST_DATA_RDWR(YL_ZWSMX)
        ZWPMX => GET_HOST_DATA_RDWR(YL_ZWPMX)
        Z_YDMF_PHYS_SURF_GSD_VV_PIVEG => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_IVEG)
        Z_YDMF_PHYS_BASE_STATE_YGSP_SG_R => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_SG%F_R)
        Z_YDMF_PHYS_SURF_GSD_VV_PARG => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_ARG)
        Z_YDMF_PHYS_SURF_GSD_VV_PSAB => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_SAB)
        Z_YDMF_PHYS_BASE_STATE_YGSP_RR_W => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_W)
        Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
        Z_YDMF_PHYS_BASE_STATE_YGSP_RR_IC => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_IC)
        Z_YDMF_PHYS_BASE_STATE_YGSP_SG_F => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_SG%F_F)
        Z_YDMF_PHYS_SURF_GSD_VF_PLSM => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_LSM)
        Z_YDMF_PHYS_SURF_GSD_VF_PVEG => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_VEG)
        Z_YDMF_PHYS_OUT_CT => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_CT)
        Z_YDMF_PHYS_BASE_STATE_YGSP_SB_TL => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_SB%F_TL)
        Z_YDMF_PHYS_SURF_GSD_VV_PD2 => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_D2)
        Z_YDMF_PHYS_SURF_GSD_VF_PZ0RLF => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_Z0RLF)
        Z_YDMF_PHYS_BASE_STATE_YGSP_SB_Q => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_SB%F_Q)
        Z_YDMF_PHYS_SURF_GSD_VF_PZ0F => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_Z0F)
        Z_YDMF_PHYS_BASE_STATE_YGSP_SG_A => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_SG%F_A)
        Z_YDMF_PHYS_SURF_GSD_VV_PLAI => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_LAI)
        Z_YDMF_PHYS_SURF_GSD_VV_PZ0H => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_Z0H)
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACSOL:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 158"
      END IF
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACSOL:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 159"
      ZDSA_C2 => NULL()
      ZC3 => NULL()
      ZWSEQ => NULL()
      ZCG => NULL()
      ZDSA_C1 => NULL()
      ZWFC => NULL()
      ZNEIJV => NULL()
      ZWWILT => NULL()
      ZNEIJG => NULL()
      ZCN => NULL()
      ZWSMX => NULL()
      ZWPMX => NULL()
      Z_YDMF_PHYS_SURF_GSD_VV_PIVEG => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_SG_R => NULL()
      Z_YDMF_PHYS_SURF_GSD_VV_PARG => NULL()
      Z_YDMF_PHYS_SURF_GSD_VV_PSAB => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_W => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_IC => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_SG_F => NULL()
      Z_YDMF_PHYS_SURF_GSD_VF_PLSM => NULL()
      Z_YDMF_PHYS_SURF_GSD_VF_PVEG => NULL()
      Z_YDMF_PHYS_OUT_CT => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_SB_TL => NULL()
      Z_YDMF_PHYS_SURF_GSD_VV_PD2 => NULL()
      Z_YDMF_PHYS_SURF_GSD_VF_PZ0RLF => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_SB_Q => NULL()
      Z_YDMF_PHYS_SURF_GSD_VF_PZ0F => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_SG_A => NULL()
      Z_YDMF_PHYS_SURF_GSD_VV_PLAI => NULL()
      Z_YDMF_PHYS_SURF_GSD_VV_PZ0H => NULL()
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACSOL:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 160"
    ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ACSOL')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACSOL:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 161"
      ZDSA_C2 => GET_DEVICE_DATA_RDWR(YL_ZDSA_C2)
      ZC3 => GET_DEVICE_DATA_RDWR(YL_ZC3)
      ZWSEQ => GET_DEVICE_DATA_RDWR(YL_ZWSEQ)
      ZCG => GET_DEVICE_DATA_RDWR(YL_ZCG)
      ZDSA_C1 => GET_DEVICE_DATA_RDWR(YL_ZDSA_C1)
      ZWFC => GET_DEVICE_DATA_RDWR(YL_ZWFC)
      ZNEIJV => GET_DEVICE_DATA_RDWR(YL_ZNEIJV)
      ZWWILT => GET_DEVICE_DATA_RDWR(YL_ZWWILT)
      ZNEIJG => GET_DEVICE_DATA_RDWR(YL_ZNEIJG)
      ZCN => GET_DEVICE_DATA_RDWR(YL_ZCN)
      ZWSMX => GET_DEVICE_DATA_RDWR(YL_ZWSMX)
      ZWPMX => GET_DEVICE_DATA_RDWR(YL_ZWPMX)
      Z_YDMF_PHYS_SURF_GSD_VV_PIVEG => GET_DEVICE_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_IVEG)
      Z_YDMF_PHYS_BASE_STATE_YGSP_SG_R => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_SG%F_R)
      Z_YDMF_PHYS_SURF_GSD_VV_PARG => GET_DEVICE_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_ARG)
      Z_YDMF_PHYS_SURF_GSD_VV_PSAB => GET_DEVICE_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_SAB)
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_W => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_W)
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_IC => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_IC)
      Z_YDMF_PHYS_BASE_STATE_YGSP_SG_F => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_SG%F_F)
      Z_YDMF_PHYS_SURF_GSD_VF_PLSM => GET_DEVICE_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_LSM)
      Z_YDMF_PHYS_SURF_GSD_VF_PVEG => GET_DEVICE_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_VEG)
      Z_YDMF_PHYS_OUT_CT => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_CT)
      Z_YDMF_PHYS_BASE_STATE_YGSP_SB_TL => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_SB%F_TL)
      Z_YDMF_PHYS_SURF_GSD_VV_PD2 => GET_DEVICE_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_D2)
      Z_YDMF_PHYS_SURF_GSD_VF_PZ0RLF => GET_DEVICE_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_Z0RLF)
      Z_YDMF_PHYS_BASE_STATE_YGSP_SB_Q => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_SB%F_Q)
      Z_YDMF_PHYS_SURF_GSD_VF_PZ0F => GET_DEVICE_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_Z0F)
      Z_YDMF_PHYS_BASE_STATE_YGSP_SG_A => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_SG%F_A)
      Z_YDMF_PHYS_SURF_GSD_VV_PLAI => GET_DEVICE_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_LAI)
      Z_YDMF_PHYS_SURF_GSD_VV_PZ0H => GET_DEVICE_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_Z0H)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACSOL:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 162"
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACSOL:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 163"
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZDSA_C2, ZC3, ZWSEQ, ZCG, ZDSA_C1, ZWFC, ZNEIJV, ZWWILT,  &
!$ACC & ZNEIJG, ZCN, ZWSMX, ZWPMX, Z_YDMF_PHYS_SURF_GSD_VV_PIVEG, Z_YDMF_PHYS_BASE_STATE_YGSP_SG_R,  &
!$ACC & Z_YDMF_PHYS_SURF_GSD_VV_PARG, Z_YDMF_PHYS_SURF_GSD_VV_PSAB, Z_YDMF_PHYS_BASE_STATE_YGSP_RR_W,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T, Z_YDMF_PHYS_BASE_STATE_YGSP_RR_IC, Z_YDMF_PHYS_BASE_STATE_YGSP_SG_F,  &
!$ACC & Z_YDMF_PHYS_SURF_GSD_VF_PLSM, Z_YDMF_PHYS_SURF_GSD_VF_PVEG, Z_YDMF_PHYS_OUT_CT, Z_YDMF_PHYS_BASE_STATE_YGSP_SB_TL,  &
!$ACC & Z_YDMF_PHYS_SURF_GSD_VV_PD2, Z_YDMF_PHYS_SURF_GSD_VF_PZ0RLF, Z_YDMF_PHYS_BASE_STATE_YGSP_SB_Q,  &
!$ACC & Z_YDMF_PHYS_SURF_GSD_VF_PZ0F, Z_YDMF_PHYS_BASE_STATE_YGSP_SG_A, Z_YDMF_PHYS_SURF_GSD_VV_PLAI,  &
!$ACC & Z_YDMF_PHYS_SURF_GSD_VV_PZ0H ) PRIVATE( JBLK ) VECTOR_LENGTH( YDCPG_OPTS%KLON )
      DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK, LLHMT )
        DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
          YLCPG_BNDS%KIDIA = JLON
          YLCPG_BNDS%KFDIA = JLON
          YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          
          IF (YDMODEL%YRML_PHY_MF%YRPHY%LSOLV) THEN
            LLHMT = .false.
            CALL ACSOL_OPENACC(YDCPG_OPTS%YRCLI, YDMODEL%YRCST, YDMODEL%YRML_PHY_MF%YRPHY, YDMODEL%YRML_PHY_MF%YRPHY1,  &
            & YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, Z_YDMF_PHYS_SURF_GSD_VV_PARG(:, JBLK),  &
            & Z_YDMF_PHYS_SURF_GSD_VV_PD2(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VF_PZ0F(:, JBLK),  &
            & Z_YDMF_PHYS_SURF_GSD_VV_PZ0H(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VF_PZ0RLF(:, JBLK),  &
            & Z_YDMF_PHYS_SURF_GSD_VF_PLSM(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VV_PIVEG(:, JBLK),  &
            & Z_YDMF_PHYS_SURF_GSD_VV_PLAI(:, JBLK), Z_YDMF_PHYS_BASE_STATE_YGSP_SG_A(:, 1, JBLK),  &
            & Z_YDMF_PHYS_BASE_STATE_YGSP_SG_R(:, 1, JBLK), Z_YDMF_PHYS_SURF_GSD_VV_PSAB(:, JBLK),  &
            & Z_YDMF_PHYS_BASE_STATE_YGSP_SG_F(:, 1, JBLK), Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T(:, JBLK),  &
            & Z_YDMF_PHYS_SURF_GSD_VF_PVEG(:, JBLK), Z_YDMF_PHYS_BASE_STATE_YGSP_SB_Q(:, 1, JBLK),  &
            & Z_YDMF_PHYS_BASE_STATE_YGSP_SB_TL(:, 1, JBLK), Z_YDMF_PHYS_BASE_STATE_YGSP_RR_W(:, JBLK),  &
            & Z_YDMF_PHYS_BASE_STATE_YGSP_RR_IC(:, JBLK), LLHMT, ZDSA_C1(:, JBLK), ZDSA_C2(:, JBLK), ZC3(:, JBLK),  &
            & ZCG(:, JBLK), ZCN(:, JBLK), Z_YDMF_PHYS_OUT_CT(:, JBLK), ZNEIJG(:, JBLK), ZNEIJV(:, JBLK), ZWFC(:, JBLK),  &
            & ZWPMX(:, JBLK), ZWSEQ(:, JBLK), ZWSMX(:, JBLK), ZWWILT(:, JBLK), YDSTACK=YLSTACK)
          ELSE
            
            !DEC$ IVDEP
            Z_YDMF_PHYS_OUT_CT(JLON, JBLK) = YDMODEL%YRML_PHY_MF%YRPHY1%HSOL / (1.0_JPRB +  &
            & YDMODEL%YRML_PHY_MF%YRPHY1%HSOLIWR*(Z_YDMF_PHYS_BASE_STATE_YGSP_RR_W(JLON, JBLK) +  &
            & Z_YDMF_PHYS_BASE_STATE_YGSP_SB_Q(JLON, 1, JBLK)) / (YDMODEL%YRML_PHY_MF%YRPHY1%WSMX +  &
            & YDMODEL%YRML_PHY_MF%YRPHY1%WPMX) &
            & *EXP(-0.5_JPRB*(YDMODEL%YRML_PHY_MF%YRPHY1%HSOLIT0*(Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T(JLON, JBLK) -  &
            & YDMODEL%YRCST%RTT))**2))
          END IF
          
        END DO
      END DO
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACSOL:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 164"
      IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ACSOL')) THEN
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACSOL:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 165"
        ZDSA_C2 => GET_HOST_DATA_RDWR(YL_ZDSA_C2)
        ZC3 => GET_HOST_DATA_RDWR(YL_ZC3)
        ZWSEQ => GET_HOST_DATA_RDWR(YL_ZWSEQ)
        ZCG => GET_HOST_DATA_RDWR(YL_ZCG)
        ZDSA_C1 => GET_HOST_DATA_RDWR(YL_ZDSA_C1)
        ZWFC => GET_HOST_DATA_RDWR(YL_ZWFC)
        ZNEIJV => GET_HOST_DATA_RDWR(YL_ZNEIJV)
        ZWWILT => GET_HOST_DATA_RDWR(YL_ZWWILT)
        ZNEIJG => GET_HOST_DATA_RDWR(YL_ZNEIJG)
        ZCN => GET_HOST_DATA_RDWR(YL_ZCN)
        ZWSMX => GET_HOST_DATA_RDWR(YL_ZWSMX)
        ZWPMX => GET_HOST_DATA_RDWR(YL_ZWPMX)
        Z_YDMF_PHYS_SURF_GSD_VV_PIVEG => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_IVEG)
        Z_YDMF_PHYS_BASE_STATE_YGSP_SG_R => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_SG%F_R)
        Z_YDMF_PHYS_SURF_GSD_VV_PARG => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_ARG)
        Z_YDMF_PHYS_SURF_GSD_VV_PSAB => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_SAB)
        Z_YDMF_PHYS_BASE_STATE_YGSP_RR_W => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_W)
        Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
        Z_YDMF_PHYS_BASE_STATE_YGSP_RR_IC => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_IC)
        Z_YDMF_PHYS_BASE_STATE_YGSP_SG_F => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_SG%F_F)
        Z_YDMF_PHYS_SURF_GSD_VF_PLSM => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_LSM)
        Z_YDMF_PHYS_SURF_GSD_VF_PVEG => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_VEG)
        Z_YDMF_PHYS_OUT_CT => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_CT)
        Z_YDMF_PHYS_BASE_STATE_YGSP_SB_TL => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_SB%F_TL)
        Z_YDMF_PHYS_SURF_GSD_VV_PD2 => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_D2)
        Z_YDMF_PHYS_SURF_GSD_VF_PZ0RLF => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_Z0RLF)
        Z_YDMF_PHYS_BASE_STATE_YGSP_SB_Q => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_SB%F_Q)
        Z_YDMF_PHYS_SURF_GSD_VF_PZ0F => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_Z0F)
        Z_YDMF_PHYS_BASE_STATE_YGSP_SG_A => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_SG%F_A)
        Z_YDMF_PHYS_SURF_GSD_VV_PLAI => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_LAI)
        Z_YDMF_PHYS_SURF_GSD_VV_PZ0H => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_Z0H)
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACSOL:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 166"
      END IF
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACSOL:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 167"
      ZDSA_C2 => NULL()
      ZC3 => NULL()
      ZWSEQ => NULL()
      ZCG => NULL()
      ZDSA_C1 => NULL()
      ZWFC => NULL()
      ZNEIJV => NULL()
      ZWWILT => NULL()
      ZNEIJG => NULL()
      ZCN => NULL()
      ZWSMX => NULL()
      ZWPMX => NULL()
      Z_YDMF_PHYS_SURF_GSD_VV_PIVEG => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_SG_R => NULL()
      Z_YDMF_PHYS_SURF_GSD_VV_PARG => NULL()
      Z_YDMF_PHYS_SURF_GSD_VV_PSAB => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_W => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_IC => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_SG_F => NULL()
      Z_YDMF_PHYS_SURF_GSD_VF_PLSM => NULL()
      Z_YDMF_PHYS_SURF_GSD_VF_PVEG => NULL()
      Z_YDMF_PHYS_OUT_CT => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_SB_TL => NULL()
      Z_YDMF_PHYS_SURF_GSD_VV_PD2 => NULL()
      Z_YDMF_PHYS_SURF_GSD_VF_PZ0RLF => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_SB_Q => NULL()
      Z_YDMF_PHYS_SURF_GSD_VF_PZ0F => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_SG_A => NULL()
      Z_YDMF_PHYS_SURF_GSD_VV_PLAI => NULL()
      Z_YDMF_PHYS_SURF_GSD_VV_PZ0H => NULL()
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACSOL:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 168"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACSOL', 1, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 169"
    
  END IF
  
  
  CALL APL_ARPEGE_INIT_SURFEX_PARALLEL(YDMF_PHYS_BASE_STATE, YDGEOMETRY, YDCPG_BNDS, YDCPG_OPTS, YDCPG_MISC, YDCPG_GPAR,  &
  & YDMF_PHYS, YDVARS, YDMODEL, YL_ZALBD, YL_ZALBP, YL_ZFLU_EMIS, YL_ZSGROUPEL, YL_ZSRAIN, YL_ZSSNOW, YL_ZTSN)
  
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDPHIV', 0, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 170"
  IF (LPARALLELMETHOD('OPENMP', 'APL_ARPEGE_PARALLEL:ZDPHIV')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDPHIV:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 171"
    ZDPHIV => GET_HOST_DATA_RDWR(YL_ZDPHIV)
    ZDPHIT => GET_HOST_DATA_RDWR(YL_ZDPHIT)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDPHIV:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 172"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDPHIV:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 173"
    CALL YLCPG_BNDS%INIT(YDCPG_OPTS)
!$OMP PARALLEL DO PRIVATE( JBLK, JLON ) FIRSTPRIVATE( YLCPG_BNDS )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      CALL YLCPG_BNDS%UPDATE(JBLK)
      
      DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
        ZDPHIV(JLON, JBLK) = YDMODEL%YRCST%RG*YDMODEL%YRML_PHY_MF%YRPHY2%HVCLS
        ZDPHIT(JLON, JBLK) = YDMODEL%YRCST%RG*YDMODEL%YRML_PHY_MF%YRPHY2%HTCLS
      END DO
      
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDPHIV:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 174"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ZDPHIV')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDPHIV:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 175"
      ZDPHIV => GET_HOST_DATA_RDWR(YL_ZDPHIV)
      ZDPHIT => GET_HOST_DATA_RDWR(YL_ZDPHIT)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDPHIV:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 176"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDPHIV:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 177"
    ZDPHIV => NULL()
    ZDPHIT => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDPHIV:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 178"
  ELSE IF (LPARALLELMETHOD('OPENMPSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ZDPHIV')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDPHIV:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 179"
    ZDPHIV => GET_HOST_DATA_RDWR(YL_ZDPHIV)
    ZDPHIT => GET_HOST_DATA_RDWR(YL_ZDPHIT)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDPHIV:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 180"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDPHIV:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 181"
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        ZDPHIV(JLON, JBLK) = YDMODEL%YRCST%RG*YDMODEL%YRML_PHY_MF%YRPHY2%HVCLS
        ZDPHIT(JLON, JBLK) = YDMODEL%YRCST%RG*YDMODEL%YRML_PHY_MF%YRPHY2%HTCLS
        
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDPHIV:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 182"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ZDPHIV')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDPHIV:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 183"
      ZDPHIV => GET_HOST_DATA_RDWR(YL_ZDPHIV)
      ZDPHIT => GET_HOST_DATA_RDWR(YL_ZDPHIT)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDPHIV:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 184"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDPHIV:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 185"
    ZDPHIV => NULL()
    ZDPHIT => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDPHIV:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 186"
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ZDPHIV')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDPHIV:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 187"
    ZDPHIV => GET_DEVICE_DATA_RDWR(YL_ZDPHIV)
    ZDPHIT => GET_DEVICE_DATA_RDWR(YL_ZDPHIT)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDPHIV:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 188"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDPHIV:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 189"
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZDPHIV, ZDPHIT ) PRIVATE( JBLK ) VECTOR_LENGTH(  &
!$ACC & YDCPG_OPTS%KLON )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK )
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        ZDPHIV(JLON, JBLK) = YDMODEL%YRCST%RG*YDMODEL%YRML_PHY_MF%YRPHY2%HVCLS
        ZDPHIT(JLON, JBLK) = YDMODEL%YRCST%RG*YDMODEL%YRML_PHY_MF%YRPHY2%HTCLS
        
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDPHIV:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 190"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ZDPHIV')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDPHIV:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 191"
      ZDPHIV => GET_HOST_DATA_RDWR(YL_ZDPHIV)
      ZDPHIT => GET_HOST_DATA_RDWR(YL_ZDPHIT)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDPHIV:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 192"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDPHIV:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 193"
    ZDPHIV => NULL()
    ZDPHIT => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDPHIV:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 194"
  END IF
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDPHIV', 1, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 195"
  
  LLCLS = .true.
  LLHMT = .true.
  IF (YDMODEL%YRML_PHY_MF%YRARPHY%LMSE) THEN
    
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMTLS', 0, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 196"
    IF (LPARALLELMETHOD('OPENMP', 'APL_ARPEGE_PARALLEL:ACHMTLS')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMTLS:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 197"
      ZGWDCS => GET_HOST_DATA_RDWR(YL_ZGWDCS)
      ZDSA_CPS => GET_HOST_DATA_RDWR(YL_ZDSA_CPS)
      ZDSA_LHS => GET_HOST_DATA_RDWR(YL_ZDSA_LHS)
      ZDPHIT => GET_HOST_DATA_RDWR(YL_ZDPHIT)
      ZRTI => GET_HOST_DATA_RDWR(YL_ZRTI)
      ZNBVNO => GET_HOST_DATA_RDWR(YL_ZNBVNO)
      ZDPHI => GET_HOST_DATA_RDWR(YL_ZDPHI)
      ZFLU_CDN => GET_HOST_DATA_RDWR(YL_ZFLU_CDN)
      ZPRS => GET_HOST_DATA_RDWR(YL_ZPRS)
      ZPCLS => GET_HOST_DATA_RDWR(YL_ZPCLS)
      ZMRIPP => GET_HOST_DATA_RDWR(YL_ZMRIPP)
      ZFLU_CD => GET_HOST_DATA_RDWR(YL_ZFLU_CD)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
      Z_YDCPG_MISC_QS => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QS)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
      Z_YDMF_PHYS_OUT_GZ0H => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_GZ0H)
      Z_YDMF_PHYS_BASE_STATE_U => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_R)
      Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
      Z_YDMF_PHYS_OUT_GZ0 => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_GZ0)
      Z_YDMF_PHYS_BASE_STATE_V => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMTLS:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 198"
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMTLS:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 199"
      CALL YLCPG_BNDS%INIT(YDCPG_OPTS)
!$OMP PARALLEL DO PRIVATE( JBLK, LLCLS, JLON, ZRVMD ) FIRSTPRIVATE( YLCPG_BNDS )
      DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
        CALL YLCPG_BNDS%UPDATE(JBLK)
        
        CALL ACHMTLS(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON,  &
        & YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_T(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_U(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_V(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T(:, JBLK), Z_YDCPG_MISC_QS(:, JBLK), ZDPHIT(:, JBLK),  &
        & Z_YDMF_PHYS_OUT_GZ0(:, JBLK), Z_YDMF_PHYS_OUT_GZ0H(:, JBLK), LLCLS, ZNBVNO(:, :, JBLK), ZMRIPP(:, :, JBLK),  &
        & ZDSA_CPS(:, JBLK), ZGWDCS(:, JBLK), ZDSA_LHS(:, JBLK), ZPCLS(:, JBLK), ZFLU_CD(:, JBLK), ZFLU_CDN(:, JBLK))
        DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
          ZDPHI(JLON, JBLK) = Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF(JLON, YDCPG_OPTS%KFLEVG, JBLK) -  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI(JLON, YDCPG_OPTS%KFLEVG, JBLK)
          ZPRS(JLON, JBLK) = YDMODEL%YRCST%RD + ZRVMD*Z_YDCPG_MISC_QS(JLON, JBLK)
          ZRTI(JLON, JBLK) = 2.0_JPRB / (Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R(JLON, YDCPG_OPTS%KFLEVG, JBLK) &
          & *Z_YDMF_PHYS_BASE_STATE_T(JLON, YDCPG_OPTS%KFLEVG, JBLK) + YDMODEL%YRCST%RKAPPA*ZDPHI(JLON, JBLK) + ZPRS(JLON,  &
          & JBLK)*Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T(JLON, JBLK))
        END DO
        
      END DO
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMTLS:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 200"
      IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ACHMTLS')) THEN
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMTLS:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 201"
        ZGWDCS => GET_HOST_DATA_RDWR(YL_ZGWDCS)
        ZDSA_CPS => GET_HOST_DATA_RDWR(YL_ZDSA_CPS)
        ZDSA_LHS => GET_HOST_DATA_RDWR(YL_ZDSA_LHS)
        ZDPHIT => GET_HOST_DATA_RDWR(YL_ZDPHIT)
        ZRTI => GET_HOST_DATA_RDWR(YL_ZRTI)
        ZNBVNO => GET_HOST_DATA_RDWR(YL_ZNBVNO)
        ZDPHI => GET_HOST_DATA_RDWR(YL_ZDPHI)
        ZFLU_CDN => GET_HOST_DATA_RDWR(YL_ZFLU_CDN)
        ZPRS => GET_HOST_DATA_RDWR(YL_ZPRS)
        ZPCLS => GET_HOST_DATA_RDWR(YL_ZPCLS)
        ZMRIPP => GET_HOST_DATA_RDWR(YL_ZMRIPP)
        ZFLU_CD => GET_HOST_DATA_RDWR(YL_ZFLU_CD)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
        Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
        Z_YDCPG_MISC_QS => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QS)
        Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
        Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
        Z_YDMF_PHYS_OUT_GZ0H => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_GZ0H)
        Z_YDMF_PHYS_BASE_STATE_U => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_R)
        Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
        Z_YDMF_PHYS_OUT_GZ0 => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_GZ0)
        Z_YDMF_PHYS_BASE_STATE_V => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMTLS:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 202"
      END IF
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMTLS:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 203"
      ZGWDCS => NULL()
      ZDSA_CPS => NULL()
      ZDSA_LHS => NULL()
      ZDPHIT => NULL()
      ZRTI => NULL()
      ZNBVNO => NULL()
      ZDPHI => NULL()
      ZFLU_CDN => NULL()
      ZPRS => NULL()
      ZPCLS => NULL()
      ZMRIPP => NULL()
      ZFLU_CD => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => NULL()
      Z_YDCPG_MISC_QS => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => NULL()
      Z_YDMF_PHYS_OUT_GZ0H => NULL()
      Z_YDMF_PHYS_BASE_STATE_U => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => NULL()
      Z_YDMF_PHYS_BASE_STATE_T => NULL()
      Z_YDMF_PHYS_OUT_GZ0 => NULL()
      Z_YDMF_PHYS_BASE_STATE_V => NULL()
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMTLS:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 204"
    ELSE IF (LPARALLELMETHOD('OPENMPSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ACHMTLS')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMTLS:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 205"
      ZGWDCS => GET_HOST_DATA_RDWR(YL_ZGWDCS)
      ZDSA_CPS => GET_HOST_DATA_RDWR(YL_ZDSA_CPS)
      ZDSA_LHS => GET_HOST_DATA_RDWR(YL_ZDSA_LHS)
      ZDPHIT => GET_HOST_DATA_RDWR(YL_ZDPHIT)
      ZRTI => GET_HOST_DATA_RDWR(YL_ZRTI)
      ZNBVNO => GET_HOST_DATA_RDWR(YL_ZNBVNO)
      ZDPHI => GET_HOST_DATA_RDWR(YL_ZDPHI)
      ZFLU_CDN => GET_HOST_DATA_RDWR(YL_ZFLU_CDN)
      ZPRS => GET_HOST_DATA_RDWR(YL_ZPRS)
      ZPCLS => GET_HOST_DATA_RDWR(YL_ZPCLS)
      ZMRIPP => GET_HOST_DATA_RDWR(YL_ZMRIPP)
      ZFLU_CD => GET_HOST_DATA_RDWR(YL_ZFLU_CD)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
      Z_YDCPG_MISC_QS => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QS)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
      Z_YDMF_PHYS_OUT_GZ0H => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_GZ0H)
      Z_YDMF_PHYS_BASE_STATE_U => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_R)
      Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
      Z_YDMF_PHYS_OUT_GZ0 => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_GZ0)
      Z_YDMF_PHYS_BASE_STATE_V => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMTLS:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 206"
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMTLS:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 207"
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK, LLCLS, ZRVMD )
      DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
        DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
          YLCPG_BNDS%KIDIA = JLON
          YLCPG_BNDS%KFDIA = JLON
          YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          
          CALL ACHMTLS_OPENACC(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON,  &
          & YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_T(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_U(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_V(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T(:, JBLK), Z_YDCPG_MISC_QS(:, JBLK),  &
          & ZDPHIT(:, JBLK), Z_YDMF_PHYS_OUT_GZ0(:, JBLK), Z_YDMF_PHYS_OUT_GZ0H(:, JBLK), LLCLS, ZNBVNO(:, :, JBLK),  &
          & ZMRIPP(:, :, JBLK), ZDSA_CPS(:, JBLK), ZGWDCS(:, JBLK), ZDSA_LHS(:, JBLK), ZPCLS(:, JBLK), ZFLU_CD(:, JBLK),  &
          & ZFLU_CDN(:, JBLK), YDSTACK=YLSTACK)
          ZDPHI(JLON, JBLK) = Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF(JLON, YDCPG_OPTS%KFLEVG, JBLK) -  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI(JLON, YDCPG_OPTS%KFLEVG, JBLK)
          ZPRS(JLON, JBLK) = YDMODEL%YRCST%RD + ZRVMD*Z_YDCPG_MISC_QS(JLON, JBLK)
          ZRTI(JLON, JBLK) = 2.0_JPRB / (Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R(JLON, YDCPG_OPTS%KFLEVG, JBLK) &
          & *Z_YDMF_PHYS_BASE_STATE_T(JLON, YDCPG_OPTS%KFLEVG, JBLK) + YDMODEL%YRCST%RKAPPA*ZDPHI(JLON, JBLK) + ZPRS(JLON,  &
          & JBLK)*Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T(JLON, JBLK))
          
        END DO
      END DO
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMTLS:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 208"
      IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ACHMTLS')) THEN
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMTLS:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 209"
        ZGWDCS => GET_HOST_DATA_RDWR(YL_ZGWDCS)
        ZDSA_CPS => GET_HOST_DATA_RDWR(YL_ZDSA_CPS)
        ZDSA_LHS => GET_HOST_DATA_RDWR(YL_ZDSA_LHS)
        ZDPHIT => GET_HOST_DATA_RDWR(YL_ZDPHIT)
        ZRTI => GET_HOST_DATA_RDWR(YL_ZRTI)
        ZNBVNO => GET_HOST_DATA_RDWR(YL_ZNBVNO)
        ZDPHI => GET_HOST_DATA_RDWR(YL_ZDPHI)
        ZFLU_CDN => GET_HOST_DATA_RDWR(YL_ZFLU_CDN)
        ZPRS => GET_HOST_DATA_RDWR(YL_ZPRS)
        ZPCLS => GET_HOST_DATA_RDWR(YL_ZPCLS)
        ZMRIPP => GET_HOST_DATA_RDWR(YL_ZMRIPP)
        ZFLU_CD => GET_HOST_DATA_RDWR(YL_ZFLU_CD)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
        Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
        Z_YDCPG_MISC_QS => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QS)
        Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
        Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
        Z_YDMF_PHYS_OUT_GZ0H => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_GZ0H)
        Z_YDMF_PHYS_BASE_STATE_U => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_R)
        Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
        Z_YDMF_PHYS_OUT_GZ0 => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_GZ0)
        Z_YDMF_PHYS_BASE_STATE_V => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMTLS:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 210"
      END IF
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMTLS:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 211"
      ZGWDCS => NULL()
      ZDSA_CPS => NULL()
      ZDSA_LHS => NULL()
      ZDPHIT => NULL()
      ZRTI => NULL()
      ZNBVNO => NULL()
      ZDPHI => NULL()
      ZFLU_CDN => NULL()
      ZPRS => NULL()
      ZPCLS => NULL()
      ZMRIPP => NULL()
      ZFLU_CD => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => NULL()
      Z_YDCPG_MISC_QS => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => NULL()
      Z_YDMF_PHYS_OUT_GZ0H => NULL()
      Z_YDMF_PHYS_BASE_STATE_U => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => NULL()
      Z_YDMF_PHYS_BASE_STATE_T => NULL()
      Z_YDMF_PHYS_OUT_GZ0 => NULL()
      Z_YDMF_PHYS_BASE_STATE_V => NULL()
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMTLS:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 212"
    ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ACHMTLS')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMTLS:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 213"
      ZGWDCS => GET_DEVICE_DATA_RDWR(YL_ZGWDCS)
      ZDSA_CPS => GET_DEVICE_DATA_RDWR(YL_ZDSA_CPS)
      ZDSA_LHS => GET_DEVICE_DATA_RDWR(YL_ZDSA_LHS)
      ZDPHIT => GET_DEVICE_DATA_RDWR(YL_ZDPHIT)
      ZRTI => GET_DEVICE_DATA_RDWR(YL_ZRTI)
      ZNBVNO => GET_DEVICE_DATA_RDWR(YL_ZNBVNO)
      ZDPHI => GET_DEVICE_DATA_RDWR(YL_ZDPHI)
      ZFLU_CDN => GET_DEVICE_DATA_RDWR(YL_ZFLU_CDN)
      ZPRS => GET_DEVICE_DATA_RDWR(YL_ZPRS)
      ZPCLS => GET_DEVICE_DATA_RDWR(YL_ZPCLS)
      ZMRIPP => GET_DEVICE_DATA_RDWR(YL_ZMRIPP)
      ZFLU_CD => GET_DEVICE_DATA_RDWR(YL_ZFLU_CD)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
      Z_YDCPG_MISC_QS => GET_DEVICE_DATA_RDWR(YDCPG_MISC%F_QS)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
      Z_YDMF_PHYS_OUT_GZ0H => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_GZ0H)
      Z_YDMF_PHYS_BASE_STATE_U => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_R)
      Z_YDMF_PHYS_BASE_STATE_T => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
      Z_YDMF_PHYS_OUT_GZ0 => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_GZ0)
      Z_YDMF_PHYS_BASE_STATE_V => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMTLS:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 214"
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMTLS:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 215"
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZGWDCS, ZDSA_CPS, ZDSA_LHS, ZDPHIT, ZRTI, ZNBVNO, ZFLU_CDN,  &
!$ACC & ZPRS, ZPCLS, ZMRIPP, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF, Z_YDCPG_MISC_QS,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF, Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T, Z_YDMF_PHYS_OUT_GZ0H,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_U, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R, Z_YDMF_PHYS_BASE_STATE_T, Z_YDMF_PHYS_BASE_STATE_V  &
!$ACC & ) PRIVATE( JBLK ) VECTOR_LENGTH( YDCPG_OPTS%KLON )
      DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK, LLCLS, ZRVMD )
        DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
          YLCPG_BNDS%KIDIA = JLON
          YLCPG_BNDS%KFDIA = JLON
          YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          
          CALL ACHMTLS_OPENACC(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON,  &
          & YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_T(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_U(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_V(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T(:, JBLK), Z_YDCPG_MISC_QS(:, JBLK),  &
          & ZDPHIT(:, JBLK), Z_YDMF_PHYS_OUT_GZ0(:, JBLK), Z_YDMF_PHYS_OUT_GZ0H(:, JBLK), LLCLS, ZNBVNO(:, :, JBLK),  &
          & ZMRIPP(:, :, JBLK), ZDSA_CPS(:, JBLK), ZGWDCS(:, JBLK), ZDSA_LHS(:, JBLK), ZPCLS(:, JBLK), ZFLU_CD(:, JBLK),  &
          & ZFLU_CDN(:, JBLK), YDSTACK=YLSTACK)
          ZDPHI(JLON, JBLK) = Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF(JLON, YDCPG_OPTS%KFLEVG, JBLK) -  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI(JLON, YDCPG_OPTS%KFLEVG, JBLK)
          ZPRS(JLON, JBLK) = YDMODEL%YRCST%RD + ZRVMD*Z_YDCPG_MISC_QS(JLON, JBLK)
          ZRTI(JLON, JBLK) = 2.0_JPRB / (Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R(JLON, YDCPG_OPTS%KFLEVG, JBLK) &
          & *Z_YDMF_PHYS_BASE_STATE_T(JLON, YDCPG_OPTS%KFLEVG, JBLK) + YDMODEL%YRCST%RKAPPA*ZDPHI(JLON, JBLK) + ZPRS(JLON,  &
          & JBLK)*Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T(JLON, JBLK))
          
        END DO
      END DO
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMTLS:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 216"
      IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ACHMTLS')) THEN
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMTLS:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 217"
        ZGWDCS => GET_HOST_DATA_RDWR(YL_ZGWDCS)
        ZDSA_CPS => GET_HOST_DATA_RDWR(YL_ZDSA_CPS)
        ZDSA_LHS => GET_HOST_DATA_RDWR(YL_ZDSA_LHS)
        ZDPHIT => GET_HOST_DATA_RDWR(YL_ZDPHIT)
        ZRTI => GET_HOST_DATA_RDWR(YL_ZRTI)
        ZNBVNO => GET_HOST_DATA_RDWR(YL_ZNBVNO)
        ZDPHI => GET_HOST_DATA_RDWR(YL_ZDPHI)
        ZFLU_CDN => GET_HOST_DATA_RDWR(YL_ZFLU_CDN)
        ZPRS => GET_HOST_DATA_RDWR(YL_ZPRS)
        ZPCLS => GET_HOST_DATA_RDWR(YL_ZPCLS)
        ZMRIPP => GET_HOST_DATA_RDWR(YL_ZMRIPP)
        ZFLU_CD => GET_HOST_DATA_RDWR(YL_ZFLU_CD)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
        Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
        Z_YDCPG_MISC_QS => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QS)
        Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
        Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
        Z_YDMF_PHYS_OUT_GZ0H => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_GZ0H)
        Z_YDMF_PHYS_BASE_STATE_U => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_R)
        Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
        Z_YDMF_PHYS_OUT_GZ0 => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_GZ0)
        Z_YDMF_PHYS_BASE_STATE_V => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMTLS:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 218"
      END IF
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMTLS:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 219"
      ZGWDCS => NULL()
      ZDSA_CPS => NULL()
      ZDSA_LHS => NULL()
      ZDPHIT => NULL()
      ZRTI => NULL()
      ZNBVNO => NULL()
      ZDPHI => NULL()
      ZFLU_CDN => NULL()
      ZPRS => NULL()
      ZPCLS => NULL()
      ZMRIPP => NULL()
      ZFLU_CD => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => NULL()
      Z_YDCPG_MISC_QS => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => NULL()
      Z_YDMF_PHYS_OUT_GZ0H => NULL()
      Z_YDMF_PHYS_BASE_STATE_U => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => NULL()
      Z_YDMF_PHYS_BASE_STATE_T => NULL()
      Z_YDMF_PHYS_OUT_GZ0 => NULL()
      Z_YDMF_PHYS_BASE_STATE_V => NULL()
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMTLS:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 220"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMTLS', 1, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 221"
    
  ELSE
    
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMT', 0, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 222"
    IF (LPARALLELMETHOD('OPENMP', 'APL_ARPEGE_PARALLEL:ACHMT')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMT:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 223"
      ZRTI => GET_HOST_DATA_RDWR(YL_ZRTI)
      ZDPHIT => GET_HOST_DATA_RDWR(YL_ZDPHIT)
      ZNBVNO => GET_HOST_DATA_RDWR(YL_ZNBVNO)
      ZPCLS => GET_HOST_DATA_RDWR(YL_ZPCLS)
      ZNEIJV => GET_HOST_DATA_RDWR(YL_ZNEIJV)
      ZFLU_QSATS => GET_HOST_DATA_RDWR(YL_ZFLU_QSATS)
      ZGWDCS => GET_HOST_DATA_RDWR(YL_ZGWDCS)
      ZDPHIV => GET_HOST_DATA_RDWR(YL_ZDPHIV)
      ZQV => GET_HOST_DATA_RDWR(YL_ZQV)
      ZHQ => GET_HOST_DATA_RDWR(YL_ZHQ)
      ZDQSTS => GET_HOST_DATA_RDWR(YL_ZDQSTS)
      ZCHROV => GET_HOST_DATA_RDWR(YL_ZCHROV)
      ZNEIJG => GET_HOST_DATA_RDWR(YL_ZNEIJG)
      ZSTAB => GET_HOST_DATA_RDWR(YL_ZSTAB)
      ZDSA_CPS => GET_HOST_DATA_RDWR(YL_ZDSA_CPS)
      ZHU => GET_HOST_DATA_RDWR(YL_ZHU)
      ZXDROV => GET_HOST_DATA_RDWR(YL_ZXDROV)
      ZWFC => GET_HOST_DATA_RDWR(YL_ZWFC)
      ZDSA_RS => GET_HOST_DATA_RDWR(YL_ZDSA_RS)
      ZFLU_CDN => GET_HOST_DATA_RDWR(YL_ZFLU_CDN)
      ZFLU_CD => GET_HOST_DATA_RDWR(YL_ZFLU_CD)
      ZMRIPP => GET_HOST_DATA_RDWR(YL_ZMRIPP)
      ZXHROV => GET_HOST_DATA_RDWR(YL_ZXHROV)
      ZCDROV => GET_HOST_DATA_RDWR(YL_ZCDROV)
      ZPFL_FPLSH => GET_HOST_DATA_RDWR(YL_ZPFL_FPLSH)
      ZFLU_CH => GET_HOST_DATA_RDWR(YL_ZFLU_CH)
      ZFLU_VEG => GET_HOST_DATA_RDWR(YL_ZFLU_VEG)
      ZFLU_NEIJ => GET_HOST_DATA_RDWR(YL_ZFLU_NEIJ)
      ZPFL_FPLCH => GET_HOST_DATA_RDWR(YL_ZPFL_FPLCH)
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_W => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_W)
      Z_YDMF_PHYS_OUT_UCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_UCLS)
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
      Z_YDMF_PHYS_BASE_STATE_U => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
      Z_YDMF_PHYS_SURF_GSD_VF_PVEG => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_VEG)
      Z_YDMF_PHYS_BASE_STATE_V => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
      Z_YDMF_PHYS_OUT_QCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_QCLS)
      Z_YDMF_PHYS_SURF_GSD_VF_PZ0F => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_Z0F)
      Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_IC => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_IC)
      Z_YDMF_PHYS_OUT_NVCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_NVCLS)
      Z_YDMF_PHYS_BASE_STATE_YGSP_SG_F => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_SG%F_F)
      Z_YDMF_PHYS_SURF_GSD_VF_PZ0RLF => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_Z0RLF)
      Z_YDMF_PHYS_OUT_VGST => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_VGST)
      Z_YDMF_PHYS_OUT_VCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_VCLS)
      Z_YDMF_PHYS_SURF_GSD_VV_PZ0H => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_Z0H)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
      Z_YDMF_PHYS_SURF_GSD_VV_PHV => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_HV)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_R)
      Z_YDMF_PHYS_OUT_NUCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_NUCLS)
      Z_YDMF_PHYS_OUT_UGST => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_UGST)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
      Z_YDMF_PHYS_OUT_GZ0H => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_GZ0H)
      Z_YDMF_PHYS_SURF_GSD_VF_PLSM => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_LSM)
      Z_YDMF_PHYS_OUT_RHCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_RHCLS)
      Z_YDMF_PHYS_OUT_GZ0 => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_GZ0)
      Z_YDMF_PHYS_OUT_TCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_TCLS)
      Z_YDCPG_MISC_QS => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QS)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMT:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 224"
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMT:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 225"
      CALL YLCPG_BNDS%INIT(YDCPG_OPTS)
!$OMP PARALLEL DO PRIVATE( JBLK, LLHMT, LLCLS ) FIRSTPRIVATE( YLCPG_BNDS )
      DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
        CALL YLCPG_BNDS%UPDATE(JBLK)
        
        CALL ACHMT(YDCPG_OPTS%YRCLI, YDMODEL%YRML_PHY_MF%YRPHY, YDMODEL%YRML_PHY_MF%YRPHY0, YDMODEL%YRML_PHY_MF%YRPHY1,  &
        & YDMODEL%YRML_PHY_MF%YRPHY2, YDMODEL%YRCST, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG,  &
        & YDCPG_OPTS%YRSURF_DIMS%YSD_VVD%NUMFLDS >= 8 .and. YDMODEL%YRML_PHY_MF%YRPHY%LSOLV,  &
        & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP(:, :, JBLK), ZQV(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_T(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_U(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_V(:, :, JBLK), ZPFL_FPLSH(:, :, JBLK),  &
        & ZPFL_FPLCH(:, :, JBLK), ZDPHIT(:, JBLK), ZDPHIV(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VF_PZ0F(:, JBLK),  &
        & Z_YDMF_PHYS_SURF_GSD_VV_PZ0H(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VF_PZ0RLF(:, JBLK),  &
        & Z_YDMF_PHYS_SURF_GSD_VV_PHV(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VF_PLSM(:, JBLK), ZNEIJG(:, JBLK), ZNEIJV(:, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YGSP_SG_F(:, 1, JBLK), Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T(:, JBLK),  &
        & Z_YDMF_PHYS_SURF_GSD_VF_PVEG(:, JBLK), ZWFC(:, JBLK), Z_YDMF_PHYS_BASE_STATE_YGSP_RR_W(:, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YGSP_RR_IC(:, JBLK), LLCLS, LLHMT, ZNBVNO(:, :, JBLK), ZMRIPP(:, :, JBLK),  &
        & ZFLU_CD(:, JBLK), ZFLU_CDN(:, JBLK), ZCDROV(:, JBLK), ZFLU_CH(:, JBLK), ZCHROV(:, JBLK), ZDSA_CPS(:, JBLK),  &
        & ZDQSTS(:, JBLK), ZGWDCS(:, JBLK), Z_YDMF_PHYS_OUT_GZ0(:, JBLK), Z_YDMF_PHYS_OUT_GZ0H(:, JBLK), ZHQ(:, JBLK),  &
        & ZHU(:, JBLK), ZFLU_NEIJ(:, JBLK), Z_YDMF_PHYS_OUT_QCLS(:, JBLK), Z_YDCPG_MISC_QS(:, JBLK), ZFLU_QSATS(:, JBLK),  &
        & Z_YDMF_PHYS_OUT_RHCLS(:, JBLK), ZDSA_RS(:, JBLK), ZRTI(:, JBLK), ZSTAB(:, JBLK), Z_YDMF_PHYS_OUT_TCLS(:, JBLK),  &
        & Z_YDMF_PHYS_OUT_UCLS(:, JBLK), Z_YDMF_PHYS_OUT_VCLS(:, JBLK), Z_YDMF_PHYS_OUT_NUCLS(:, JBLK),  &
        & Z_YDMF_PHYS_OUT_NVCLS(:, JBLK), ZPCLS(:, JBLK), ZFLU_VEG(:, JBLK), ZXDROV(:, JBLK), ZXHROV(:, JBLK),  &
        & Z_YDMF_PHYS_OUT_UGST(:, JBLK), Z_YDMF_PHYS_OUT_VGST(:, JBLK))
        
      END DO
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMT:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 226"
      IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ACHMT')) THEN
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMT:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 227"
        ZRTI => GET_HOST_DATA_RDWR(YL_ZRTI)
        ZDPHIT => GET_HOST_DATA_RDWR(YL_ZDPHIT)
        ZNBVNO => GET_HOST_DATA_RDWR(YL_ZNBVNO)
        ZPCLS => GET_HOST_DATA_RDWR(YL_ZPCLS)
        ZNEIJV => GET_HOST_DATA_RDWR(YL_ZNEIJV)
        ZFLU_QSATS => GET_HOST_DATA_RDWR(YL_ZFLU_QSATS)
        ZGWDCS => GET_HOST_DATA_RDWR(YL_ZGWDCS)
        ZDPHIV => GET_HOST_DATA_RDWR(YL_ZDPHIV)
        ZQV => GET_HOST_DATA_RDWR(YL_ZQV)
        ZHQ => GET_HOST_DATA_RDWR(YL_ZHQ)
        ZDQSTS => GET_HOST_DATA_RDWR(YL_ZDQSTS)
        ZCHROV => GET_HOST_DATA_RDWR(YL_ZCHROV)
        ZNEIJG => GET_HOST_DATA_RDWR(YL_ZNEIJG)
        ZSTAB => GET_HOST_DATA_RDWR(YL_ZSTAB)
        ZDSA_CPS => GET_HOST_DATA_RDWR(YL_ZDSA_CPS)
        ZHU => GET_HOST_DATA_RDWR(YL_ZHU)
        ZXDROV => GET_HOST_DATA_RDWR(YL_ZXDROV)
        ZWFC => GET_HOST_DATA_RDWR(YL_ZWFC)
        ZDSA_RS => GET_HOST_DATA_RDWR(YL_ZDSA_RS)
        ZFLU_CDN => GET_HOST_DATA_RDWR(YL_ZFLU_CDN)
        ZFLU_CD => GET_HOST_DATA_RDWR(YL_ZFLU_CD)
        ZMRIPP => GET_HOST_DATA_RDWR(YL_ZMRIPP)
        ZXHROV => GET_HOST_DATA_RDWR(YL_ZXHROV)
        ZCDROV => GET_HOST_DATA_RDWR(YL_ZCDROV)
        ZPFL_FPLSH => GET_HOST_DATA_RDWR(YL_ZPFL_FPLSH)
        ZFLU_CH => GET_HOST_DATA_RDWR(YL_ZFLU_CH)
        ZFLU_VEG => GET_HOST_DATA_RDWR(YL_ZFLU_VEG)
        ZFLU_NEIJ => GET_HOST_DATA_RDWR(YL_ZFLU_NEIJ)
        ZPFL_FPLCH => GET_HOST_DATA_RDWR(YL_ZPFL_FPLCH)
        Z_YDMF_PHYS_BASE_STATE_YGSP_RR_W => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_W)
        Z_YDMF_PHYS_OUT_UCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_UCLS)
        Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
        Z_YDMF_PHYS_BASE_STATE_U => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
        Z_YDMF_PHYS_SURF_GSD_VF_PVEG => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_VEG)
        Z_YDMF_PHYS_BASE_STATE_V => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
        Z_YDMF_PHYS_OUT_QCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_QCLS)
        Z_YDMF_PHYS_SURF_GSD_VF_PZ0F => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_Z0F)
        Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
        Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
        Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
        Z_YDMF_PHYS_BASE_STATE_YGSP_RR_IC => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_IC)
        Z_YDMF_PHYS_OUT_NVCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_NVCLS)
        Z_YDMF_PHYS_BASE_STATE_YGSP_SG_F => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_SG%F_F)
        Z_YDMF_PHYS_SURF_GSD_VF_PZ0RLF => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_Z0RLF)
        Z_YDMF_PHYS_OUT_VGST => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_VGST)
        Z_YDMF_PHYS_OUT_VCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_VCLS)
        Z_YDMF_PHYS_SURF_GSD_VV_PZ0H => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_Z0H)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
        Z_YDMF_PHYS_SURF_GSD_VV_PHV => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_HV)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_R)
        Z_YDMF_PHYS_OUT_NUCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_NUCLS)
        Z_YDMF_PHYS_OUT_UGST => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_UGST)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
        Z_YDMF_PHYS_OUT_GZ0H => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_GZ0H)
        Z_YDMF_PHYS_SURF_GSD_VF_PLSM => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_LSM)
        Z_YDMF_PHYS_OUT_RHCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_RHCLS)
        Z_YDMF_PHYS_OUT_GZ0 => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_GZ0)
        Z_YDMF_PHYS_OUT_TCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_TCLS)
        Z_YDCPG_MISC_QS => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QS)
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMT:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 228"
      END IF
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMT:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 229"
      ZRTI => NULL()
      ZDPHIT => NULL()
      ZNBVNO => NULL()
      ZPCLS => NULL()
      ZNEIJV => NULL()
      ZFLU_QSATS => NULL()
      ZGWDCS => NULL()
      ZDPHIV => NULL()
      ZQV => NULL()
      ZHQ => NULL()
      ZDQSTS => NULL()
      ZCHROV => NULL()
      ZNEIJG => NULL()
      ZSTAB => NULL()
      ZDSA_CPS => NULL()
      ZHU => NULL()
      ZXDROV => NULL()
      ZWFC => NULL()
      ZDSA_RS => NULL()
      ZFLU_CDN => NULL()
      ZFLU_CD => NULL()
      ZMRIPP => NULL()
      ZXHROV => NULL()
      ZCDROV => NULL()
      ZPFL_FPLSH => NULL()
      ZFLU_CH => NULL()
      ZFLU_VEG => NULL()
      ZFLU_NEIJ => NULL()
      ZPFL_FPLCH => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_W => NULL()
      Z_YDMF_PHYS_OUT_UCLS => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => NULL()
      Z_YDMF_PHYS_BASE_STATE_U => NULL()
      Z_YDMF_PHYS_SURF_GSD_VF_PVEG => NULL()
      Z_YDMF_PHYS_BASE_STATE_V => NULL()
      Z_YDMF_PHYS_OUT_QCLS => NULL()
      Z_YDMF_PHYS_SURF_GSD_VF_PZ0F => NULL()
      Z_YDMF_PHYS_BASE_STATE_T => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_IC => NULL()
      Z_YDMF_PHYS_OUT_NVCLS => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_SG_F => NULL()
      Z_YDMF_PHYS_SURF_GSD_VF_PZ0RLF => NULL()
      Z_YDMF_PHYS_OUT_VGST => NULL()
      Z_YDMF_PHYS_OUT_VCLS => NULL()
      Z_YDMF_PHYS_SURF_GSD_VV_PZ0H => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => NULL()
      Z_YDMF_PHYS_SURF_GSD_VV_PHV => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => NULL()
      Z_YDMF_PHYS_OUT_NUCLS => NULL()
      Z_YDMF_PHYS_OUT_UGST => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => NULL()
      Z_YDMF_PHYS_OUT_GZ0H => NULL()
      Z_YDMF_PHYS_SURF_GSD_VF_PLSM => NULL()
      Z_YDMF_PHYS_OUT_RHCLS => NULL()
      Z_YDMF_PHYS_OUT_GZ0 => NULL()
      Z_YDMF_PHYS_OUT_TCLS => NULL()
      Z_YDCPG_MISC_QS => NULL()
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMT:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 230"
    ELSE IF (LPARALLELMETHOD('OPENMPSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ACHMT')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMT:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 231"
      ZRTI => GET_HOST_DATA_RDWR(YL_ZRTI)
      ZDPHIT => GET_HOST_DATA_RDWR(YL_ZDPHIT)
      ZNBVNO => GET_HOST_DATA_RDWR(YL_ZNBVNO)
      ZPCLS => GET_HOST_DATA_RDWR(YL_ZPCLS)
      ZNEIJV => GET_HOST_DATA_RDWR(YL_ZNEIJV)
      ZFLU_QSATS => GET_HOST_DATA_RDWR(YL_ZFLU_QSATS)
      ZGWDCS => GET_HOST_DATA_RDWR(YL_ZGWDCS)
      ZDPHIV => GET_HOST_DATA_RDWR(YL_ZDPHIV)
      ZQV => GET_HOST_DATA_RDWR(YL_ZQV)
      ZHQ => GET_HOST_DATA_RDWR(YL_ZHQ)
      ZDQSTS => GET_HOST_DATA_RDWR(YL_ZDQSTS)
      ZCHROV => GET_HOST_DATA_RDWR(YL_ZCHROV)
      ZNEIJG => GET_HOST_DATA_RDWR(YL_ZNEIJG)
      ZSTAB => GET_HOST_DATA_RDWR(YL_ZSTAB)
      ZDSA_CPS => GET_HOST_DATA_RDWR(YL_ZDSA_CPS)
      ZHU => GET_HOST_DATA_RDWR(YL_ZHU)
      ZXDROV => GET_HOST_DATA_RDWR(YL_ZXDROV)
      ZWFC => GET_HOST_DATA_RDWR(YL_ZWFC)
      ZDSA_RS => GET_HOST_DATA_RDWR(YL_ZDSA_RS)
      ZFLU_CDN => GET_HOST_DATA_RDWR(YL_ZFLU_CDN)
      ZFLU_CD => GET_HOST_DATA_RDWR(YL_ZFLU_CD)
      ZMRIPP => GET_HOST_DATA_RDWR(YL_ZMRIPP)
      ZXHROV => GET_HOST_DATA_RDWR(YL_ZXHROV)
      ZCDROV => GET_HOST_DATA_RDWR(YL_ZCDROV)
      ZPFL_FPLSH => GET_HOST_DATA_RDWR(YL_ZPFL_FPLSH)
      ZFLU_CH => GET_HOST_DATA_RDWR(YL_ZFLU_CH)
      ZFLU_VEG => GET_HOST_DATA_RDWR(YL_ZFLU_VEG)
      ZFLU_NEIJ => GET_HOST_DATA_RDWR(YL_ZFLU_NEIJ)
      ZPFL_FPLCH => GET_HOST_DATA_RDWR(YL_ZPFL_FPLCH)
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_W => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_W)
      Z_YDMF_PHYS_OUT_UCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_UCLS)
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
      Z_YDMF_PHYS_BASE_STATE_U => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
      Z_YDMF_PHYS_SURF_GSD_VF_PVEG => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_VEG)
      Z_YDMF_PHYS_BASE_STATE_V => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
      Z_YDMF_PHYS_OUT_QCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_QCLS)
      Z_YDMF_PHYS_SURF_GSD_VF_PZ0F => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_Z0F)
      Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_IC => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_IC)
      Z_YDMF_PHYS_OUT_NVCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_NVCLS)
      Z_YDMF_PHYS_BASE_STATE_YGSP_SG_F => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_SG%F_F)
      Z_YDMF_PHYS_SURF_GSD_VF_PZ0RLF => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_Z0RLF)
      Z_YDMF_PHYS_OUT_VGST => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_VGST)
      Z_YDMF_PHYS_OUT_VCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_VCLS)
      Z_YDMF_PHYS_SURF_GSD_VV_PZ0H => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_Z0H)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
      Z_YDMF_PHYS_SURF_GSD_VV_PHV => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_HV)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_R)
      Z_YDMF_PHYS_OUT_NUCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_NUCLS)
      Z_YDMF_PHYS_OUT_UGST => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_UGST)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
      Z_YDMF_PHYS_OUT_GZ0H => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_GZ0H)
      Z_YDMF_PHYS_SURF_GSD_VF_PLSM => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_LSM)
      Z_YDMF_PHYS_OUT_RHCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_RHCLS)
      Z_YDMF_PHYS_OUT_GZ0 => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_GZ0)
      Z_YDMF_PHYS_OUT_TCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_TCLS)
      Z_YDCPG_MISC_QS => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QS)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMT:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 232"
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMT:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 233"
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK, LLHMT, LLCLS )
      DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
        DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
          YLCPG_BNDS%KIDIA = JLON
          YLCPG_BNDS%KFDIA = JLON
          YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          
          CALL ACHMT_OPENACC(YDCPG_OPTS%YRCLI, YDMODEL%YRML_PHY_MF%YRPHY, YDMODEL%YRML_PHY_MF%YRPHY0,  &
          & YDMODEL%YRML_PHY_MF%YRPHY1, YDMODEL%YRML_PHY_MF%YRPHY2, YDMODEL%YRCST, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA,  &
          & YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG,  &
          & YDCPG_OPTS%YRSURF_DIMS%YSD_VVD%NUMFLDS >= 8 .and. YDMODEL%YRML_PHY_MF%YRPHY%LSOLV,  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP(:, :, JBLK), ZQV(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_T(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_U(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_V(:, :, JBLK), ZPFL_FPLSH(:, :, JBLK),  &
          & ZPFL_FPLCH(:, :, JBLK), ZDPHIT(:, JBLK), ZDPHIV(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VF_PZ0F(:, JBLK),  &
          & Z_YDMF_PHYS_SURF_GSD_VV_PZ0H(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VF_PZ0RLF(:, JBLK),  &
          & Z_YDMF_PHYS_SURF_GSD_VV_PHV(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VF_PLSM(:, JBLK), ZNEIJG(:, JBLK), ZNEIJV(:, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YGSP_SG_F(:, 1, JBLK), Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T(:, JBLK),  &
          & Z_YDMF_PHYS_SURF_GSD_VF_PVEG(:, JBLK), ZWFC(:, JBLK), Z_YDMF_PHYS_BASE_STATE_YGSP_RR_W(:, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YGSP_RR_IC(:, JBLK), LLCLS, LLHMT, ZNBVNO(:, :, JBLK), ZMRIPP(:, :, JBLK),  &
          & ZFLU_CD(:, JBLK), ZFLU_CDN(:, JBLK), ZCDROV(:, JBLK), ZFLU_CH(:, JBLK), ZCHROV(:, JBLK), ZDSA_CPS(:, JBLK),  &
          & ZDQSTS(:, JBLK), ZGWDCS(:, JBLK), Z_YDMF_PHYS_OUT_GZ0(:, JBLK), Z_YDMF_PHYS_OUT_GZ0H(:, JBLK), ZHQ(:, JBLK),  &
          & ZHU(:, JBLK), ZFLU_NEIJ(:, JBLK), Z_YDMF_PHYS_OUT_QCLS(:, JBLK), Z_YDCPG_MISC_QS(:, JBLK), ZFLU_QSATS(:, JBLK),  &
          & Z_YDMF_PHYS_OUT_RHCLS(:, JBLK), ZDSA_RS(:, JBLK), ZRTI(:, JBLK), ZSTAB(:, JBLK), Z_YDMF_PHYS_OUT_TCLS(:, JBLK),  &
          & Z_YDMF_PHYS_OUT_UCLS(:, JBLK), Z_YDMF_PHYS_OUT_VCLS(:, JBLK), Z_YDMF_PHYS_OUT_NUCLS(:, JBLK),  &
          & Z_YDMF_PHYS_OUT_NVCLS(:, JBLK), ZPCLS(:, JBLK), ZFLU_VEG(:, JBLK), ZXDROV(:, JBLK), ZXHROV(:, JBLK),  &
          & Z_YDMF_PHYS_OUT_UGST(:, JBLK), Z_YDMF_PHYS_OUT_VGST(:, JBLK), YDSTACK=YLSTACK)
          
        END DO
      END DO
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMT:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 234"
      IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ACHMT')) THEN
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMT:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 235"
        ZRTI => GET_HOST_DATA_RDWR(YL_ZRTI)
        ZDPHIT => GET_HOST_DATA_RDWR(YL_ZDPHIT)
        ZNBVNO => GET_HOST_DATA_RDWR(YL_ZNBVNO)
        ZPCLS => GET_HOST_DATA_RDWR(YL_ZPCLS)
        ZNEIJV => GET_HOST_DATA_RDWR(YL_ZNEIJV)
        ZFLU_QSATS => GET_HOST_DATA_RDWR(YL_ZFLU_QSATS)
        ZGWDCS => GET_HOST_DATA_RDWR(YL_ZGWDCS)
        ZDPHIV => GET_HOST_DATA_RDWR(YL_ZDPHIV)
        ZQV => GET_HOST_DATA_RDWR(YL_ZQV)
        ZHQ => GET_HOST_DATA_RDWR(YL_ZHQ)
        ZDQSTS => GET_HOST_DATA_RDWR(YL_ZDQSTS)
        ZCHROV => GET_HOST_DATA_RDWR(YL_ZCHROV)
        ZNEIJG => GET_HOST_DATA_RDWR(YL_ZNEIJG)
        ZSTAB => GET_HOST_DATA_RDWR(YL_ZSTAB)
        ZDSA_CPS => GET_HOST_DATA_RDWR(YL_ZDSA_CPS)
        ZHU => GET_HOST_DATA_RDWR(YL_ZHU)
        ZXDROV => GET_HOST_DATA_RDWR(YL_ZXDROV)
        ZWFC => GET_HOST_DATA_RDWR(YL_ZWFC)
        ZDSA_RS => GET_HOST_DATA_RDWR(YL_ZDSA_RS)
        ZFLU_CDN => GET_HOST_DATA_RDWR(YL_ZFLU_CDN)
        ZFLU_CD => GET_HOST_DATA_RDWR(YL_ZFLU_CD)
        ZMRIPP => GET_HOST_DATA_RDWR(YL_ZMRIPP)
        ZXHROV => GET_HOST_DATA_RDWR(YL_ZXHROV)
        ZCDROV => GET_HOST_DATA_RDWR(YL_ZCDROV)
        ZPFL_FPLSH => GET_HOST_DATA_RDWR(YL_ZPFL_FPLSH)
        ZFLU_CH => GET_HOST_DATA_RDWR(YL_ZFLU_CH)
        ZFLU_VEG => GET_HOST_DATA_RDWR(YL_ZFLU_VEG)
        ZFLU_NEIJ => GET_HOST_DATA_RDWR(YL_ZFLU_NEIJ)
        ZPFL_FPLCH => GET_HOST_DATA_RDWR(YL_ZPFL_FPLCH)
        Z_YDMF_PHYS_BASE_STATE_YGSP_RR_W => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_W)
        Z_YDMF_PHYS_OUT_UCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_UCLS)
        Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
        Z_YDMF_PHYS_BASE_STATE_U => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
        Z_YDMF_PHYS_SURF_GSD_VF_PVEG => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_VEG)
        Z_YDMF_PHYS_BASE_STATE_V => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
        Z_YDMF_PHYS_OUT_QCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_QCLS)
        Z_YDMF_PHYS_SURF_GSD_VF_PZ0F => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_Z0F)
        Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
        Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
        Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
        Z_YDMF_PHYS_BASE_STATE_YGSP_RR_IC => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_IC)
        Z_YDMF_PHYS_OUT_NVCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_NVCLS)
        Z_YDMF_PHYS_BASE_STATE_YGSP_SG_F => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_SG%F_F)
        Z_YDMF_PHYS_SURF_GSD_VF_PZ0RLF => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_Z0RLF)
        Z_YDMF_PHYS_OUT_VGST => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_VGST)
        Z_YDMF_PHYS_OUT_VCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_VCLS)
        Z_YDMF_PHYS_SURF_GSD_VV_PZ0H => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_Z0H)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
        Z_YDMF_PHYS_SURF_GSD_VV_PHV => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_HV)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_R)
        Z_YDMF_PHYS_OUT_NUCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_NUCLS)
        Z_YDMF_PHYS_OUT_UGST => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_UGST)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
        Z_YDMF_PHYS_OUT_GZ0H => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_GZ0H)
        Z_YDMF_PHYS_SURF_GSD_VF_PLSM => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_LSM)
        Z_YDMF_PHYS_OUT_RHCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_RHCLS)
        Z_YDMF_PHYS_OUT_GZ0 => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_GZ0)
        Z_YDMF_PHYS_OUT_TCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_TCLS)
        Z_YDCPG_MISC_QS => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QS)
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMT:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 236"
      END IF
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMT:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 237"
      ZRTI => NULL()
      ZDPHIT => NULL()
      ZNBVNO => NULL()
      ZPCLS => NULL()
      ZNEIJV => NULL()
      ZFLU_QSATS => NULL()
      ZGWDCS => NULL()
      ZDPHIV => NULL()
      ZQV => NULL()
      ZHQ => NULL()
      ZDQSTS => NULL()
      ZCHROV => NULL()
      ZNEIJG => NULL()
      ZSTAB => NULL()
      ZDSA_CPS => NULL()
      ZHU => NULL()
      ZXDROV => NULL()
      ZWFC => NULL()
      ZDSA_RS => NULL()
      ZFLU_CDN => NULL()
      ZFLU_CD => NULL()
      ZMRIPP => NULL()
      ZXHROV => NULL()
      ZCDROV => NULL()
      ZPFL_FPLSH => NULL()
      ZFLU_CH => NULL()
      ZFLU_VEG => NULL()
      ZFLU_NEIJ => NULL()
      ZPFL_FPLCH => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_W => NULL()
      Z_YDMF_PHYS_OUT_UCLS => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => NULL()
      Z_YDMF_PHYS_BASE_STATE_U => NULL()
      Z_YDMF_PHYS_SURF_GSD_VF_PVEG => NULL()
      Z_YDMF_PHYS_BASE_STATE_V => NULL()
      Z_YDMF_PHYS_OUT_QCLS => NULL()
      Z_YDMF_PHYS_SURF_GSD_VF_PZ0F => NULL()
      Z_YDMF_PHYS_BASE_STATE_T => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_IC => NULL()
      Z_YDMF_PHYS_OUT_NVCLS => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_SG_F => NULL()
      Z_YDMF_PHYS_SURF_GSD_VF_PZ0RLF => NULL()
      Z_YDMF_PHYS_OUT_VGST => NULL()
      Z_YDMF_PHYS_OUT_VCLS => NULL()
      Z_YDMF_PHYS_SURF_GSD_VV_PZ0H => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => NULL()
      Z_YDMF_PHYS_SURF_GSD_VV_PHV => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => NULL()
      Z_YDMF_PHYS_OUT_NUCLS => NULL()
      Z_YDMF_PHYS_OUT_UGST => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => NULL()
      Z_YDMF_PHYS_OUT_GZ0H => NULL()
      Z_YDMF_PHYS_SURF_GSD_VF_PLSM => NULL()
      Z_YDMF_PHYS_OUT_RHCLS => NULL()
      Z_YDMF_PHYS_OUT_GZ0 => NULL()
      Z_YDMF_PHYS_OUT_TCLS => NULL()
      Z_YDCPG_MISC_QS => NULL()
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMT:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 238"
    ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ACHMT')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMT:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 239"
      ZRTI => GET_DEVICE_DATA_RDWR(YL_ZRTI)
      ZDPHIT => GET_DEVICE_DATA_RDWR(YL_ZDPHIT)
      ZNBVNO => GET_DEVICE_DATA_RDWR(YL_ZNBVNO)
      ZPCLS => GET_DEVICE_DATA_RDWR(YL_ZPCLS)
      ZNEIJV => GET_DEVICE_DATA_RDWR(YL_ZNEIJV)
      ZFLU_QSATS => GET_DEVICE_DATA_RDWR(YL_ZFLU_QSATS)
      ZGWDCS => GET_DEVICE_DATA_RDWR(YL_ZGWDCS)
      ZDPHIV => GET_DEVICE_DATA_RDWR(YL_ZDPHIV)
      ZQV => GET_DEVICE_DATA_RDWR(YL_ZQV)
      ZHQ => GET_DEVICE_DATA_RDWR(YL_ZHQ)
      ZDQSTS => GET_DEVICE_DATA_RDWR(YL_ZDQSTS)
      ZCHROV => GET_DEVICE_DATA_RDWR(YL_ZCHROV)
      ZNEIJG => GET_DEVICE_DATA_RDWR(YL_ZNEIJG)
      ZSTAB => GET_DEVICE_DATA_RDWR(YL_ZSTAB)
      ZDSA_CPS => GET_DEVICE_DATA_RDWR(YL_ZDSA_CPS)
      ZHU => GET_DEVICE_DATA_RDWR(YL_ZHU)
      ZXDROV => GET_DEVICE_DATA_RDWR(YL_ZXDROV)
      ZWFC => GET_DEVICE_DATA_RDWR(YL_ZWFC)
      ZDSA_RS => GET_DEVICE_DATA_RDWR(YL_ZDSA_RS)
      ZFLU_CDN => GET_DEVICE_DATA_RDWR(YL_ZFLU_CDN)
      ZFLU_CD => GET_DEVICE_DATA_RDWR(YL_ZFLU_CD)
      ZMRIPP => GET_DEVICE_DATA_RDWR(YL_ZMRIPP)
      ZXHROV => GET_DEVICE_DATA_RDWR(YL_ZXHROV)
      ZCDROV => GET_DEVICE_DATA_RDWR(YL_ZCDROV)
      ZPFL_FPLSH => GET_DEVICE_DATA_RDWR(YL_ZPFL_FPLSH)
      ZFLU_CH => GET_DEVICE_DATA_RDWR(YL_ZFLU_CH)
      ZFLU_VEG => GET_DEVICE_DATA_RDWR(YL_ZFLU_VEG)
      ZFLU_NEIJ => GET_DEVICE_DATA_RDWR(YL_ZFLU_NEIJ)
      ZPFL_FPLCH => GET_DEVICE_DATA_RDWR(YL_ZPFL_FPLCH)
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_W => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_W)
      Z_YDMF_PHYS_OUT_UCLS => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_UCLS)
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
      Z_YDMF_PHYS_BASE_STATE_U => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
      Z_YDMF_PHYS_SURF_GSD_VF_PVEG => GET_DEVICE_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_VEG)
      Z_YDMF_PHYS_BASE_STATE_V => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
      Z_YDMF_PHYS_OUT_QCLS => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_QCLS)
      Z_YDMF_PHYS_SURF_GSD_VF_PZ0F => GET_DEVICE_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_Z0F)
      Z_YDMF_PHYS_BASE_STATE_T => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_IC => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_IC)
      Z_YDMF_PHYS_OUT_NVCLS => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_NVCLS)
      Z_YDMF_PHYS_BASE_STATE_YGSP_SG_F => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_SG%F_F)
      Z_YDMF_PHYS_SURF_GSD_VF_PZ0RLF => GET_DEVICE_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_Z0RLF)
      Z_YDMF_PHYS_OUT_VGST => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_VGST)
      Z_YDMF_PHYS_OUT_VCLS => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_VCLS)
      Z_YDMF_PHYS_SURF_GSD_VV_PZ0H => GET_DEVICE_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_Z0H)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
      Z_YDMF_PHYS_SURF_GSD_VV_PHV => GET_DEVICE_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_HV)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_R)
      Z_YDMF_PHYS_OUT_NUCLS => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_NUCLS)
      Z_YDMF_PHYS_OUT_UGST => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_UGST)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
      Z_YDMF_PHYS_OUT_GZ0H => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_GZ0H)
      Z_YDMF_PHYS_SURF_GSD_VF_PLSM => GET_DEVICE_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_LSM)
      Z_YDMF_PHYS_OUT_RHCLS => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_RHCLS)
      Z_YDMF_PHYS_OUT_GZ0 => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_GZ0)
      Z_YDMF_PHYS_OUT_TCLS => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_TCLS)
      Z_YDCPG_MISC_QS => GET_DEVICE_DATA_RDWR(YDCPG_MISC%F_QS)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMT:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 240"
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMT:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 241"
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZRTI, ZDPHIT, ZNBVNO, ZPCLS, ZNEIJV, ZFLU_QSATS, ZGWDCS,  &
!$ACC & ZDPHIV, ZQV, ZHQ, ZDQSTS, ZCHROV, ZNEIJG, ZSTAB, ZDSA_CPS, ZHU, ZXDROV, ZWFC, ZDSA_RS, ZFLU_CDN, ZMRIPP, ZXHROV,  &
!$ACC & ZCDROV, ZPFL_FPLSH, ZFLU_CH, ZFLU_VEG, ZFLU_NEIJ, ZPFL_FPLCH, Z_YDMF_PHYS_BASE_STATE_YGSP_RR_W,  &
!$ACC & Z_YDMF_PHYS_OUT_UCLS, Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T, Z_YDMF_PHYS_BASE_STATE_U, Z_YDMF_PHYS_SURF_GSD_VF_PVEG,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_V, Z_YDMF_PHYS_OUT_QCLS, Z_YDMF_PHYS_SURF_GSD_VF_PZ0F, Z_YDMF_PHYS_BASE_STATE_T,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF, Z_YDMF_PHYS_BASE_STATE_YGSP_RR_IC, Z_YDMF_PHYS_OUT_NVCLS,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YGSP_SG_F, Z_YDMF_PHYS_SURF_GSD_VF_PZ0RLF, Z_YDMF_PHYS_OUT_VGST, Z_YDMF_PHYS_OUT_VCLS,  &
!$ACC & Z_YDMF_PHYS_SURF_GSD_VV_PZ0H, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI, Z_YDMF_PHYS_SURF_GSD_VV_PHV,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R, Z_YDMF_PHYS_OUT_NUCLS,  &
!$ACC & Z_YDMF_PHYS_OUT_UGST, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF, Z_YDMF_PHYS_OUT_GZ0H, Z_YDMF_PHYS_SURF_GSD_VF_PLSM,  &
!$ACC & Z_YDMF_PHYS_OUT_RHCLS, Z_YDMF_PHYS_OUT_TCLS, Z_YDCPG_MISC_QS ) PRIVATE( JBLK ) VECTOR_LENGTH( YDCPG_OPTS%KLON )
      DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK, LLHMT, LLCLS )
        DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
          YLCPG_BNDS%KIDIA = JLON
          YLCPG_BNDS%KFDIA = JLON
          YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          
          CALL ACHMT_OPENACC(YDCPG_OPTS%YRCLI, YDMODEL%YRML_PHY_MF%YRPHY, YDMODEL%YRML_PHY_MF%YRPHY0,  &
          & YDMODEL%YRML_PHY_MF%YRPHY1, YDMODEL%YRML_PHY_MF%YRPHY2, YDMODEL%YRCST, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA,  &
          & YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG,  &
          & YDCPG_OPTS%YRSURF_DIMS%YSD_VVD%NUMFLDS >= 8 .and. YDMODEL%YRML_PHY_MF%YRPHY%LSOLV,  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP(:, :, JBLK), ZQV(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_T(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_U(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_V(:, :, JBLK), ZPFL_FPLSH(:, :, JBLK),  &
          & ZPFL_FPLCH(:, :, JBLK), ZDPHIT(:, JBLK), ZDPHIV(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VF_PZ0F(:, JBLK),  &
          & Z_YDMF_PHYS_SURF_GSD_VV_PZ0H(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VF_PZ0RLF(:, JBLK),  &
          & Z_YDMF_PHYS_SURF_GSD_VV_PHV(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VF_PLSM(:, JBLK), ZNEIJG(:, JBLK), ZNEIJV(:, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YGSP_SG_F(:, 1, JBLK), Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T(:, JBLK),  &
          & Z_YDMF_PHYS_SURF_GSD_VF_PVEG(:, JBLK), ZWFC(:, JBLK), Z_YDMF_PHYS_BASE_STATE_YGSP_RR_W(:, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YGSP_RR_IC(:, JBLK), LLCLS, LLHMT, ZNBVNO(:, :, JBLK), ZMRIPP(:, :, JBLK),  &
          & ZFLU_CD(:, JBLK), ZFLU_CDN(:, JBLK), ZCDROV(:, JBLK), ZFLU_CH(:, JBLK), ZCHROV(:, JBLK), ZDSA_CPS(:, JBLK),  &
          & ZDQSTS(:, JBLK), ZGWDCS(:, JBLK), Z_YDMF_PHYS_OUT_GZ0(:, JBLK), Z_YDMF_PHYS_OUT_GZ0H(:, JBLK), ZHQ(:, JBLK),  &
          & ZHU(:, JBLK), ZFLU_NEIJ(:, JBLK), Z_YDMF_PHYS_OUT_QCLS(:, JBLK), Z_YDCPG_MISC_QS(:, JBLK), ZFLU_QSATS(:, JBLK),  &
          & Z_YDMF_PHYS_OUT_RHCLS(:, JBLK), ZDSA_RS(:, JBLK), ZRTI(:, JBLK), ZSTAB(:, JBLK), Z_YDMF_PHYS_OUT_TCLS(:, JBLK),  &
          & Z_YDMF_PHYS_OUT_UCLS(:, JBLK), Z_YDMF_PHYS_OUT_VCLS(:, JBLK), Z_YDMF_PHYS_OUT_NUCLS(:, JBLK),  &
          & Z_YDMF_PHYS_OUT_NVCLS(:, JBLK), ZPCLS(:, JBLK), ZFLU_VEG(:, JBLK), ZXDROV(:, JBLK), ZXHROV(:, JBLK),  &
          & Z_YDMF_PHYS_OUT_UGST(:, JBLK), Z_YDMF_PHYS_OUT_VGST(:, JBLK), YDSTACK=YLSTACK)
          
        END DO
      END DO
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMT:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 242"
      IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ACHMT')) THEN
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMT:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 243"
        ZRTI => GET_HOST_DATA_RDWR(YL_ZRTI)
        ZDPHIT => GET_HOST_DATA_RDWR(YL_ZDPHIT)
        ZNBVNO => GET_HOST_DATA_RDWR(YL_ZNBVNO)
        ZPCLS => GET_HOST_DATA_RDWR(YL_ZPCLS)
        ZNEIJV => GET_HOST_DATA_RDWR(YL_ZNEIJV)
        ZFLU_QSATS => GET_HOST_DATA_RDWR(YL_ZFLU_QSATS)
        ZGWDCS => GET_HOST_DATA_RDWR(YL_ZGWDCS)
        ZDPHIV => GET_HOST_DATA_RDWR(YL_ZDPHIV)
        ZQV => GET_HOST_DATA_RDWR(YL_ZQV)
        ZHQ => GET_HOST_DATA_RDWR(YL_ZHQ)
        ZDQSTS => GET_HOST_DATA_RDWR(YL_ZDQSTS)
        ZCHROV => GET_HOST_DATA_RDWR(YL_ZCHROV)
        ZNEIJG => GET_HOST_DATA_RDWR(YL_ZNEIJG)
        ZSTAB => GET_HOST_DATA_RDWR(YL_ZSTAB)
        ZDSA_CPS => GET_HOST_DATA_RDWR(YL_ZDSA_CPS)
        ZHU => GET_HOST_DATA_RDWR(YL_ZHU)
        ZXDROV => GET_HOST_DATA_RDWR(YL_ZXDROV)
        ZWFC => GET_HOST_DATA_RDWR(YL_ZWFC)
        ZDSA_RS => GET_HOST_DATA_RDWR(YL_ZDSA_RS)
        ZFLU_CDN => GET_HOST_DATA_RDWR(YL_ZFLU_CDN)
        ZFLU_CD => GET_HOST_DATA_RDWR(YL_ZFLU_CD)
        ZMRIPP => GET_HOST_DATA_RDWR(YL_ZMRIPP)
        ZXHROV => GET_HOST_DATA_RDWR(YL_ZXHROV)
        ZCDROV => GET_HOST_DATA_RDWR(YL_ZCDROV)
        ZPFL_FPLSH => GET_HOST_DATA_RDWR(YL_ZPFL_FPLSH)
        ZFLU_CH => GET_HOST_DATA_RDWR(YL_ZFLU_CH)
        ZFLU_VEG => GET_HOST_DATA_RDWR(YL_ZFLU_VEG)
        ZFLU_NEIJ => GET_HOST_DATA_RDWR(YL_ZFLU_NEIJ)
        ZPFL_FPLCH => GET_HOST_DATA_RDWR(YL_ZPFL_FPLCH)
        Z_YDMF_PHYS_BASE_STATE_YGSP_RR_W => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_W)
        Z_YDMF_PHYS_OUT_UCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_UCLS)
        Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
        Z_YDMF_PHYS_BASE_STATE_U => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
        Z_YDMF_PHYS_SURF_GSD_VF_PVEG => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_VEG)
        Z_YDMF_PHYS_BASE_STATE_V => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
        Z_YDMF_PHYS_OUT_QCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_QCLS)
        Z_YDMF_PHYS_SURF_GSD_VF_PZ0F => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_Z0F)
        Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
        Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
        Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
        Z_YDMF_PHYS_BASE_STATE_YGSP_RR_IC => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_IC)
        Z_YDMF_PHYS_OUT_NVCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_NVCLS)
        Z_YDMF_PHYS_BASE_STATE_YGSP_SG_F => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_SG%F_F)
        Z_YDMF_PHYS_SURF_GSD_VF_PZ0RLF => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_Z0RLF)
        Z_YDMF_PHYS_OUT_VGST => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_VGST)
        Z_YDMF_PHYS_OUT_VCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_VCLS)
        Z_YDMF_PHYS_SURF_GSD_VV_PZ0H => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_Z0H)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
        Z_YDMF_PHYS_SURF_GSD_VV_PHV => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VV%F_HV)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_R)
        Z_YDMF_PHYS_OUT_NUCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_NUCLS)
        Z_YDMF_PHYS_OUT_UGST => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_UGST)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
        Z_YDMF_PHYS_OUT_GZ0H => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_GZ0H)
        Z_YDMF_PHYS_SURF_GSD_VF_PLSM => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_LSM)
        Z_YDMF_PHYS_OUT_RHCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_RHCLS)
        Z_YDMF_PHYS_OUT_GZ0 => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_GZ0)
        Z_YDMF_PHYS_OUT_TCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_TCLS)
        Z_YDCPG_MISC_QS => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QS)
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMT:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 244"
      END IF
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMT:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 245"
      ZRTI => NULL()
      ZDPHIT => NULL()
      ZNBVNO => NULL()
      ZPCLS => NULL()
      ZNEIJV => NULL()
      ZFLU_QSATS => NULL()
      ZGWDCS => NULL()
      ZDPHIV => NULL()
      ZQV => NULL()
      ZHQ => NULL()
      ZDQSTS => NULL()
      ZCHROV => NULL()
      ZNEIJG => NULL()
      ZSTAB => NULL()
      ZDSA_CPS => NULL()
      ZHU => NULL()
      ZXDROV => NULL()
      ZWFC => NULL()
      ZDSA_RS => NULL()
      ZFLU_CDN => NULL()
      ZFLU_CD => NULL()
      ZMRIPP => NULL()
      ZXHROV => NULL()
      ZCDROV => NULL()
      ZPFL_FPLSH => NULL()
      ZFLU_CH => NULL()
      ZFLU_VEG => NULL()
      ZFLU_NEIJ => NULL()
      ZPFL_FPLCH => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_W => NULL()
      Z_YDMF_PHYS_OUT_UCLS => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => NULL()
      Z_YDMF_PHYS_BASE_STATE_U => NULL()
      Z_YDMF_PHYS_SURF_GSD_VF_PVEG => NULL()
      Z_YDMF_PHYS_BASE_STATE_V => NULL()
      Z_YDMF_PHYS_OUT_QCLS => NULL()
      Z_YDMF_PHYS_SURF_GSD_VF_PZ0F => NULL()
      Z_YDMF_PHYS_BASE_STATE_T => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_IC => NULL()
      Z_YDMF_PHYS_OUT_NVCLS => NULL()
      Z_YDMF_PHYS_BASE_STATE_YGSP_SG_F => NULL()
      Z_YDMF_PHYS_SURF_GSD_VF_PZ0RLF => NULL()
      Z_YDMF_PHYS_OUT_VGST => NULL()
      Z_YDMF_PHYS_OUT_VCLS => NULL()
      Z_YDMF_PHYS_SURF_GSD_VV_PZ0H => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => NULL()
      Z_YDMF_PHYS_SURF_GSD_VV_PHV => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => NULL()
      Z_YDMF_PHYS_OUT_NUCLS => NULL()
      Z_YDMF_PHYS_OUT_UGST => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => NULL()
      Z_YDMF_PHYS_OUT_GZ0H => NULL()
      Z_YDMF_PHYS_SURF_GSD_VF_PLSM => NULL()
      Z_YDMF_PHYS_OUT_RHCLS => NULL()
      Z_YDMF_PHYS_OUT_GZ0 => NULL()
      Z_YDMF_PHYS_OUT_TCLS => NULL()
      Z_YDCPG_MISC_QS => NULL()
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMT:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 246"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMT', 1, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 247"
    
  END IF
  
  
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACCLPH', 0, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 248"
  IF (LPARALLELMETHOD('OPENMP', 'APL_ARPEGE_PARALLEL:ACCLPH')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACCLPH:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 249"
    ZUGST => GET_HOST_DATA_RDWR(YL_ZUGST)
    ZCP => GET_HOST_DATA_RDWR(YL_ZCP)
    ZQV => GET_HOST_DATA_RDWR(YL_ZQV)
    ZBLH => GET_HOST_DATA_RDWR(YL_ZBLH)
    ZTHETAVS => GET_HOST_DATA_RDWR(YL_ZTHETAVS)
    ZTHETAV => GET_HOST_DATA_RDWR(YL_ZTHETAV)
    ZVGST => GET_HOST_DATA_RDWR(YL_ZVGST)
    IMOC_CLPH => GET_HOST_DATA_RDWR(YL_IMOC_CLPH)
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
    Z_YDCPG_MISC_QS => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QS)
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP)
    Z_YDMF_PHYS_OUT_VEIN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_VEIN)
    Z_YDMF_PHYS_OUT_CLPH => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_CLPH)
    Z_YDMF_PHYS_BASE_STATE_U => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
    Z_YDMF_PHYS_BASE_STATE_V => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
    Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
    Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACCLPH:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 250"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACCLPH:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 251"
    CALL YLCPG_BNDS%INIT(YDCPG_OPTS)
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, JLEV ) FIRSTPRIVATE( YLCPG_BNDS )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      CALL YLCPG_BNDS%UPDATE(JBLK)
      
      DO JLEV=1,YDCPG_OPTS%KFLEVG
        DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
          ZCP(JLON, JLEV, JBLK) = Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP(JLON, JLEV, JBLK)
        END DO
      END DO
      
      DO JLEV=YDCPG_OPTS%KTDIA,YDCPG_OPTS%KFLEVG
        DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
          ZTHETAV(JLON, JLEV, JBLK) = Z_YDMF_PHYS_BASE_STATE_T(JLON, JLEV, JBLK)*(1.0_JPRB + YDMODEL%YRCST%RETV*ZQV(JLON,  &
          & JLEV, JBLK))*(YDMODEL%YRCST%RATM / Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(JLON, JLEV, JBLK)) &
          & **YDMODEL%YRCST%RKAPPA
        END DO
      END DO
      
      DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
        ZTHETAVS(JLON, JBLK) = Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T(JLON, JBLK)*(1.0_JPRB +  &
        & YDMODEL%YRCST%RETV*Z_YDCPG_MISC_QS(JLON, JBLK))*(YDMODEL%YRCST%RATM / Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD(JLON,  &
        & YDCPG_OPTS%KFLEVG, JBLK))**YDMODEL%YRCST%RKAPPA
      END DO
      
      CALL ACCLPH(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF%YRPHY0, YDMODEL%YRML_PHY_MF%YRPHY2, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA,  &
      & YDCPG_OPTS%KLON, YDCPG_OPTS%KTDIA, YDCPG_OPTS%KFLEVG, ZTHETAV(:, :, JBLK),  &
      & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF(:, :, JBLK),  &
      & Z_YDMF_PHYS_BASE_STATE_U(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_V(:, :, JBLK), ZTHETAVS(:, JBLK), IMOC_CLPH(:, JBLK),  &
      & Z_YDMF_PHYS_OUT_CLPH(:, JBLK), Z_YDMF_PHYS_OUT_VEIN(:, JBLK), ZUGST(:, JBLK), ZVGST(:, JBLK))
      
      
      DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
        ZBLH(JLON, JBLK) = Z_YDMF_PHYS_OUT_CLPH(JLON, JBLK)
      END DO
      
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACCLPH:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 252"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ACCLPH')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACCLPH:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 253"
      ZUGST => GET_HOST_DATA_RDWR(YL_ZUGST)
      ZCP => GET_HOST_DATA_RDWR(YL_ZCP)
      ZQV => GET_HOST_DATA_RDWR(YL_ZQV)
      ZBLH => GET_HOST_DATA_RDWR(YL_ZBLH)
      ZTHETAVS => GET_HOST_DATA_RDWR(YL_ZTHETAVS)
      ZTHETAV => GET_HOST_DATA_RDWR(YL_ZTHETAV)
      ZVGST => GET_HOST_DATA_RDWR(YL_ZVGST)
      IMOC_CLPH => GET_HOST_DATA_RDWR(YL_IMOC_CLPH)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
      Z_YDCPG_MISC_QS => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QS)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP)
      Z_YDMF_PHYS_OUT_VEIN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_VEIN)
      Z_YDMF_PHYS_OUT_CLPH => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_CLPH)
      Z_YDMF_PHYS_BASE_STATE_U => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
      Z_YDMF_PHYS_BASE_STATE_V => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
      Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACCLPH:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 254"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACCLPH:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 255"
    ZUGST => NULL()
    ZCP => NULL()
    ZQV => NULL()
    ZBLH => NULL()
    ZTHETAVS => NULL()
    ZTHETAV => NULL()
    ZVGST => NULL()
    IMOC_CLPH => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => NULL()
    Z_YDCPG_MISC_QS => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => NULL()
    Z_YDMF_PHYS_OUT_VEIN => NULL()
    Z_YDMF_PHYS_OUT_CLPH => NULL()
    Z_YDMF_PHYS_BASE_STATE_U => NULL()
    Z_YDMF_PHYS_BASE_STATE_V => NULL()
    Z_YDMF_PHYS_BASE_STATE_T => NULL()
    Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACCLPH:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 256"
  ELSE IF (LPARALLELMETHOD('OPENMPSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ACCLPH')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACCLPH:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 257"
    ZUGST => GET_HOST_DATA_RDWR(YL_ZUGST)
    ZCP => GET_HOST_DATA_RDWR(YL_ZCP)
    ZQV => GET_HOST_DATA_RDWR(YL_ZQV)
    ZBLH => GET_HOST_DATA_RDWR(YL_ZBLH)
    ZTHETAVS => GET_HOST_DATA_RDWR(YL_ZTHETAVS)
    ZTHETAV => GET_HOST_DATA_RDWR(YL_ZTHETAV)
    ZVGST => GET_HOST_DATA_RDWR(YL_ZVGST)
    IMOC_CLPH => GET_HOST_DATA_RDWR(YL_IMOC_CLPH)
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
    Z_YDCPG_MISC_QS => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QS)
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP)
    Z_YDMF_PHYS_OUT_VEIN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_VEIN)
    Z_YDMF_PHYS_OUT_CLPH => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_CLPH)
    Z_YDMF_PHYS_BASE_STATE_U => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
    Z_YDMF_PHYS_BASE_STATE_V => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
    Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
    Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACCLPH:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 258"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACCLPH:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 259"
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK, JLEV )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        DO JLEV=1,YDCPG_OPTS%KFLEVG
          ZCP(JLON, JLEV, JBLK) = Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP(JLON, JLEV, JBLK)
        END DO
        
        DO JLEV=YDCPG_OPTS%KTDIA,YDCPG_OPTS%KFLEVG
          ZTHETAV(JLON, JLEV, JBLK) = Z_YDMF_PHYS_BASE_STATE_T(JLON, JLEV, JBLK)*(1.0_JPRB + YDMODEL%YRCST%RETV*ZQV(JLON,  &
          & JLEV, JBLK))*(YDMODEL%YRCST%RATM / Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(JLON, JLEV, JBLK)) &
          & **YDMODEL%YRCST%RKAPPA
        END DO
        
        ZTHETAVS(JLON, JBLK) = Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T(JLON, JBLK)*(1.0_JPRB +  &
        & YDMODEL%YRCST%RETV*Z_YDCPG_MISC_QS(JLON, JBLK))*(YDMODEL%YRCST%RATM / Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD(JLON,  &
        & YDCPG_OPTS%KFLEVG, JBLK))**YDMODEL%YRCST%RKAPPA
        
        CALL ACCLPH_OPENACC(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF%YRPHY0, YDMODEL%YRML_PHY_MF%YRPHY2, YLCPG_BNDS%KIDIA,  &
        & YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KTDIA, YDCPG_OPTS%KFLEVG, ZTHETAV(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_U(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_V(:, :, JBLK), ZTHETAVS(:, JBLK), IMOC_CLPH(:, JBLK),  &
        & Z_YDMF_PHYS_OUT_CLPH(:, JBLK), Z_YDMF_PHYS_OUT_VEIN(:, JBLK), ZUGST(:, JBLK), ZVGST(:, JBLK), YDSTACK=YLSTACK)
        
        
        ZBLH(JLON, JBLK) = Z_YDMF_PHYS_OUT_CLPH(JLON, JBLK)
        
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACCLPH:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 260"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ACCLPH')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACCLPH:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 261"
      ZUGST => GET_HOST_DATA_RDWR(YL_ZUGST)
      ZCP => GET_HOST_DATA_RDWR(YL_ZCP)
      ZQV => GET_HOST_DATA_RDWR(YL_ZQV)
      ZBLH => GET_HOST_DATA_RDWR(YL_ZBLH)
      ZTHETAVS => GET_HOST_DATA_RDWR(YL_ZTHETAVS)
      ZTHETAV => GET_HOST_DATA_RDWR(YL_ZTHETAV)
      ZVGST => GET_HOST_DATA_RDWR(YL_ZVGST)
      IMOC_CLPH => GET_HOST_DATA_RDWR(YL_IMOC_CLPH)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
      Z_YDCPG_MISC_QS => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QS)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP)
      Z_YDMF_PHYS_OUT_VEIN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_VEIN)
      Z_YDMF_PHYS_OUT_CLPH => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_CLPH)
      Z_YDMF_PHYS_BASE_STATE_U => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
      Z_YDMF_PHYS_BASE_STATE_V => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
      Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACCLPH:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 262"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACCLPH:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 263"
    ZUGST => NULL()
    ZCP => NULL()
    ZQV => NULL()
    ZBLH => NULL()
    ZTHETAVS => NULL()
    ZTHETAV => NULL()
    ZVGST => NULL()
    IMOC_CLPH => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => NULL()
    Z_YDCPG_MISC_QS => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => NULL()
    Z_YDMF_PHYS_OUT_VEIN => NULL()
    Z_YDMF_PHYS_OUT_CLPH => NULL()
    Z_YDMF_PHYS_BASE_STATE_U => NULL()
    Z_YDMF_PHYS_BASE_STATE_V => NULL()
    Z_YDMF_PHYS_BASE_STATE_T => NULL()
    Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACCLPH:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 264"
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ACCLPH')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACCLPH:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 265"
    ZUGST => GET_DEVICE_DATA_RDWR(YL_ZUGST)
    ZCP => GET_DEVICE_DATA_RDWR(YL_ZCP)
    ZQV => GET_DEVICE_DATA_RDWR(YL_ZQV)
    ZBLH => GET_DEVICE_DATA_RDWR(YL_ZBLH)
    ZTHETAVS => GET_DEVICE_DATA_RDWR(YL_ZTHETAVS)
    ZTHETAV => GET_DEVICE_DATA_RDWR(YL_ZTHETAV)
    ZVGST => GET_DEVICE_DATA_RDWR(YL_ZVGST)
    IMOC_CLPH => GET_DEVICE_DATA_RDWR(YL_IMOC_CLPH)
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
    Z_YDCPG_MISC_QS => GET_DEVICE_DATA_RDWR(YDCPG_MISC%F_QS)
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP)
    Z_YDMF_PHYS_OUT_VEIN => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_VEIN)
    Z_YDMF_PHYS_OUT_CLPH => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_CLPH)
    Z_YDMF_PHYS_BASE_STATE_U => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
    Z_YDMF_PHYS_BASE_STATE_V => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
    Z_YDMF_PHYS_BASE_STATE_T => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
    Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACCLPH:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 266"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACCLPH:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 267"
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZUGST, ZCP, ZQV, ZBLH, ZTHETAVS, ZVGST, IMOC_CLPH,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI, Z_YDCPG_MISC_QS, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP, Z_YDMF_PHYS_OUT_VEIN,  &
!$ACC & Z_YDMF_PHYS_OUT_CLPH, Z_YDMF_PHYS_BASE_STATE_U, Z_YDMF_PHYS_BASE_STATE_V, Z_YDMF_PHYS_BASE_STATE_T,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T ) PRIVATE( JBLK ) VECTOR_LENGTH( YDCPG_OPTS%KLON )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK, JLEV )
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        DO JLEV=1,YDCPG_OPTS%KFLEVG
          ZCP(JLON, JLEV, JBLK) = Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP(JLON, JLEV, JBLK)
        END DO
        
        DO JLEV=YDCPG_OPTS%KTDIA,YDCPG_OPTS%KFLEVG
          ZTHETAV(JLON, JLEV, JBLK) = Z_YDMF_PHYS_BASE_STATE_T(JLON, JLEV, JBLK)*(1.0_JPRB + YDMODEL%YRCST%RETV*ZQV(JLON,  &
          & JLEV, JBLK))*(YDMODEL%YRCST%RATM / Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(JLON, JLEV, JBLK)) &
          & **YDMODEL%YRCST%RKAPPA
        END DO
        
        ZTHETAVS(JLON, JBLK) = Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T(JLON, JBLK)*(1.0_JPRB +  &
        & YDMODEL%YRCST%RETV*Z_YDCPG_MISC_QS(JLON, JBLK))*(YDMODEL%YRCST%RATM / Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD(JLON,  &
        & YDCPG_OPTS%KFLEVG, JBLK))**YDMODEL%YRCST%RKAPPA
        
        CALL ACCLPH_OPENACC(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF%YRPHY0, YDMODEL%YRML_PHY_MF%YRPHY2, YLCPG_BNDS%KIDIA,  &
        & YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KTDIA, YDCPG_OPTS%KFLEVG, ZTHETAV(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_U(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_V(:, :, JBLK), ZTHETAVS(:, JBLK), IMOC_CLPH(:, JBLK),  &
        & Z_YDMF_PHYS_OUT_CLPH(:, JBLK), Z_YDMF_PHYS_OUT_VEIN(:, JBLK), ZUGST(:, JBLK), ZVGST(:, JBLK), YDSTACK=YLSTACK)
        
        
        ZBLH(JLON, JBLK) = Z_YDMF_PHYS_OUT_CLPH(JLON, JBLK)
        
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACCLPH:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 268"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ACCLPH')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACCLPH:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 269"
      ZUGST => GET_HOST_DATA_RDWR(YL_ZUGST)
      ZCP => GET_HOST_DATA_RDWR(YL_ZCP)
      ZQV => GET_HOST_DATA_RDWR(YL_ZQV)
      ZBLH => GET_HOST_DATA_RDWR(YL_ZBLH)
      ZTHETAVS => GET_HOST_DATA_RDWR(YL_ZTHETAVS)
      ZTHETAV => GET_HOST_DATA_RDWR(YL_ZTHETAV)
      ZVGST => GET_HOST_DATA_RDWR(YL_ZVGST)
      IMOC_CLPH => GET_HOST_DATA_RDWR(YL_IMOC_CLPH)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
      Z_YDCPG_MISC_QS => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QS)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP)
      Z_YDMF_PHYS_OUT_VEIN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_VEIN)
      Z_YDMF_PHYS_OUT_CLPH => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_CLPH)
      Z_YDMF_PHYS_BASE_STATE_U => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
      Z_YDMF_PHYS_BASE_STATE_V => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
      Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACCLPH:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 270"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACCLPH:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 271"
    ZUGST => NULL()
    ZCP => NULL()
    ZQV => NULL()
    ZBLH => NULL()
    ZTHETAVS => NULL()
    ZTHETAV => NULL()
    ZVGST => NULL()
    IMOC_CLPH => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => NULL()
    Z_YDCPG_MISC_QS => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => NULL()
    Z_YDMF_PHYS_OUT_VEIN => NULL()
    Z_YDMF_PHYS_OUT_CLPH => NULL()
    Z_YDMF_PHYS_BASE_STATE_U => NULL()
    Z_YDMF_PHYS_BASE_STATE_V => NULL()
    Z_YDMF_PHYS_BASE_STATE_T => NULL()
    Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACCLPH:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 272"
  END IF
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACCLPH', 1, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 273"
  
  CALL APL_ARPEGE_OCEANIC_FLUXES_PARALLEL(YDMF_PHYS_BASE_STATE, YDCPG_BNDS, YDCPG_OPTS, YDMF_PHYS, YDMF_PHYS_SURF, YDMODEL,  &
  & LLHMT, YL_ZCDROV, YL_ZCEROV, YL_ZCHROV, YL_ZDPHIT, YL_ZDPHIV, YL_ZDSA_RS, YL_ZFLU_CD, YL_ZFLU_CDN, YL_ZFLU_CH,  &
  & YL_ZFLU_QSATS)
  
  
  CALL APL_WIND_GUST_PARALLEL(YDMF_PHYS_BASE_STATE, YDCPG_BNDS, YDCPG_OPTS, YDMF_PHYS, YDVARS, YDMODEL, YL_IMOC_CLPH,  &
  & YL_ZBLH, YL_ZDCAPE)
  
  CALL APL_ARPEGE_SHALLOW_CONVECTION_AND_TURBULENCE_PARALLEL(YDMF_PHYS_BASE_STATE, YDCPG_BNDS, YDCPG_OPTS, YDCPG_MISC,  &
  & YDMF_PHYS, YDMF_PHYS_SURF, YDMODEL, YDDDH, YL_INLAB_CVPP, YL_ZCDROV, YL_ZCHROV, YL_ZCOEFN, YL_ZCONDCVPPI, YL_ZCONDCVPPL,  &
  & YL_ZDIFCVPPQ, YL_ZDIFCVPPS, YDMODEL%YRML_PHY_MF%YRPHY0%REPS, YL_ZFLU_CD, YL_ZFLU_CH, YL_ZKQLROV, YL_ZKQROV, YL_ZKTROV,  &
  & YL_ZKUROV, YL_ZKUROV2, YL_ZMSC_LSCPE, YL_ZNBVNO, YL_ZNEBS, YL_ZNEBS0, YL_ZNEB_CVPP, YL_ZPFL_FPLCH, YL_ZPFL_FTKE,  &
  & YL_ZPFL_FTKEI, YL_ZPRODTH_CVPP, YL_ZQI, YL_ZQIC, YL_ZQL, YL_ZQLC, YL_ZQLIS, YL_ZQLIS0, YL_ZQLI_CVPP, YL_ZQV, YL_ZQR,  &
  & YL_ZQS, YL_ZTKE1, YL_ZXTROV, YL_ZXUROV, YL_ZUSLE, YL_ZKCLS, YL_ZECTCLS, YL_ZPRODTH, YL_ZEDMFQ, YL_ZEDMFS, YL_ZEDMFU,  &
  & YL_ZEDMFV, YL_ZMF_UP)
  
  CALL APL_ARPEGE_ALBEDO_COMPUTATION_PARALLEL(YDMF_PHYS_BASE_STATE, YDCPG_BNDS, YDCPG_OPTS, YDMF_PHYS, YDMF_PHYS_SURF,  &
  & YDMODEL, YL_ZALBD, YL_ZALBP, ZEPS0, YL_ZFLU_EMIS, YL_ZFLU_NEIJ, YL_ZRDG_MU0, YL_ZALBDIR)
  
  CALL APL_ARPEGE_AEROSOLS_FOR_RADIATION_PARALLEL(YDMF_PHYS_BASE_STATE, YDCPG_BNDS, YDCPG_OPTS, YDMF_PHYS_SURF, YDMODEL,  &
  & YL_ZAER, YL_ZAERINDS)
  
  CALL APL_ARPEGE_CLOUDINESS_PARALLEL(YDMF_PHYS_BASE_STATE, YDCPG_BNDS, YDCPG_OPTS, YDCPG_MISC, YDMF_PHYS, YDVARS, YDMODEL,  &
  & LLREDPR, YL_ZAIPCMT, YL_ZBLH, YL_ZDECRD, YL_ZFLU_QSAT, YL_ZMSC_QW, YL_ZNEBC0, YL_ZNEBCH, YL_ZNEBS, YL_ZNEBS0,  &
  & YL_ZNEB_CVPP, YL_ZPFL_FPLCH, YL_ZQI, YL_ZQL, YL_ZQLIS, YL_ZQLIS0, YL_ZQLI_CVP, YL_ZQLI_CVPP, YL_ZQV, YL_ZUNEBH,  &
  & YL_ZCLCT_RAD, YDGEOMETRY%YRSTA)
  
  CALL APL_ARPEGE_RADIATION_PARALLEL(YDMF_PHYS_BASE_STATE, YDCFU, YDGEOMETRY, YDCPG_BNDS, YDCPG_OPTS, YDCPG_MISC,  &
  & YDCPG_GPAR, YDMF_PHYS, YDMF_PHYS_SURF, YDVARS, YDMODEL, YL_ZCLCT_RAD, YL_ZALBDIR, YL_ZDECRD, YL_ZAER, YL_ZAERINDS,  &
  & YL_ZALBD, YL_ZALBP, YL_ZQCO2, YL_ZCEMTR, YL_ZCTRSO, YL_ZFLU_EMIS, YL_ZFLU_QSAT, YL_ZQO3, YL_ZQR, YL_ZQS, YL_ZQV,  &
  & YL_ZRDG_MU0, YL_ZRDG_SOLO, YL_ZRDG_MU0LU, YL_ZRDG_MU0M, YL_ZSFSWDIF, YL_ZSFSWDIR, YL_ZSUDU, YL_ZTENT, YL_ZTRSOD,  &
  & YL_ZTRSODIF, YL_ZTRSODIR)
  
  CALL APL_ARPEGE_SOIL_HYDRO_PARALLEL(YDMF_PHYS_BASE_STATE, YDCPG_BNDS, YDCPG_OPTS, YDMF_PHYS, YDMF_PHYS_SURF, YDMODEL,  &
  & YL_ZCHROV, YL_ZFLU_NEIJ, YL_ZFLU_QSAT, YL_ZFLU_QSATS, YL_ZFLU_VEG, YL_ZGWDCS, YL_ZHQ, YL_ZHTR, YL_ZHU, YL_ZQV, YL_ZWFC,  &
  & YL_ZWLMX, YL_ZWWILT)
  
  CALL APL_ARPEGE_SURFACE_PARALLEL(YDMF_PHYS_BASE_STATE, YDGEOMETRY, YDCPG_BNDS, YDCPG_OPTS, YDCPG_MISC, YDCPG_GPAR,  &
  & YDMF_PHYS, YDMF_PHYS_SURF, YDVARS, YDMODEL, YL_ZALBD, YL_ZALBP, YL_ZALPHA1, YL_ZCDROV, YL_ZCEROV, YL_ZCFATH, YL_ZCFAU,  &
  & YL_ZCFBTH, YL_ZCFBU, YL_ZCFBV, YL_ZCHROV, YL_ZCOEFA, YL_ZCOEFN, YL_ZCP, YL_ZDIFEXT, YL_ZDIFWQ, YL_ZDIFWS, YL_ZDQSTS,  &
  & YL_ZDSA_CPS, YL_ZDSA_LHS, ZDTMSE, YL_ZEDMFQ, YL_ZEDMFS, YL_ZEDMFU, YL_ZEDMFV, YL_ZFLU_CD, YL_ZFLU_CDN, YL_ZFLU_EMIS,  &
  & YL_ZFLU_FEVI, YL_ZFLU_NEIJ, YL_ZFLU_QSATS, YL_ZFLU_VEG, YL_ZHQ, YL_ZHTR, YL_ZHU, YL_ZKQLROV, YL_ZKQROV, YL_ZKTROV,  &
  & YL_ZKUROV, YL_ZLVT, YL_ZMF_UP, YL_ZNEBCH, YL_ZNEBDIFF, YL_ZNEBS, YL_ZPOID, YL_ZQI, YL_ZQICE, YL_ZQL, YL_ZQV,  &
  & YL_ZRDG_MU0, YL_ZRDG_MU0N, ZRHGMT, YL_ZSC_FCLL, YL_ZSC_FCLN, YL_ZSC_FEVI, YL_ZSC_FEVN, YL_ZSFSWDIF, YL_ZSFSWDIR,  &
  & YL_ZSGROUPEL, YL_ZSRAIN, YL_ZSSNOW, ZSTATI, YL_ZTSN, YL_ZXDROV, YL_ZXHROV, YL_ZXQRO, YL_ZXTRO, YL_ZXTROV, YL_ZXURO,  &
  & YL_ZXUROV)
  
  ! The deep convection will see the shallow part from KFB as it is with Louis scheme and the modified RI
  
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:DIFTQ', 0, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 274"
  IF (LPARALLELMETHOD('OPENMP', 'APL_ARPEGE_PARALLEL:DIFTQ')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:DIFTQ:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 275"
    ZEDMFQ => GET_HOST_DATA_RDWR(YL_ZEDMFQ)
    ZEDMFU => GET_HOST_DATA_RDWR(YL_ZEDMFU)
    ZDIFCVPPQ => GET_HOST_DATA_RDWR(YL_ZDIFCVPPQ)
    ZDIFCVPPV => GET_HOST_DATA_RDWR(YL_ZDIFCVPPV)
    ZEDMFV => GET_HOST_DATA_RDWR(YL_ZEDMFV)
    ZDIFCVPPS => GET_HOST_DATA_RDWR(YL_ZDIFCVPPS)
    ZDIFCVPPU => GET_HOST_DATA_RDWR(YL_ZDIFCVPPU)
    ZEDMFS => GET_HOST_DATA_RDWR(YL_ZEDMFS)
    Z_YDMF_PHYS_OUT_DIFTQ => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFTQ)
    Z_YDMF_PHYS_OUT_STRTU => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRTU)
    Z_YDMF_PHYS_OUT_DIFTS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFTS)
    Z_YDMF_PHYS_OUT_STRTV => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRTV)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:DIFTQ:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 276"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:DIFTQ:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 277"
    CALL YLCPG_BNDS%INIT(YDCPG_OPTS)
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, JLEV ) FIRSTPRIVATE( YLCPG_BNDS )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      CALL YLCPG_BNDS%UPDATE(JBLK)
      
      IF (YDMODEL%YRML_PHY_MF%YRPHY%LCVPPKF .or. YDMODEL%YRML_PHY_MF%YRPHY%LEDKF .and. .not.YDMODEL%YRML_PHY_MF%YRPHY%LEDMFI &
      & ) THEN
        IF (YDMODEL%YRML_PHY_MF%YRPHY%LCVPPKF) THEN
          DO JLEV=YDCPG_OPTS%KTDIA,YDCPG_OPTS%KFLEVG
            DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
              Z_YDMF_PHYS_OUT_DIFTQ(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_DIFTQ(JLON, JLEV, JBLK) + ZDIFCVPPQ(JLON, JLEV, JBLK)
              Z_YDMF_PHYS_OUT_DIFTS(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_DIFTS(JLON, JLEV, JBLK) + ZDIFCVPPS(JLON, JLEV, JBLK)
              Z_YDMF_PHYS_OUT_STRTU(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_STRTU(JLON, JLEV, JBLK) + ZDIFCVPPU(JLON, JLEV, JBLK)
              Z_YDMF_PHYS_OUT_STRTV(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_STRTV(JLON, JLEV, JBLK) + ZDIFCVPPV(JLON, JLEV, JBLK)
            END DO
          END DO
        ELSE IF (YDMODEL%YRML_PHY_MF%YRPHY%LEDKF) THEN
          DO JLEV=YDCPG_OPTS%KTDIA,YDCPG_OPTS%KFLEVG
            DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
              Z_YDMF_PHYS_OUT_DIFTQ(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_DIFTQ(JLON, JLEV, JBLK) + ZEDMFQ(JLON, JLEV, JBLK)
              Z_YDMF_PHYS_OUT_DIFTS(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_DIFTS(JLON, JLEV, JBLK) + ZEDMFS(JLON, JLEV, JBLK)
              Z_YDMF_PHYS_OUT_STRTU(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_STRTU(JLON, JLEV, JBLK) + ZEDMFU(JLON, JLEV, JBLK)
              Z_YDMF_PHYS_OUT_STRTV(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_STRTV(JLON, JLEV, JBLK) + ZEDMFV(JLON, JLEV, JBLK)
            END DO
          END DO
        END IF
      END IF
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:DIFTQ:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 278"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:DIFTQ')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:DIFTQ:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 279"
      ZEDMFQ => GET_HOST_DATA_RDWR(YL_ZEDMFQ)
      ZEDMFU => GET_HOST_DATA_RDWR(YL_ZEDMFU)
      ZDIFCVPPQ => GET_HOST_DATA_RDWR(YL_ZDIFCVPPQ)
      ZDIFCVPPV => GET_HOST_DATA_RDWR(YL_ZDIFCVPPV)
      ZEDMFV => GET_HOST_DATA_RDWR(YL_ZEDMFV)
      ZDIFCVPPS => GET_HOST_DATA_RDWR(YL_ZDIFCVPPS)
      ZDIFCVPPU => GET_HOST_DATA_RDWR(YL_ZDIFCVPPU)
      ZEDMFS => GET_HOST_DATA_RDWR(YL_ZEDMFS)
      Z_YDMF_PHYS_OUT_DIFTQ => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFTQ)
      Z_YDMF_PHYS_OUT_STRTU => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRTU)
      Z_YDMF_PHYS_OUT_DIFTS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFTS)
      Z_YDMF_PHYS_OUT_STRTV => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRTV)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:DIFTQ:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 280"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:DIFTQ:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 281"
    ZEDMFQ => NULL()
    ZEDMFU => NULL()
    ZDIFCVPPQ => NULL()
    ZDIFCVPPV => NULL()
    ZEDMFV => NULL()
    ZDIFCVPPS => NULL()
    ZDIFCVPPU => NULL()
    ZEDMFS => NULL()
    Z_YDMF_PHYS_OUT_DIFTQ => NULL()
    Z_YDMF_PHYS_OUT_STRTU => NULL()
    Z_YDMF_PHYS_OUT_DIFTS => NULL()
    Z_YDMF_PHYS_OUT_STRTV => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:DIFTQ:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 282"
  ELSE IF (LPARALLELMETHOD('OPENMPSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:DIFTQ')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:DIFTQ:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 283"
    ZEDMFQ => GET_HOST_DATA_RDWR(YL_ZEDMFQ)
    ZEDMFU => GET_HOST_DATA_RDWR(YL_ZEDMFU)
    ZDIFCVPPQ => GET_HOST_DATA_RDWR(YL_ZDIFCVPPQ)
    ZDIFCVPPV => GET_HOST_DATA_RDWR(YL_ZDIFCVPPV)
    ZEDMFV => GET_HOST_DATA_RDWR(YL_ZEDMFV)
    ZDIFCVPPS => GET_HOST_DATA_RDWR(YL_ZDIFCVPPS)
    ZDIFCVPPU => GET_HOST_DATA_RDWR(YL_ZDIFCVPPU)
    ZEDMFS => GET_HOST_DATA_RDWR(YL_ZEDMFS)
    Z_YDMF_PHYS_OUT_DIFTQ => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFTQ)
    Z_YDMF_PHYS_OUT_STRTU => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRTU)
    Z_YDMF_PHYS_OUT_DIFTS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFTS)
    Z_YDMF_PHYS_OUT_STRTV => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRTV)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:DIFTQ:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 284"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:DIFTQ:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 285"
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK, JLEV )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        IF ( &
        & YDMODEL%YRML_PHY_MF%YRPHY%LCVPPKF .or. YDMODEL%YRML_PHY_MF%YRPHY%LEDKF .and. .not.YDMODEL%YRML_PHY_MF%YRPHY%LEDMFI &
        & ) THEN
          IF (YDMODEL%YRML_PHY_MF%YRPHY%LCVPPKF) THEN
            DO JLEV=YDCPG_OPTS%KTDIA,YDCPG_OPTS%KFLEVG
              Z_YDMF_PHYS_OUT_DIFTQ(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_DIFTQ(JLON, JLEV, JBLK) + ZDIFCVPPQ(JLON, JLEV, JBLK)
              Z_YDMF_PHYS_OUT_DIFTS(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_DIFTS(JLON, JLEV, JBLK) + ZDIFCVPPS(JLON, JLEV, JBLK)
              Z_YDMF_PHYS_OUT_STRTU(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_STRTU(JLON, JLEV, JBLK) + ZDIFCVPPU(JLON, JLEV, JBLK)
              Z_YDMF_PHYS_OUT_STRTV(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_STRTV(JLON, JLEV, JBLK) + ZDIFCVPPV(JLON, JLEV, JBLK)
            END DO
          ELSE IF (YDMODEL%YRML_PHY_MF%YRPHY%LEDKF) THEN
            DO JLEV=YDCPG_OPTS%KTDIA,YDCPG_OPTS%KFLEVG
              Z_YDMF_PHYS_OUT_DIFTQ(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_DIFTQ(JLON, JLEV, JBLK) + ZEDMFQ(JLON, JLEV, JBLK)
              Z_YDMF_PHYS_OUT_DIFTS(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_DIFTS(JLON, JLEV, JBLK) + ZEDMFS(JLON, JLEV, JBLK)
              Z_YDMF_PHYS_OUT_STRTU(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_STRTU(JLON, JLEV, JBLK) + ZEDMFU(JLON, JLEV, JBLK)
              Z_YDMF_PHYS_OUT_STRTV(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_STRTV(JLON, JLEV, JBLK) + ZEDMFV(JLON, JLEV, JBLK)
            END DO
          END IF
        END IF
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:DIFTQ:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 286"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:DIFTQ')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:DIFTQ:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 287"
      ZEDMFQ => GET_HOST_DATA_RDWR(YL_ZEDMFQ)
      ZEDMFU => GET_HOST_DATA_RDWR(YL_ZEDMFU)
      ZDIFCVPPQ => GET_HOST_DATA_RDWR(YL_ZDIFCVPPQ)
      ZDIFCVPPV => GET_HOST_DATA_RDWR(YL_ZDIFCVPPV)
      ZEDMFV => GET_HOST_DATA_RDWR(YL_ZEDMFV)
      ZDIFCVPPS => GET_HOST_DATA_RDWR(YL_ZDIFCVPPS)
      ZDIFCVPPU => GET_HOST_DATA_RDWR(YL_ZDIFCVPPU)
      ZEDMFS => GET_HOST_DATA_RDWR(YL_ZEDMFS)
      Z_YDMF_PHYS_OUT_DIFTQ => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFTQ)
      Z_YDMF_PHYS_OUT_STRTU => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRTU)
      Z_YDMF_PHYS_OUT_DIFTS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFTS)
      Z_YDMF_PHYS_OUT_STRTV => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRTV)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:DIFTQ:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 288"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:DIFTQ:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 289"
    ZEDMFQ => NULL()
    ZEDMFU => NULL()
    ZDIFCVPPQ => NULL()
    ZDIFCVPPV => NULL()
    ZEDMFV => NULL()
    ZDIFCVPPS => NULL()
    ZDIFCVPPU => NULL()
    ZEDMFS => NULL()
    Z_YDMF_PHYS_OUT_DIFTQ => NULL()
    Z_YDMF_PHYS_OUT_STRTU => NULL()
    Z_YDMF_PHYS_OUT_DIFTS => NULL()
    Z_YDMF_PHYS_OUT_STRTV => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:DIFTQ:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 290"
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:DIFTQ')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:DIFTQ:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 291"
    ZEDMFQ => GET_DEVICE_DATA_RDWR(YL_ZEDMFQ)
    ZEDMFU => GET_DEVICE_DATA_RDWR(YL_ZEDMFU)
    ZDIFCVPPQ => GET_DEVICE_DATA_RDWR(YL_ZDIFCVPPQ)
    ZDIFCVPPV => GET_DEVICE_DATA_RDWR(YL_ZDIFCVPPV)
    ZEDMFV => GET_DEVICE_DATA_RDWR(YL_ZEDMFV)
    ZDIFCVPPS => GET_DEVICE_DATA_RDWR(YL_ZDIFCVPPS)
    ZDIFCVPPU => GET_DEVICE_DATA_RDWR(YL_ZDIFCVPPU)
    ZEDMFS => GET_DEVICE_DATA_RDWR(YL_ZEDMFS)
    Z_YDMF_PHYS_OUT_DIFTQ => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_DIFTQ)
    Z_YDMF_PHYS_OUT_STRTU => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_STRTU)
    Z_YDMF_PHYS_OUT_DIFTS => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_DIFTS)
    Z_YDMF_PHYS_OUT_STRTV => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_STRTV)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:DIFTQ:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 292"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:DIFTQ:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 293"
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZEDMFQ, ZEDMFU, ZDIFCVPPQ, ZDIFCVPPV, ZEDMFV, ZDIFCVPPS,  &
!$ACC & ZDIFCVPPU, ZEDMFS, Z_YDMF_PHYS_OUT_DIFTQ, Z_YDMF_PHYS_OUT_STRTU, Z_YDMF_PHYS_OUT_DIFTS, Z_YDMF_PHYS_OUT_STRTV )  &
!$ACC & PRIVATE( JBLK ) VECTOR_LENGTH( YDCPG_OPTS%KLON )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK, JLEV )
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        IF ( &
        & YDMODEL%YRML_PHY_MF%YRPHY%LCVPPKF .or. YDMODEL%YRML_PHY_MF%YRPHY%LEDKF .and. .not.YDMODEL%YRML_PHY_MF%YRPHY%LEDMFI &
        & ) THEN
          IF (YDMODEL%YRML_PHY_MF%YRPHY%LCVPPKF) THEN
            DO JLEV=YDCPG_OPTS%KTDIA,YDCPG_OPTS%KFLEVG
              Z_YDMF_PHYS_OUT_DIFTQ(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_DIFTQ(JLON, JLEV, JBLK) + ZDIFCVPPQ(JLON, JLEV, JBLK)
              Z_YDMF_PHYS_OUT_DIFTS(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_DIFTS(JLON, JLEV, JBLK) + ZDIFCVPPS(JLON, JLEV, JBLK)
              Z_YDMF_PHYS_OUT_STRTU(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_STRTU(JLON, JLEV, JBLK) + ZDIFCVPPU(JLON, JLEV, JBLK)
              Z_YDMF_PHYS_OUT_STRTV(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_STRTV(JLON, JLEV, JBLK) + ZDIFCVPPV(JLON, JLEV, JBLK)
            END DO
          ELSE IF (YDMODEL%YRML_PHY_MF%YRPHY%LEDKF) THEN
            DO JLEV=YDCPG_OPTS%KTDIA,YDCPG_OPTS%KFLEVG
              Z_YDMF_PHYS_OUT_DIFTQ(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_DIFTQ(JLON, JLEV, JBLK) + ZEDMFQ(JLON, JLEV, JBLK)
              Z_YDMF_PHYS_OUT_DIFTS(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_DIFTS(JLON, JLEV, JBLK) + ZEDMFS(JLON, JLEV, JBLK)
              Z_YDMF_PHYS_OUT_STRTU(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_STRTU(JLON, JLEV, JBLK) + ZEDMFU(JLON, JLEV, JBLK)
              Z_YDMF_PHYS_OUT_STRTV(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_STRTV(JLON, JLEV, JBLK) + ZEDMFV(JLON, JLEV, JBLK)
            END DO
          END IF
        END IF
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:DIFTQ:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 294"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:DIFTQ')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:DIFTQ:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 295"
      ZEDMFQ => GET_HOST_DATA_RDWR(YL_ZEDMFQ)
      ZEDMFU => GET_HOST_DATA_RDWR(YL_ZEDMFU)
      ZDIFCVPPQ => GET_HOST_DATA_RDWR(YL_ZDIFCVPPQ)
      ZDIFCVPPV => GET_HOST_DATA_RDWR(YL_ZDIFCVPPV)
      ZEDMFV => GET_HOST_DATA_RDWR(YL_ZEDMFV)
      ZDIFCVPPS => GET_HOST_DATA_RDWR(YL_ZDIFCVPPS)
      ZDIFCVPPU => GET_HOST_DATA_RDWR(YL_ZDIFCVPPU)
      ZEDMFS => GET_HOST_DATA_RDWR(YL_ZEDMFS)
      Z_YDMF_PHYS_OUT_DIFTQ => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFTQ)
      Z_YDMF_PHYS_OUT_STRTU => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRTU)
      Z_YDMF_PHYS_OUT_DIFTS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFTS)
      Z_YDMF_PHYS_OUT_STRTV => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRTV)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:DIFTQ:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 296"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:DIFTQ:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 297"
    ZEDMFQ => NULL()
    ZEDMFU => NULL()
    ZDIFCVPPQ => NULL()
    ZDIFCVPPV => NULL()
    ZEDMFV => NULL()
    ZDIFCVPPS => NULL()
    ZDIFCVPPU => NULL()
    ZEDMFS => NULL()
    Z_YDMF_PHYS_OUT_DIFTQ => NULL()
    Z_YDMF_PHYS_OUT_STRTU => NULL()
    Z_YDMF_PHYS_OUT_DIFTS => NULL()
    Z_YDMF_PHYS_OUT_STRTV => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:DIFTQ:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 298"
  END IF
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:DIFTQ', 1, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 299"
  
  ! Additional diagnostics of radiative fluxes (PFRTHDS and PFRSOPT)
  
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:FRSOPT', 0, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 300"
  IF (LPARALLELMETHOD('OPENMP', 'APL_ARPEGE_PARALLEL:FRSOPT')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:FRSOPT:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 301"
    ZFLU_EMIS => GET_HOST_DATA_RDWR(YL_ZFLU_EMIS)
    ZRDG_MU0 => GET_HOST_DATA_RDWR(YL_ZRDG_MU0)
    Z_YDMF_PHYS_OUT_FRTH => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FRTH)
    Z_YDMF_PHYS_OUT_FRSOPT => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FRSOPT)
    Z_YDMF_PHYS_OUT_FRTHDS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FRTHDS)
    Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:FRSOPT:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 302"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:FRSOPT:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 303"
    CALL YLCPG_BNDS%INIT(YDCPG_OPTS)
!$OMP PARALLEL DO PRIVATE( JBLK, JLON ) FIRSTPRIVATE( YLCPG_BNDS )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      CALL YLCPG_BNDS%UPDATE(JBLK)
      
      !DEC$ IVDEP
      DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
        IF (.not.YDMODEL%YRML_PHY_MF%YRARPHY%LMSE) Z_YDMF_PHYS_OUT_FRTHDS(JLON, JBLK) = Z_YDMF_PHYS_OUT_FRTH(JLON,  &
        & YDCPG_OPTS%KFLEVG, 1, JBLK) / ZFLU_EMIS(JLON, JBLK) +   YDMODEL%YRCST%RSIGMA*Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T(JLON, &
        &  JBLK)**4
        IF (ZRDG_MU0(JLON, JBLK) <= 0.0_JPRB) THEN
          Z_YDMF_PHYS_OUT_FRSOPT(JLON, JBLK) = 0.0_JPRB
        ELSE
          Z_YDMF_PHYS_OUT_FRSOPT(JLON, JBLK) = YDMODEL%YRML_PHY_MF%YRPHY3%RII0*ZRDG_MU0(JLON, JBLK)
        END IF
      END DO
      
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:FRSOPT:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 304"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:FRSOPT')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:FRSOPT:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 305"
      ZFLU_EMIS => GET_HOST_DATA_RDWR(YL_ZFLU_EMIS)
      ZRDG_MU0 => GET_HOST_DATA_RDWR(YL_ZRDG_MU0)
      Z_YDMF_PHYS_OUT_FRTH => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FRTH)
      Z_YDMF_PHYS_OUT_FRSOPT => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FRSOPT)
      Z_YDMF_PHYS_OUT_FRTHDS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FRTHDS)
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:FRSOPT:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 306"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:FRSOPT:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 307"
    ZFLU_EMIS => NULL()
    ZRDG_MU0 => NULL()
    Z_YDMF_PHYS_OUT_FRTH => NULL()
    Z_YDMF_PHYS_OUT_FRSOPT => NULL()
    Z_YDMF_PHYS_OUT_FRTHDS => NULL()
    Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:FRSOPT:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 308"
  ELSE IF (LPARALLELMETHOD('OPENMPSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:FRSOPT')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:FRSOPT:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 309"
    ZFLU_EMIS => GET_HOST_DATA_RDWR(YL_ZFLU_EMIS)
    ZRDG_MU0 => GET_HOST_DATA_RDWR(YL_ZRDG_MU0)
    Z_YDMF_PHYS_OUT_FRTH => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FRTH)
    Z_YDMF_PHYS_OUT_FRSOPT => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FRSOPT)
    Z_YDMF_PHYS_OUT_FRTHDS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FRTHDS)
    Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:FRSOPT:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 310"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:FRSOPT:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 311"
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        !DEC$ IVDEP
        IF (.not.YDMODEL%YRML_PHY_MF%YRARPHY%LMSE) Z_YDMF_PHYS_OUT_FRTHDS(JLON, JBLK) = Z_YDMF_PHYS_OUT_FRTH(JLON,  &
        & YDCPG_OPTS%KFLEVG, 1, JBLK) / ZFLU_EMIS(JLON, JBLK) +   YDMODEL%YRCST%RSIGMA*Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T(JLON, &
        &  JBLK)**4
        IF (ZRDG_MU0(JLON, JBLK) <= 0.0_JPRB) THEN
          Z_YDMF_PHYS_OUT_FRSOPT(JLON, JBLK) = 0.0_JPRB
        ELSE
          Z_YDMF_PHYS_OUT_FRSOPT(JLON, JBLK) = YDMODEL%YRML_PHY_MF%YRPHY3%RII0*ZRDG_MU0(JLON, JBLK)
        END IF
        
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:FRSOPT:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 312"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:FRSOPT')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:FRSOPT:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 313"
      ZFLU_EMIS => GET_HOST_DATA_RDWR(YL_ZFLU_EMIS)
      ZRDG_MU0 => GET_HOST_DATA_RDWR(YL_ZRDG_MU0)
      Z_YDMF_PHYS_OUT_FRTH => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FRTH)
      Z_YDMF_PHYS_OUT_FRSOPT => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FRSOPT)
      Z_YDMF_PHYS_OUT_FRTHDS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FRTHDS)
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:FRSOPT:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 314"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:FRSOPT:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 315"
    ZFLU_EMIS => NULL()
    ZRDG_MU0 => NULL()
    Z_YDMF_PHYS_OUT_FRTH => NULL()
    Z_YDMF_PHYS_OUT_FRSOPT => NULL()
    Z_YDMF_PHYS_OUT_FRTHDS => NULL()
    Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:FRSOPT:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 316"
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:FRSOPT')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:FRSOPT:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 317"
    ZFLU_EMIS => GET_DEVICE_DATA_RDWR(YL_ZFLU_EMIS)
    ZRDG_MU0 => GET_DEVICE_DATA_RDWR(YL_ZRDG_MU0)
    Z_YDMF_PHYS_OUT_FRTH => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_FRTH)
    Z_YDMF_PHYS_OUT_FRSOPT => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_FRSOPT)
    Z_YDMF_PHYS_OUT_FRTHDS => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_FRTHDS)
    Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:FRSOPT:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 318"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:FRSOPT:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 319"
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZFLU_EMIS, ZRDG_MU0, Z_YDMF_PHYS_OUT_FRTH,  &
!$ACC & Z_YDMF_PHYS_OUT_FRSOPT, Z_YDMF_PHYS_OUT_FRTHDS, Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T ) PRIVATE( JBLK ) VECTOR_LENGTH(  &
!$ACC & YDCPG_OPTS%KLON )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK )
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        !DEC$ IVDEP
        IF (.not.YDMODEL%YRML_PHY_MF%YRARPHY%LMSE) Z_YDMF_PHYS_OUT_FRTHDS(JLON, JBLK) = Z_YDMF_PHYS_OUT_FRTH(JLON,  &
        & YDCPG_OPTS%KFLEVG, 1, JBLK) / ZFLU_EMIS(JLON, JBLK) +   YDMODEL%YRCST%RSIGMA*Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T(JLON, &
        &  JBLK)**4
        IF (ZRDG_MU0(JLON, JBLK) <= 0.0_JPRB) THEN
          Z_YDMF_PHYS_OUT_FRSOPT(JLON, JBLK) = 0.0_JPRB
        ELSE
          Z_YDMF_PHYS_OUT_FRSOPT(JLON, JBLK) = YDMODEL%YRML_PHY_MF%YRPHY3%RII0*ZRDG_MU0(JLON, JBLK)
        END IF
        
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:FRSOPT:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 320"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:FRSOPT')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:FRSOPT:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 321"
      ZFLU_EMIS => GET_HOST_DATA_RDWR(YL_ZFLU_EMIS)
      ZRDG_MU0 => GET_HOST_DATA_RDWR(YL_ZRDG_MU0)
      Z_YDMF_PHYS_OUT_FRTH => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FRTH)
      Z_YDMF_PHYS_OUT_FRSOPT => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FRSOPT)
      Z_YDMF_PHYS_OUT_FRTHDS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FRTHDS)
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:FRSOPT:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 322"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:FRSOPT:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 323"
    ZFLU_EMIS => NULL()
    ZRDG_MU0 => NULL()
    Z_YDMF_PHYS_OUT_FRTH => NULL()
    Z_YDMF_PHYS_OUT_FRSOPT => NULL()
    Z_YDMF_PHYS_OUT_FRTHDS => NULL()
    Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:FRSOPT:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 324"
  END IF
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:FRSOPT', 1, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 325"
  
  ! Additional diagnostic of the derivative of the non solar surface; heat flux with respect to surface temperature
  
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRAG', 0, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 326"
  IF (LPARALLELMETHOD('OPENMP', 'APL_ARPEGE_PARALLEL:ACDRAG')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRAG:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 327"
    ZGWDCS => GET_HOST_DATA_RDWR(YL_ZGWDCS)
    ZQLIS => GET_HOST_DATA_RDWR(YL_ZQLIS)
    ZFLU_EMIS => GET_HOST_DATA_RDWR(YL_ZFLU_EMIS)
    ZQV => GET_HOST_DATA_RDWR(YL_ZQV)
    ZDQSTS => GET_HOST_DATA_RDWR(YL_ZDQSTS)
    ZMSC_QW => GET_HOST_DATA_RDWR(YL_ZMSC_QW)
    ZFLU_NEIJ => GET_HOST_DATA_RDWR(YL_ZFLU_NEIJ)
    ZTRAJGWD => GET_HOST_DATA_RDWR(YL_ZTRAJGWD)
    ZNBVNO => GET_HOST_DATA_RDWR(YL_ZNBVNO)
    ZCHROV => GET_HOST_DATA_RDWR(YL_ZCHROV)
    ZNEBS => GET_HOST_DATA_RDWR(YL_ZNEBS)
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
    Z_YDMF_PHYS_OUT_FPEVPSL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPEVPSL)
    Z_YDMF_PHYS_SURF_GSD_VF_PVRLAN => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_VRLAN)
    Z_YDMF_PHYS_SURF_GSD_VF_PVRLDI => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_VRLDI)
    Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
    Z_YDVARS_GEOMETRY_RCORI_T0 => GET_HOST_DATA_RDWR(YDVARS%GEOMETRY%RCORI%FT0)
    Z_YDMF_PHYS_OUT_STRDU => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRDU)
    Z_YDMF_PHYS_OUT_DRNSHF => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DRNSHF)
    Z_YDMF_PHYS_BASE_STATE_U => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
    Z_YDMF_PHYS_SURF_GSD_VF_PLSM => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_LSM)
    Z_YDMF_PHYS_OUT_FCSQL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FCSQL)
    Z_YDMF_PHYS_OUT_FPEVPSN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPEVPSN)
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_R)
    Z_YDMF_PHYS_SURF_GSD_VF_PGETRL => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_GETRL)
    Z_YDMF_PHYS_BASE_STATE_V => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
    Z_YDMF_PHYS_OUT_FPLSL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSL)
    Z_YDMF_PHYS_OUT_STRDV => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRDV)
    Z_YDMF_PHYS_OUT_FPFPSN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPFPSN)
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_DELP)
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_RDELP)
    Z_YDMF_PHYS_OUT_FPFPSL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPFPSL)
    Z_YDMF_PHYS_OUT_FPLSN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSN)
    Z_YDCPG_MISC_QS => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QS)
    Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
    Z_YDMF_PHYS_OUT_FCSQN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FCSQN)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRAG:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 328"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRAG:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 329"
    CALL YLCPG_BNDS%INIT(YDCPG_OPTS)
!$OMP PARALLEL DO PRIVATE( JBLK ) FIRSTPRIVATE( YLCPG_BNDS )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      CALL YLCPG_BNDS%UPDATE(JBLK)
      
      IF (YDMODEL%YRML_AOC%YRMCC%LMCC03) THEN
        CALL ACDNSHF(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF%YRPHY, YDMODEL%YRML_PHY_MF%YRPHY1, YLCPG_BNDS%KIDIA,  &
        & YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KTDIA, YDCPG_OPTS%KFLEVG, ZFLU_EMIS(:, JBLK),  &
        & Z_YDMF_PHYS_SURF_GSD_VF_PLSM(:, JBLK), ZFLU_NEIJ(:, JBLK), ZQV(:, :, JBLK), Z_YDCPG_MISC_QS(:, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T(:, JBLK), ZCHROV(:, JBLK), ZDQSTS(:, JBLK), Z_YDMF_PHYS_OUT_DRNSHF(:, JBLK))
      END IF
      
      CALL ACDRAG(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON,  &
      & YDMODEL%YRML_PHY_MF%YRTOPH%NTDRAG, YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD(:, :, JBLK),  &
      & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP(:, :, JBLK),  &
      & ZNBVNO(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_U(:, :, JBLK),  &
      & Z_YDMF_PHYS_BASE_STATE_V(:, :, JBLK), Z_YDVARS_GEOMETRY_RCORI_T0(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VF_PGETRL(:, JBLK),  &
      & ZGWDCS(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VF_PVRLAN(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VF_PVRLDI(:, JBLK),  &
      & Z_YDMF_PHYS_OUT_STRDU(:, :, JBLK), Z_YDMF_PHYS_OUT_STRDV(:, :, JBLK), ZTRAJGWD(:, :, JBLK))
      
      IF (YDMODEL%YRML_PHY_MF%YRPHY%LSTRAS) THEN
        CALL ACPLUIS(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON,  &
        & YDMODEL%YRML_PHY_MF%YRTOPH%NTPLUI, YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP(:, :, JBLK), ZNEBS(:, :, JBLK), ZQV(:, :, JBLK), ZQLIS(:, :, JBLK),  &
        & ZMSC_QW(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_T(:, :, JBLK),  &
        & Z_YDMF_PHYS_OUT_FCSQL(:, :, JBLK), Z_YDMF_PHYS_OUT_FCSQN(:, :, JBLK), Z_YDMF_PHYS_OUT_FPLSL(:, :, JBLK),  &
        & Z_YDMF_PHYS_OUT_FPLSN(:, :, JBLK), Z_YDMF_PHYS_OUT_FPEVPSL(:, :, JBLK), Z_YDMF_PHYS_OUT_FPEVPSN(:, :, JBLK),  &
        & Z_YDMF_PHYS_OUT_FPFPSL(:, :, JBLK), Z_YDMF_PHYS_OUT_FPFPSN(:, :, JBLK))
      END IF
      
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRAG:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 330"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ACDRAG')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRAG:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 331"
      ZGWDCS => GET_HOST_DATA_RDWR(YL_ZGWDCS)
      ZQLIS => GET_HOST_DATA_RDWR(YL_ZQLIS)
      ZFLU_EMIS => GET_HOST_DATA_RDWR(YL_ZFLU_EMIS)
      ZQV => GET_HOST_DATA_RDWR(YL_ZQV)
      ZDQSTS => GET_HOST_DATA_RDWR(YL_ZDQSTS)
      ZMSC_QW => GET_HOST_DATA_RDWR(YL_ZMSC_QW)
      ZFLU_NEIJ => GET_HOST_DATA_RDWR(YL_ZFLU_NEIJ)
      ZTRAJGWD => GET_HOST_DATA_RDWR(YL_ZTRAJGWD)
      ZNBVNO => GET_HOST_DATA_RDWR(YL_ZNBVNO)
      ZCHROV => GET_HOST_DATA_RDWR(YL_ZCHROV)
      ZNEBS => GET_HOST_DATA_RDWR(YL_ZNEBS)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
      Z_YDMF_PHYS_OUT_FPEVPSL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPEVPSL)
      Z_YDMF_PHYS_SURF_GSD_VF_PVRLAN => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_VRLAN)
      Z_YDMF_PHYS_SURF_GSD_VF_PVRLDI => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_VRLDI)
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
      Z_YDVARS_GEOMETRY_RCORI_T0 => GET_HOST_DATA_RDWR(YDVARS%GEOMETRY%RCORI%FT0)
      Z_YDMF_PHYS_OUT_STRDU => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRDU)
      Z_YDMF_PHYS_OUT_DRNSHF => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DRNSHF)
      Z_YDMF_PHYS_BASE_STATE_U => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
      Z_YDMF_PHYS_SURF_GSD_VF_PLSM => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_LSM)
      Z_YDMF_PHYS_OUT_FCSQL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FCSQL)
      Z_YDMF_PHYS_OUT_FPEVPSN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPEVPSN)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_R)
      Z_YDMF_PHYS_SURF_GSD_VF_PGETRL => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_GETRL)
      Z_YDMF_PHYS_BASE_STATE_V => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
      Z_YDMF_PHYS_OUT_FPLSL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSL)
      Z_YDMF_PHYS_OUT_STRDV => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRDV)
      Z_YDMF_PHYS_OUT_FPFPSN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPFPSN)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_DELP)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_RDELP)
      Z_YDMF_PHYS_OUT_FPFPSL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPFPSL)
      Z_YDMF_PHYS_OUT_FPLSN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSN)
      Z_YDCPG_MISC_QS => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QS)
      Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
      Z_YDMF_PHYS_OUT_FCSQN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FCSQN)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRAG:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 332"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRAG:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 333"
    ZGWDCS => NULL()
    ZQLIS => NULL()
    ZFLU_EMIS => NULL()
    ZQV => NULL()
    ZDQSTS => NULL()
    ZMSC_QW => NULL()
    ZFLU_NEIJ => NULL()
    ZTRAJGWD => NULL()
    ZNBVNO => NULL()
    ZCHROV => NULL()
    ZNEBS => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => NULL()
    Z_YDMF_PHYS_OUT_FPEVPSL => NULL()
    Z_YDMF_PHYS_SURF_GSD_VF_PVRLAN => NULL()
    Z_YDMF_PHYS_SURF_GSD_VF_PVRLDI => NULL()
    Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => NULL()
    Z_YDVARS_GEOMETRY_RCORI_T0 => NULL()
    Z_YDMF_PHYS_OUT_STRDU => NULL()
    Z_YDMF_PHYS_OUT_DRNSHF => NULL()
    Z_YDMF_PHYS_BASE_STATE_U => NULL()
    Z_YDMF_PHYS_SURF_GSD_VF_PLSM => NULL()
    Z_YDMF_PHYS_OUT_FCSQL => NULL()
    Z_YDMF_PHYS_OUT_FPEVPSN => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => NULL()
    Z_YDMF_PHYS_SURF_GSD_VF_PGETRL => NULL()
    Z_YDMF_PHYS_BASE_STATE_V => NULL()
    Z_YDMF_PHYS_OUT_FPLSL => NULL()
    Z_YDMF_PHYS_OUT_STRDV => NULL()
    Z_YDMF_PHYS_OUT_FPFPSN => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP => NULL()
    Z_YDMF_PHYS_OUT_FPFPSL => NULL()
    Z_YDMF_PHYS_OUT_FPLSN => NULL()
    Z_YDCPG_MISC_QS => NULL()
    Z_YDMF_PHYS_BASE_STATE_T => NULL()
    Z_YDMF_PHYS_OUT_FCSQN => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRAG:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 334"
  ELSE IF (LPARALLELMETHOD('OPENMPSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ACDRAG')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRAG:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 335"
    ZGWDCS => GET_HOST_DATA_RDWR(YL_ZGWDCS)
    ZQLIS => GET_HOST_DATA_RDWR(YL_ZQLIS)
    ZFLU_EMIS => GET_HOST_DATA_RDWR(YL_ZFLU_EMIS)
    ZQV => GET_HOST_DATA_RDWR(YL_ZQV)
    ZDQSTS => GET_HOST_DATA_RDWR(YL_ZDQSTS)
    ZMSC_QW => GET_HOST_DATA_RDWR(YL_ZMSC_QW)
    ZFLU_NEIJ => GET_HOST_DATA_RDWR(YL_ZFLU_NEIJ)
    ZTRAJGWD => GET_HOST_DATA_RDWR(YL_ZTRAJGWD)
    ZNBVNO => GET_HOST_DATA_RDWR(YL_ZNBVNO)
    ZCHROV => GET_HOST_DATA_RDWR(YL_ZCHROV)
    ZNEBS => GET_HOST_DATA_RDWR(YL_ZNEBS)
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
    Z_YDMF_PHYS_OUT_FPEVPSL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPEVPSL)
    Z_YDMF_PHYS_SURF_GSD_VF_PVRLAN => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_VRLAN)
    Z_YDMF_PHYS_SURF_GSD_VF_PVRLDI => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_VRLDI)
    Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
    Z_YDVARS_GEOMETRY_RCORI_T0 => GET_HOST_DATA_RDWR(YDVARS%GEOMETRY%RCORI%FT0)
    Z_YDMF_PHYS_OUT_STRDU => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRDU)
    Z_YDMF_PHYS_OUT_DRNSHF => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DRNSHF)
    Z_YDMF_PHYS_BASE_STATE_U => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
    Z_YDMF_PHYS_SURF_GSD_VF_PLSM => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_LSM)
    Z_YDMF_PHYS_OUT_FCSQL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FCSQL)
    Z_YDMF_PHYS_OUT_FPEVPSN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPEVPSN)
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_R)
    Z_YDMF_PHYS_SURF_GSD_VF_PGETRL => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_GETRL)
    Z_YDMF_PHYS_BASE_STATE_V => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
    Z_YDMF_PHYS_OUT_FPLSL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSL)
    Z_YDMF_PHYS_OUT_STRDV => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRDV)
    Z_YDMF_PHYS_OUT_FPFPSN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPFPSN)
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_DELP)
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_RDELP)
    Z_YDMF_PHYS_OUT_FPFPSL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPFPSL)
    Z_YDMF_PHYS_OUT_FPLSN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSN)
    Z_YDCPG_MISC_QS => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QS)
    Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
    Z_YDMF_PHYS_OUT_FCSQN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FCSQN)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRAG:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 336"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRAG:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 337"
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        IF (YDMODEL%YRML_AOC%YRMCC%LMCC03) THEN
          CALL ACDNSHF_OPENACC(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF%YRPHY, YDMODEL%YRML_PHY_MF%YRPHY1, YLCPG_BNDS%KIDIA,  &
          & YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KTDIA, YDCPG_OPTS%KFLEVG, ZFLU_EMIS(:, JBLK),  &
          & Z_YDMF_PHYS_SURF_GSD_VF_PLSM(:, JBLK), ZFLU_NEIJ(:, JBLK), ZQV(:, :, JBLK), Z_YDCPG_MISC_QS(:, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T(:, JBLK), ZCHROV(:, JBLK), ZDQSTS(:, JBLK), Z_YDMF_PHYS_OUT_DRNSHF(:, JBLK),  &
          & YDSTACK=YLSTACK)
        END IF
        
        CALL ACDRAG_OPENACC(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON,  &
        & YDMODEL%YRML_PHY_MF%YRTOPH%NTDRAG, YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP(:, :, JBLK),  &
        & ZNBVNO(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_U(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_V(:, :, JBLK), Z_YDVARS_GEOMETRY_RCORI_T0(:, JBLK),  &
        & Z_YDMF_PHYS_SURF_GSD_VF_PGETRL(:, JBLK), ZGWDCS(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VF_PVRLAN(:, JBLK),  &
        & Z_YDMF_PHYS_SURF_GSD_VF_PVRLDI(:, JBLK), Z_YDMF_PHYS_OUT_STRDU(:, :, JBLK), Z_YDMF_PHYS_OUT_STRDV(:, :, JBLK),  &
        & ZTRAJGWD(:, :, JBLK), YDSTACK=YLSTACK)
        
        IF (YDMODEL%YRML_PHY_MF%YRPHY%LSTRAS) THEN
          CALL ACPLUIS_OPENACC(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON,  &
          & YDMODEL%YRML_PHY_MF%YRTOPH%NTPLUI, YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP(:, :, JBLK), ZNEBS(:, :, JBLK), ZQV(:, :, JBLK), ZQLIS(:, :, JBLK),  &
          & ZMSC_QW(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_T(:, :, JBLK),  &
          & Z_YDMF_PHYS_OUT_FCSQL(:, :, JBLK), Z_YDMF_PHYS_OUT_FCSQN(:, :, JBLK), Z_YDMF_PHYS_OUT_FPLSL(:, :, JBLK),  &
          & Z_YDMF_PHYS_OUT_FPLSN(:, :, JBLK), Z_YDMF_PHYS_OUT_FPEVPSL(:, :, JBLK), Z_YDMF_PHYS_OUT_FPEVPSN(:, :, JBLK),  &
          & Z_YDMF_PHYS_OUT_FPFPSL(:, :, JBLK), Z_YDMF_PHYS_OUT_FPFPSN(:, :, JBLK), YDSTACK=YLSTACK)
        END IF
        
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRAG:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 338"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ACDRAG')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRAG:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 339"
      ZGWDCS => GET_HOST_DATA_RDWR(YL_ZGWDCS)
      ZQLIS => GET_HOST_DATA_RDWR(YL_ZQLIS)
      ZFLU_EMIS => GET_HOST_DATA_RDWR(YL_ZFLU_EMIS)
      ZQV => GET_HOST_DATA_RDWR(YL_ZQV)
      ZDQSTS => GET_HOST_DATA_RDWR(YL_ZDQSTS)
      ZMSC_QW => GET_HOST_DATA_RDWR(YL_ZMSC_QW)
      ZFLU_NEIJ => GET_HOST_DATA_RDWR(YL_ZFLU_NEIJ)
      ZTRAJGWD => GET_HOST_DATA_RDWR(YL_ZTRAJGWD)
      ZNBVNO => GET_HOST_DATA_RDWR(YL_ZNBVNO)
      ZCHROV => GET_HOST_DATA_RDWR(YL_ZCHROV)
      ZNEBS => GET_HOST_DATA_RDWR(YL_ZNEBS)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
      Z_YDMF_PHYS_OUT_FPEVPSL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPEVPSL)
      Z_YDMF_PHYS_SURF_GSD_VF_PVRLAN => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_VRLAN)
      Z_YDMF_PHYS_SURF_GSD_VF_PVRLDI => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_VRLDI)
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
      Z_YDVARS_GEOMETRY_RCORI_T0 => GET_HOST_DATA_RDWR(YDVARS%GEOMETRY%RCORI%FT0)
      Z_YDMF_PHYS_OUT_STRDU => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRDU)
      Z_YDMF_PHYS_OUT_DRNSHF => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DRNSHF)
      Z_YDMF_PHYS_BASE_STATE_U => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
      Z_YDMF_PHYS_SURF_GSD_VF_PLSM => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_LSM)
      Z_YDMF_PHYS_OUT_FCSQL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FCSQL)
      Z_YDMF_PHYS_OUT_FPEVPSN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPEVPSN)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_R)
      Z_YDMF_PHYS_SURF_GSD_VF_PGETRL => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_GETRL)
      Z_YDMF_PHYS_BASE_STATE_V => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
      Z_YDMF_PHYS_OUT_FPLSL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSL)
      Z_YDMF_PHYS_OUT_STRDV => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRDV)
      Z_YDMF_PHYS_OUT_FPFPSN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPFPSN)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_DELP)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_RDELP)
      Z_YDMF_PHYS_OUT_FPFPSL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPFPSL)
      Z_YDMF_PHYS_OUT_FPLSN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSN)
      Z_YDCPG_MISC_QS => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QS)
      Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
      Z_YDMF_PHYS_OUT_FCSQN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FCSQN)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRAG:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 340"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRAG:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 341"
    ZGWDCS => NULL()
    ZQLIS => NULL()
    ZFLU_EMIS => NULL()
    ZQV => NULL()
    ZDQSTS => NULL()
    ZMSC_QW => NULL()
    ZFLU_NEIJ => NULL()
    ZTRAJGWD => NULL()
    ZNBVNO => NULL()
    ZCHROV => NULL()
    ZNEBS => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => NULL()
    Z_YDMF_PHYS_OUT_FPEVPSL => NULL()
    Z_YDMF_PHYS_SURF_GSD_VF_PVRLAN => NULL()
    Z_YDMF_PHYS_SURF_GSD_VF_PVRLDI => NULL()
    Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => NULL()
    Z_YDVARS_GEOMETRY_RCORI_T0 => NULL()
    Z_YDMF_PHYS_OUT_STRDU => NULL()
    Z_YDMF_PHYS_OUT_DRNSHF => NULL()
    Z_YDMF_PHYS_BASE_STATE_U => NULL()
    Z_YDMF_PHYS_SURF_GSD_VF_PLSM => NULL()
    Z_YDMF_PHYS_OUT_FCSQL => NULL()
    Z_YDMF_PHYS_OUT_FPEVPSN => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => NULL()
    Z_YDMF_PHYS_SURF_GSD_VF_PGETRL => NULL()
    Z_YDMF_PHYS_BASE_STATE_V => NULL()
    Z_YDMF_PHYS_OUT_FPLSL => NULL()
    Z_YDMF_PHYS_OUT_STRDV => NULL()
    Z_YDMF_PHYS_OUT_FPFPSN => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP => NULL()
    Z_YDMF_PHYS_OUT_FPFPSL => NULL()
    Z_YDMF_PHYS_OUT_FPLSN => NULL()
    Z_YDCPG_MISC_QS => NULL()
    Z_YDMF_PHYS_BASE_STATE_T => NULL()
    Z_YDMF_PHYS_OUT_FCSQN => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRAG:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 342"
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ACDRAG')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRAG:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 343"
    ZGWDCS => GET_DEVICE_DATA_RDWR(YL_ZGWDCS)
    ZQLIS => GET_DEVICE_DATA_RDWR(YL_ZQLIS)
    ZFLU_EMIS => GET_DEVICE_DATA_RDWR(YL_ZFLU_EMIS)
    ZQV => GET_DEVICE_DATA_RDWR(YL_ZQV)
    ZDQSTS => GET_DEVICE_DATA_RDWR(YL_ZDQSTS)
    ZMSC_QW => GET_DEVICE_DATA_RDWR(YL_ZMSC_QW)
    ZFLU_NEIJ => GET_DEVICE_DATA_RDWR(YL_ZFLU_NEIJ)
    ZTRAJGWD => GET_DEVICE_DATA_RDWR(YL_ZTRAJGWD)
    ZNBVNO => GET_DEVICE_DATA_RDWR(YL_ZNBVNO)
    ZCHROV => GET_DEVICE_DATA_RDWR(YL_ZCHROV)
    ZNEBS => GET_DEVICE_DATA_RDWR(YL_ZNEBS)
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
    Z_YDMF_PHYS_OUT_FPEVPSL => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_FPEVPSL)
    Z_YDMF_PHYS_SURF_GSD_VF_PVRLAN => GET_DEVICE_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_VRLAN)
    Z_YDMF_PHYS_SURF_GSD_VF_PVRLDI => GET_DEVICE_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_VRLDI)
    Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
    Z_YDVARS_GEOMETRY_RCORI_T0 => GET_DEVICE_DATA_RDWR(YDVARS%GEOMETRY%RCORI%FT0)
    Z_YDMF_PHYS_OUT_STRDU => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_STRDU)
    Z_YDMF_PHYS_OUT_DRNSHF => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_DRNSHF)
    Z_YDMF_PHYS_BASE_STATE_U => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
    Z_YDMF_PHYS_SURF_GSD_VF_PLSM => GET_DEVICE_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_LSM)
    Z_YDMF_PHYS_OUT_FCSQL => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_FCSQL)
    Z_YDMF_PHYS_OUT_FPEVPSN => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_FPEVPSN)
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_R)
    Z_YDMF_PHYS_SURF_GSD_VF_PGETRL => GET_DEVICE_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_GETRL)
    Z_YDMF_PHYS_BASE_STATE_V => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
    Z_YDMF_PHYS_OUT_FPLSL => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSL)
    Z_YDMF_PHYS_OUT_STRDV => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_STRDV)
    Z_YDMF_PHYS_OUT_FPFPSN => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_FPFPSN)
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_DELP)
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_RDELP)
    Z_YDMF_PHYS_OUT_FPFPSL => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_FPFPSL)
    Z_YDMF_PHYS_OUT_FPLSN => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSN)
    Z_YDCPG_MISC_QS => GET_DEVICE_DATA_RDWR(YDCPG_MISC%F_QS)
    Z_YDMF_PHYS_BASE_STATE_T => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
    Z_YDMF_PHYS_OUT_FCSQN => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_FCSQN)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRAG:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 344"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRAG:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 345"
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZGWDCS, ZQLIS, ZFLU_EMIS, ZQV, ZDQSTS, ZMSC_QW, ZFLU_NEIJ,  &
!$ACC & ZTRAJGWD, ZNBVNO, ZCHROV, ZNEBS, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF, Z_YDMF_PHYS_OUT_FPEVPSL,  &
!$ACC & Z_YDMF_PHYS_SURF_GSD_VF_PVRLAN, Z_YDMF_PHYS_SURF_GSD_VF_PVRLDI, Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T,  &
!$ACC & Z_YDVARS_GEOMETRY_RCORI_T0, Z_YDMF_PHYS_OUT_STRDU, Z_YDMF_PHYS_OUT_DRNSHF, Z_YDMF_PHYS_BASE_STATE_U,  &
!$ACC & Z_YDMF_PHYS_SURF_GSD_VF_PLSM, Z_YDMF_PHYS_OUT_FCSQL, Z_YDMF_PHYS_OUT_FPEVPSN, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R,  &
!$ACC & Z_YDMF_PHYS_SURF_GSD_VF_PGETRL, Z_YDMF_PHYS_BASE_STATE_V, Z_YDMF_PHYS_OUT_FPLSL, Z_YDMF_PHYS_OUT_STRDV,  &
!$ACC & Z_YDMF_PHYS_OUT_FPFPSN, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP,  &
!$ACC & Z_YDMF_PHYS_OUT_FPFPSL, Z_YDMF_PHYS_OUT_FPLSN, Z_YDCPG_MISC_QS, Z_YDMF_PHYS_BASE_STATE_T, Z_YDMF_PHYS_OUT_FCSQN )  &
!$ACC & PRIVATE( JBLK ) VECTOR_LENGTH( YDCPG_OPTS%KLON )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK )
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        IF (YDMODEL%YRML_AOC%YRMCC%LMCC03) THEN
          CALL ACDNSHF_OPENACC(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF%YRPHY, YDMODEL%YRML_PHY_MF%YRPHY1, YLCPG_BNDS%KIDIA,  &
          & YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KTDIA, YDCPG_OPTS%KFLEVG, ZFLU_EMIS(:, JBLK),  &
          & Z_YDMF_PHYS_SURF_GSD_VF_PLSM(:, JBLK), ZFLU_NEIJ(:, JBLK), ZQV(:, :, JBLK), Z_YDCPG_MISC_QS(:, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T(:, JBLK), ZCHROV(:, JBLK), ZDQSTS(:, JBLK), Z_YDMF_PHYS_OUT_DRNSHF(:, JBLK),  &
          & YDSTACK=YLSTACK)
        END IF
        
        CALL ACDRAG_OPENACC(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON,  &
        & YDMODEL%YRML_PHY_MF%YRTOPH%NTDRAG, YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP(:, :, JBLK),  &
        & ZNBVNO(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_U(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_V(:, :, JBLK), Z_YDVARS_GEOMETRY_RCORI_T0(:, JBLK),  &
        & Z_YDMF_PHYS_SURF_GSD_VF_PGETRL(:, JBLK), ZGWDCS(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VF_PVRLAN(:, JBLK),  &
        & Z_YDMF_PHYS_SURF_GSD_VF_PVRLDI(:, JBLK), Z_YDMF_PHYS_OUT_STRDU(:, :, JBLK), Z_YDMF_PHYS_OUT_STRDV(:, :, JBLK),  &
        & ZTRAJGWD(:, :, JBLK), YDSTACK=YLSTACK)
        
        IF (YDMODEL%YRML_PHY_MF%YRPHY%LSTRAS) THEN
          CALL ACPLUIS_OPENACC(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON,  &
          & YDMODEL%YRML_PHY_MF%YRTOPH%NTPLUI, YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP(:, :, JBLK), ZNEBS(:, :, JBLK), ZQV(:, :, JBLK), ZQLIS(:, :, JBLK),  &
          & ZMSC_QW(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_T(:, :, JBLK),  &
          & Z_YDMF_PHYS_OUT_FCSQL(:, :, JBLK), Z_YDMF_PHYS_OUT_FCSQN(:, :, JBLK), Z_YDMF_PHYS_OUT_FPLSL(:, :, JBLK),  &
          & Z_YDMF_PHYS_OUT_FPLSN(:, :, JBLK), Z_YDMF_PHYS_OUT_FPEVPSL(:, :, JBLK), Z_YDMF_PHYS_OUT_FPEVPSN(:, :, JBLK),  &
          & Z_YDMF_PHYS_OUT_FPFPSL(:, :, JBLK), Z_YDMF_PHYS_OUT_FPFPSN(:, :, JBLK), YDSTACK=YLSTACK)
        END IF
        
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRAG:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 346"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ACDRAG')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRAG:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 347"
      ZGWDCS => GET_HOST_DATA_RDWR(YL_ZGWDCS)
      ZQLIS => GET_HOST_DATA_RDWR(YL_ZQLIS)
      ZFLU_EMIS => GET_HOST_DATA_RDWR(YL_ZFLU_EMIS)
      ZQV => GET_HOST_DATA_RDWR(YL_ZQV)
      ZDQSTS => GET_HOST_DATA_RDWR(YL_ZDQSTS)
      ZMSC_QW => GET_HOST_DATA_RDWR(YL_ZMSC_QW)
      ZFLU_NEIJ => GET_HOST_DATA_RDWR(YL_ZFLU_NEIJ)
      ZTRAJGWD => GET_HOST_DATA_RDWR(YL_ZTRAJGWD)
      ZNBVNO => GET_HOST_DATA_RDWR(YL_ZNBVNO)
      ZCHROV => GET_HOST_DATA_RDWR(YL_ZCHROV)
      ZNEBS => GET_HOST_DATA_RDWR(YL_ZNEBS)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
      Z_YDMF_PHYS_OUT_FPEVPSL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPEVPSL)
      Z_YDMF_PHYS_SURF_GSD_VF_PVRLAN => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_VRLAN)
      Z_YDMF_PHYS_SURF_GSD_VF_PVRLDI => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_VRLDI)
      Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YGSP_RR%F_T)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
      Z_YDVARS_GEOMETRY_RCORI_T0 => GET_HOST_DATA_RDWR(YDVARS%GEOMETRY%RCORI%FT0)
      Z_YDMF_PHYS_OUT_STRDU => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRDU)
      Z_YDMF_PHYS_OUT_DRNSHF => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DRNSHF)
      Z_YDMF_PHYS_BASE_STATE_U => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
      Z_YDMF_PHYS_SURF_GSD_VF_PLSM => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_LSM)
      Z_YDMF_PHYS_OUT_FCSQL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FCSQL)
      Z_YDMF_PHYS_OUT_FPEVPSN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPEVPSN)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_R)
      Z_YDMF_PHYS_SURF_GSD_VF_PGETRL => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VF%F_GETRL)
      Z_YDMF_PHYS_BASE_STATE_V => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
      Z_YDMF_PHYS_OUT_FPLSL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSL)
      Z_YDMF_PHYS_OUT_STRDV => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRDV)
      Z_YDMF_PHYS_OUT_FPFPSN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPFPSN)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_DELP)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_RDELP)
      Z_YDMF_PHYS_OUT_FPFPSL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPFPSL)
      Z_YDMF_PHYS_OUT_FPLSN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSN)
      Z_YDCPG_MISC_QS => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QS)
      Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
      Z_YDMF_PHYS_OUT_FCSQN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FCSQN)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRAG:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 348"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRAG:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 349"
    ZGWDCS => NULL()
    ZQLIS => NULL()
    ZFLU_EMIS => NULL()
    ZQV => NULL()
    ZDQSTS => NULL()
    ZMSC_QW => NULL()
    ZFLU_NEIJ => NULL()
    ZTRAJGWD => NULL()
    ZNBVNO => NULL()
    ZCHROV => NULL()
    ZNEBS => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => NULL()
    Z_YDMF_PHYS_OUT_FPEVPSL => NULL()
    Z_YDMF_PHYS_SURF_GSD_VF_PVRLAN => NULL()
    Z_YDMF_PHYS_SURF_GSD_VF_PVRLDI => NULL()
    Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => NULL()
    Z_YDVARS_GEOMETRY_RCORI_T0 => NULL()
    Z_YDMF_PHYS_OUT_STRDU => NULL()
    Z_YDMF_PHYS_OUT_DRNSHF => NULL()
    Z_YDMF_PHYS_BASE_STATE_U => NULL()
    Z_YDMF_PHYS_SURF_GSD_VF_PLSM => NULL()
    Z_YDMF_PHYS_OUT_FCSQL => NULL()
    Z_YDMF_PHYS_OUT_FPEVPSN => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => NULL()
    Z_YDMF_PHYS_SURF_GSD_VF_PGETRL => NULL()
    Z_YDMF_PHYS_BASE_STATE_V => NULL()
    Z_YDMF_PHYS_OUT_FPLSL => NULL()
    Z_YDMF_PHYS_OUT_STRDV => NULL()
    Z_YDMF_PHYS_OUT_FPFPSN => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP => NULL()
    Z_YDMF_PHYS_OUT_FPFPSL => NULL()
    Z_YDMF_PHYS_OUT_FPLSN => NULL()
    Z_YDCPG_MISC_QS => NULL()
    Z_YDMF_PHYS_BASE_STATE_T => NULL()
    Z_YDMF_PHYS_OUT_FCSQN => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRAG:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 350"
  END IF
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRAG', 1, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 351"
  
  IF (YDMODEL%YRML_PHY_MF%YRPHY%LCVTDK) THEN
    CALL APL_ARPEGE_DEEP_CONVECTION_PARALLEL(YDMF_PHYS_BASE_STATE, YDGEOMETRY, YDCPG_BNDS, YDCPG_OPTS, YDMF_PHYS,  &
    & YDCPG_DYN0, YDMF_PHYS_SURF, YDVARS, YDMODEL, YL_ZFPCOR, YL_ZQV, YL_ZTENHA, YL_ZTENQVA, YL_ZTENT, YL_ZRDG_CVGQ,  &
    & YL_ZRDG_CVGT)
  END IF
  
  IF (YDMODEL%YRML_PHY_MF%YRPHY%LECTPRODCVP) THEN
    
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:TURBULENCE_PART2', 0, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 352"
    IF (LPARALLELMETHOD('OPENMP', 'APL_ARPEGE_PARALLEL:TURBULENCE_PART2')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:TURBULENCE_PART2:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 353"
      ZECTCLS => GET_HOST_DATA_RDWR(YL_ZECTCLS)
      ZKCLS => GET_HOST_DATA_RDWR(YL_ZKCLS)
      ZKUROV2 => GET_HOST_DATA_RDWR(YL_ZKUROV2)
      ZTKE1 => GET_HOST_DATA_RDWR(YL_ZTKE1)
      ZPFL_FTKE => GET_HOST_DATA_RDWR(YL_ZPFL_FTKE)
      ZUSLE => GET_HOST_DATA_RDWR(YL_ZUSLE)
      ZPRODTH => GET_HOST_DATA_RDWR(YL_ZPRODTH)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
      Z_YDMF_PHYS_OUT_STRCV => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRCV)
      Z_YDMF_PHYS_OUT_STRCU => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRCU)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP)
      Z_YDMF_PHYS_BASE_STATE_U => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
      Z_YDMF_PHYS_OUT_DIFCS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFCS)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_R)
      Z_YDMF_PHYS_BASE_STATE_V => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
      Z_YDMF_PHYS_BASE_STATE_TKE => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_TKE)
      Z_YDMF_PHYS_BASE_STATE_Q => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_Q)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_DELP)
      Z_YDMF_PHYS_OUT_EDR => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_EDR)
      Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
      Z_YDMF_PHYS_OUT_DIFCQ => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFCQ)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:TURBULENCE_PART2:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 354"
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:TURBULENCE_PART2:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 355"
      CALL YLCPG_BNDS%INIT(YDCPG_OPTS)
!$OMP PARALLEL DO PRIVATE( JBLK ) FIRSTPRIVATE( YLCPG_BNDS )
      DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
        CALL YLCPG_BNDS%UPDATE(JBLK)
        
        CALL APL_ARPEGE_TURBULENCE_PART2(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA,  &
        & YDCPG_OPTS%KLON, YDMODEL%YRML_PHY_MF%YRTOPH%NTCOEF, YDMODEL%YRML_PHY_MF%YRTOPH%NTCOET, YDCPG_OPTS%KFLEVG,  &
        & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_T(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_Q(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_U(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_V(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_TKE(:, :, JBLK), ZPRODTH(:, :, JBLK), ZKUROV2(:, :, JBLK),  &
        & Z_YDMF_PHYS_OUT_DIFCQ(:, :, JBLK), Z_YDMF_PHYS_OUT_DIFCS(:, :, JBLK), Z_YDMF_PHYS_OUT_STRCU(:, :, JBLK),  &
        & Z_YDMF_PHYS_OUT_STRCV(:, :, JBLK), ZUSLE(:, :, JBLK), ZKCLS(:, JBLK), ZECTCLS(:, JBLK), ZPFL_FTKE(:, :, JBLK),  &
        & ZTKE1(:, :, JBLK), Z_YDMF_PHYS_OUT_EDR(:, :, JBLK))
        
      END DO
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:TURBULENCE_PART2:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 356"
      IF (LSYNCHOST('APL_ARPEGE_PARALLEL:TURBULENCE_PART2')) THEN
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:TURBULENCE_PART2:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 357"
        ZECTCLS => GET_HOST_DATA_RDWR(YL_ZECTCLS)
        ZKCLS => GET_HOST_DATA_RDWR(YL_ZKCLS)
        ZKUROV2 => GET_HOST_DATA_RDWR(YL_ZKUROV2)
        ZTKE1 => GET_HOST_DATA_RDWR(YL_ZTKE1)
        ZPFL_FTKE => GET_HOST_DATA_RDWR(YL_ZPFL_FTKE)
        ZUSLE => GET_HOST_DATA_RDWR(YL_ZUSLE)
        ZPRODTH => GET_HOST_DATA_RDWR(YL_ZPRODTH)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
        Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
        Z_YDMF_PHYS_OUT_STRCV => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRCV)
        Z_YDMF_PHYS_OUT_STRCU => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRCU)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
        Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP)
        Z_YDMF_PHYS_BASE_STATE_U => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
        Z_YDMF_PHYS_OUT_DIFCS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFCS)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_R)
        Z_YDMF_PHYS_BASE_STATE_V => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
        Z_YDMF_PHYS_BASE_STATE_TKE => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_TKE)
        Z_YDMF_PHYS_BASE_STATE_Q => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_Q)
        Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_DELP)
        Z_YDMF_PHYS_OUT_EDR => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_EDR)
        Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
        Z_YDMF_PHYS_OUT_DIFCQ => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFCQ)
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:TURBULENCE_PART2:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 358"
      END IF
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:TURBULENCE_PART2:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 359"
      ZECTCLS => NULL()
      ZKCLS => NULL()
      ZKUROV2 => NULL()
      ZTKE1 => NULL()
      ZPFL_FTKE => NULL()
      ZUSLE => NULL()
      ZPRODTH => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => NULL()
      Z_YDMF_PHYS_OUT_STRCV => NULL()
      Z_YDMF_PHYS_OUT_STRCU => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => NULL()
      Z_YDMF_PHYS_BASE_STATE_U => NULL()
      Z_YDMF_PHYS_OUT_DIFCS => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => NULL()
      Z_YDMF_PHYS_BASE_STATE_V => NULL()
      Z_YDMF_PHYS_BASE_STATE_TKE => NULL()
      Z_YDMF_PHYS_BASE_STATE_Q => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP => NULL()
      Z_YDMF_PHYS_OUT_EDR => NULL()
      Z_YDMF_PHYS_BASE_STATE_T => NULL()
      Z_YDMF_PHYS_OUT_DIFCQ => NULL()
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:TURBULENCE_PART2:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 360"
    ELSE IF (LPARALLELMETHOD('OPENMPSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:TURBULENCE_PART2')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:TURBULENCE_PART2:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 361"
      ZECTCLS => GET_HOST_DATA_RDWR(YL_ZECTCLS)
      ZKCLS => GET_HOST_DATA_RDWR(YL_ZKCLS)
      ZKUROV2 => GET_HOST_DATA_RDWR(YL_ZKUROV2)
      ZTKE1 => GET_HOST_DATA_RDWR(YL_ZTKE1)
      ZPFL_FTKE => GET_HOST_DATA_RDWR(YL_ZPFL_FTKE)
      ZUSLE => GET_HOST_DATA_RDWR(YL_ZUSLE)
      ZPRODTH => GET_HOST_DATA_RDWR(YL_ZPRODTH)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
      Z_YDMF_PHYS_OUT_STRCV => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRCV)
      Z_YDMF_PHYS_OUT_STRCU => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRCU)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP)
      Z_YDMF_PHYS_BASE_STATE_U => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
      Z_YDMF_PHYS_OUT_DIFCS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFCS)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_R)
      Z_YDMF_PHYS_BASE_STATE_V => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
      Z_YDMF_PHYS_BASE_STATE_TKE => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_TKE)
      Z_YDMF_PHYS_BASE_STATE_Q => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_Q)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_DELP)
      Z_YDMF_PHYS_OUT_EDR => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_EDR)
      Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
      Z_YDMF_PHYS_OUT_DIFCQ => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFCQ)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:TURBULENCE_PART2:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 362"
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:TURBULENCE_PART2:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 363"
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
      DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
        DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
          YLCPG_BNDS%KIDIA = JLON
          YLCPG_BNDS%KFDIA = JLON
          YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          
          CALL APL_ARPEGE_TURBULENCE_PART2_OPENACC(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA,  &
          & YDCPG_OPTS%KLON, YDMODEL%YRML_PHY_MF%YRTOPH%NTCOEF, YDMODEL%YRML_PHY_MF%YRTOPH%NTCOET, YDCPG_OPTS%KFLEVG,  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_T(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_Q(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_U(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_V(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_TKE(:, :, JBLK), ZPRODTH(:, :, JBLK),  &
          & ZKUROV2(:, :, JBLK), Z_YDMF_PHYS_OUT_DIFCQ(:, :, JBLK), Z_YDMF_PHYS_OUT_DIFCS(:, :, JBLK),  &
          & Z_YDMF_PHYS_OUT_STRCU(:, :, JBLK), Z_YDMF_PHYS_OUT_STRCV(:, :, JBLK), ZUSLE(:, :, JBLK), ZKCLS(:, JBLK),  &
          & ZECTCLS(:, JBLK), ZPFL_FTKE(:, :, JBLK), ZTKE1(:, :, JBLK), Z_YDMF_PHYS_OUT_EDR(:, :, JBLK), YDSTACK=YLSTACK)
          
        END DO
      END DO
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:TURBULENCE_PART2:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 364"
      IF (LSYNCHOST('APL_ARPEGE_PARALLEL:TURBULENCE_PART2')) THEN
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:TURBULENCE_PART2:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 365"
        ZECTCLS => GET_HOST_DATA_RDWR(YL_ZECTCLS)
        ZKCLS => GET_HOST_DATA_RDWR(YL_ZKCLS)
        ZKUROV2 => GET_HOST_DATA_RDWR(YL_ZKUROV2)
        ZTKE1 => GET_HOST_DATA_RDWR(YL_ZTKE1)
        ZPFL_FTKE => GET_HOST_DATA_RDWR(YL_ZPFL_FTKE)
        ZUSLE => GET_HOST_DATA_RDWR(YL_ZUSLE)
        ZPRODTH => GET_HOST_DATA_RDWR(YL_ZPRODTH)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
        Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
        Z_YDMF_PHYS_OUT_STRCV => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRCV)
        Z_YDMF_PHYS_OUT_STRCU => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRCU)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
        Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP)
        Z_YDMF_PHYS_BASE_STATE_U => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
        Z_YDMF_PHYS_OUT_DIFCS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFCS)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_R)
        Z_YDMF_PHYS_BASE_STATE_V => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
        Z_YDMF_PHYS_BASE_STATE_TKE => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_TKE)
        Z_YDMF_PHYS_BASE_STATE_Q => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_Q)
        Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_DELP)
        Z_YDMF_PHYS_OUT_EDR => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_EDR)
        Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
        Z_YDMF_PHYS_OUT_DIFCQ => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFCQ)
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:TURBULENCE_PART2:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 366"
      END IF
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:TURBULENCE_PART2:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 367"
      ZECTCLS => NULL()
      ZKCLS => NULL()
      ZKUROV2 => NULL()
      ZTKE1 => NULL()
      ZPFL_FTKE => NULL()
      ZUSLE => NULL()
      ZPRODTH => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => NULL()
      Z_YDMF_PHYS_OUT_STRCV => NULL()
      Z_YDMF_PHYS_OUT_STRCU => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => NULL()
      Z_YDMF_PHYS_BASE_STATE_U => NULL()
      Z_YDMF_PHYS_OUT_DIFCS => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => NULL()
      Z_YDMF_PHYS_BASE_STATE_V => NULL()
      Z_YDMF_PHYS_BASE_STATE_TKE => NULL()
      Z_YDMF_PHYS_BASE_STATE_Q => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP => NULL()
      Z_YDMF_PHYS_OUT_EDR => NULL()
      Z_YDMF_PHYS_BASE_STATE_T => NULL()
      Z_YDMF_PHYS_OUT_DIFCQ => NULL()
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:TURBULENCE_PART2:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 368"
    ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:TURBULENCE_PART2')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:TURBULENCE_PART2:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 369"
      ZECTCLS => GET_DEVICE_DATA_RDWR(YL_ZECTCLS)
      ZKCLS => GET_DEVICE_DATA_RDWR(YL_ZKCLS)
      ZKUROV2 => GET_DEVICE_DATA_RDWR(YL_ZKUROV2)
      ZTKE1 => GET_DEVICE_DATA_RDWR(YL_ZTKE1)
      ZPFL_FTKE => GET_DEVICE_DATA_RDWR(YL_ZPFL_FTKE)
      ZUSLE => GET_DEVICE_DATA_RDWR(YL_ZUSLE)
      ZPRODTH => GET_DEVICE_DATA_RDWR(YL_ZPRODTH)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
      Z_YDMF_PHYS_OUT_STRCV => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_STRCV)
      Z_YDMF_PHYS_OUT_STRCU => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_STRCU)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP)
      Z_YDMF_PHYS_BASE_STATE_U => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
      Z_YDMF_PHYS_OUT_DIFCS => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_DIFCS)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_R)
      Z_YDMF_PHYS_BASE_STATE_V => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
      Z_YDMF_PHYS_BASE_STATE_TKE => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_TKE)
      Z_YDMF_PHYS_BASE_STATE_Q => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_Q)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_DELP)
      Z_YDMF_PHYS_OUT_EDR => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_EDR)
      Z_YDMF_PHYS_BASE_STATE_T => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
      Z_YDMF_PHYS_OUT_DIFCQ => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_DIFCQ)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:TURBULENCE_PART2:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 370"
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:TURBULENCE_PART2:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 371"
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZECTCLS, ZKCLS, ZKUROV2, ZTKE1, ZPFL_FTKE, ZUSLE, ZPRODTH,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF, Z_YDMF_PHYS_OUT_STRCV,  &
!$ACC & Z_YDMF_PHYS_OUT_STRCU, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_U, Z_YDMF_PHYS_OUT_DIFCS, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R, Z_YDMF_PHYS_BASE_STATE_V,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_TKE, Z_YDMF_PHYS_BASE_STATE_Q, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP, Z_YDMF_PHYS_OUT_EDR,  &
!$ACC & Z_YDMF_PHYS_OUT_DIFCQ ) PRIVATE( JBLK ) VECTOR_LENGTH( YDCPG_OPTS%KLON )
      DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK )
        DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
          YLCPG_BNDS%KIDIA = JLON
          YLCPG_BNDS%KFDIA = JLON
          YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          
          CALL APL_ARPEGE_TURBULENCE_PART2_OPENACC(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA,  &
          & YDCPG_OPTS%KLON, YDMODEL%YRML_PHY_MF%YRTOPH%NTCOEF, YDMODEL%YRML_PHY_MF%YRTOPH%NTCOET, YDCPG_OPTS%KFLEVG,  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_T(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_Q(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_U(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_V(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_TKE(:, :, JBLK), ZPRODTH(:, :, JBLK),  &
          & ZKUROV2(:, :, JBLK), Z_YDMF_PHYS_OUT_DIFCQ(:, :, JBLK), Z_YDMF_PHYS_OUT_DIFCS(:, :, JBLK),  &
          & Z_YDMF_PHYS_OUT_STRCU(:, :, JBLK), Z_YDMF_PHYS_OUT_STRCV(:, :, JBLK), ZUSLE(:, :, JBLK), ZKCLS(:, JBLK),  &
          & ZECTCLS(:, JBLK), ZPFL_FTKE(:, :, JBLK), ZTKE1(:, :, JBLK), Z_YDMF_PHYS_OUT_EDR(:, :, JBLK), YDSTACK=YLSTACK)
          
        END DO
      END DO
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:TURBULENCE_PART2:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 372"
      IF (LSYNCHOST('APL_ARPEGE_PARALLEL:TURBULENCE_PART2')) THEN
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:TURBULENCE_PART2:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 373"
        ZECTCLS => GET_HOST_DATA_RDWR(YL_ZECTCLS)
        ZKCLS => GET_HOST_DATA_RDWR(YL_ZKCLS)
        ZKUROV2 => GET_HOST_DATA_RDWR(YL_ZKUROV2)
        ZTKE1 => GET_HOST_DATA_RDWR(YL_ZTKE1)
        ZPFL_FTKE => GET_HOST_DATA_RDWR(YL_ZPFL_FTKE)
        ZUSLE => GET_HOST_DATA_RDWR(YL_ZUSLE)
        ZPRODTH => GET_HOST_DATA_RDWR(YL_ZPRODTH)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
        Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
        Z_YDMF_PHYS_OUT_STRCV => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRCV)
        Z_YDMF_PHYS_OUT_STRCU => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRCU)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
        Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP)
        Z_YDMF_PHYS_BASE_STATE_U => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
        Z_YDMF_PHYS_OUT_DIFCS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFCS)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_R)
        Z_YDMF_PHYS_BASE_STATE_V => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
        Z_YDMF_PHYS_BASE_STATE_TKE => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_TKE)
        Z_YDMF_PHYS_BASE_STATE_Q => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_Q)
        Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_DELP)
        Z_YDMF_PHYS_OUT_EDR => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_EDR)
        Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
        Z_YDMF_PHYS_OUT_DIFCQ => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFCQ)
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:TURBULENCE_PART2:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 374"
      END IF
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:TURBULENCE_PART2:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 375"
      ZECTCLS => NULL()
      ZKCLS => NULL()
      ZKUROV2 => NULL()
      ZTKE1 => NULL()
      ZPFL_FTKE => NULL()
      ZUSLE => NULL()
      ZPRODTH => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => NULL()
      Z_YDMF_PHYS_OUT_STRCV => NULL()
      Z_YDMF_PHYS_OUT_STRCU => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => NULL()
      Z_YDMF_PHYS_BASE_STATE_U => NULL()
      Z_YDMF_PHYS_OUT_DIFCS => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => NULL()
      Z_YDMF_PHYS_BASE_STATE_V => NULL()
      Z_YDMF_PHYS_BASE_STATE_TKE => NULL()
      Z_YDMF_PHYS_BASE_STATE_Q => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP => NULL()
      Z_YDMF_PHYS_OUT_EDR => NULL()
      Z_YDMF_PHYS_BASE_STATE_T => NULL()
      Z_YDMF_PHYS_OUT_DIFCQ => NULL()
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:TURBULENCE_PART2:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 376"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:TURBULENCE_PART2', 1, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 377"
    
  END IF
  
  CALL APL_ARPEGE_PRECIPITATION_PARALLEL(YDMF_PHYS_BASE_STATE, YDCPG_BNDS, YDCPG_OPTS, YDMF_PHYS, YDMF_PHYS_SURF, YDVARS,  &
  & YDMODEL, YL_ZFLU_NEIJ, YL_ZNEBS, YL_ZNEB_CVPP, YL_ZQC_DET_PCMT, YL_ZQI, YL_ZQL, YL_ZQLIS, YL_ZQLI_CVPP, YL_ZQR, YL_ZQS,  &
  & YL_ZQV, YL_ZSEDIQI, YL_ZSEDIQL, YL_ZTENHA, YL_ZTENQVA, YDGEOMETRY%YRSTA)
  
  
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:QNGCOR', 0, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 378"
  IF (LPARALLELMETHOD('OPENMP', 'APL_ARPEGE_PARALLEL:QNGCOR')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:QNGCOR:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 379"
    ZQV => GET_HOST_DATA_RDWR(YL_ZQV)
    ZPFL_FPLCH => GET_HOST_DATA_RDWR(YL_ZPFL_FPLCH)
    ZPFL_FPLSH => GET_HOST_DATA_RDWR(YL_ZPFL_FPLSH)
    ZQI => GET_HOST_DATA_RDWR(YL_ZQI)
    ZSEDIQI => GET_HOST_DATA_RDWR(YL_ZSEDIQI)
    ZFPCOR => GET_HOST_DATA_RDWR(YL_ZFPCOR)
    ZFLX_LOTT_GWV => GET_HOST_DATA_RDWR(YL_ZFLX_LOTT_GWV)
    ZQL => GET_HOST_DATA_RDWR(YL_ZQL)
    ZSEDIQL => GET_HOST_DATA_RDWR(YL_ZSEDIQL)
    ZFPLSN => GET_HOST_DATA_RDWR(YL_ZFPLSN)
    ZFLX_LOTT_GWU => GET_HOST_DATA_RDWR(YL_ZFLX_LOTT_GWU)
    ZFPLSL => GET_HOST_DATA_RDWR(YL_ZFPLSL)
    Z_YDMF_PHYS_OUT_DIFTQN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFTQN)
    Z_YDMF_PHYS_OUT_FPEVPCL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPEVPCL)
    Z_YDMF_PHYS_OUT_DIFTQL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFTQL)
    Z_YDMF_PHYS_OUT_FPEVPSL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPEVPSL)
    Z_YDMF_PHYS_OUT_DIFCQN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFCQN)
    Z_YDMF_PHYS_OUT_FPEVPCN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPEVPCN)
    Z_YDMF_PHYS_OUT_FPLSG => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSG)
    Z_YDMF_PHYS_OUT_FCCQN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FCCQN)
    Z_YDMF_PHYS_OUT_FCSQL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FCSQL)
    Z_YDMF_PHYS_OUT_FPEVPSN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPEVPSN)
    Z_YDMF_PHYS_OUT_FPLSN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSN)
    Z_YDMF_PHYS_OUT_FPFPCL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPFPCL)
    Z_YDMF_PHYS_OUT_FPLSL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSL)
    Z_YDMF_PHYS_OUT_FPFPSN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPFPSN)
    Z_YDMF_PHYS_OUT_DIFCQL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFCQL)
    Z_YDMF_PHYS_OUT_FPFPCN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPFPCN)
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_RDELP)
    Z_YDMF_PHYS_OUT_FPFPSL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPFPSL)
    Z_YDMF_PHYS_OUT_DIFTQ => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFTQ)
    Z_YDMF_PHYS_OUT_DIFCQ => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFCQ)
    Z_YDMF_PHYS_OUT_FCCQL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FCCQL)
    Z_YDMF_PHYS_OUT_FCQNG => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FCQNG)
    Z_YDMF_PHYS_OUT_FCSQN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FCSQN)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:QNGCOR:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 380"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:QNGCOR:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 381"
    CALL YLCPG_BNDS%INIT(YDCPG_OPTS)
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, JLEV ) FIRSTPRIVATE( YLCPG_BNDS )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      CALL YLCPG_BNDS%UPDATE(JBLK)
      
      DO JLEV=0,YDCPG_OPTS%KFLEVG
        DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
          ZFLX_LOTT_GWU(JLON, JLEV, JBLK) = 0.0_JPRB
          ZFLX_LOTT_GWV(JLON, JLEV, JBLK) = 0.0_JPRB
        END DO
      END DO
      
      IF (YDMODEL%YRML_PHY_MF%YRPHY%LNEBN .or. YDMODEL%YRML_PHY_MF%YRPHY%LRRGUST) THEN
        DO JLEV=YDCPG_OPTS%KTDIA - 1,YDCPG_OPTS%KFLEVG
          DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
            ZPFL_FPLCH(JLON, JLEV, JBLK) = ZFPCOR(JLON, JLEV, JBLK)
          END DO
        END DO
      END IF
      
      IF (YDMODEL%YRML_PHY_MF%YRPHY%LRRGUST) THEN
        DO JLEV=YDCPG_OPTS%KTDIA - 1,YDCPG_OPTS%KFLEVG
          DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
            ZPFL_FPLSH(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_FPLSL(JLON, JLEV, JBLK) + Z_YDMF_PHYS_OUT_FPLSN(JLON, JLEV, JBLK)  &
            & + Z_YDMF_PHYS_OUT_FPLSG(JLON, JLEV, JBLK)
          END DO
        END DO
      END IF
      
      
      ! Update transport fluxes due to sedimentation of clouds.
      
      DO JLEV=YDCPG_OPTS%KTDIA,YDCPG_OPTS%KFLEVG
        DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
          Z_YDMF_PHYS_OUT_DIFTQL(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_DIFTQL(JLON, JLEV, JBLK) + ZSEDIQL(JLON, JLEV, JBLK)
          Z_YDMF_PHYS_OUT_DIFTQN(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_DIFTQN(JLON, JLEV, JBLK) + ZSEDIQI(JLON, JLEV, JBLK)
          ZFPLSL(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_DIFTQL(JLON, JLEV, JBLK) + Z_YDMF_PHYS_OUT_FPLSL(JLON, JLEV, JBLK)
          ZFPLSN(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_DIFTQN(JLON, JLEV, JBLK) + Z_YDMF_PHYS_OUT_FPLSN(JLON, JLEV, JBLK)
        END DO
      END DO
      
      ! Correct negative water contents.
      
      CALL QNGCOR(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF%YRPHY2, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON,  &
      & YDMODEL%YRML_PHY_MF%YRTOPH%NTPLUI, YDCPG_OPTS%KFLEVG, ZQV(:, :, JBLK), ZQL(:, :, JBLK), ZQI(:, :, JBLK),  &
      & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP(:, :, JBLK), Z_YDMF_PHYS_OUT_DIFCQ(:, :, JBLK),  &
      & Z_YDMF_PHYS_OUT_DIFCQN(:, :, JBLK), Z_YDMF_PHYS_OUT_DIFCQL(:, :, JBLK), Z_YDMF_PHYS_OUT_DIFTQ(:, :, JBLK),  &
      & Z_YDMF_PHYS_OUT_DIFTQN(:, :, JBLK), Z_YDMF_PHYS_OUT_DIFTQL(:, :, JBLK), Z_YDMF_PHYS_OUT_FPEVPSL(:, :, JBLK),  &
      & Z_YDMF_PHYS_OUT_FPEVPSN(:, :, JBLK), Z_YDMF_PHYS_OUT_FPEVPCL(:, :, JBLK), Z_YDMF_PHYS_OUT_FPEVPCN(:, :, JBLK),  &
      & Z_YDMF_PHYS_OUT_FPFPSL(:, :, JBLK), Z_YDMF_PHYS_OUT_FPFPSN(:, :, JBLK), Z_YDMF_PHYS_OUT_FPFPCL(:, :, JBLK),  &
      & Z_YDMF_PHYS_OUT_FPFPCN(:, :, JBLK), Z_YDMF_PHYS_OUT_FCCQL(:, :, JBLK), Z_YDMF_PHYS_OUT_FCCQN(:, :, JBLK),  &
      & Z_YDMF_PHYS_OUT_FCSQL(:, :, JBLK), Z_YDMF_PHYS_OUT_FCSQN(:, :, JBLK), Z_YDMF_PHYS_OUT_FCQNG(:, :, JBLK))
      
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:QNGCOR:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 382"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:QNGCOR')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:QNGCOR:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 383"
      ZQV => GET_HOST_DATA_RDWR(YL_ZQV)
      ZPFL_FPLCH => GET_HOST_DATA_RDWR(YL_ZPFL_FPLCH)
      ZPFL_FPLSH => GET_HOST_DATA_RDWR(YL_ZPFL_FPLSH)
      ZQI => GET_HOST_DATA_RDWR(YL_ZQI)
      ZSEDIQI => GET_HOST_DATA_RDWR(YL_ZSEDIQI)
      ZFPCOR => GET_HOST_DATA_RDWR(YL_ZFPCOR)
      ZFLX_LOTT_GWV => GET_HOST_DATA_RDWR(YL_ZFLX_LOTT_GWV)
      ZQL => GET_HOST_DATA_RDWR(YL_ZQL)
      ZSEDIQL => GET_HOST_DATA_RDWR(YL_ZSEDIQL)
      ZFPLSN => GET_HOST_DATA_RDWR(YL_ZFPLSN)
      ZFLX_LOTT_GWU => GET_HOST_DATA_RDWR(YL_ZFLX_LOTT_GWU)
      ZFPLSL => GET_HOST_DATA_RDWR(YL_ZFPLSL)
      Z_YDMF_PHYS_OUT_DIFTQN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFTQN)
      Z_YDMF_PHYS_OUT_FPEVPCL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPEVPCL)
      Z_YDMF_PHYS_OUT_DIFTQL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFTQL)
      Z_YDMF_PHYS_OUT_FPEVPSL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPEVPSL)
      Z_YDMF_PHYS_OUT_DIFCQN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFCQN)
      Z_YDMF_PHYS_OUT_FPEVPCN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPEVPCN)
      Z_YDMF_PHYS_OUT_FPLSG => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSG)
      Z_YDMF_PHYS_OUT_FCCQN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FCCQN)
      Z_YDMF_PHYS_OUT_FCSQL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FCSQL)
      Z_YDMF_PHYS_OUT_FPEVPSN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPEVPSN)
      Z_YDMF_PHYS_OUT_FPLSN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSN)
      Z_YDMF_PHYS_OUT_FPFPCL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPFPCL)
      Z_YDMF_PHYS_OUT_FPLSL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSL)
      Z_YDMF_PHYS_OUT_FPFPSN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPFPSN)
      Z_YDMF_PHYS_OUT_DIFCQL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFCQL)
      Z_YDMF_PHYS_OUT_FPFPCN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPFPCN)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_RDELP)
      Z_YDMF_PHYS_OUT_FPFPSL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPFPSL)
      Z_YDMF_PHYS_OUT_DIFTQ => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFTQ)
      Z_YDMF_PHYS_OUT_DIFCQ => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFCQ)
      Z_YDMF_PHYS_OUT_FCCQL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FCCQL)
      Z_YDMF_PHYS_OUT_FCQNG => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FCQNG)
      Z_YDMF_PHYS_OUT_FCSQN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FCSQN)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:QNGCOR:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 384"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:QNGCOR:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 385"
    ZQV => NULL()
    ZPFL_FPLCH => NULL()
    ZPFL_FPLSH => NULL()
    ZQI => NULL()
    ZSEDIQI => NULL()
    ZFPCOR => NULL()
    ZFLX_LOTT_GWV => NULL()
    ZQL => NULL()
    ZSEDIQL => NULL()
    ZFPLSN => NULL()
    ZFLX_LOTT_GWU => NULL()
    ZFPLSL => NULL()
    Z_YDMF_PHYS_OUT_DIFTQN => NULL()
    Z_YDMF_PHYS_OUT_FPEVPCL => NULL()
    Z_YDMF_PHYS_OUT_DIFTQL => NULL()
    Z_YDMF_PHYS_OUT_FPEVPSL => NULL()
    Z_YDMF_PHYS_OUT_DIFCQN => NULL()
    Z_YDMF_PHYS_OUT_FPEVPCN => NULL()
    Z_YDMF_PHYS_OUT_FPLSG => NULL()
    Z_YDMF_PHYS_OUT_FCCQN => NULL()
    Z_YDMF_PHYS_OUT_FCSQL => NULL()
    Z_YDMF_PHYS_OUT_FPEVPSN => NULL()
    Z_YDMF_PHYS_OUT_FPLSN => NULL()
    Z_YDMF_PHYS_OUT_FPFPCL => NULL()
    Z_YDMF_PHYS_OUT_FPLSL => NULL()
    Z_YDMF_PHYS_OUT_FPFPSN => NULL()
    Z_YDMF_PHYS_OUT_DIFCQL => NULL()
    Z_YDMF_PHYS_OUT_FPFPCN => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP => NULL()
    Z_YDMF_PHYS_OUT_FPFPSL => NULL()
    Z_YDMF_PHYS_OUT_DIFTQ => NULL()
    Z_YDMF_PHYS_OUT_DIFCQ => NULL()
    Z_YDMF_PHYS_OUT_FCCQL => NULL()
    Z_YDMF_PHYS_OUT_FCQNG => NULL()
    Z_YDMF_PHYS_OUT_FCSQN => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:QNGCOR:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 386"
  ELSE IF (LPARALLELMETHOD('OPENMPSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:QNGCOR')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:QNGCOR:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 387"
    ZQV => GET_HOST_DATA_RDWR(YL_ZQV)
    ZPFL_FPLCH => GET_HOST_DATA_RDWR(YL_ZPFL_FPLCH)
    ZPFL_FPLSH => GET_HOST_DATA_RDWR(YL_ZPFL_FPLSH)
    ZQI => GET_HOST_DATA_RDWR(YL_ZQI)
    ZSEDIQI => GET_HOST_DATA_RDWR(YL_ZSEDIQI)
    ZFPCOR => GET_HOST_DATA_RDWR(YL_ZFPCOR)
    ZFLX_LOTT_GWV => GET_HOST_DATA_RDWR(YL_ZFLX_LOTT_GWV)
    ZQL => GET_HOST_DATA_RDWR(YL_ZQL)
    ZSEDIQL => GET_HOST_DATA_RDWR(YL_ZSEDIQL)
    ZFPLSN => GET_HOST_DATA_RDWR(YL_ZFPLSN)
    ZFLX_LOTT_GWU => GET_HOST_DATA_RDWR(YL_ZFLX_LOTT_GWU)
    ZFPLSL => GET_HOST_DATA_RDWR(YL_ZFPLSL)
    Z_YDMF_PHYS_OUT_DIFTQN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFTQN)
    Z_YDMF_PHYS_OUT_FPEVPCL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPEVPCL)
    Z_YDMF_PHYS_OUT_DIFTQL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFTQL)
    Z_YDMF_PHYS_OUT_FPEVPSL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPEVPSL)
    Z_YDMF_PHYS_OUT_DIFCQN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFCQN)
    Z_YDMF_PHYS_OUT_FPEVPCN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPEVPCN)
    Z_YDMF_PHYS_OUT_FPLSG => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSG)
    Z_YDMF_PHYS_OUT_FCCQN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FCCQN)
    Z_YDMF_PHYS_OUT_FCSQL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FCSQL)
    Z_YDMF_PHYS_OUT_FPEVPSN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPEVPSN)
    Z_YDMF_PHYS_OUT_FPLSN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSN)
    Z_YDMF_PHYS_OUT_FPFPCL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPFPCL)
    Z_YDMF_PHYS_OUT_FPLSL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSL)
    Z_YDMF_PHYS_OUT_FPFPSN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPFPSN)
    Z_YDMF_PHYS_OUT_DIFCQL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFCQL)
    Z_YDMF_PHYS_OUT_FPFPCN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPFPCN)
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_RDELP)
    Z_YDMF_PHYS_OUT_FPFPSL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPFPSL)
    Z_YDMF_PHYS_OUT_DIFTQ => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFTQ)
    Z_YDMF_PHYS_OUT_DIFCQ => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFCQ)
    Z_YDMF_PHYS_OUT_FCCQL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FCCQL)
    Z_YDMF_PHYS_OUT_FCQNG => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FCQNG)
    Z_YDMF_PHYS_OUT_FCSQN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FCSQN)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:QNGCOR:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 388"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:QNGCOR:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 389"
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK, JLEV )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        DO JLEV=0,YDCPG_OPTS%KFLEVG
          ZFLX_LOTT_GWU(JLON, JLEV, JBLK) = 0.0_JPRB
          ZFLX_LOTT_GWV(JLON, JLEV, JBLK) = 0.0_JPRB
        END DO
        
        IF (YDMODEL%YRML_PHY_MF%YRPHY%LNEBN .or. YDMODEL%YRML_PHY_MF%YRPHY%LRRGUST) THEN
          DO JLEV=YDCPG_OPTS%KTDIA - 1,YDCPG_OPTS%KFLEVG
            ZPFL_FPLCH(JLON, JLEV, JBLK) = ZFPCOR(JLON, JLEV, JBLK)
          END DO
        END IF
        
        IF (YDMODEL%YRML_PHY_MF%YRPHY%LRRGUST) THEN
          DO JLEV=YDCPG_OPTS%KTDIA - 1,YDCPG_OPTS%KFLEVG
            ZPFL_FPLSH(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_FPLSL(JLON, JLEV, JBLK) + Z_YDMF_PHYS_OUT_FPLSN(JLON, JLEV, JBLK)  &
            & + Z_YDMF_PHYS_OUT_FPLSG(JLON, JLEV, JBLK)
          END DO
        END IF
        
        
        ! Update transport fluxes due to sedimentation of clouds.
        
        DO JLEV=YDCPG_OPTS%KTDIA,YDCPG_OPTS%KFLEVG
          Z_YDMF_PHYS_OUT_DIFTQL(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_DIFTQL(JLON, JLEV, JBLK) + ZSEDIQL(JLON, JLEV, JBLK)
          Z_YDMF_PHYS_OUT_DIFTQN(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_DIFTQN(JLON, JLEV, JBLK) + ZSEDIQI(JLON, JLEV, JBLK)
          ZFPLSL(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_DIFTQL(JLON, JLEV, JBLK) + Z_YDMF_PHYS_OUT_FPLSL(JLON, JLEV, JBLK)
          ZFPLSN(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_DIFTQN(JLON, JLEV, JBLK) + Z_YDMF_PHYS_OUT_FPLSN(JLON, JLEV, JBLK)
        END DO
        
        ! Correct negative water contents.
        
        CALL QNGCOR_OPENACC(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF%YRPHY2, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON,  &
        & YDMODEL%YRML_PHY_MF%YRTOPH%NTPLUI, YDCPG_OPTS%KFLEVG, ZQV(:, :, JBLK), ZQL(:, :, JBLK), ZQI(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP(:, :, JBLK), Z_YDMF_PHYS_OUT_DIFCQ(:, :, JBLK),  &
        & Z_YDMF_PHYS_OUT_DIFCQN(:, :, JBLK), Z_YDMF_PHYS_OUT_DIFCQL(:, :, JBLK), Z_YDMF_PHYS_OUT_DIFTQ(:, :, JBLK),  &
        & Z_YDMF_PHYS_OUT_DIFTQN(:, :, JBLK), Z_YDMF_PHYS_OUT_DIFTQL(:, :, JBLK), Z_YDMF_PHYS_OUT_FPEVPSL(:, :, JBLK),  &
        & Z_YDMF_PHYS_OUT_FPEVPSN(:, :, JBLK), Z_YDMF_PHYS_OUT_FPEVPCL(:, :, JBLK), Z_YDMF_PHYS_OUT_FPEVPCN(:, :, JBLK),  &
        & Z_YDMF_PHYS_OUT_FPFPSL(:, :, JBLK), Z_YDMF_PHYS_OUT_FPFPSN(:, :, JBLK), Z_YDMF_PHYS_OUT_FPFPCL(:, :, JBLK),  &
        & Z_YDMF_PHYS_OUT_FPFPCN(:, :, JBLK), Z_YDMF_PHYS_OUT_FCCQL(:, :, JBLK), Z_YDMF_PHYS_OUT_FCCQN(:, :, JBLK),  &
        & Z_YDMF_PHYS_OUT_FCSQL(:, :, JBLK), Z_YDMF_PHYS_OUT_FCSQN(:, :, JBLK), Z_YDMF_PHYS_OUT_FCQNG(:, :, JBLK),  &
        & YDSTACK=YLSTACK)
        
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:QNGCOR:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 390"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:QNGCOR')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:QNGCOR:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 391"
      ZQV => GET_HOST_DATA_RDWR(YL_ZQV)
      ZPFL_FPLCH => GET_HOST_DATA_RDWR(YL_ZPFL_FPLCH)
      ZPFL_FPLSH => GET_HOST_DATA_RDWR(YL_ZPFL_FPLSH)
      ZQI => GET_HOST_DATA_RDWR(YL_ZQI)
      ZSEDIQI => GET_HOST_DATA_RDWR(YL_ZSEDIQI)
      ZFPCOR => GET_HOST_DATA_RDWR(YL_ZFPCOR)
      ZFLX_LOTT_GWV => GET_HOST_DATA_RDWR(YL_ZFLX_LOTT_GWV)
      ZQL => GET_HOST_DATA_RDWR(YL_ZQL)
      ZSEDIQL => GET_HOST_DATA_RDWR(YL_ZSEDIQL)
      ZFPLSN => GET_HOST_DATA_RDWR(YL_ZFPLSN)
      ZFLX_LOTT_GWU => GET_HOST_DATA_RDWR(YL_ZFLX_LOTT_GWU)
      ZFPLSL => GET_HOST_DATA_RDWR(YL_ZFPLSL)
      Z_YDMF_PHYS_OUT_DIFTQN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFTQN)
      Z_YDMF_PHYS_OUT_FPEVPCL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPEVPCL)
      Z_YDMF_PHYS_OUT_DIFTQL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFTQL)
      Z_YDMF_PHYS_OUT_FPEVPSL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPEVPSL)
      Z_YDMF_PHYS_OUT_DIFCQN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFCQN)
      Z_YDMF_PHYS_OUT_FPEVPCN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPEVPCN)
      Z_YDMF_PHYS_OUT_FPLSG => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSG)
      Z_YDMF_PHYS_OUT_FCCQN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FCCQN)
      Z_YDMF_PHYS_OUT_FCSQL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FCSQL)
      Z_YDMF_PHYS_OUT_FPEVPSN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPEVPSN)
      Z_YDMF_PHYS_OUT_FPLSN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSN)
      Z_YDMF_PHYS_OUT_FPFPCL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPFPCL)
      Z_YDMF_PHYS_OUT_FPLSL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSL)
      Z_YDMF_PHYS_OUT_FPFPSN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPFPSN)
      Z_YDMF_PHYS_OUT_DIFCQL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFCQL)
      Z_YDMF_PHYS_OUT_FPFPCN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPFPCN)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_RDELP)
      Z_YDMF_PHYS_OUT_FPFPSL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPFPSL)
      Z_YDMF_PHYS_OUT_DIFTQ => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFTQ)
      Z_YDMF_PHYS_OUT_DIFCQ => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFCQ)
      Z_YDMF_PHYS_OUT_FCCQL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FCCQL)
      Z_YDMF_PHYS_OUT_FCQNG => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FCQNG)
      Z_YDMF_PHYS_OUT_FCSQN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FCSQN)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:QNGCOR:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 392"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:QNGCOR:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 393"
    ZQV => NULL()
    ZPFL_FPLCH => NULL()
    ZPFL_FPLSH => NULL()
    ZQI => NULL()
    ZSEDIQI => NULL()
    ZFPCOR => NULL()
    ZFLX_LOTT_GWV => NULL()
    ZQL => NULL()
    ZSEDIQL => NULL()
    ZFPLSN => NULL()
    ZFLX_LOTT_GWU => NULL()
    ZFPLSL => NULL()
    Z_YDMF_PHYS_OUT_DIFTQN => NULL()
    Z_YDMF_PHYS_OUT_FPEVPCL => NULL()
    Z_YDMF_PHYS_OUT_DIFTQL => NULL()
    Z_YDMF_PHYS_OUT_FPEVPSL => NULL()
    Z_YDMF_PHYS_OUT_DIFCQN => NULL()
    Z_YDMF_PHYS_OUT_FPEVPCN => NULL()
    Z_YDMF_PHYS_OUT_FPLSG => NULL()
    Z_YDMF_PHYS_OUT_FCCQN => NULL()
    Z_YDMF_PHYS_OUT_FCSQL => NULL()
    Z_YDMF_PHYS_OUT_FPEVPSN => NULL()
    Z_YDMF_PHYS_OUT_FPLSN => NULL()
    Z_YDMF_PHYS_OUT_FPFPCL => NULL()
    Z_YDMF_PHYS_OUT_FPLSL => NULL()
    Z_YDMF_PHYS_OUT_FPFPSN => NULL()
    Z_YDMF_PHYS_OUT_DIFCQL => NULL()
    Z_YDMF_PHYS_OUT_FPFPCN => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP => NULL()
    Z_YDMF_PHYS_OUT_FPFPSL => NULL()
    Z_YDMF_PHYS_OUT_DIFTQ => NULL()
    Z_YDMF_PHYS_OUT_DIFCQ => NULL()
    Z_YDMF_PHYS_OUT_FCCQL => NULL()
    Z_YDMF_PHYS_OUT_FCQNG => NULL()
    Z_YDMF_PHYS_OUT_FCSQN => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:QNGCOR:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 394"
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:QNGCOR')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:QNGCOR:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 395"
    ZQV => GET_DEVICE_DATA_RDWR(YL_ZQV)
    ZPFL_FPLCH => GET_DEVICE_DATA_RDWR(YL_ZPFL_FPLCH)
    ZPFL_FPLSH => GET_DEVICE_DATA_RDWR(YL_ZPFL_FPLSH)
    ZQI => GET_DEVICE_DATA_RDWR(YL_ZQI)
    ZSEDIQI => GET_DEVICE_DATA_RDWR(YL_ZSEDIQI)
    ZFPCOR => GET_DEVICE_DATA_RDWR(YL_ZFPCOR)
    ZFLX_LOTT_GWV => GET_DEVICE_DATA_RDWR(YL_ZFLX_LOTT_GWV)
    ZQL => GET_DEVICE_DATA_RDWR(YL_ZQL)
    ZSEDIQL => GET_DEVICE_DATA_RDWR(YL_ZSEDIQL)
    ZFPLSN => GET_DEVICE_DATA_RDWR(YL_ZFPLSN)
    ZFLX_LOTT_GWU => GET_DEVICE_DATA_RDWR(YL_ZFLX_LOTT_GWU)
    ZFPLSL => GET_DEVICE_DATA_RDWR(YL_ZFPLSL)
    Z_YDMF_PHYS_OUT_DIFTQN => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_DIFTQN)
    Z_YDMF_PHYS_OUT_FPEVPCL => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_FPEVPCL)
    Z_YDMF_PHYS_OUT_DIFTQL => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_DIFTQL)
    Z_YDMF_PHYS_OUT_FPEVPSL => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_FPEVPSL)
    Z_YDMF_PHYS_OUT_DIFCQN => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_DIFCQN)
    Z_YDMF_PHYS_OUT_FPEVPCN => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_FPEVPCN)
    Z_YDMF_PHYS_OUT_FPLSG => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSG)
    Z_YDMF_PHYS_OUT_FCCQN => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_FCCQN)
    Z_YDMF_PHYS_OUT_FCSQL => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_FCSQL)
    Z_YDMF_PHYS_OUT_FPEVPSN => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_FPEVPSN)
    Z_YDMF_PHYS_OUT_FPLSN => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSN)
    Z_YDMF_PHYS_OUT_FPFPCL => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_FPFPCL)
    Z_YDMF_PHYS_OUT_FPLSL => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSL)
    Z_YDMF_PHYS_OUT_FPFPSN => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_FPFPSN)
    Z_YDMF_PHYS_OUT_DIFCQL => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_DIFCQL)
    Z_YDMF_PHYS_OUT_FPFPCN => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_FPFPCN)
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_RDELP)
    Z_YDMF_PHYS_OUT_FPFPSL => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_FPFPSL)
    Z_YDMF_PHYS_OUT_DIFTQ => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_DIFTQ)
    Z_YDMF_PHYS_OUT_DIFCQ => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_DIFCQ)
    Z_YDMF_PHYS_OUT_FCCQL => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_FCCQL)
    Z_YDMF_PHYS_OUT_FCQNG => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_FCQNG)
    Z_YDMF_PHYS_OUT_FCSQN => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_FCSQN)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:QNGCOR:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 396"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:QNGCOR:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 397"
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZQV, ZPFL_FPLCH, ZPFL_FPLSH, ZQI, ZSEDIQI, ZFPCOR,  &
!$ACC & ZFLX_LOTT_GWV, ZQL, ZSEDIQL, ZFPLSN, ZFLX_LOTT_GWU, ZFPLSL, Z_YDMF_PHYS_OUT_DIFTQN, Z_YDMF_PHYS_OUT_FPEVPCL,  &
!$ACC & Z_YDMF_PHYS_OUT_DIFTQL, Z_YDMF_PHYS_OUT_FPEVPSL, Z_YDMF_PHYS_OUT_DIFCQN, Z_YDMF_PHYS_OUT_FPEVPCN,  &
!$ACC & Z_YDMF_PHYS_OUT_FPLSG, Z_YDMF_PHYS_OUT_FCCQN, Z_YDMF_PHYS_OUT_FCSQL, Z_YDMF_PHYS_OUT_FPEVPSN, Z_YDMF_PHYS_OUT_FPLSN,  &
!$ACC & Z_YDMF_PHYS_OUT_FPFPCL, Z_YDMF_PHYS_OUT_FPLSL, Z_YDMF_PHYS_OUT_FPFPSN, Z_YDMF_PHYS_OUT_DIFCQL,  &
!$ACC & Z_YDMF_PHYS_OUT_FPFPCN, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP, Z_YDMF_PHYS_OUT_FPFPSL, Z_YDMF_PHYS_OUT_FCCQL,  &
!$ACC & Z_YDMF_PHYS_OUT_FCQNG, Z_YDMF_PHYS_OUT_FCSQN ) PRIVATE( JBLK ) VECTOR_LENGTH( YDCPG_OPTS%KLON )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK, JLEV )
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        DO JLEV=0,YDCPG_OPTS%KFLEVG
          ZFLX_LOTT_GWU(JLON, JLEV, JBLK) = 0.0_JPRB
          ZFLX_LOTT_GWV(JLON, JLEV, JBLK) = 0.0_JPRB
        END DO
        
        IF (YDMODEL%YRML_PHY_MF%YRPHY%LNEBN .or. YDMODEL%YRML_PHY_MF%YRPHY%LRRGUST) THEN
          DO JLEV=YDCPG_OPTS%KTDIA - 1,YDCPG_OPTS%KFLEVG
            ZPFL_FPLCH(JLON, JLEV, JBLK) = ZFPCOR(JLON, JLEV, JBLK)
          END DO
        END IF
        
        IF (YDMODEL%YRML_PHY_MF%YRPHY%LRRGUST) THEN
          DO JLEV=YDCPG_OPTS%KTDIA - 1,YDCPG_OPTS%KFLEVG
            ZPFL_FPLSH(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_FPLSL(JLON, JLEV, JBLK) + Z_YDMF_PHYS_OUT_FPLSN(JLON, JLEV, JBLK)  &
            & + Z_YDMF_PHYS_OUT_FPLSG(JLON, JLEV, JBLK)
          END DO
        END IF
        
        
        ! Update transport fluxes due to sedimentation of clouds.
        
        DO JLEV=YDCPG_OPTS%KTDIA,YDCPG_OPTS%KFLEVG
          Z_YDMF_PHYS_OUT_DIFTQL(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_DIFTQL(JLON, JLEV, JBLK) + ZSEDIQL(JLON, JLEV, JBLK)
          Z_YDMF_PHYS_OUT_DIFTQN(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_DIFTQN(JLON, JLEV, JBLK) + ZSEDIQI(JLON, JLEV, JBLK)
          ZFPLSL(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_DIFTQL(JLON, JLEV, JBLK) + Z_YDMF_PHYS_OUT_FPLSL(JLON, JLEV, JBLK)
          ZFPLSN(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_DIFTQN(JLON, JLEV, JBLK) + Z_YDMF_PHYS_OUT_FPLSN(JLON, JLEV, JBLK)
        END DO
        
        ! Correct negative water contents.
        
        CALL QNGCOR_OPENACC(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF%YRPHY2, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON,  &
        & YDMODEL%YRML_PHY_MF%YRTOPH%NTPLUI, YDCPG_OPTS%KFLEVG, ZQV(:, :, JBLK), ZQL(:, :, JBLK), ZQI(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP(:, :, JBLK), Z_YDMF_PHYS_OUT_DIFCQ(:, :, JBLK),  &
        & Z_YDMF_PHYS_OUT_DIFCQN(:, :, JBLK), Z_YDMF_PHYS_OUT_DIFCQL(:, :, JBLK), Z_YDMF_PHYS_OUT_DIFTQ(:, :, JBLK),  &
        & Z_YDMF_PHYS_OUT_DIFTQN(:, :, JBLK), Z_YDMF_PHYS_OUT_DIFTQL(:, :, JBLK), Z_YDMF_PHYS_OUT_FPEVPSL(:, :, JBLK),  &
        & Z_YDMF_PHYS_OUT_FPEVPSN(:, :, JBLK), Z_YDMF_PHYS_OUT_FPEVPCL(:, :, JBLK), Z_YDMF_PHYS_OUT_FPEVPCN(:, :, JBLK),  &
        & Z_YDMF_PHYS_OUT_FPFPSL(:, :, JBLK), Z_YDMF_PHYS_OUT_FPFPSN(:, :, JBLK), Z_YDMF_PHYS_OUT_FPFPCL(:, :, JBLK),  &
        & Z_YDMF_PHYS_OUT_FPFPCN(:, :, JBLK), Z_YDMF_PHYS_OUT_FCCQL(:, :, JBLK), Z_YDMF_PHYS_OUT_FCCQN(:, :, JBLK),  &
        & Z_YDMF_PHYS_OUT_FCSQL(:, :, JBLK), Z_YDMF_PHYS_OUT_FCSQN(:, :, JBLK), Z_YDMF_PHYS_OUT_FCQNG(:, :, JBLK),  &
        & YDSTACK=YLSTACK)
        
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:QNGCOR:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 398"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:QNGCOR')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:QNGCOR:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 399"
      ZQV => GET_HOST_DATA_RDWR(YL_ZQV)
      ZPFL_FPLCH => GET_HOST_DATA_RDWR(YL_ZPFL_FPLCH)
      ZPFL_FPLSH => GET_HOST_DATA_RDWR(YL_ZPFL_FPLSH)
      ZQI => GET_HOST_DATA_RDWR(YL_ZQI)
      ZSEDIQI => GET_HOST_DATA_RDWR(YL_ZSEDIQI)
      ZFPCOR => GET_HOST_DATA_RDWR(YL_ZFPCOR)
      ZFLX_LOTT_GWV => GET_HOST_DATA_RDWR(YL_ZFLX_LOTT_GWV)
      ZQL => GET_HOST_DATA_RDWR(YL_ZQL)
      ZSEDIQL => GET_HOST_DATA_RDWR(YL_ZSEDIQL)
      ZFPLSN => GET_HOST_DATA_RDWR(YL_ZFPLSN)
      ZFLX_LOTT_GWU => GET_HOST_DATA_RDWR(YL_ZFLX_LOTT_GWU)
      ZFPLSL => GET_HOST_DATA_RDWR(YL_ZFPLSL)
      Z_YDMF_PHYS_OUT_DIFTQN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFTQN)
      Z_YDMF_PHYS_OUT_FPEVPCL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPEVPCL)
      Z_YDMF_PHYS_OUT_DIFTQL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFTQL)
      Z_YDMF_PHYS_OUT_FPEVPSL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPEVPSL)
      Z_YDMF_PHYS_OUT_DIFCQN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFCQN)
      Z_YDMF_PHYS_OUT_FPEVPCN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPEVPCN)
      Z_YDMF_PHYS_OUT_FPLSG => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSG)
      Z_YDMF_PHYS_OUT_FCCQN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FCCQN)
      Z_YDMF_PHYS_OUT_FCSQL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FCSQL)
      Z_YDMF_PHYS_OUT_FPEVPSN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPEVPSN)
      Z_YDMF_PHYS_OUT_FPLSN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSN)
      Z_YDMF_PHYS_OUT_FPFPCL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPFPCL)
      Z_YDMF_PHYS_OUT_FPLSL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSL)
      Z_YDMF_PHYS_OUT_FPFPSN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPFPSN)
      Z_YDMF_PHYS_OUT_DIFCQL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFCQL)
      Z_YDMF_PHYS_OUT_FPFPCN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPFPCN)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_RDELP)
      Z_YDMF_PHYS_OUT_FPFPSL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPFPSL)
      Z_YDMF_PHYS_OUT_DIFTQ => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFTQ)
      Z_YDMF_PHYS_OUT_DIFCQ => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_DIFCQ)
      Z_YDMF_PHYS_OUT_FCCQL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FCCQL)
      Z_YDMF_PHYS_OUT_FCQNG => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FCQNG)
      Z_YDMF_PHYS_OUT_FCSQN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FCSQN)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:QNGCOR:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 400"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:QNGCOR:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 401"
    ZQV => NULL()
    ZPFL_FPLCH => NULL()
    ZPFL_FPLSH => NULL()
    ZQI => NULL()
    ZSEDIQI => NULL()
    ZFPCOR => NULL()
    ZFLX_LOTT_GWV => NULL()
    ZQL => NULL()
    ZSEDIQL => NULL()
    ZFPLSN => NULL()
    ZFLX_LOTT_GWU => NULL()
    ZFPLSL => NULL()
    Z_YDMF_PHYS_OUT_DIFTQN => NULL()
    Z_YDMF_PHYS_OUT_FPEVPCL => NULL()
    Z_YDMF_PHYS_OUT_DIFTQL => NULL()
    Z_YDMF_PHYS_OUT_FPEVPSL => NULL()
    Z_YDMF_PHYS_OUT_DIFCQN => NULL()
    Z_YDMF_PHYS_OUT_FPEVPCN => NULL()
    Z_YDMF_PHYS_OUT_FPLSG => NULL()
    Z_YDMF_PHYS_OUT_FCCQN => NULL()
    Z_YDMF_PHYS_OUT_FCSQL => NULL()
    Z_YDMF_PHYS_OUT_FPEVPSN => NULL()
    Z_YDMF_PHYS_OUT_FPLSN => NULL()
    Z_YDMF_PHYS_OUT_FPFPCL => NULL()
    Z_YDMF_PHYS_OUT_FPLSL => NULL()
    Z_YDMF_PHYS_OUT_FPFPSN => NULL()
    Z_YDMF_PHYS_OUT_DIFCQL => NULL()
    Z_YDMF_PHYS_OUT_FPFPCN => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP => NULL()
    Z_YDMF_PHYS_OUT_FPFPSL => NULL()
    Z_YDMF_PHYS_OUT_DIFTQ => NULL()
    Z_YDMF_PHYS_OUT_DIFCQ => NULL()
    Z_YDMF_PHYS_OUT_FCCQL => NULL()
    Z_YDMF_PHYS_OUT_FCQNG => NULL()
    Z_YDMF_PHYS_OUT_FCSQN => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:QNGCOR:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 402"
  END IF
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:QNGCOR', 1, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 403"
  
  ! Bilan hydrique du sol
  
  CALL APL_ARPEGE_HYDRO_BUDGET_PARALLEL(YDMF_PHYS_BASE_STATE, YDCPG_BNDS, YDCPG_OPTS, YDCPG_GPAR, YDMF_PHYS, YDMF_PHYS_SURF,  &
  & YDMODEL, YL_ZC3, YL_ZCN, YL_ZDSA_C1, YL_ZDSA_C2, YL_ZFLU_FEVI, YL_ZFLU_NEIJ, YL_ZFLU_VEG, YL_ZFPLSL, YL_ZFPLSN, YL_ZWFC,  &
  & YL_ZWLMX, YL_ZWPMX, YL_ZWSEQ, YL_ZWSMX)
  
  ! Drag mesospherique pour un modele possedant des niveaux au-dessus de 50 km (i.e. dans la mesosphere)
  
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRME', 0, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 404"
  IF (LPARALLELMETHOD('OPENMP', 'APL_ARPEGE_PARALLEL:ACDRME')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRME:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 405"
    ZQV => GET_HOST_DATA_RDWR(YL_ZQV)
    ZMSC_FRMQ => GET_HOST_DATA_RDWR(YL_ZMSC_FRMQ)
    Z_YDVARS_A_T1 => GET_HOST_DATA_RDWR(YDVARS%A%FT1)
    Z_YDMF_PHYS_OUT_FRMH => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FRMH)
    Z_YDMF_PHYS_OUT_STRMU => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRMU)
    Z_YDMF_PHYS_OUT_STRMV => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRMV)
    Z_YDCPG_MISC_QLI => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QLI)
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP)
    Z_YDMF_PHYS_BASE_STATE_U => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
    Z_YDVARS_LRAD_T1 => GET_HOST_DATA_RDWR(YDVARS%LRAD%FT1)
    Z_YDVARS_IRAD_T1 => GET_HOST_DATA_RDWR(YDVARS%IRAD%FT1)
    Z_YDCPG_MISC_QICE => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QICE)
    Z_YDMF_PHYS_BASE_STATE_V => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
    Z_YDCPG_MISC_NEB => GET_HOST_DATA_RDWR(YDCPG_MISC%F_NEB)
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_DELP)
    Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRME:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 406"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRME:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 407"
    CALL YLCPG_BNDS%INIT(YDCPG_OPTS)
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, JLEV ) FIRSTPRIVATE( YLCPG_BNDS )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      CALL YLCPG_BNDS%UPDATE(JBLK)
      
      CALL ACDRME(YDMODEL%YRCST, YDGEOMETRY%YRSTA, YDMODEL%YRML_PHY_MF%YRPHY2, YDMODEL%YRML_PHY_MF%YRTOPH, YLCPG_BNDS%KIDIA,  &
      & YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDMODEL%YRML_PHY_MF%YRTOPH%NTDRME, YDCPG_OPTS%KFLEVG,  &
      & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP(:, :, JBLK),  &
      & Z_YDMF_PHYS_BASE_STATE_T(:, :, JBLK), ZQV(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_U(:, :, JBLK),  &
      & Z_YDMF_PHYS_BASE_STATE_V(:, :, JBLK), Z_YDMF_PHYS_OUT_FRMH(:, :, JBLK), ZMSC_FRMQ(:, :, JBLK),  &
      & Z_YDMF_PHYS_OUT_STRMU(:, :, JBLK), Z_YDMF_PHYS_OUT_STRMV(:, :, JBLK))
      
      ! Store radiative cloudiness in GFL structure for ISP, Historical files or PostProcessing
      
      IF (YDMODEL%YRML_GCONF%YGFL%YIRAD%LGP) THEN
        DO JLEV=1,YDCPG_OPTS%KFLEVG
          DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
            Z_YDVARS_IRAD_T1(JLON, JLEV, JBLK) = Z_YDCPG_MISC_QICE(JLON, JLEV, JBLK)
          END DO
        END DO
      END IF
      
      IF (YDMODEL%YRML_GCONF%YGFL%YLRAD%LGP) THEN
        DO JLEV=1,YDCPG_OPTS%KFLEVG
          DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
            Z_YDVARS_LRAD_T1(JLON, JLEV, JBLK) = Z_YDCPG_MISC_QLI(JLON, JLEV, JBLK)
          END DO
        END DO
      END IF
      
      IF (YDMODEL%YRML_GCONF%YGFL%YA%LGP) THEN
        DO JLEV=1,YDCPG_OPTS%KFLEVG
          DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
            Z_YDVARS_A_T1(JLON, JLEV, JBLK) = Z_YDCPG_MISC_NEB(JLON, JLEV, JBLK)
          END DO
        END DO
      END IF
      
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRME:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 408"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ACDRME')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRME:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 409"
      ZQV => GET_HOST_DATA_RDWR(YL_ZQV)
      ZMSC_FRMQ => GET_HOST_DATA_RDWR(YL_ZMSC_FRMQ)
      Z_YDVARS_A_T1 => GET_HOST_DATA_RDWR(YDVARS%A%FT1)
      Z_YDMF_PHYS_OUT_FRMH => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FRMH)
      Z_YDMF_PHYS_OUT_STRMU => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRMU)
      Z_YDMF_PHYS_OUT_STRMV => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRMV)
      Z_YDCPG_MISC_QLI => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QLI)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP)
      Z_YDMF_PHYS_BASE_STATE_U => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
      Z_YDVARS_LRAD_T1 => GET_HOST_DATA_RDWR(YDVARS%LRAD%FT1)
      Z_YDVARS_IRAD_T1 => GET_HOST_DATA_RDWR(YDVARS%IRAD%FT1)
      Z_YDCPG_MISC_QICE => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QICE)
      Z_YDMF_PHYS_BASE_STATE_V => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
      Z_YDCPG_MISC_NEB => GET_HOST_DATA_RDWR(YDCPG_MISC%F_NEB)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_DELP)
      Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRME:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 410"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRME:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 411"
    ZQV => NULL()
    ZMSC_FRMQ => NULL()
    Z_YDVARS_A_T1 => NULL()
    Z_YDMF_PHYS_OUT_FRMH => NULL()
    Z_YDMF_PHYS_OUT_STRMU => NULL()
    Z_YDMF_PHYS_OUT_STRMV => NULL()
    Z_YDCPG_MISC_QLI => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => NULL()
    Z_YDMF_PHYS_BASE_STATE_U => NULL()
    Z_YDVARS_LRAD_T1 => NULL()
    Z_YDVARS_IRAD_T1 => NULL()
    Z_YDCPG_MISC_QICE => NULL()
    Z_YDMF_PHYS_BASE_STATE_V => NULL()
    Z_YDCPG_MISC_NEB => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP => NULL()
    Z_YDMF_PHYS_BASE_STATE_T => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRME:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 412"
  ELSE IF (LPARALLELMETHOD('OPENMPSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ACDRME')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRME:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 413"
    ZQV => GET_HOST_DATA_RDWR(YL_ZQV)
    ZMSC_FRMQ => GET_HOST_DATA_RDWR(YL_ZMSC_FRMQ)
    Z_YDVARS_A_T1 => GET_HOST_DATA_RDWR(YDVARS%A%FT1)
    Z_YDMF_PHYS_OUT_FRMH => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FRMH)
    Z_YDMF_PHYS_OUT_STRMU => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRMU)
    Z_YDMF_PHYS_OUT_STRMV => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRMV)
    Z_YDCPG_MISC_QLI => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QLI)
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP)
    Z_YDMF_PHYS_BASE_STATE_U => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
    Z_YDVARS_LRAD_T1 => GET_HOST_DATA_RDWR(YDVARS%LRAD%FT1)
    Z_YDVARS_IRAD_T1 => GET_HOST_DATA_RDWR(YDVARS%IRAD%FT1)
    Z_YDCPG_MISC_QICE => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QICE)
    Z_YDMF_PHYS_BASE_STATE_V => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
    Z_YDCPG_MISC_NEB => GET_HOST_DATA_RDWR(YDCPG_MISC%F_NEB)
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_DELP)
    Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRME:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 414"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRME:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 415"
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK, JLEV )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        CALL ACDRME_OPENACC(YDMODEL%YRCST, YDGEOMETRY%YRSTA, YDMODEL%YRML_PHY_MF%YRPHY2, YDMODEL%YRML_PHY_MF%YRTOPH,  &
        & YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDMODEL%YRML_PHY_MF%YRTOPH%NTDRME, YDCPG_OPTS%KFLEVG,  &
        & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_T(:, :, JBLK), ZQV(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_U(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_V(:, :, JBLK), Z_YDMF_PHYS_OUT_FRMH(:, :, JBLK), ZMSC_FRMQ(:, :, JBLK),  &
        & Z_YDMF_PHYS_OUT_STRMU(:, :, JBLK), Z_YDMF_PHYS_OUT_STRMV(:, :, JBLK), YDSTACK=YLSTACK)
        
        ! Store radiative cloudiness in GFL structure for ISP, Historical files or PostProcessing
        
        IF (YDMODEL%YRML_GCONF%YGFL%YIRAD%LGP) THEN
          DO JLEV=1,YDCPG_OPTS%KFLEVG
            Z_YDVARS_IRAD_T1(JLON, JLEV, JBLK) = Z_YDCPG_MISC_QICE(JLON, JLEV, JBLK)
          END DO
        END IF
        
        IF (YDMODEL%YRML_GCONF%YGFL%YLRAD%LGP) THEN
          DO JLEV=1,YDCPG_OPTS%KFLEVG
            Z_YDVARS_LRAD_T1(JLON, JLEV, JBLK) = Z_YDCPG_MISC_QLI(JLON, JLEV, JBLK)
          END DO
        END IF
        
        IF (YDMODEL%YRML_GCONF%YGFL%YA%LGP) THEN
          DO JLEV=1,YDCPG_OPTS%KFLEVG
            Z_YDVARS_A_T1(JLON, JLEV, JBLK) = Z_YDCPG_MISC_NEB(JLON, JLEV, JBLK)
          END DO
        END IF
        
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRME:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 416"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ACDRME')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRME:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 417"
      ZQV => GET_HOST_DATA_RDWR(YL_ZQV)
      ZMSC_FRMQ => GET_HOST_DATA_RDWR(YL_ZMSC_FRMQ)
      Z_YDVARS_A_T1 => GET_HOST_DATA_RDWR(YDVARS%A%FT1)
      Z_YDMF_PHYS_OUT_FRMH => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FRMH)
      Z_YDMF_PHYS_OUT_STRMU => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRMU)
      Z_YDMF_PHYS_OUT_STRMV => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRMV)
      Z_YDCPG_MISC_QLI => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QLI)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP)
      Z_YDMF_PHYS_BASE_STATE_U => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
      Z_YDVARS_LRAD_T1 => GET_HOST_DATA_RDWR(YDVARS%LRAD%FT1)
      Z_YDVARS_IRAD_T1 => GET_HOST_DATA_RDWR(YDVARS%IRAD%FT1)
      Z_YDCPG_MISC_QICE => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QICE)
      Z_YDMF_PHYS_BASE_STATE_V => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
      Z_YDCPG_MISC_NEB => GET_HOST_DATA_RDWR(YDCPG_MISC%F_NEB)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_DELP)
      Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRME:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 418"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRME:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 419"
    ZQV => NULL()
    ZMSC_FRMQ => NULL()
    Z_YDVARS_A_T1 => NULL()
    Z_YDMF_PHYS_OUT_FRMH => NULL()
    Z_YDMF_PHYS_OUT_STRMU => NULL()
    Z_YDMF_PHYS_OUT_STRMV => NULL()
    Z_YDCPG_MISC_QLI => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => NULL()
    Z_YDMF_PHYS_BASE_STATE_U => NULL()
    Z_YDVARS_LRAD_T1 => NULL()
    Z_YDVARS_IRAD_T1 => NULL()
    Z_YDCPG_MISC_QICE => NULL()
    Z_YDMF_PHYS_BASE_STATE_V => NULL()
    Z_YDCPG_MISC_NEB => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP => NULL()
    Z_YDMF_PHYS_BASE_STATE_T => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRME:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 420"
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ACDRME')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRME:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 421"
    ZQV => GET_DEVICE_DATA_RDWR(YL_ZQV)
    ZMSC_FRMQ => GET_DEVICE_DATA_RDWR(YL_ZMSC_FRMQ)
    Z_YDVARS_A_T1 => GET_DEVICE_DATA_RDWR(YDVARS%A%FT1)
    Z_YDMF_PHYS_OUT_FRMH => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_FRMH)
    Z_YDMF_PHYS_OUT_STRMU => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_STRMU)
    Z_YDMF_PHYS_OUT_STRMV => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_STRMV)
    Z_YDCPG_MISC_QLI => GET_DEVICE_DATA_RDWR(YDCPG_MISC%F_QLI)
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP)
    Z_YDMF_PHYS_BASE_STATE_U => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
    Z_YDVARS_LRAD_T1 => GET_DEVICE_DATA_RDWR(YDVARS%LRAD%FT1)
    Z_YDVARS_IRAD_T1 => GET_DEVICE_DATA_RDWR(YDVARS%IRAD%FT1)
    Z_YDCPG_MISC_QICE => GET_DEVICE_DATA_RDWR(YDCPG_MISC%F_QICE)
    Z_YDMF_PHYS_BASE_STATE_V => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
    Z_YDCPG_MISC_NEB => GET_DEVICE_DATA_RDWR(YDCPG_MISC%F_NEB)
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_DELP)
    Z_YDMF_PHYS_BASE_STATE_T => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRME:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 422"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRME:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 423"
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, YDGEOMETRY, ZQV, ZMSC_FRMQ, Z_YDVARS_A_T1,  &
!$ACC & Z_YDMF_PHYS_OUT_FRMH, Z_YDMF_PHYS_OUT_STRMU, Z_YDMF_PHYS_OUT_STRMV, Z_YDCPG_MISC_QLI,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP, Z_YDMF_PHYS_BASE_STATE_U, Z_YDVARS_LRAD_T1, Z_YDVARS_IRAD_T1,  &
!$ACC & Z_YDCPG_MISC_QICE, Z_YDMF_PHYS_BASE_STATE_V, Z_YDCPG_MISC_NEB, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_T ) PRIVATE( JBLK ) VECTOR_LENGTH( YDCPG_OPTS%KLON )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK, JLEV )
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        CALL ACDRME_OPENACC(YDMODEL%YRCST, YDGEOMETRY%YRSTA, YDMODEL%YRML_PHY_MF%YRPHY2, YDMODEL%YRML_PHY_MF%YRTOPH,  &
        & YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDMODEL%YRML_PHY_MF%YRTOPH%NTDRME, YDCPG_OPTS%KFLEVG,  &
        & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_T(:, :, JBLK), ZQV(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_U(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_V(:, :, JBLK), Z_YDMF_PHYS_OUT_FRMH(:, :, JBLK), ZMSC_FRMQ(:, :, JBLK),  &
        & Z_YDMF_PHYS_OUT_STRMU(:, :, JBLK), Z_YDMF_PHYS_OUT_STRMV(:, :, JBLK), YDSTACK=YLSTACK)
        
        ! Store radiative cloudiness in GFL structure for ISP, Historical files or PostProcessing
        
        IF (YDMODEL%YRML_GCONF%YGFL%YIRAD%LGP) THEN
          DO JLEV=1,YDCPG_OPTS%KFLEVG
            Z_YDVARS_IRAD_T1(JLON, JLEV, JBLK) = Z_YDCPG_MISC_QICE(JLON, JLEV, JBLK)
          END DO
        END IF
        
        IF (YDMODEL%YRML_GCONF%YGFL%YLRAD%LGP) THEN
          DO JLEV=1,YDCPG_OPTS%KFLEVG
            Z_YDVARS_LRAD_T1(JLON, JLEV, JBLK) = Z_YDCPG_MISC_QLI(JLON, JLEV, JBLK)
          END DO
        END IF
        
        IF (YDMODEL%YRML_GCONF%YGFL%YA%LGP) THEN
          DO JLEV=1,YDCPG_OPTS%KFLEVG
            Z_YDVARS_A_T1(JLON, JLEV, JBLK) = Z_YDCPG_MISC_NEB(JLON, JLEV, JBLK)
          END DO
        END IF
        
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRME:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 424"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ACDRME')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRME:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 425"
      ZQV => GET_HOST_DATA_RDWR(YL_ZQV)
      ZMSC_FRMQ => GET_HOST_DATA_RDWR(YL_ZMSC_FRMQ)
      Z_YDVARS_A_T1 => GET_HOST_DATA_RDWR(YDVARS%A%FT1)
      Z_YDMF_PHYS_OUT_FRMH => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FRMH)
      Z_YDMF_PHYS_OUT_STRMU => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRMU)
      Z_YDMF_PHYS_OUT_STRMV => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_STRMV)
      Z_YDCPG_MISC_QLI => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QLI)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP)
      Z_YDMF_PHYS_BASE_STATE_U => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
      Z_YDVARS_LRAD_T1 => GET_HOST_DATA_RDWR(YDVARS%LRAD%FT1)
      Z_YDVARS_IRAD_T1 => GET_HOST_DATA_RDWR(YDVARS%IRAD%FT1)
      Z_YDCPG_MISC_QICE => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QICE)
      Z_YDMF_PHYS_BASE_STATE_V => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
      Z_YDCPG_MISC_NEB => GET_HOST_DATA_RDWR(YDCPG_MISC%F_NEB)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_DELP)
      Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRME:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 426"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRME:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 427"
    ZQV => NULL()
    ZMSC_FRMQ => NULL()
    Z_YDVARS_A_T1 => NULL()
    Z_YDMF_PHYS_OUT_FRMH => NULL()
    Z_YDMF_PHYS_OUT_STRMU => NULL()
    Z_YDMF_PHYS_OUT_STRMV => NULL()
    Z_YDCPG_MISC_QLI => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => NULL()
    Z_YDMF_PHYS_BASE_STATE_U => NULL()
    Z_YDVARS_LRAD_T1 => NULL()
    Z_YDVARS_IRAD_T1 => NULL()
    Z_YDCPG_MISC_QICE => NULL()
    Z_YDMF_PHYS_BASE_STATE_V => NULL()
    Z_YDCPG_MISC_NEB => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP => NULL()
    Z_YDMF_PHYS_BASE_STATE_T => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRME:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 428"
  END IF
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRME', 1, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 429"
  
  ! DDH flexibles pour les champs de surface variables/flux/tendances.
  
  
  
  IF (YDMODEL%YRML_PHY_MF%YRPHY2%LRAFTKE) THEN
    
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACEVADCAPE', 0, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 430"
    IF (LPARALLELMETHOD('OPENMP', 'APL_ARPEGE_PARALLEL:ACEVADCAPE')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACEVADCAPE:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 431"
      ZDCAPE => GET_HOST_DATA_RDWR(YL_ZDCAPE)
      ZBLH => GET_HOST_DATA_RDWR(YL_ZBLH)
      ZTKE1 => GET_HOST_DATA_RDWR(YL_ZTKE1)
      IMOC_CLPH => GET_HOST_DATA_RDWR(YL_IMOC_CLPH)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
      Z_YDMF_PHYS_OUT_UGST => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_UGST)
      Z_YDMF_PHYS_OUT_FPLCL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLCL)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
      Z_YDMF_PHYS_OUT_UCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_UCLS)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP)
      Z_YDMF_PHYS_OUT_CAPE => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_CAPE)
      Z_YDMF_PHYS_OUT_CLPH => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_CLPH)
      Z_YDMF_PHYS_BASE_STATE_U => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
      Z_YDMF_PHYS_OUT_FPLCN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLCN)
      Z_YDMF_PHYS_BASE_STATE_V => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
      Z_YDMF_PHYS_OUT_FPLSL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSL)
      Z_YDMF_PHYS_OUT_VCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_VCLS)
      Z_YDMF_PHYS_OUT_VGST => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_VGST)
      Z_YDVARS_GEOMETRY_OROG_T0 => GET_HOST_DATA_RDWR(YDVARS%GEOMETRY%OROG%FT0)
      Z_YDMF_PHYS_OUT_FPLSN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSN)
      Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACEVADCAPE:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 432"
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACEVADCAPE:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 433"
      CALL YLCPG_BNDS%INIT(YDCPG_OPTS)
!$OMP PARALLEL DO PRIVATE( JBLK, JLON ) FIRSTPRIVATE( YLCPG_BNDS )
      DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
        CALL YLCPG_BNDS%UPDATE(JBLK)
        
        ! DCAPE due to precipitation evaporation.
        
        CALL ACEVADCAPE(YDMODEL%YRML_PHY_MF%YRPHY2, YDMODEL%YRCST, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON,  &
        & YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_OUT_FPLSL(:, :, JBLK), Z_YDMF_PHYS_OUT_FPLSN(:, :, JBLK),  &
        & Z_YDMF_PHYS_OUT_FPLCL(:, :, JBLK), Z_YDMF_PHYS_OUT_FPLCN(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_T(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI(:, :, JBLK), ZDCAPE(:, JBLK))
        
        ! Gusts.
        
        CALL ACCLDIA(YDMODEL%YRCST, YDCPG_OPTS%LXCLP, YDCPG_OPTS%LXTGST, YDCPG_OPTS%LXXGST, YDMODEL%YRML_PHY_MF%YRPHY,  &
        & YDMODEL%YRML_PHY_MF%YRPHY2, YDMODEL%YRML_PHY_MF%YRTOPH, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON,  &
        & YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_OUT_UCLS(:, JBLK), Z_YDMF_PHYS_OUT_VCLS(:, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_U(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_V(:, :, JBLK), Z_YDMF_PHYS_OUT_CAPE(:, JBLK),  &
        & ZDCAPE(:, JBLK), ZTKE1(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF(:, :, JBLK),  &
        & Z_YDVARS_GEOMETRY_OROG_T0(:, JBLK), Z_YDMF_PHYS_OUT_UGST(:, JBLK), Z_YDMF_PHYS_OUT_VGST(:, JBLK), ZBLH(:, JBLK),  &
        & IMOC_CLPH(:, JBLK))
        
        DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
          Z_YDMF_PHYS_OUT_CLPH(JLON, JBLK) =  &
          & MIN(YDMODEL%YRML_PHY_MF%YRPHY0%XMAXLM, MAX(YDMODEL%YRML_PHY_MF%YRPHY0%XMINLM, ZBLH(JLON, JBLK)))
        END DO
        
      END DO
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACEVADCAPE:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 434"
      IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ACEVADCAPE')) THEN
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACEVADCAPE:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 435"
        ZDCAPE => GET_HOST_DATA_RDWR(YL_ZDCAPE)
        ZBLH => GET_HOST_DATA_RDWR(YL_ZBLH)
        ZTKE1 => GET_HOST_DATA_RDWR(YL_ZTKE1)
        IMOC_CLPH => GET_HOST_DATA_RDWR(YL_IMOC_CLPH)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
        Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
        Z_YDMF_PHYS_OUT_UGST => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_UGST)
        Z_YDMF_PHYS_OUT_FPLCL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLCL)
        Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
        Z_YDMF_PHYS_OUT_UCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_UCLS)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP)
        Z_YDMF_PHYS_OUT_CAPE => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_CAPE)
        Z_YDMF_PHYS_OUT_CLPH => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_CLPH)
        Z_YDMF_PHYS_BASE_STATE_U => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
        Z_YDMF_PHYS_OUT_FPLCN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLCN)
        Z_YDMF_PHYS_BASE_STATE_V => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
        Z_YDMF_PHYS_OUT_FPLSL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSL)
        Z_YDMF_PHYS_OUT_VCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_VCLS)
        Z_YDMF_PHYS_OUT_VGST => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_VGST)
        Z_YDVARS_GEOMETRY_OROG_T0 => GET_HOST_DATA_RDWR(YDVARS%GEOMETRY%OROG%FT0)
        Z_YDMF_PHYS_OUT_FPLSN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSN)
        Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACEVADCAPE:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 436"
      END IF
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACEVADCAPE:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 437"
      ZDCAPE => NULL()
      ZBLH => NULL()
      ZTKE1 => NULL()
      IMOC_CLPH => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => NULL()
      Z_YDMF_PHYS_OUT_UGST => NULL()
      Z_YDMF_PHYS_OUT_FPLCL => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => NULL()
      Z_YDMF_PHYS_OUT_UCLS => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => NULL()
      Z_YDMF_PHYS_OUT_CAPE => NULL()
      Z_YDMF_PHYS_OUT_CLPH => NULL()
      Z_YDMF_PHYS_BASE_STATE_U => NULL()
      Z_YDMF_PHYS_OUT_FPLCN => NULL()
      Z_YDMF_PHYS_BASE_STATE_V => NULL()
      Z_YDMF_PHYS_OUT_FPLSL => NULL()
      Z_YDMF_PHYS_OUT_VCLS => NULL()
      Z_YDMF_PHYS_OUT_VGST => NULL()
      Z_YDVARS_GEOMETRY_OROG_T0 => NULL()
      Z_YDMF_PHYS_OUT_FPLSN => NULL()
      Z_YDMF_PHYS_BASE_STATE_T => NULL()
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACEVADCAPE:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 438"
    ELSE IF (LPARALLELMETHOD('OPENMPSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ACEVADCAPE')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACEVADCAPE:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 439"
      ZDCAPE => GET_HOST_DATA_RDWR(YL_ZDCAPE)
      ZBLH => GET_HOST_DATA_RDWR(YL_ZBLH)
      ZTKE1 => GET_HOST_DATA_RDWR(YL_ZTKE1)
      IMOC_CLPH => GET_HOST_DATA_RDWR(YL_IMOC_CLPH)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
      Z_YDMF_PHYS_OUT_UGST => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_UGST)
      Z_YDMF_PHYS_OUT_FPLCL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLCL)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
      Z_YDMF_PHYS_OUT_UCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_UCLS)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP)
      Z_YDMF_PHYS_OUT_CAPE => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_CAPE)
      Z_YDMF_PHYS_OUT_CLPH => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_CLPH)
      Z_YDMF_PHYS_BASE_STATE_U => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
      Z_YDMF_PHYS_OUT_FPLCN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLCN)
      Z_YDMF_PHYS_BASE_STATE_V => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
      Z_YDMF_PHYS_OUT_FPLSL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSL)
      Z_YDMF_PHYS_OUT_VCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_VCLS)
      Z_YDMF_PHYS_OUT_VGST => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_VGST)
      Z_YDVARS_GEOMETRY_OROG_T0 => GET_HOST_DATA_RDWR(YDVARS%GEOMETRY%OROG%FT0)
      Z_YDMF_PHYS_OUT_FPLSN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSN)
      Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACEVADCAPE:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 440"
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACEVADCAPE:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 441"
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
      DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
        DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
          YLCPG_BNDS%KIDIA = JLON
          YLCPG_BNDS%KFDIA = JLON
          YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          
          ! DCAPE due to precipitation evaporation.
          
          CALL ACEVADCAPE_OPENACC(YDMODEL%YRML_PHY_MF%YRPHY2, YDMODEL%YRCST, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA,  &
          & YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_OUT_FPLSL(:, :, JBLK), Z_YDMF_PHYS_OUT_FPLSN(:, :, JBLK),  &
          & Z_YDMF_PHYS_OUT_FPLCL(:, :, JBLK), Z_YDMF_PHYS_OUT_FPLCN(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_T(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI(:, :, JBLK),  &
          & ZDCAPE(:, JBLK), YDSTACK=YLSTACK)
          
          ! Gusts.
          
          CALL ACCLDIA_OPENACC(YDMODEL%YRCST, YDCPG_OPTS%LXCLP, YDCPG_OPTS%LXTGST, YDCPG_OPTS%LXXGST,  &
          & YDMODEL%YRML_PHY_MF%YRPHY, YDMODEL%YRML_PHY_MF%YRPHY2, YDMODEL%YRML_PHY_MF%YRTOPH, YLCPG_BNDS%KIDIA,  &
          & YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_OUT_UCLS(:, JBLK),  &
          & Z_YDMF_PHYS_OUT_VCLS(:, JBLK), Z_YDMF_PHYS_BASE_STATE_U(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_V(:, :, JBLK),  &
          & Z_YDMF_PHYS_OUT_CAPE(:, JBLK), ZDCAPE(:, JBLK), ZTKE1(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF(:, :, JBLK), Z_YDVARS_GEOMETRY_OROG_T0(:, JBLK),  &
          & Z_YDMF_PHYS_OUT_UGST(:, JBLK), Z_YDMF_PHYS_OUT_VGST(:, JBLK), ZBLH(:, JBLK), IMOC_CLPH(:, JBLK), YDSTACK=YLSTACK)
          
          Z_YDMF_PHYS_OUT_CLPH(JLON, JBLK) =  &
          & MIN(YDMODEL%YRML_PHY_MF%YRPHY0%XMAXLM, MAX(YDMODEL%YRML_PHY_MF%YRPHY0%XMINLM, ZBLH(JLON, JBLK)))
          
        END DO
      END DO
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACEVADCAPE:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 442"
      IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ACEVADCAPE')) THEN
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACEVADCAPE:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 443"
        ZDCAPE => GET_HOST_DATA_RDWR(YL_ZDCAPE)
        ZBLH => GET_HOST_DATA_RDWR(YL_ZBLH)
        ZTKE1 => GET_HOST_DATA_RDWR(YL_ZTKE1)
        IMOC_CLPH => GET_HOST_DATA_RDWR(YL_IMOC_CLPH)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
        Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
        Z_YDMF_PHYS_OUT_UGST => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_UGST)
        Z_YDMF_PHYS_OUT_FPLCL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLCL)
        Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
        Z_YDMF_PHYS_OUT_UCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_UCLS)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP)
        Z_YDMF_PHYS_OUT_CAPE => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_CAPE)
        Z_YDMF_PHYS_OUT_CLPH => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_CLPH)
        Z_YDMF_PHYS_BASE_STATE_U => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
        Z_YDMF_PHYS_OUT_FPLCN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLCN)
        Z_YDMF_PHYS_BASE_STATE_V => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
        Z_YDMF_PHYS_OUT_FPLSL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSL)
        Z_YDMF_PHYS_OUT_VCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_VCLS)
        Z_YDMF_PHYS_OUT_VGST => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_VGST)
        Z_YDVARS_GEOMETRY_OROG_T0 => GET_HOST_DATA_RDWR(YDVARS%GEOMETRY%OROG%FT0)
        Z_YDMF_PHYS_OUT_FPLSN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSN)
        Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACEVADCAPE:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 444"
      END IF
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACEVADCAPE:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 445"
      ZDCAPE => NULL()
      ZBLH => NULL()
      ZTKE1 => NULL()
      IMOC_CLPH => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => NULL()
      Z_YDMF_PHYS_OUT_UGST => NULL()
      Z_YDMF_PHYS_OUT_FPLCL => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => NULL()
      Z_YDMF_PHYS_OUT_UCLS => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => NULL()
      Z_YDMF_PHYS_OUT_CAPE => NULL()
      Z_YDMF_PHYS_OUT_CLPH => NULL()
      Z_YDMF_PHYS_BASE_STATE_U => NULL()
      Z_YDMF_PHYS_OUT_FPLCN => NULL()
      Z_YDMF_PHYS_BASE_STATE_V => NULL()
      Z_YDMF_PHYS_OUT_FPLSL => NULL()
      Z_YDMF_PHYS_OUT_VCLS => NULL()
      Z_YDMF_PHYS_OUT_VGST => NULL()
      Z_YDVARS_GEOMETRY_OROG_T0 => NULL()
      Z_YDMF_PHYS_OUT_FPLSN => NULL()
      Z_YDMF_PHYS_BASE_STATE_T => NULL()
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACEVADCAPE:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 446"
    ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ACEVADCAPE')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACEVADCAPE:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 447"
      ZDCAPE => GET_DEVICE_DATA_RDWR(YL_ZDCAPE)
      ZBLH => GET_DEVICE_DATA_RDWR(YL_ZBLH)
      ZTKE1 => GET_DEVICE_DATA_RDWR(YL_ZTKE1)
      IMOC_CLPH => GET_DEVICE_DATA_RDWR(YL_IMOC_CLPH)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
      Z_YDMF_PHYS_OUT_UGST => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_UGST)
      Z_YDMF_PHYS_OUT_FPLCL => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_FPLCL)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
      Z_YDMF_PHYS_OUT_UCLS => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_UCLS)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP)
      Z_YDMF_PHYS_OUT_CAPE => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_CAPE)
      Z_YDMF_PHYS_OUT_CLPH => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_CLPH)
      Z_YDMF_PHYS_BASE_STATE_U => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
      Z_YDMF_PHYS_OUT_FPLCN => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_FPLCN)
      Z_YDMF_PHYS_BASE_STATE_V => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
      Z_YDMF_PHYS_OUT_FPLSL => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSL)
      Z_YDMF_PHYS_OUT_VCLS => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_VCLS)
      Z_YDMF_PHYS_OUT_VGST => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_VGST)
      Z_YDVARS_GEOMETRY_OROG_T0 => GET_DEVICE_DATA_RDWR(YDVARS%GEOMETRY%OROG%FT0)
      Z_YDMF_PHYS_OUT_FPLSN => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSN)
      Z_YDMF_PHYS_BASE_STATE_T => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACEVADCAPE:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 448"
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACEVADCAPE:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 449"
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZDCAPE, ZBLH, ZTKE1, IMOC_CLPH,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF, Z_YDMF_PHYS_OUT_UGST,  &
!$ACC & Z_YDMF_PHYS_OUT_FPLCL, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF, Z_YDMF_PHYS_OUT_UCLS,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP, Z_YDMF_PHYS_OUT_CAPE, Z_YDMF_PHYS_OUT_CLPH, Z_YDMF_PHYS_BASE_STATE_U,  &
!$ACC & Z_YDMF_PHYS_OUT_FPLCN, Z_YDMF_PHYS_BASE_STATE_V, Z_YDMF_PHYS_OUT_FPLSL, Z_YDMF_PHYS_OUT_VCLS, Z_YDMF_PHYS_OUT_VGST,  &
!$ACC & Z_YDVARS_GEOMETRY_OROG_T0, Z_YDMF_PHYS_OUT_FPLSN, Z_YDMF_PHYS_BASE_STATE_T ) PRIVATE( JBLK ) VECTOR_LENGTH(  &
!$ACC & YDCPG_OPTS%KLON )
      DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK )
        DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
          YLCPG_BNDS%KIDIA = JLON
          YLCPG_BNDS%KFDIA = JLON
          YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
          
          ! DCAPE due to precipitation evaporation.
          
          CALL ACEVADCAPE_OPENACC(YDMODEL%YRML_PHY_MF%YRPHY2, YDMODEL%YRCST, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA,  &
          & YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_OUT_FPLSL(:, :, JBLK), Z_YDMF_PHYS_OUT_FPLSN(:, :, JBLK),  &
          & Z_YDMF_PHYS_OUT_FPLCL(:, :, JBLK), Z_YDMF_PHYS_OUT_FPLCN(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_T(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI(:, :, JBLK),  &
          & ZDCAPE(:, JBLK), YDSTACK=YLSTACK)
          
          ! Gusts.
          
          CALL ACCLDIA_OPENACC(YDMODEL%YRCST, YDCPG_OPTS%LXCLP, YDCPG_OPTS%LXTGST, YDCPG_OPTS%LXXGST,  &
          & YDMODEL%YRML_PHY_MF%YRPHY, YDMODEL%YRML_PHY_MF%YRPHY2, YDMODEL%YRML_PHY_MF%YRTOPH, YLCPG_BNDS%KIDIA,  &
          & YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_OUT_UCLS(:, JBLK),  &
          & Z_YDMF_PHYS_OUT_VCLS(:, JBLK), Z_YDMF_PHYS_BASE_STATE_U(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_V(:, :, JBLK),  &
          & Z_YDMF_PHYS_OUT_CAPE(:, JBLK), ZDCAPE(:, JBLK), ZTKE1(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF(:, :, JBLK), Z_YDVARS_GEOMETRY_OROG_T0(:, JBLK),  &
          & Z_YDMF_PHYS_OUT_UGST(:, JBLK), Z_YDMF_PHYS_OUT_VGST(:, JBLK), ZBLH(:, JBLK), IMOC_CLPH(:, JBLK), YDSTACK=YLSTACK)
          
          Z_YDMF_PHYS_OUT_CLPH(JLON, JBLK) =  &
          & MIN(YDMODEL%YRML_PHY_MF%YRPHY0%XMAXLM, MAX(YDMODEL%YRML_PHY_MF%YRPHY0%XMINLM, ZBLH(JLON, JBLK)))
          
        END DO
      END DO
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACEVADCAPE:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 450"
      IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ACEVADCAPE')) THEN
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACEVADCAPE:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 451"
        ZDCAPE => GET_HOST_DATA_RDWR(YL_ZDCAPE)
        ZBLH => GET_HOST_DATA_RDWR(YL_ZBLH)
        ZTKE1 => GET_HOST_DATA_RDWR(YL_ZTKE1)
        IMOC_CLPH => GET_HOST_DATA_RDWR(YL_IMOC_CLPH)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
        Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
        Z_YDMF_PHYS_OUT_UGST => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_UGST)
        Z_YDMF_PHYS_OUT_FPLCL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLCL)
        Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYD)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
        Z_YDMF_PHYS_OUT_UCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_UCLS)
        Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP)
        Z_YDMF_PHYS_OUT_CAPE => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_CAPE)
        Z_YDMF_PHYS_OUT_CLPH => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_CLPH)
        Z_YDMF_PHYS_BASE_STATE_U => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_U)
        Z_YDMF_PHYS_OUT_FPLCN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLCN)
        Z_YDMF_PHYS_BASE_STATE_V => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_V)
        Z_YDMF_PHYS_OUT_FPLSL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSL)
        Z_YDMF_PHYS_OUT_VCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_VCLS)
        Z_YDMF_PHYS_OUT_VGST => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_VGST)
        Z_YDVARS_GEOMETRY_OROG_T0 => GET_HOST_DATA_RDWR(YDVARS%GEOMETRY%OROG%FT0)
        Z_YDMF_PHYS_OUT_FPLSN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSN)
        Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACEVADCAPE:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 452"
      END IF
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACEVADCAPE:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 453"
      ZDCAPE => NULL()
      ZBLH => NULL()
      ZTKE1 => NULL()
      IMOC_CLPH => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => NULL()
      Z_YDMF_PHYS_OUT_UGST => NULL()
      Z_YDMF_PHYS_OUT_FPLCL => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => NULL()
      Z_YDMF_PHYS_OUT_UCLS => NULL()
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP => NULL()
      Z_YDMF_PHYS_OUT_CAPE => NULL()
      Z_YDMF_PHYS_OUT_CLPH => NULL()
      Z_YDMF_PHYS_BASE_STATE_U => NULL()
      Z_YDMF_PHYS_OUT_FPLCN => NULL()
      Z_YDMF_PHYS_BASE_STATE_V => NULL()
      Z_YDMF_PHYS_OUT_FPLSL => NULL()
      Z_YDMF_PHYS_OUT_VCLS => NULL()
      Z_YDMF_PHYS_OUT_VGST => NULL()
      Z_YDVARS_GEOMETRY_OROG_T0 => NULL()
      Z_YDMF_PHYS_OUT_FPLSN => NULL()
      Z_YDMF_PHYS_BASE_STATE_T => NULL()
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACEVADCAPE:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 454"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACEVADCAPE', 1, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 455"
    
  END IF
  
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACVISIH', 0, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 456"
  IF (LPARALLELMETHOD('OPENMP', 'APL_ARPEGE_PARALLEL:ACVISIH')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACVISIH:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 457"
    ZQR => GET_HOST_DATA_RDWR(YL_ZQR)
    ZQS => GET_HOST_DATA_RDWR(YL_ZQS)
    ZQGM => GET_HOST_DATA_RDWR(YL_ZQGM)
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
    Z_YDCPG_MISC_QLI => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QLI)
    Z_YDMF_PHYS_OUT_VISICLD => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_VISICLD)
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_R)
    Z_YDMF_PHYS_OUT_MXCLWC => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_MXCLWC)
    Z_YDCPG_MISC_QICE => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QICE)
    Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
    Z_YDMF_PHYS_OUT_VISIHYD => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_VISIHYD)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACVISIH:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 458"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACVISIH:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 459"
    CALL YLCPG_BNDS%INIT(YDCPG_OPTS)
!$OMP PARALLEL DO PRIVATE( JBLK, ZEPSNEB ) FIRSTPRIVATE( YLCPG_BNDS )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      CALL YLCPG_BNDS%UPDATE(JBLK)
      
      ZQGM(1:YDCPG_OPTS%KLON, 1:YDCPG_OPTS%KFLEVG, JBLK) = ZEPSNEB
      
      CALL ACVISIH(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF%YRPHY%YRDVISI, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON,  &
      & YDCPG_OPTS%KTDIA, YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI(:, :, JBLK),  &
      & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(:, :, JBLK),  &
      & Z_YDMF_PHYS_BASE_STATE_T(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R(:, :, JBLK),  &
      & Z_YDCPG_MISC_QLI(:, :, JBLK), Z_YDCPG_MISC_QICE(:, :, JBLK), ZQR(:, :, JBLK), ZQS(:, :, JBLK), ZQGM(:, :, JBLK),  &
      & Z_YDMF_PHYS_OUT_VISICLD(:, JBLK), Z_YDMF_PHYS_OUT_VISIHYD(:, JBLK), Z_YDMF_PHYS_OUT_MXCLWC(:, JBLK))
      
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACVISIH:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 460"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ACVISIH')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACVISIH:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 461"
      ZQR => GET_HOST_DATA_RDWR(YL_ZQR)
      ZQS => GET_HOST_DATA_RDWR(YL_ZQS)
      ZQGM => GET_HOST_DATA_RDWR(YL_ZQGM)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
      Z_YDCPG_MISC_QLI => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QLI)
      Z_YDMF_PHYS_OUT_VISICLD => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_VISICLD)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_R)
      Z_YDMF_PHYS_OUT_MXCLWC => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_MXCLWC)
      Z_YDCPG_MISC_QICE => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QICE)
      Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
      Z_YDMF_PHYS_OUT_VISIHYD => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_VISIHYD)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACVISIH:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 462"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACVISIH:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 463"
    ZQR => NULL()
    ZQS => NULL()
    ZQGM => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => NULL()
    Z_YDCPG_MISC_QLI => NULL()
    Z_YDMF_PHYS_OUT_VISICLD => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => NULL()
    Z_YDMF_PHYS_OUT_MXCLWC => NULL()
    Z_YDCPG_MISC_QICE => NULL()
    Z_YDMF_PHYS_BASE_STATE_T => NULL()
    Z_YDMF_PHYS_OUT_VISIHYD => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACVISIH:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 464"
  ELSE IF (LPARALLELMETHOD('OPENMPSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ACVISIH')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACVISIH:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 465"
    ZQR => GET_HOST_DATA_RDWR(YL_ZQR)
    ZQS => GET_HOST_DATA_RDWR(YL_ZQS)
    ZQGM => GET_HOST_DATA_RDWR(YL_ZQGM)
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
    Z_YDCPG_MISC_QLI => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QLI)
    Z_YDMF_PHYS_OUT_VISICLD => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_VISICLD)
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_R)
    Z_YDMF_PHYS_OUT_MXCLWC => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_MXCLWC)
    Z_YDCPG_MISC_QICE => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QICE)
    Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
    Z_YDMF_PHYS_OUT_VISIHYD => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_VISIHYD)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACVISIH:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 466"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACVISIH:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 467"
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK, ZEPSNEB )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        ZQGM(JLON, 1:YDCPG_OPTS%KFLEVG, JBLK) = ZEPSNEB
        
        CALL ACVISIH_OPENACC(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF%YRPHY%YRDVISI, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA,  &
        & YDCPG_OPTS%KLON, YDCPG_OPTS%KTDIA, YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_T(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R(:, :, JBLK),  &
        & Z_YDCPG_MISC_QLI(:, :, JBLK), Z_YDCPG_MISC_QICE(:, :, JBLK), ZQR(:, :, JBLK), ZQS(:, :, JBLK), ZQGM(:, :, JBLK),  &
        & Z_YDMF_PHYS_OUT_VISICLD(:, JBLK), Z_YDMF_PHYS_OUT_VISIHYD(:, JBLK), Z_YDMF_PHYS_OUT_MXCLWC(:, JBLK),  &
        & YDSTACK=YLSTACK)
        
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACVISIH:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 468"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ACVISIH')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACVISIH:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 469"
      ZQR => GET_HOST_DATA_RDWR(YL_ZQR)
      ZQS => GET_HOST_DATA_RDWR(YL_ZQS)
      ZQGM => GET_HOST_DATA_RDWR(YL_ZQGM)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
      Z_YDCPG_MISC_QLI => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QLI)
      Z_YDMF_PHYS_OUT_VISICLD => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_VISICLD)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_R)
      Z_YDMF_PHYS_OUT_MXCLWC => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_MXCLWC)
      Z_YDCPG_MISC_QICE => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QICE)
      Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
      Z_YDMF_PHYS_OUT_VISIHYD => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_VISIHYD)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACVISIH:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 470"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACVISIH:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 471"
    ZQR => NULL()
    ZQS => NULL()
    ZQGM => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => NULL()
    Z_YDCPG_MISC_QLI => NULL()
    Z_YDMF_PHYS_OUT_VISICLD => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => NULL()
    Z_YDMF_PHYS_OUT_MXCLWC => NULL()
    Z_YDCPG_MISC_QICE => NULL()
    Z_YDMF_PHYS_BASE_STATE_T => NULL()
    Z_YDMF_PHYS_OUT_VISIHYD => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACVISIH:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 472"
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ACVISIH')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACVISIH:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 473"
    ZQR => GET_DEVICE_DATA_RDWR(YL_ZQR)
    ZQS => GET_DEVICE_DATA_RDWR(YL_ZQS)
    ZQGM => GET_DEVICE_DATA_RDWR(YL_ZQGM)
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
    Z_YDCPG_MISC_QLI => GET_DEVICE_DATA_RDWR(YDCPG_MISC%F_QLI)
    Z_YDMF_PHYS_OUT_VISICLD => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_VISICLD)
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_R)
    Z_YDMF_PHYS_OUT_MXCLWC => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_MXCLWC)
    Z_YDCPG_MISC_QICE => GET_DEVICE_DATA_RDWR(YDCPG_MISC%F_QICE)
    Z_YDMF_PHYS_BASE_STATE_T => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
    Z_YDMF_PHYS_OUT_VISIHYD => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_VISIHYD)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACVISIH:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 474"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACVISIH:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 475"
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZQR, ZQS, ZQGM, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF, Z_YDCPG_MISC_QLI, Z_YDMF_PHYS_OUT_VISICLD,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R, Z_YDMF_PHYS_OUT_MXCLWC,  &
!$ACC & Z_YDCPG_MISC_QICE, Z_YDMF_PHYS_BASE_STATE_T, Z_YDMF_PHYS_OUT_VISIHYD ) PRIVATE( JBLK ) VECTOR_LENGTH(  &
!$ACC & YDCPG_OPTS%KLON )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK, ZEPSNEB )
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        ZQGM(JLON, 1:YDCPG_OPTS%KFLEVG, JBLK) = ZEPSNEB
        
        CALL ACVISIH_OPENACC(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF%YRPHY%YRDVISI, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA,  &
        & YDCPG_OPTS%KLON, YDCPG_OPTS%KTDIA, YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_T(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R(:, :, JBLK),  &
        & Z_YDCPG_MISC_QLI(:, :, JBLK), Z_YDCPG_MISC_QICE(:, :, JBLK), ZQR(:, :, JBLK), ZQS(:, :, JBLK), ZQGM(:, :, JBLK),  &
        & Z_YDMF_PHYS_OUT_VISICLD(:, JBLK), Z_YDMF_PHYS_OUT_VISIHYD(:, JBLK), Z_YDMF_PHYS_OUT_MXCLWC(:, JBLK),  &
        & YDSTACK=YLSTACK)
        
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACVISIH:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 476"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ACVISIH')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACVISIH:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 477"
      ZQR => GET_HOST_DATA_RDWR(YL_ZQR)
      ZQS => GET_HOST_DATA_RDWR(YL_ZQS)
      ZQGM => GET_HOST_DATA_RDWR(YL_ZQGM)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
      Z_YDCPG_MISC_QLI => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QLI)
      Z_YDMF_PHYS_OUT_VISICLD => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_VISICLD)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF)
      Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_R)
      Z_YDMF_PHYS_OUT_MXCLWC => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_MXCLWC)
      Z_YDCPG_MISC_QICE => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QICE)
      Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
      Z_YDMF_PHYS_OUT_VISIHYD => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_VISIHYD)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACVISIH:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 478"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACVISIH:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 479"
    ZQR => NULL()
    ZQS => NULL()
    ZQGM => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => NULL()
    Z_YDCPG_MISC_QLI => NULL()
    Z_YDMF_PHYS_OUT_VISICLD => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R => NULL()
    Z_YDMF_PHYS_OUT_MXCLWC => NULL()
    Z_YDCPG_MISC_QICE => NULL()
    Z_YDMF_PHYS_BASE_STATE_T => NULL()
    Z_YDMF_PHYS_OUT_VISIHYD => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACVISIH:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 480"
  END IF
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACVISIH', 1, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 481"
  
  ! Convert from flux [kg/m2/s] to density [kg/m3] using old RTTOV-SCATT
  !  a    b         ! RR = a * LWC^b, [RR]=mm/h, [LWC]=g/m^3
  ! 20.89 1.15      ! rain
  ! 29.51 1.10      ! snow
  
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZBAY_QRCONV', 0, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 482"
  IF (LPARALLELMETHOD('OPENMP', 'APL_ARPEGE_PARALLEL:ZBAY_QRCONV')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZBAY_QRCONV:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 483"
    ZBAY_QSCONV => GET_HOST_DATA_RDWR(YL_ZBAY_QSCONV)
    ZBAY_QRCONV => GET_HOST_DATA_RDWR(YL_ZBAY_QRCONV)
    Z_YDMF_PHYS_OUT_FPLCN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLCN)
    Z_YDMF_PHYS_OUT_FPLCL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLCL)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZBAY_QRCONV:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 484"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZBAY_QRCONV:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 485"
    CALL YLCPG_BNDS%INIT(YDCPG_OPTS)
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, JLEV ) FIRSTPRIVATE( YLCPG_BNDS )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      CALL YLCPG_BNDS%UPDATE(JBLK)
      
      DO JLEV=1,YDCPG_OPTS%KFLEVG
        DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
          ZBAY_QRCONV(JLON, JLEV, JBLK) = 0.001_JPRB*((ABS(Z_YDMF_PHYS_OUT_FPLCL(JLON, JLEV, JBLK)) &
          & *YDMODEL%YRML_PHY_MF%YRPHY0%RCOEFRAIN(1))**YDMODEL%YRML_PHY_MF%YRPHY0%RCOEFRAIN(2))
          ZBAY_QSCONV(JLON, JLEV, JBLK) = 0.001_JPRB*((ABS(Z_YDMF_PHYS_OUT_FPLCN(JLON, JLEV, JBLK)) &
          & *YDMODEL%YRML_PHY_MF%YRPHY0%RCOEFSNOW(1))**YDMODEL%YRML_PHY_MF%YRPHY0%RCOEFSNOW(2))
        END DO
      END DO
      
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZBAY_QRCONV:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 486"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ZBAY_QRCONV')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZBAY_QRCONV:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 487"
      ZBAY_QSCONV => GET_HOST_DATA_RDWR(YL_ZBAY_QSCONV)
      ZBAY_QRCONV => GET_HOST_DATA_RDWR(YL_ZBAY_QRCONV)
      Z_YDMF_PHYS_OUT_FPLCN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLCN)
      Z_YDMF_PHYS_OUT_FPLCL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLCL)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZBAY_QRCONV:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 488"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZBAY_QRCONV:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 489"
    ZBAY_QSCONV => NULL()
    ZBAY_QRCONV => NULL()
    Z_YDMF_PHYS_OUT_FPLCN => NULL()
    Z_YDMF_PHYS_OUT_FPLCL => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZBAY_QRCONV:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 490"
  ELSE IF (LPARALLELMETHOD('OPENMPSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ZBAY_QRCONV')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZBAY_QRCONV:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 491"
    ZBAY_QSCONV => GET_HOST_DATA_RDWR(YL_ZBAY_QSCONV)
    ZBAY_QRCONV => GET_HOST_DATA_RDWR(YL_ZBAY_QRCONV)
    Z_YDMF_PHYS_OUT_FPLCN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLCN)
    Z_YDMF_PHYS_OUT_FPLCL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLCL)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZBAY_QRCONV:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 492"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZBAY_QRCONV:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 493"
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK, JLEV )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        DO JLEV=1,YDCPG_OPTS%KFLEVG
          ZBAY_QRCONV(JLON, JLEV, JBLK) = 0.001_JPRB*((ABS(Z_YDMF_PHYS_OUT_FPLCL(JLON, JLEV, JBLK)) &
          & *YDMODEL%YRML_PHY_MF%YRPHY0%RCOEFRAIN(1))**YDMODEL%YRML_PHY_MF%YRPHY0%RCOEFRAIN(2))
          ZBAY_QSCONV(JLON, JLEV, JBLK) = 0.001_JPRB*((ABS(Z_YDMF_PHYS_OUT_FPLCN(JLON, JLEV, JBLK)) &
          & *YDMODEL%YRML_PHY_MF%YRPHY0%RCOEFSNOW(1))**YDMODEL%YRML_PHY_MF%YRPHY0%RCOEFSNOW(2))
        END DO
        
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZBAY_QRCONV:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 494"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ZBAY_QRCONV')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZBAY_QRCONV:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 495"
      ZBAY_QSCONV => GET_HOST_DATA_RDWR(YL_ZBAY_QSCONV)
      ZBAY_QRCONV => GET_HOST_DATA_RDWR(YL_ZBAY_QRCONV)
      Z_YDMF_PHYS_OUT_FPLCN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLCN)
      Z_YDMF_PHYS_OUT_FPLCL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLCL)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZBAY_QRCONV:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 496"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZBAY_QRCONV:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 497"
    ZBAY_QSCONV => NULL()
    ZBAY_QRCONV => NULL()
    Z_YDMF_PHYS_OUT_FPLCN => NULL()
    Z_YDMF_PHYS_OUT_FPLCL => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZBAY_QRCONV:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 498"
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ZBAY_QRCONV')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZBAY_QRCONV:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 499"
    ZBAY_QSCONV => GET_DEVICE_DATA_RDWR(YL_ZBAY_QSCONV)
    ZBAY_QRCONV => GET_DEVICE_DATA_RDWR(YL_ZBAY_QRCONV)
    Z_YDMF_PHYS_OUT_FPLCN => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_FPLCN)
    Z_YDMF_PHYS_OUT_FPLCL => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_FPLCL)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZBAY_QRCONV:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 500"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZBAY_QRCONV:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 501"
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZBAY_QSCONV, ZBAY_QRCONV, Z_YDMF_PHYS_OUT_FPLCN,  &
!$ACC & Z_YDMF_PHYS_OUT_FPLCL ) PRIVATE( JBLK ) VECTOR_LENGTH( YDCPG_OPTS%KLON )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK, JLEV )
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        DO JLEV=1,YDCPG_OPTS%KFLEVG
          ZBAY_QRCONV(JLON, JLEV, JBLK) = 0.001_JPRB*((ABS(Z_YDMF_PHYS_OUT_FPLCL(JLON, JLEV, JBLK)) &
          & *YDMODEL%YRML_PHY_MF%YRPHY0%RCOEFRAIN(1))**YDMODEL%YRML_PHY_MF%YRPHY0%RCOEFRAIN(2))
          ZBAY_QSCONV(JLON, JLEV, JBLK) = 0.001_JPRB*((ABS(Z_YDMF_PHYS_OUT_FPLCN(JLON, JLEV, JBLK)) &
          & *YDMODEL%YRML_PHY_MF%YRPHY0%RCOEFSNOW(1))**YDMODEL%YRML_PHY_MF%YRPHY0%RCOEFSNOW(2))
        END DO
        
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZBAY_QRCONV:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 502"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ZBAY_QRCONV')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZBAY_QRCONV:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 503"
      ZBAY_QSCONV => GET_HOST_DATA_RDWR(YL_ZBAY_QSCONV)
      ZBAY_QRCONV => GET_HOST_DATA_RDWR(YL_ZBAY_QRCONV)
      Z_YDMF_PHYS_OUT_FPLCN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLCN)
      Z_YDMF_PHYS_OUT_FPLCL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLCL)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZBAY_QRCONV:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 504"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZBAY_QRCONV:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 505"
    ZBAY_QSCONV => NULL()
    ZBAY_QRCONV => NULL()
    Z_YDMF_PHYS_OUT_FPLCN => NULL()
    Z_YDMF_PHYS_OUT_FPLCL => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZBAY_QRCONV:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 506"
  END IF
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZBAY_QRCONV', 1, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 507"
  
  ! Convert density [kg/m3] to mixing ratio [kg/kg]
  ! R_dry (dry air constant)
  
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDE2MR', 0, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 508"
  IF (LPARALLELMETHOD('OPENMP', 'APL_ARPEGE_PARALLEL:ZDE2MR')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDE2MR:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 509"
    ZBAY_QSCONV => GET_HOST_DATA_RDWR(YL_ZBAY_QSCONV)
    ZDE2MR => GET_HOST_DATA_RDWR(YL_ZDE2MR)
    ZBAY_QRCONV => GET_HOST_DATA_RDWR(YL_ZBAY_QRCONV)
    Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDE2MR:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 510"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDE2MR:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 511"
    CALL YLCPG_BNDS%INIT(YDCPG_OPTS)
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, JLEV ) FIRSTPRIVATE( YLCPG_BNDS )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      CALL YLCPG_BNDS%UPDATE(JBLK)
      
      DO JLEV=1,YDCPG_OPTS%KFLEVG
        DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
          ZDE2MR(JLON, JLEV, JBLK) = YDMODEL%YRCST%RD*Z_YDMF_PHYS_BASE_STATE_T(JLON, JLEV, JBLK) /  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(JLON, JLEV, JBLK)
          ZBAY_QRCONV(JLON, JLEV, JBLK) = ZBAY_QRCONV(JLON, JLEV, JBLK)*ZDE2MR(JLON, JLEV, JBLK)
          ZBAY_QSCONV(JLON, JLEV, JBLK) = ZBAY_QSCONV(JLON, JLEV, JBLK)*ZDE2MR(JLON, JLEV, JBLK)
        END DO
      END DO
      
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDE2MR:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 512"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ZDE2MR')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDE2MR:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 513"
      ZBAY_QSCONV => GET_HOST_DATA_RDWR(YL_ZBAY_QSCONV)
      ZDE2MR => GET_HOST_DATA_RDWR(YL_ZDE2MR)
      ZBAY_QRCONV => GET_HOST_DATA_RDWR(YL_ZBAY_QRCONV)
      Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDE2MR:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 514"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDE2MR:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 515"
    ZBAY_QSCONV => NULL()
    ZDE2MR => NULL()
    ZBAY_QRCONV => NULL()
    Z_YDMF_PHYS_BASE_STATE_T => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDE2MR:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 516"
  ELSE IF (LPARALLELMETHOD('OPENMPSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ZDE2MR')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDE2MR:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 517"
    ZBAY_QSCONV => GET_HOST_DATA_RDWR(YL_ZBAY_QSCONV)
    ZDE2MR => GET_HOST_DATA_RDWR(YL_ZDE2MR)
    ZBAY_QRCONV => GET_HOST_DATA_RDWR(YL_ZBAY_QRCONV)
    Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDE2MR:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 518"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDE2MR:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 519"
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK, JLEV )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        DO JLEV=1,YDCPG_OPTS%KFLEVG
          ZDE2MR(JLON, JLEV, JBLK) = YDMODEL%YRCST%RD*Z_YDMF_PHYS_BASE_STATE_T(JLON, JLEV, JBLK) /  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(JLON, JLEV, JBLK)
          ZBAY_QRCONV(JLON, JLEV, JBLK) = ZBAY_QRCONV(JLON, JLEV, JBLK)*ZDE2MR(JLON, JLEV, JBLK)
          ZBAY_QSCONV(JLON, JLEV, JBLK) = ZBAY_QSCONV(JLON, JLEV, JBLK)*ZDE2MR(JLON, JLEV, JBLK)
        END DO
        
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDE2MR:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 520"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ZDE2MR')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDE2MR:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 521"
      ZBAY_QSCONV => GET_HOST_DATA_RDWR(YL_ZBAY_QSCONV)
      ZDE2MR => GET_HOST_DATA_RDWR(YL_ZDE2MR)
      ZBAY_QRCONV => GET_HOST_DATA_RDWR(YL_ZBAY_QRCONV)
      Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDE2MR:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 522"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDE2MR:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 523"
    ZBAY_QSCONV => NULL()
    ZDE2MR => NULL()
    ZBAY_QRCONV => NULL()
    Z_YDMF_PHYS_BASE_STATE_T => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDE2MR:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 524"
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ZDE2MR')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDE2MR:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 525"
    ZBAY_QSCONV => GET_DEVICE_DATA_RDWR(YL_ZBAY_QSCONV)
    ZDE2MR => GET_DEVICE_DATA_RDWR(YL_ZDE2MR)
    ZBAY_QRCONV => GET_DEVICE_DATA_RDWR(YL_ZBAY_QRCONV)
    Z_YDMF_PHYS_BASE_STATE_T => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDE2MR:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 526"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDE2MR:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 527"
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZBAY_QSCONV, ZDE2MR, ZBAY_QRCONV, Z_YDMF_PHYS_BASE_STATE_T,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF ) PRIVATE( JBLK ) VECTOR_LENGTH( YDCPG_OPTS%KLON )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK, JLEV )
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        DO JLEV=1,YDCPG_OPTS%KFLEVG
          ZDE2MR(JLON, JLEV, JBLK) = YDMODEL%YRCST%RD*Z_YDMF_PHYS_BASE_STATE_T(JLON, JLEV, JBLK) /  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(JLON, JLEV, JBLK)
          ZBAY_QRCONV(JLON, JLEV, JBLK) = ZBAY_QRCONV(JLON, JLEV, JBLK)*ZDE2MR(JLON, JLEV, JBLK)
          ZBAY_QSCONV(JLON, JLEV, JBLK) = ZBAY_QSCONV(JLON, JLEV, JBLK)*ZDE2MR(JLON, JLEV, JBLK)
        END DO
        
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDE2MR:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 528"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:ZDE2MR')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDE2MR:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 529"
      ZBAY_QSCONV => GET_HOST_DATA_RDWR(YL_ZBAY_QSCONV)
      ZDE2MR => GET_HOST_DATA_RDWR(YL_ZDE2MR)
      ZBAY_QRCONV => GET_HOST_DATA_RDWR(YL_ZBAY_QRCONV)
      Z_YDMF_PHYS_BASE_STATE_T => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_T)
      Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDE2MR:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 530"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDE2MR:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 531"
    ZBAY_QSCONV => NULL()
    ZDE2MR => NULL()
    ZBAY_QRCONV => NULL()
    Z_YDMF_PHYS_BASE_STATE_T => NULL()
    Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDE2MR:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 532"
  END IF
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDE2MR', 1, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 533"
  
  ! Precipitation Type
  
  ! Compute wet-bulb temperature at 2 meters (suppose homogeneity of qv/ql/qi )
  
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PPWETPOINT', 0, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 534"
  IF (LPARALLELMETHOD('OPENMP', 'APL_ARPEGE_PARALLEL:PPWETPOINT')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PPWETPOINT:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 535"
    ZPCLS => GET_HOST_DATA_RDWR(YL_ZPCLS)
    Z_YDMF_PHYS_BASE_STATE_I => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_I)
    Z_YDMF_PHYS_OUT_TCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_TCLS)
    Z_YDMF_PHYS_BASE_STATE_Q => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_Q)
    Z_YDMF_PHYS_BASE_STATE_L => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_L)
    Z_YDMF_PHYS_OUT_TPWCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_TPWCLS)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PPWETPOINT:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 536"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PPWETPOINT:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 537"
    CALL YLCPG_BNDS%INIT(YDCPG_OPTS)
!$OMP PARALLEL DO PRIVATE( JBLK ) FIRSTPRIVATE( YLCPG_BNDS )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      CALL YLCPG_BNDS%UPDATE(JBLK)
      
      CALL PPWETPOINT(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF%YRPHY, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, 1,  &
      & ZPCLS(:, JBLK), Z_YDMF_PHYS_OUT_TCLS(:, JBLK), Z_YDMF_PHYS_BASE_STATE_Q(:, YDCPG_OPTS%KFLEVG, JBLK),  &
      & Z_YDMF_PHYS_BASE_STATE_L(:, YDCPG_OPTS%KFLEVG, JBLK), Z_YDMF_PHYS_BASE_STATE_I(:, YDCPG_OPTS%KFLEVG, JBLK),  &
      & Z_YDMF_PHYS_OUT_TPWCLS(:, JBLK))
      
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PPWETPOINT:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 538"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:PPWETPOINT')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PPWETPOINT:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 539"
      ZPCLS => GET_HOST_DATA_RDWR(YL_ZPCLS)
      Z_YDMF_PHYS_BASE_STATE_I => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_I)
      Z_YDMF_PHYS_OUT_TCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_TCLS)
      Z_YDMF_PHYS_BASE_STATE_Q => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_Q)
      Z_YDMF_PHYS_BASE_STATE_L => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_L)
      Z_YDMF_PHYS_OUT_TPWCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_TPWCLS)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PPWETPOINT:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 540"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PPWETPOINT:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 541"
    ZPCLS => NULL()
    Z_YDMF_PHYS_BASE_STATE_I => NULL()
    Z_YDMF_PHYS_OUT_TCLS => NULL()
    Z_YDMF_PHYS_BASE_STATE_Q => NULL()
    Z_YDMF_PHYS_BASE_STATE_L => NULL()
    Z_YDMF_PHYS_OUT_TPWCLS => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PPWETPOINT:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 542"
  ELSE IF (LPARALLELMETHOD('OPENMPSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:PPWETPOINT')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PPWETPOINT:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 543"
    ZPCLS => GET_HOST_DATA_RDWR(YL_ZPCLS)
    Z_YDMF_PHYS_BASE_STATE_I => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_I)
    Z_YDMF_PHYS_OUT_TCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_TCLS)
    Z_YDMF_PHYS_BASE_STATE_Q => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_Q)
    Z_YDMF_PHYS_BASE_STATE_L => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_L)
    Z_YDMF_PHYS_OUT_TPWCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_TPWCLS)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PPWETPOINT:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 544"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PPWETPOINT:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 545"
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        CALL PPWETPOINT_OPENACC(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF%YRPHY, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA,  &
        & YDCPG_OPTS%KLON, 1, ZPCLS(:, JBLK), Z_YDMF_PHYS_OUT_TCLS(:, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_Q(:, YDCPG_OPTS%KFLEVG, JBLK), Z_YDMF_PHYS_BASE_STATE_L(:, YDCPG_OPTS%KFLEVG, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_I(:, YDCPG_OPTS%KFLEVG, JBLK), Z_YDMF_PHYS_OUT_TPWCLS(:, JBLK), YDSTACK=YLSTACK)
        
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PPWETPOINT:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 546"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:PPWETPOINT')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PPWETPOINT:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 547"
      ZPCLS => GET_HOST_DATA_RDWR(YL_ZPCLS)
      Z_YDMF_PHYS_BASE_STATE_I => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_I)
      Z_YDMF_PHYS_OUT_TCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_TCLS)
      Z_YDMF_PHYS_BASE_STATE_Q => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_Q)
      Z_YDMF_PHYS_BASE_STATE_L => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_L)
      Z_YDMF_PHYS_OUT_TPWCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_TPWCLS)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PPWETPOINT:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 548"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PPWETPOINT:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 549"
    ZPCLS => NULL()
    Z_YDMF_PHYS_BASE_STATE_I => NULL()
    Z_YDMF_PHYS_OUT_TCLS => NULL()
    Z_YDMF_PHYS_BASE_STATE_Q => NULL()
    Z_YDMF_PHYS_BASE_STATE_L => NULL()
    Z_YDMF_PHYS_OUT_TPWCLS => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PPWETPOINT:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 550"
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:PPWETPOINT')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PPWETPOINT:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 551"
    ZPCLS => GET_DEVICE_DATA_RDWR(YL_ZPCLS)
    Z_YDMF_PHYS_BASE_STATE_I => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_I)
    Z_YDMF_PHYS_OUT_TCLS => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_TCLS)
    Z_YDMF_PHYS_BASE_STATE_Q => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_Q)
    Z_YDMF_PHYS_BASE_STATE_L => GET_DEVICE_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_L)
    Z_YDMF_PHYS_OUT_TPWCLS => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_TPWCLS)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PPWETPOINT:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 552"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PPWETPOINT:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 553"
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZPCLS, Z_YDMF_PHYS_BASE_STATE_I, Z_YDMF_PHYS_OUT_TCLS,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_Q, Z_YDMF_PHYS_BASE_STATE_L, Z_YDMF_PHYS_OUT_TPWCLS ) PRIVATE( JBLK ) VECTOR_LENGTH(  &
!$ACC & YDCPG_OPTS%KLON )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK )
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        CALL PPWETPOINT_OPENACC(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF%YRPHY, YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA,  &
        & YDCPG_OPTS%KLON, 1, ZPCLS(:, JBLK), Z_YDMF_PHYS_OUT_TCLS(:, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_Q(:, YDCPG_OPTS%KFLEVG, JBLK), Z_YDMF_PHYS_BASE_STATE_L(:, YDCPG_OPTS%KFLEVG, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_I(:, YDCPG_OPTS%KFLEVG, JBLK), Z_YDMF_PHYS_OUT_TPWCLS(:, JBLK), YDSTACK=YLSTACK)
        
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PPWETPOINT:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 554"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:PPWETPOINT')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PPWETPOINT:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 555"
      ZPCLS => GET_HOST_DATA_RDWR(YL_ZPCLS)
      Z_YDMF_PHYS_BASE_STATE_I => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_I)
      Z_YDMF_PHYS_OUT_TCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_TCLS)
      Z_YDMF_PHYS_BASE_STATE_Q => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_Q)
      Z_YDMF_PHYS_BASE_STATE_L => GET_HOST_DATA_RDWR(YDMF_PHYS_BASE_STATE%F_L)
      Z_YDMF_PHYS_OUT_TPWCLS => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_TPWCLS)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PPWETPOINT:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 556"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PPWETPOINT:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 557"
    ZPCLS => NULL()
    Z_YDMF_PHYS_BASE_STATE_I => NULL()
    Z_YDMF_PHYS_OUT_TCLS => NULL()
    Z_YDMF_PHYS_BASE_STATE_Q => NULL()
    Z_YDMF_PHYS_BASE_STATE_L => NULL()
    Z_YDMF_PHYS_OUT_TPWCLS => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PPWETPOINT:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 558"
  END IF
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PPWETPOINT', 1, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 559"
  
  CALL APL_ARPEGE_DPRECIPS_PARALLEL(YDMF_PHYS_BASE_STATE, YDCPG_BNDS, YDCPG_OPTS, YDMF_PHYS, YDVARS, YDMODEL,  &
  & YL_ZPRC_DPRECIPS, YL_ZPRC_DPRECIPS2)
  
  CALL MF_PHYS_MOCON_PARALLEL(YDCPG_BNDS, YDCPG_OPTS, YL_ZRDG_LCVQ, YL_IMOC_CLPH, YDMF_PHYS, YDMF_PHYS_BASE_STATE)
  
  ! Store surface water flux P and E for water conservation
  IF (YDCPG_OPTS%LCORWAT) THEN
    
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:0', 0, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 560"
    IF (LPARALLELMETHOD('OPENMP', 'APL_ARPEGE_PARALLEL:0')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:0:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 561"
      Z_YDMF_PHYS_OUT_FPLCN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLCN)
      Z_YDMF_PHYS_OUT_FPLCL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLCL)
      Z_YDMF_PHYS_OUT_FEVN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FEVN)
      Z_YDMF_PHYS_OUT_FEVL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FEVL)
      Z_YDMF_PHYS_SURF_GSD_VH_PPCN => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VH%F_PCN)
      Z_YDMF_PHYS_OUT_FPLSN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSN)
      Z_YDMF_PHYS_SURF_GSD_VH_PPSL => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VH%F_PSL)
      Z_YDMF_PHYS_SURF_GSD_VH_PPSN => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VH%F_PSN)
      Z_YDMF_PHYS_OUT_FPLSL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSL)
      Z_YDMF_PHYS_SURF_GSD_VH_PEVA => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VH%F_EVA)
      Z_YDMF_PHYS_SURF_GSD_VH_PPCL => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VH%F_PCL)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:0:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 562"
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:0:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 563"
      CALL YLCPG_BNDS%INIT(YDCPG_OPTS)
!$OMP PARALLEL DO PRIVATE( JBLK ) FIRSTPRIVATE( YLCPG_BNDS )
      DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
        CALL YLCPG_BNDS%UPDATE(JBLK)
        
        CALL MF_PHYS_CORWAT(YLCPG_BNDS, YDCPG_OPTS, Z_YDMF_PHYS_OUT_FEVL(:, :, JBLK), Z_YDMF_PHYS_OUT_FEVN(:, :, JBLK),  &
        & Z_YDMF_PHYS_OUT_FPLCL(:, :, JBLK), Z_YDMF_PHYS_OUT_FPLCN(:, :, JBLK), Z_YDMF_PHYS_OUT_FPLSL(:, :, JBLK),  &
        & Z_YDMF_PHYS_OUT_FPLSN(:, :, JBLK), Z_YDMF_PHYS_SURF_GSD_VH_PEVA(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VH_PPCL(:, JBLK),  &
        & Z_YDMF_PHYS_SURF_GSD_VH_PPCN(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VH_PPSL(:, JBLK), Z_YDMF_PHYS_SURF_GSD_VH_PPSN(:, JBLK) &
        & )
        
      END DO
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:0:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 564"
      IF (LSYNCHOST('APL_ARPEGE_PARALLEL:0')) THEN
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:0:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 565"
        Z_YDMF_PHYS_OUT_FPLCN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLCN)
        Z_YDMF_PHYS_OUT_FPLCL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLCL)
        Z_YDMF_PHYS_OUT_FEVN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FEVN)
        Z_YDMF_PHYS_OUT_FEVL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FEVL)
        Z_YDMF_PHYS_SURF_GSD_VH_PPCN => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VH%F_PCN)
        Z_YDMF_PHYS_OUT_FPLSN => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSN)
        Z_YDMF_PHYS_SURF_GSD_VH_PPSL => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VH%F_PSL)
        Z_YDMF_PHYS_SURF_GSD_VH_PPSN => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VH%F_PSN)
        Z_YDMF_PHYS_OUT_FPLSL => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_FPLSL)
        Z_YDMF_PHYS_SURF_GSD_VH_PEVA => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VH%F_EVA)
        Z_YDMF_PHYS_SURF_GSD_VH_PPCL => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_VH%F_PCL)
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:0:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 566"
      END IF
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:0:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 567"
      Z_YDMF_PHYS_OUT_FPLCN => NULL()
      Z_YDMF_PHYS_OUT_FPLCL => NULL()
      Z_YDMF_PHYS_OUT_FEVN => NULL()
      Z_YDMF_PHYS_OUT_FEVL => NULL()
      Z_YDMF_PHYS_SURF_GSD_VH_PPCN => NULL()
      Z_YDMF_PHYS_OUT_FPLSN => NULL()
      Z_YDMF_PHYS_SURF_GSD_VH_PPSL => NULL()
      Z_YDMF_PHYS_SURF_GSD_VH_PPSN => NULL()
      Z_YDMF_PHYS_OUT_FPLSL => NULL()
      Z_YDMF_PHYS_SURF_GSD_VH_PEVA => NULL()
      Z_YDMF_PHYS_SURF_GSD_VH_PPCL => NULL()
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:0:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 568"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:0', 1, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 569"
    
  END IF
  
  ! Surface specific humidity necessary to compute the vertical advection of q in the case "delta m=1" (unlagged physics only).
  
  IF (YDMODEL%YRML_PHY_MF%YRPHY%NDPSFI == 1) THEN
    
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:1', 0, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 570"
    IF (LPARALLELMETHOD('OPENMP', 'APL_ARPEGE_PARALLEL:1')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:1:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 571"
      ZFLU_QSATS => GET_HOST_DATA_RDWR(YL_ZFLU_QSATS)
      Z_YDCPG_PHY0_PREHYD => GET_HOST_DATA_RDWR(YDCPG_PHY0%F_PREHYD)
      Z_YDMF_PHYS_SURF_GSP_RR_PT_T0 => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSP_RR%F_T_T0)
      Z_YDCPG_MISC_QSOL => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QSOL)
      Z_YDCPG_MISC_QS => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QS)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:1:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 572"
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:1:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 573"
      CALL YLCPG_BNDS%INIT(YDCPG_OPTS)
!$OMP PARALLEL DO PRIVATE( JBLK ) FIRSTPRIVATE( YLCPG_BNDS )
      DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
        CALL YLCPG_BNDS%UPDATE(JBLK)
        
        CALL CPQSOL(YDMODEL%YRCST, YDGEOMETRY%YRDIMV, YDMODEL%YRML_PHY_MF%YRPHY, YDCPG_OPTS%KLON, YLCPG_BNDS%KIDIA,  &
        & YLCPG_BNDS%KFDIA, Z_YDCPG_PHY0_PREHYD(:, :, JBLK), Z_YDMF_PHYS_SURF_GSP_RR_PT_T0(:, JBLK),  &
        & Z_YDCPG_MISC_QS(:, JBLK), ZFLU_QSATS(:, JBLK), Z_YDCPG_MISC_QSOL(:, JBLK))
        
      END DO
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:1:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 574"
      IF (LSYNCHOST('APL_ARPEGE_PARALLEL:1')) THEN
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:1:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 575"
        ZFLU_QSATS => GET_HOST_DATA_RDWR(YL_ZFLU_QSATS)
        Z_YDCPG_PHY0_PREHYD => GET_HOST_DATA_RDWR(YDCPG_PHY0%F_PREHYD)
        Z_YDMF_PHYS_SURF_GSP_RR_PT_T0 => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSP_RR%F_T_T0)
        Z_YDCPG_MISC_QSOL => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QSOL)
        Z_YDCPG_MISC_QS => GET_HOST_DATA_RDWR(YDCPG_MISC%F_QS)
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:1:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 576"
      END IF
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:1:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 577"
      ZFLU_QSATS => NULL()
      Z_YDCPG_PHY0_PREHYD => NULL()
      Z_YDMF_PHYS_SURF_GSP_RR_PT_T0 => NULL()
      Z_YDCPG_MISC_QSOL => NULL()
      Z_YDCPG_MISC_QS => NULL()
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:1:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 578"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:1', 1, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 579"
    
  END IF
  
  CALL APL_ARPEGE_ATMOSPHERE_UPDATE_PARALLEL(YDMF_PHYS_BASE_STATE, YDMF_PHYS_NEXT_STATE, YDGEOMETRY, YDCPG_BNDS, YDCPG_OPTS,  &
  & YDCPG_MISC, YDMF_PHYS, YDCPG_DYN0, YDMF_PHYS_SURF, YDVARS, YDMODEL, YDDDH, YL_ZDIFEXT, YL_ZDSA_CPS, YL_ZMSC_FRMQ,  &
  & YL_ZPFL_FEFB1, YL_ZPFL_FEFB2, YL_ZPFL_FEFB3, YL_ZPFL_FTKE, YL_ZPFL_FTKEI, YL_ZFCHQ)
  
  CALL MF_PHYS_FPL_PART2_PARALLEL(YDCPG_BNDS, YDCPG_OPTS, YL_ZPFL_FPLCH, YL_ZPFL_FPLSH, YDVARS%CPF%FT1, YDVARS%SPF%FT1,  &
  & YDMODEL)
  
  CALL MF_PHYS_TRANSFER_PARALLEL(YDCPG_BNDS, YDCPG_OPTS, YDVARS, YDMODEL%YRML_PHY_MF%YRPHY, YDMODEL%YRML_GCONF%YGFL)
  
  CALL APL_ARPEGE_SURFACE_UPDATE_PARALLEL(YDCPG_BNDS, YDCPG_OPTS, YDCPG_GPAR, YDMF_PHYS, YDMF_PHYS_SURF, YDMODEL,  &
  & YDCPG_OPTS%LCONFX, YDCPG_OPTS%ZDTPHY, YL_ZDSA_C1, YL_ZDSA_C2, YL_ZFLU_FEVI, YL_ZFLU_VEG)
  
  IF (YDMODEL%YRML_PHY_MF%YRPHY%LCVPGY) THEN
    
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:2', 0, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 580"
    IF (LPARALLELMETHOD('OPENMP', 'APL_ARPEGE_PARALLEL:2')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:2:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 581"
      Z_YDVARS_CVV_T0 => GET_HOST_DATA_RDWR(YDVARS%CVV%FT0)
      Z_YDVARS_CVV_T1 => GET_HOST_DATA_RDWR(YDVARS%CVV%FT1)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:2:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 582"
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:2:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 583"
      CALL YLCPG_BNDS%INIT(YDCPG_OPTS)
!$OMP PARALLEL DO PRIVATE( JBLK ) FIRSTPRIVATE( YLCPG_BNDS )
      DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
        CALL YLCPG_BNDS%UPDATE(JBLK)
        
        CALL MF_PHYS_CVV(YLCPG_BNDS, YDCPG_OPTS, Z_YDVARS_CVV_T0(:, :, JBLK), Z_YDVARS_CVV_T1(:, :, JBLK))
        
      END DO
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:2:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 584"
      IF (LSYNCHOST('APL_ARPEGE_PARALLEL:2')) THEN
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:2:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 585"
        Z_YDVARS_CVV_T0 => GET_HOST_DATA_RDWR(YDVARS%CVV%FT0)
        Z_YDVARS_CVV_T1 => GET_HOST_DATA_RDWR(YDVARS%CVV%FT1)
        IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:2:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 586"
      END IF
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:2:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 587"
      Z_YDVARS_CVV_T0 => NULL()
      Z_YDVARS_CVV_T1 => NULL()
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:2:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 588"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:2', 1, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 589"
    
  END IF
  
  ! Restore the initial value of some pseudo-historical surface buffers if relevant.
  
  IF (YDCPG_OPTS%LCONFX) THEN
    
    CALL MF_PHYS_SAVE_PHSURF_PART2_PARALLEL(YDCPG_BNDS, YDCPG_OPTS, YL_ZSAV_DDAL, YL_ZSAV_DDOM, YL_ZSAV_ENTCH, YL_ZSAV_FHPS,  &
    & YL_ZSAV_GZ0F, YL_ZSAV_GZ0HF, YL_ZSAV_HV, YL_ZSAV_PBLH, YL_ZSAV_QSH, YL_ZSAV_UDAL, YL_ZSAV_UDGRO, YL_ZSAV_UDOM,  &
    & YL_ZSAV_UNEBH, YDMF_PHYS_SURF%GSD_VF%F_Z0F, YDMF_PHYS_SURF%GSD_VH%F_PBLH, YDMF_PHYS_SURF%GSD_VH%F_QSH,  &
    & YDMF_PHYS_SURF%GSD_VH%F_SPSH, YDMF_PHYS_SURF%GSD_VK%F_UDGRO, YDMF_PHYS_SURF%GSD_VV%F_HV, YDMF_PHYS_SURF%GSD_VV%F_Z0H,  &
    & YDVARS%DAL%FT0, YDVARS%DOM%FT0, YDVARS%UAL%FT0, YDVARS%UEN%FT0, YDVARS%UNEBH%FT0, YDVARS%UOM%FT0, YDMODEL)
    
  END IF
  
  ! Store horizontal exchange coefficients (3D turbulence) to SL2 buffers
  
  
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:BAYRAD', 0, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 590"
  IF (LPARALLELMETHOD('OPENMP', 'APL_ARPEGE_PARALLEL:BAYRAD')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:BAYRAD:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 591"
    ZBAY_QSCONV => GET_HOST_DATA_RDWR(YL_ZBAY_QSCONV)
    ZBAY_QRCONV => GET_HOST_DATA_RDWR(YL_ZBAY_QRCONV)
    Z_YDVARS_SCONV_T1 => GET_HOST_DATA_RDWR(YDVARS%SCONV%FT1)
    Z_YDVARS_RCONV_T1 => GET_HOST_DATA_RDWR(YDVARS%RCONV%FT1)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:BAYRAD:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 592"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:BAYRAD:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 593"
    CALL YLCPG_BNDS%INIT(YDCPG_OPTS)
!$OMP PARALLEL DO PRIVATE( JBLK ) FIRSTPRIVATE( YLCPG_BNDS )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      CALL YLCPG_BNDS%UPDATE(JBLK)
      
      CALL MF_PHYS_BAYRAD(YLCPG_BNDS, YDCPG_OPTS, ZBAY_QRCONV(:, :, JBLK), ZBAY_QSCONV(:, :, JBLK),  &
      & Z_YDVARS_RCONV_T1(:, :, JBLK), Z_YDVARS_SCONV_T1(:, :, JBLK), YDMODEL)
      
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:BAYRAD:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 594"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:BAYRAD')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:BAYRAD:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 595"
      ZBAY_QSCONV => GET_HOST_DATA_RDWR(YL_ZBAY_QSCONV)
      ZBAY_QRCONV => GET_HOST_DATA_RDWR(YL_ZBAY_QRCONV)
      Z_YDVARS_SCONV_T1 => GET_HOST_DATA_RDWR(YDVARS%SCONV%FT1)
      Z_YDVARS_RCONV_T1 => GET_HOST_DATA_RDWR(YDVARS%RCONV%FT1)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:BAYRAD:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 596"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:BAYRAD:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 597"
    ZBAY_QSCONV => NULL()
    ZBAY_QRCONV => NULL()
    Z_YDVARS_SCONV_T1 => NULL()
    Z_YDVARS_RCONV_T1 => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:BAYRAD:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 598"
  ELSE IF (LPARALLELMETHOD('OPENMPSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:BAYRAD')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:BAYRAD:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 599"
    ZBAY_QSCONV => GET_HOST_DATA_RDWR(YL_ZBAY_QSCONV)
    ZBAY_QRCONV => GET_HOST_DATA_RDWR(YL_ZBAY_QRCONV)
    Z_YDVARS_SCONV_T1 => GET_HOST_DATA_RDWR(YDVARS%SCONV%FT1)
    Z_YDVARS_RCONV_T1 => GET_HOST_DATA_RDWR(YDVARS%RCONV%FT1)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:BAYRAD:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 600"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:BAYRAD:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 601"
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        CALL MF_PHYS_BAYRAD_OPENACC(YLCPG_BNDS, YDCPG_OPTS, ZBAY_QRCONV(:, :, JBLK), ZBAY_QSCONV(:, :, JBLK),  &
        & Z_YDVARS_RCONV_T1(:, :, JBLK), Z_YDVARS_SCONV_T1(:, :, JBLK), YDMODEL, YDSTACK=YLSTACK)
        
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:BAYRAD:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 602"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:BAYRAD')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:BAYRAD:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 603"
      ZBAY_QSCONV => GET_HOST_DATA_RDWR(YL_ZBAY_QSCONV)
      ZBAY_QRCONV => GET_HOST_DATA_RDWR(YL_ZBAY_QRCONV)
      Z_YDVARS_SCONV_T1 => GET_HOST_DATA_RDWR(YDVARS%SCONV%FT1)
      Z_YDVARS_RCONV_T1 => GET_HOST_DATA_RDWR(YDVARS%RCONV%FT1)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:BAYRAD:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 604"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:BAYRAD:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 605"
    ZBAY_QSCONV => NULL()
    ZBAY_QRCONV => NULL()
    Z_YDVARS_SCONV_T1 => NULL()
    Z_YDVARS_RCONV_T1 => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:BAYRAD:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 606"
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:BAYRAD')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:BAYRAD:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 607"
    ZBAY_QSCONV => GET_DEVICE_DATA_RDWR(YL_ZBAY_QSCONV)
    ZBAY_QRCONV => GET_DEVICE_DATA_RDWR(YL_ZBAY_QRCONV)
    Z_YDVARS_SCONV_T1 => GET_DEVICE_DATA_RDWR(YDVARS%SCONV%FT1)
    Z_YDVARS_RCONV_T1 => GET_DEVICE_DATA_RDWR(YDVARS%RCONV%FT1)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:BAYRAD:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 608"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:BAYRAD:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 609"
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZBAY_QSCONV, ZBAY_QRCONV, Z_YDVARS_SCONV_T1,  &
!$ACC & Z_YDVARS_RCONV_T1 ) PRIVATE( JBLK ) VECTOR_LENGTH( YDCPG_OPTS%KLON )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK )
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        CALL MF_PHYS_BAYRAD_OPENACC(YLCPG_BNDS, YDCPG_OPTS, ZBAY_QRCONV(:, :, JBLK), ZBAY_QSCONV(:, :, JBLK),  &
        & Z_YDVARS_RCONV_T1(:, :, JBLK), Z_YDVARS_SCONV_T1(:, :, JBLK), YDMODEL, YDSTACK=YLSTACK)
        
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:BAYRAD:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 610"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:BAYRAD')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:BAYRAD:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 611"
      ZBAY_QSCONV => GET_HOST_DATA_RDWR(YL_ZBAY_QSCONV)
      ZBAY_QRCONV => GET_HOST_DATA_RDWR(YL_ZBAY_QRCONV)
      Z_YDVARS_SCONV_T1 => GET_HOST_DATA_RDWR(YDVARS%SCONV%FT1)
      Z_YDVARS_RCONV_T1 => GET_HOST_DATA_RDWR(YDVARS%RCONV%FT1)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:BAYRAD:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 612"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:BAYRAD:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 613"
    ZBAY_QSCONV => NULL()
    ZBAY_QRCONV => NULL()
    Z_YDVARS_SCONV_T1 => NULL()
    Z_YDVARS_RCONV_T1 => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:BAYRAD:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 614"
  END IF
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:BAYRAD', 1, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 615"
  
  
  IF (YDMODEL%YRML_PHY_EC%YREPHY%LEMWAVE) THEN
    CALL APL_ARPEGE_MWAVE_PARALLEL(YDCPG_MISC, YDMF_PHYS, YDCPG_BNDS, YDCPG_OPTS, YDPHYSMWAVE)
  END IF
  
  
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:EDR', 0, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 616"
  IF (LPARALLELMETHOD('OPENMP', 'APL_ARPEGE_PARALLEL:EDR')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:EDR:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 617"
    Z_YDMF_PHYS_OUT_EDR => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_EDR)
    Z_YDMF_PHYS_SURF_GSD_DI_PXEDR => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_DI%F_XEDR)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:EDR:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 618"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:EDR:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 619"
    CALL YLCPG_BNDS%INIT(YDCPG_OPTS)
!$OMP PARALLEL DO PRIVATE( JBLK, JLEV ) FIRSTPRIVATE( YLCPG_BNDS )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      CALL YLCPG_BNDS%UPDATE(JBLK)
      
      IF (YDMODEL%YRML_PHY_MF%YRPHY%LEDR) THEN
        DO JLEV=1,YDCPG_OPTS%YRSURF_DIMS%YSD_DID%NLEVS
          Z_YDMF_PHYS_SURF_GSD_DI_PXEDR(:, JLEV, JBLK) = Z_YDMF_PHYS_OUT_EDR(:, JLEV, JBLK)
        END DO
      END IF
      
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:EDR:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 620"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:EDR')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:EDR:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 621"
      Z_YDMF_PHYS_OUT_EDR => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_EDR)
      Z_YDMF_PHYS_SURF_GSD_DI_PXEDR => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_DI%F_XEDR)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:EDR:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 622"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:EDR:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 623"
    Z_YDMF_PHYS_OUT_EDR => NULL()
    Z_YDMF_PHYS_SURF_GSD_DI_PXEDR => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:EDR:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 624"
  ELSE IF (LPARALLELMETHOD('OPENMPSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:EDR')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:EDR:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 625"
    Z_YDMF_PHYS_OUT_EDR => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_EDR)
    Z_YDMF_PHYS_SURF_GSD_DI_PXEDR => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_DI%F_XEDR)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:EDR:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 626"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:EDR:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 627"
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK, JLEV )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        IF (YDMODEL%YRML_PHY_MF%YRPHY%LEDR) THEN
          DO JLEV=1,YDCPG_OPTS%YRSURF_DIMS%YSD_DID%NLEVS
            Z_YDMF_PHYS_SURF_GSD_DI_PXEDR(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_EDR(JLON, JLEV, JBLK)
          END DO
        END IF
        
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:EDR:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 628"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:EDR')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:EDR:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 629"
      Z_YDMF_PHYS_OUT_EDR => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_EDR)
      Z_YDMF_PHYS_SURF_GSD_DI_PXEDR => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_DI%F_XEDR)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:EDR:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 630"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:EDR:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 631"
    Z_YDMF_PHYS_OUT_EDR => NULL()
    Z_YDMF_PHYS_SURF_GSD_DI_PXEDR => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:EDR:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 632"
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:EDR')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:EDR:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 633"
    Z_YDMF_PHYS_OUT_EDR => GET_DEVICE_DATA_RDWR(YDMF_PHYS%OUT%F_EDR)
    Z_YDMF_PHYS_SURF_GSD_DI_PXEDR => GET_DEVICE_DATA_RDWR(YDMF_PHYS_SURF%GSD_DI%F_XEDR)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:EDR:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 634"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:EDR:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 635"
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, Z_YDMF_PHYS_OUT_EDR, Z_YDMF_PHYS_SURF_GSD_DI_PXEDR ) PRIVATE(  &
!$ACC & JBLK ) VECTOR_LENGTH( YDCPG_OPTS%KLON )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK, JLEV )
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        IF (YDMODEL%YRML_PHY_MF%YRPHY%LEDR) THEN
          DO JLEV=1,YDCPG_OPTS%YRSURF_DIMS%YSD_DID%NLEVS
            Z_YDMF_PHYS_SURF_GSD_DI_PXEDR(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_EDR(JLON, JLEV, JBLK)
          END DO
        END IF
        
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:EDR:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 636"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:EDR')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:EDR:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 637"
      Z_YDMF_PHYS_OUT_EDR => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_EDR)
      Z_YDMF_PHYS_SURF_GSD_DI_PXEDR => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_DI%F_XEDR)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:EDR:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 638"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:EDR:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 639"
    Z_YDMF_PHYS_OUT_EDR => NULL()
    Z_YDMF_PHYS_SURF_GSD_DI_PXEDR => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:EDR:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 640"
  END IF
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:EDR', 1, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 641"
  
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PRECIPS', 0, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 642"
  IF (LPARALLELMETHOD('OPENMP', 'APL_ARPEGE_PARALLEL:PRECIPS')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PRECIPS:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 643"
    ZPRC_DPRECIPS => GET_HOST_DATA_RDWR(YL_ZPRC_DPRECIPS)
    ZPRC_DPRECIPS2 => GET_HOST_DATA_RDWR(YL_ZPRC_DPRECIPS2)
    Z_YDMF_PHYS_SURF_GSD_XP_PPRECIP => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_XP%F_PRECIP)
    Z_YDMF_PHYS_SURF_GSD_XP2_PPRECIP2 => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_XP2%F_PRECIP2)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PRECIPS:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 644"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PRECIPS:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 645"
    CALL YLCPG_BNDS%INIT(YDCPG_OPTS)
!$OMP PARALLEL DO PRIVATE( JBLK ) FIRSTPRIVATE( YLCPG_BNDS )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      CALL YLCPG_BNDS%UPDATE(JBLK)
      
      CALL MF_PHYS_PRECIPS(YLCPG_BNDS, YDCPG_OPTS, ZPRC_DPRECIPS(:, :, JBLK), ZPRC_DPRECIPS2(:, :, JBLK),  &
      & Z_YDMF_PHYS_SURF_GSD_XP_PPRECIP(:, :, JBLK), Z_YDMF_PHYS_SURF_GSD_XP2_PPRECIP2(:, :, JBLK), YDMODEL)
      
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PRECIPS:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 646"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:PRECIPS')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PRECIPS:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 647"
      ZPRC_DPRECIPS => GET_HOST_DATA_RDWR(YL_ZPRC_DPRECIPS)
      ZPRC_DPRECIPS2 => GET_HOST_DATA_RDWR(YL_ZPRC_DPRECIPS2)
      Z_YDMF_PHYS_SURF_GSD_XP_PPRECIP => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_XP%F_PRECIP)
      Z_YDMF_PHYS_SURF_GSD_XP2_PPRECIP2 => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_XP2%F_PRECIP2)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PRECIPS:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 648"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PRECIPS:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 649"
    ZPRC_DPRECIPS => NULL()
    ZPRC_DPRECIPS2 => NULL()
    Z_YDMF_PHYS_SURF_GSD_XP_PPRECIP => NULL()
    Z_YDMF_PHYS_SURF_GSD_XP2_PPRECIP2 => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PRECIPS:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 650"
  ELSE IF (LPARALLELMETHOD('OPENMPSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:PRECIPS')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PRECIPS:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 651"
    ZPRC_DPRECIPS => GET_HOST_DATA_RDWR(YL_ZPRC_DPRECIPS)
    ZPRC_DPRECIPS2 => GET_HOST_DATA_RDWR(YL_ZPRC_DPRECIPS2)
    Z_YDMF_PHYS_SURF_GSD_XP_PPRECIP => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_XP%F_PRECIP)
    Z_YDMF_PHYS_SURF_GSD_XP2_PPRECIP2 => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_XP2%F_PRECIP2)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PRECIPS:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 652"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PRECIPS:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 653"
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        CALL MF_PHYS_PRECIPS_OPENACC(YLCPG_BNDS, YDCPG_OPTS, ZPRC_DPRECIPS(:, :, JBLK), ZPRC_DPRECIPS2(:, :, JBLK),  &
        & Z_YDMF_PHYS_SURF_GSD_XP_PPRECIP(:, :, JBLK), Z_YDMF_PHYS_SURF_GSD_XP2_PPRECIP2(:, :, JBLK), YDMODEL,  &
        & YDSTACK=YLSTACK)
        
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PRECIPS:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 654"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:PRECIPS')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PRECIPS:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 655"
      ZPRC_DPRECIPS => GET_HOST_DATA_RDWR(YL_ZPRC_DPRECIPS)
      ZPRC_DPRECIPS2 => GET_HOST_DATA_RDWR(YL_ZPRC_DPRECIPS2)
      Z_YDMF_PHYS_SURF_GSD_XP_PPRECIP => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_XP%F_PRECIP)
      Z_YDMF_PHYS_SURF_GSD_XP2_PPRECIP2 => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_XP2%F_PRECIP2)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PRECIPS:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 656"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PRECIPS:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 657"
    ZPRC_DPRECIPS => NULL()
    ZPRC_DPRECIPS2 => NULL()
    Z_YDMF_PHYS_SURF_GSD_XP_PPRECIP => NULL()
    Z_YDMF_PHYS_SURF_GSD_XP2_PPRECIP2 => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PRECIPS:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 658"
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:PRECIPS')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PRECIPS:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 659"
    ZPRC_DPRECIPS => GET_DEVICE_DATA_RDWR(YL_ZPRC_DPRECIPS)
    ZPRC_DPRECIPS2 => GET_DEVICE_DATA_RDWR(YL_ZPRC_DPRECIPS2)
    Z_YDMF_PHYS_SURF_GSD_XP_PPRECIP => GET_DEVICE_DATA_RDWR(YDMF_PHYS_SURF%GSD_XP%F_PRECIP)
    Z_YDMF_PHYS_SURF_GSD_XP2_PPRECIP2 => GET_DEVICE_DATA_RDWR(YDMF_PHYS_SURF%GSD_XP2%F_PRECIP2)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PRECIPS:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 660"
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PRECIPS:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 661"
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZPRC_DPRECIPS, ZPRC_DPRECIPS2,  &
!$ACC & Z_YDMF_PHYS_SURF_GSD_XP_PPRECIP, Z_YDMF_PHYS_SURF_GSD_XP2_PPRECIP2 ) PRIVATE( JBLK ) VECTOR_LENGTH( YDCPG_OPTS%KLON )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK )
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
        YLCPG_BNDS%KFDIA = JLON
        YLSTACK%L8 = stack_l8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U8 = stack_u8(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%L4 = stack_l4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        YLSTACK%U4 = stack_u4(YSTACK, JBLK - YDCPG_OPTS%JBLKMIN + 1, YDCPG_OPTS%KGPBLKS)
        
        CALL MF_PHYS_PRECIPS_OPENACC(YLCPG_BNDS, YDCPG_OPTS, ZPRC_DPRECIPS(:, :, JBLK), ZPRC_DPRECIPS2(:, :, JBLK),  &
        & Z_YDMF_PHYS_SURF_GSD_XP_PPRECIP(:, :, JBLK), Z_YDMF_PHYS_SURF_GSD_XP2_PPRECIP2(:, :, JBLK), YDMODEL,  &
        & YDSTACK=YLSTACK)
        
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PRECIPS:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
PRINT *,"apl_arpege_parallel_dbg ::: 662"
    IF (LSYNCHOST('APL_ARPEGE_PARALLEL:PRECIPS')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PRECIPS:SYNCHOST', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 663"
      ZPRC_DPRECIPS => GET_HOST_DATA_RDWR(YL_ZPRC_DPRECIPS)
      ZPRC_DPRECIPS2 => GET_HOST_DATA_RDWR(YL_ZPRC_DPRECIPS2)
      Z_YDMF_PHYS_SURF_GSD_XP_PPRECIP => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_XP%F_PRECIP)
      Z_YDMF_PHYS_SURF_GSD_XP2_PPRECIP2 => GET_HOST_DATA_RDWR(YDMF_PHYS_SURF%GSD_XP2%F_PRECIP2)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PRECIPS:SYNCHOST', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 664"
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PRECIPS:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 665"
    ZPRC_DPRECIPS => NULL()
    ZPRC_DPRECIPS2 => NULL()
    Z_YDMF_PHYS_SURF_GSD_XP_PPRECIP => NULL()
    Z_YDMF_PHYS_SURF_GSD_XP2_PPRECIP2 => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PRECIPS:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 666"
  END IF
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PRECIPS', 1, ZHOOK_HANDLE_PARALLEL)
PRINT *,"apl_arpege_parallel_dbg ::: 667"
  
  
  IF (LHOOK) CALL DR_HOOK('DELETE_TEMPORARIES', 0, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 668"
  
  IF (ASSOCIATED(YL_ZRDG_MU0)) CALL FIELD_DELETE(YL_ZRDG_MU0)
  IF (ASSOCIATED(YL_ZRDG_CVGQ)) CALL FIELD_DELETE(YL_ZRDG_CVGQ)
  IF (ASSOCIATED(YL_ZRDG_MU0M)) CALL FIELD_DELETE(YL_ZRDG_MU0M)
  IF (ASSOCIATED(YL_ZRDG_MU0LU)) CALL FIELD_DELETE(YL_ZRDG_MU0LU)
  IF (ASSOCIATED(YL_ZRDG_MU0N)) CALL FIELD_DELETE(YL_ZRDG_MU0N)
  IF (ASSOCIATED(YL_ZRDG_CVGT)) CALL FIELD_DELETE(YL_ZRDG_CVGT)
  IF (ASSOCIATED(YL_ZRDG_SOLO)) CALL FIELD_DELETE(YL_ZRDG_SOLO)
  IF (ASSOCIATED(YL_ZFLU_QSATS)) CALL FIELD_DELETE(YL_ZFLU_QSATS)
  IF (ASSOCIATED(YL_ZRDG_LCVQ)) CALL FIELD_DELETE(YL_ZRDG_LCVQ)
  IF (ASSOCIATED(YL_ZQCO2)) CALL FIELD_DELETE(YL_ZQCO2)
  IF (ASSOCIATED(YL_ZDSA_CPS)) CALL FIELD_DELETE(YL_ZDSA_CPS)
  IF (ASSOCIATED(YL_ZDSA_C2)) CALL FIELD_DELETE(YL_ZDSA_C2)
  IF (ASSOCIATED(YL_ZDSA_LHS)) CALL FIELD_DELETE(YL_ZDSA_LHS)
  IF (ASSOCIATED(YL_ZFLU_EMIS)) CALL FIELD_DELETE(YL_ZFLU_EMIS)
  IF (ASSOCIATED(YL_ZPFL_FEFB1)) CALL FIELD_DELETE(YL_ZPFL_FEFB1)
  IF (ASSOCIATED(YL_ZFLU_QSAT)) CALL FIELD_DELETE(YL_ZFLU_QSAT)
  IF (ASSOCIATED(YL_ZFLU_CH)) CALL FIELD_DELETE(YL_ZFLU_CH)
  IF (ASSOCIATED(YL_ZFLU_VEG)) CALL FIELD_DELETE(YL_ZFLU_VEG)
  IF (ASSOCIATED(YL_ZPFL_FEFB3)) CALL FIELD_DELETE(YL_ZPFL_FEFB3)
  IF (ASSOCIATED(YL_ZDSA_C1)) CALL FIELD_DELETE(YL_ZDSA_C1)
  IF (ASSOCIATED(YL_ZMSC_QW)) CALL FIELD_DELETE(YL_ZMSC_QW)
  IF (ASSOCIATED(YL_ZPFL_FTKEI)) CALL FIELD_DELETE(YL_ZPFL_FTKEI)
  IF (ASSOCIATED(YL_ZFLU_NEIJ)) CALL FIELD_DELETE(YL_ZFLU_NEIJ)
  IF (ASSOCIATED(YL_ZMSC_LH)) CALL FIELD_DELETE(YL_ZMSC_LH)
  IF (ASSOCIATED(YL_ZDSA_RS)) CALL FIELD_DELETE(YL_ZDSA_RS)
  IF (ASSOCIATED(YL_ZMSC_FRMQ)) CALL FIELD_DELETE(YL_ZMSC_FRMQ)
  IF (ASSOCIATED(YL_ZMSC_LSCPE)) CALL FIELD_DELETE(YL_ZMSC_LSCPE)
  IF (ASSOCIATED(YL_ZMSC_TW)) CALL FIELD_DELETE(YL_ZMSC_TW)
  IF (ASSOCIATED(YL_ZFLU_CHN)) CALL FIELD_DELETE(YL_ZFLU_CHN)
  IF (ASSOCIATED(YL_ZPFL_FTKE)) CALL FIELD_DELETE(YL_ZPFL_FTKE)
  IF (ASSOCIATED(YL_ZFLU_FEVI)) CALL FIELD_DELETE(YL_ZFLU_FEVI)
  IF (ASSOCIATED(YL_ZFLU_CDN)) CALL FIELD_DELETE(YL_ZFLU_CDN)
  IF (ASSOCIATED(YL_ZPFL_FEFB2)) CALL FIELD_DELETE(YL_ZPFL_FEFB2)
  IF (ASSOCIATED(YL_ZFLU_CD)) CALL FIELD_DELETE(YL_ZFLU_CD)
  IF (ASSOCIATED(YL_IMOC_CLPH)) CALL FIELD_DELETE(YL_IMOC_CLPH)
  IF (ASSOCIATED(YL_ZFCHQ)) CALL FIELD_DELETE(YL_ZFCHQ)
  IF (ASSOCIATED(YL_ZQV)) CALL FIELD_DELETE(YL_ZQV)
  IF (ASSOCIATED(YL_ZGEOSLC)) CALL FIELD_DELETE(YL_ZGEOSLC)
  IF (ASSOCIATED(YL_ZC3)) CALL FIELD_DELETE(YL_ZC3)
  IF (ASSOCIATED(YL_ZWSEQ)) CALL FIELD_DELETE(YL_ZWSEQ)
  IF (ASSOCIATED(YL_ZCG)) CALL FIELD_DELETE(YL_ZCG)
  IF (ASSOCIATED(YL_ZWFC)) CALL FIELD_DELETE(YL_ZWFC)
  IF (ASSOCIATED(YL_ZNEIJV)) CALL FIELD_DELETE(YL_ZNEIJV)
  IF (ASSOCIATED(YL_ZWWILT)) CALL FIELD_DELETE(YL_ZWWILT)
  IF (ASSOCIATED(YL_ZNEIJG)) CALL FIELD_DELETE(YL_ZNEIJG)
  IF (ASSOCIATED(YL_ZCN)) CALL FIELD_DELETE(YL_ZCN)
  IF (ASSOCIATED(YL_ZWSMX)) CALL FIELD_DELETE(YL_ZWSMX)
  IF (ASSOCIATED(YL_ZWPMX)) CALL FIELD_DELETE(YL_ZWPMX)
  IF (ASSOCIATED(YL_ZDPHIV)) CALL FIELD_DELETE(YL_ZDPHIV)
  IF (ASSOCIATED(YL_ZDPHIT)) CALL FIELD_DELETE(YL_ZDPHIT)
  IF (ASSOCIATED(YL_ZGWDCS)) CALL FIELD_DELETE(YL_ZGWDCS)
  IF (ASSOCIATED(YL_ZRTI)) CALL FIELD_DELETE(YL_ZRTI)
  IF (ASSOCIATED(YL_ZNBVNO)) CALL FIELD_DELETE(YL_ZNBVNO)
  IF (ASSOCIATED(YL_ZDPHI)) CALL FIELD_DELETE(YL_ZDPHI)
  IF (ASSOCIATED(YL_ZPRS)) CALL FIELD_DELETE(YL_ZPRS)
  IF (ASSOCIATED(YL_ZPCLS)) CALL FIELD_DELETE(YL_ZPCLS)
  IF (ASSOCIATED(YL_ZMRIPP)) CALL FIELD_DELETE(YL_ZMRIPP)
  IF (ASSOCIATED(YL_ZHQ)) CALL FIELD_DELETE(YL_ZHQ)
  IF (ASSOCIATED(YL_ZDQSTS)) CALL FIELD_DELETE(YL_ZDQSTS)
  IF (ASSOCIATED(YL_ZCHROV)) CALL FIELD_DELETE(YL_ZCHROV)
  IF (ASSOCIATED(YL_ZSTAB)) CALL FIELD_DELETE(YL_ZSTAB)
  IF (ASSOCIATED(YL_ZHU)) CALL FIELD_DELETE(YL_ZHU)
  IF (ASSOCIATED(YL_ZXDROV)) CALL FIELD_DELETE(YL_ZXDROV)
  IF (ASSOCIATED(YL_ZXHROV)) CALL FIELD_DELETE(YL_ZXHROV)
  IF (ASSOCIATED(YL_ZCDROV)) CALL FIELD_DELETE(YL_ZCDROV)
  IF (ASSOCIATED(YL_ZPFL_FPLSH)) CALL FIELD_DELETE(YL_ZPFL_FPLSH)
  IF (ASSOCIATED(YL_ZPFL_FPLCH)) CALL FIELD_DELETE(YL_ZPFL_FPLCH)
  IF (ASSOCIATED(YL_ZUGST)) CALL FIELD_DELETE(YL_ZUGST)
  IF (ASSOCIATED(YL_ZCP)) CALL FIELD_DELETE(YL_ZCP)
  IF (ASSOCIATED(YL_ZBLH)) CALL FIELD_DELETE(YL_ZBLH)
  IF (ASSOCIATED(YL_ZTHETAVS)) CALL FIELD_DELETE(YL_ZTHETAVS)
  IF (ASSOCIATED(YL_ZTHETAV)) CALL FIELD_DELETE(YL_ZTHETAV)
  IF (ASSOCIATED(YL_ZVGST)) CALL FIELD_DELETE(YL_ZVGST)
  IF (ASSOCIATED(YL_ZEDMFQ)) CALL FIELD_DELETE(YL_ZEDMFQ)
  IF (ASSOCIATED(YL_ZEDMFU)) CALL FIELD_DELETE(YL_ZEDMFU)
  IF (ASSOCIATED(YL_ZDIFCVPPQ)) CALL FIELD_DELETE(YL_ZDIFCVPPQ)
  IF (ASSOCIATED(YL_ZDIFCVPPV)) CALL FIELD_DELETE(YL_ZDIFCVPPV)
  IF (ASSOCIATED(YL_ZEDMFV)) CALL FIELD_DELETE(YL_ZEDMFV)
  IF (ASSOCIATED(YL_ZDIFCVPPS)) CALL FIELD_DELETE(YL_ZDIFCVPPS)
  IF (ASSOCIATED(YL_ZDIFCVPPU)) CALL FIELD_DELETE(YL_ZDIFCVPPU)
  IF (ASSOCIATED(YL_ZEDMFS)) CALL FIELD_DELETE(YL_ZEDMFS)
  IF (ASSOCIATED(YL_ZQLIS)) CALL FIELD_DELETE(YL_ZQLIS)
  IF (ASSOCIATED(YL_ZTRAJGWD)) CALL FIELD_DELETE(YL_ZTRAJGWD)
  IF (ASSOCIATED(YL_ZNEBS)) CALL FIELD_DELETE(YL_ZNEBS)
  IF (ASSOCIATED(YL_ZECTCLS)) CALL FIELD_DELETE(YL_ZECTCLS)
  IF (ASSOCIATED(YL_ZKCLS)) CALL FIELD_DELETE(YL_ZKCLS)
  IF (ASSOCIATED(YL_ZKUROV2)) CALL FIELD_DELETE(YL_ZKUROV2)
  IF (ASSOCIATED(YL_ZTKE1)) CALL FIELD_DELETE(YL_ZTKE1)
  IF (ASSOCIATED(YL_ZUSLE)) CALL FIELD_DELETE(YL_ZUSLE)
  IF (ASSOCIATED(YL_ZPRODTH)) CALL FIELD_DELETE(YL_ZPRODTH)
  IF (ASSOCIATED(YL_ZQI)) CALL FIELD_DELETE(YL_ZQI)
  IF (ASSOCIATED(YL_ZSEDIQI)) CALL FIELD_DELETE(YL_ZSEDIQI)
  IF (ASSOCIATED(YL_ZFPCOR)) CALL FIELD_DELETE(YL_ZFPCOR)
  IF (ASSOCIATED(YL_ZFLX_LOTT_GWV)) CALL FIELD_DELETE(YL_ZFLX_LOTT_GWV)
  IF (ASSOCIATED(YL_ZQL)) CALL FIELD_DELETE(YL_ZQL)
  IF (ASSOCIATED(YL_ZSEDIQL)) CALL FIELD_DELETE(YL_ZSEDIQL)
  IF (ASSOCIATED(YL_ZFPLSN)) CALL FIELD_DELETE(YL_ZFPLSN)
  IF (ASSOCIATED(YL_ZFLX_LOTT_GWU)) CALL FIELD_DELETE(YL_ZFLX_LOTT_GWU)
  IF (ASSOCIATED(YL_ZFPLSL)) CALL FIELD_DELETE(YL_ZFPLSL)
  IF (ASSOCIATED(YL_ZDCAPE)) CALL FIELD_DELETE(YL_ZDCAPE)
  IF (ASSOCIATED(YL_ZQR)) CALL FIELD_DELETE(YL_ZQR)
  IF (ASSOCIATED(YL_ZQS)) CALL FIELD_DELETE(YL_ZQS)
  IF (ASSOCIATED(YL_ZQGM)) CALL FIELD_DELETE(YL_ZQGM)
  IF (ASSOCIATED(YL_ZBAY_QSCONV)) CALL FIELD_DELETE(YL_ZBAY_QSCONV)
  IF (ASSOCIATED(YL_ZBAY_QRCONV)) CALL FIELD_DELETE(YL_ZBAY_QRCONV)
  IF (ASSOCIATED(YL_ZDE2MR)) CALL FIELD_DELETE(YL_ZDE2MR)
  IF (ASSOCIATED(YL_ZPRC_DPRECIPS)) CALL FIELD_DELETE(YL_ZPRC_DPRECIPS)
  IF (ASSOCIATED(YL_ZPRC_DPRECIPS2)) CALL FIELD_DELETE(YL_ZPRC_DPRECIPS2)
  IF (ASSOCIATED(YL_ZSAV_QSH)) CALL FIELD_DELETE(YL_ZSAV_QSH)
  IF (ASSOCIATED(YL_ZSAV_DDAL)) CALL FIELD_DELETE(YL_ZSAV_DDAL)
  IF (ASSOCIATED(YL_ZSAV_UDAL)) CALL FIELD_DELETE(YL_ZSAV_UDAL)
  IF (ASSOCIATED(YL_ZSAV_ENTCH)) CALL FIELD_DELETE(YL_ZSAV_ENTCH)
  IF (ASSOCIATED(YL_ZSAV_FHPS)) CALL FIELD_DELETE(YL_ZSAV_FHPS)
  IF (ASSOCIATED(YL_ZSAV_HV)) CALL FIELD_DELETE(YL_ZSAV_HV)
  IF (ASSOCIATED(YL_ZSAV_PBLH)) CALL FIELD_DELETE(YL_ZSAV_PBLH)
  IF (ASSOCIATED(YL_ZSAV_GZ0HF)) CALL FIELD_DELETE(YL_ZSAV_GZ0HF)
  IF (ASSOCIATED(YL_ZSAV_DDOM)) CALL FIELD_DELETE(YL_ZSAV_DDOM)
  IF (ASSOCIATED(YL_ZSAV_UDOM)) CALL FIELD_DELETE(YL_ZSAV_UDOM)
  IF (ASSOCIATED(YL_ZSAV_GZ0F)) CALL FIELD_DELETE(YL_ZSAV_GZ0F)
  IF (ASSOCIATED(YL_ZSAV_UNEBH)) CALL FIELD_DELETE(YL_ZSAV_UNEBH)
  IF (ASSOCIATED(YL_ZSAV_UDGRO)) CALL FIELD_DELETE(YL_ZSAV_UDGRO)
  IF (ASSOCIATED(YL_ZNEB_CVPP)) CALL FIELD_DELETE(YL_ZNEB_CVPP)
  IF (ASSOCIATED(YL_ZXUROV)) CALL FIELD_DELETE(YL_ZXUROV)
  IF (ASSOCIATED(YL_ZCFBTH)) CALL FIELD_DELETE(YL_ZCFBTH)
  IF (ASSOCIATED(YL_ZKUROV)) CALL FIELD_DELETE(YL_ZKUROV)
  IF (ASSOCIATED(YL_ZSC_FEVI)) CALL FIELD_DELETE(YL_ZSC_FEVI)
  IF (ASSOCIATED(YL_INLAB_CVPP)) CALL FIELD_DELETE(YL_INLAB_CVPP)
  IF (ASSOCIATED(YL_ZKTROV)) CALL FIELD_DELETE(YL_ZKTROV)
  IF (ASSOCIATED(YL_ZCFBV)) CALL FIELD_DELETE(YL_ZCFBV)
  IF (ASSOCIATED(YL_ZNEBCH)) CALL FIELD_DELETE(YL_ZNEBCH)
  IF (ASSOCIATED(YL_ZQC_DET_PCMT)) CALL FIELD_DELETE(YL_ZQC_DET_PCMT)
  IF (ASSOCIATED(YL_ZQO3)) CALL FIELD_DELETE(YL_ZQO3)
  IF (ASSOCIATED(YL_ZXTROV)) CALL FIELD_DELETE(YL_ZXTROV)
  IF (ASSOCIATED(YL_ZPRODTH_CVPP)) CALL FIELD_DELETE(YL_ZPRODTH_CVPP)
  IF (ASSOCIATED(YL_ZQLI_CVP)) CALL FIELD_DELETE(YL_ZQLI_CVP)
  IF (ASSOCIATED(YL_ZXQRO)) CALL FIELD_DELETE(YL_ZXQRO)
  IF (ASSOCIATED(YL_ZTENQVA)) CALL FIELD_DELETE(YL_ZTENQVA)
  IF (ASSOCIATED(YL_ZTENHA)) CALL FIELD_DELETE(YL_ZTENHA)
  IF (ASSOCIATED(YL_ZKQROV)) CALL FIELD_DELETE(YL_ZKQROV)
  IF (ASSOCIATED(YL_ZTRSODIF)) CALL FIELD_DELETE(YL_ZTRSODIF)
  IF (ASSOCIATED(YL_ZNEBS0)) CALL FIELD_DELETE(YL_ZNEBS0)
  IF (ASSOCIATED(YL_ZQICE)) CALL FIELD_DELETE(YL_ZQICE)
  IF (ASSOCIATED(YL_ZXTRO)) CALL FIELD_DELETE(YL_ZXTRO)
  IF (ASSOCIATED(YL_ZTRSODIR)) CALL FIELD_DELETE(YL_ZTRSODIR)
  IF (ASSOCIATED(YL_ZQLIS0)) CALL FIELD_DELETE(YL_ZQLIS0)
  IF (ASSOCIATED(YL_ZSFSWDIF)) CALL FIELD_DELETE(YL_ZSFSWDIF)
  IF (ASSOCIATED(YL_ZAERINDS)) CALL FIELD_DELETE(YL_ZAERINDS)
  IF (ASSOCIATED(YL_ZXURO)) CALL FIELD_DELETE(YL_ZXURO)
  IF (ASSOCIATED(YL_ZNEBDIFF)) CALL FIELD_DELETE(YL_ZNEBDIFF)
  IF (ASSOCIATED(YL_ZCFATH)) CALL FIELD_DELETE(YL_ZCFATH)
  IF (ASSOCIATED(YL_ZCFBU)) CALL FIELD_DELETE(YL_ZCFBU)
  IF (ASSOCIATED(YL_ZDIFWQ)) CALL FIELD_DELETE(YL_ZDIFWQ)
  IF (ASSOCIATED(YL_ZAER)) CALL FIELD_DELETE(YL_ZAER)
  IF (ASSOCIATED(YL_ZPOID)) CALL FIELD_DELETE(YL_ZPOID)
  IF (ASSOCIATED(YL_ZMF_UP)) CALL FIELD_DELETE(YL_ZMF_UP)
  IF (ASSOCIATED(YL_ZCONDCVPPI)) CALL FIELD_DELETE(YL_ZCONDCVPPI)
  IF (ASSOCIATED(YL_ZSC_FCLN)) CALL FIELD_DELETE(YL_ZSC_FCLN)
  IF (ASSOCIATED(YL_ZCOEFA)) CALL FIELD_DELETE(YL_ZCOEFA)
  IF (ASSOCIATED(YL_ZQIC)) CALL FIELD_DELETE(YL_ZQIC)
  IF (ASSOCIATED(YL_ZQLC)) CALL FIELD_DELETE(YL_ZQLC)
  IF (ASSOCIATED(YL_ZSFSWDIR)) CALL FIELD_DELETE(YL_ZSFSWDIR)
  IF (ASSOCIATED(YL_ZUNEBH)) CALL FIELD_DELETE(YL_ZUNEBH)
  IF (ASSOCIATED(YL_ZCTRSO)) CALL FIELD_DELETE(YL_ZCTRSO)
  IF (ASSOCIATED(YL_ZCFAU)) CALL FIELD_DELETE(YL_ZCFAU)
  IF (ASSOCIATED(YL_ZAIPCMT)) CALL FIELD_DELETE(YL_ZAIPCMT)
  IF (ASSOCIATED(YL_ZSC_FCLL)) CALL FIELD_DELETE(YL_ZSC_FCLL)
  IF (ASSOCIATED(YL_ZDIFWS)) CALL FIELD_DELETE(YL_ZDIFWS)
  IF (ASSOCIATED(YL_ZSUDU)) CALL FIELD_DELETE(YL_ZSUDU)
  IF (ASSOCIATED(YL_ZALPHA1)) CALL FIELD_DELETE(YL_ZALPHA1)
  IF (ASSOCIATED(YL_ZTRSOD)) CALL FIELD_DELETE(YL_ZTRSOD)
  IF (ASSOCIATED(YL_ZALBD)) CALL FIELD_DELETE(YL_ZALBD)
  IF (ASSOCIATED(YL_ZNEBC0)) CALL FIELD_DELETE(YL_ZNEBC0)
  IF (ASSOCIATED(YL_ZCEMTR)) CALL FIELD_DELETE(YL_ZCEMTR)
  IF (ASSOCIATED(YL_ZALBP)) CALL FIELD_DELETE(YL_ZALBP)
  IF (ASSOCIATED(YL_ZLVT)) CALL FIELD_DELETE(YL_ZLVT)
  IF (ASSOCIATED(YL_ZQLI_CVPP)) CALL FIELD_DELETE(YL_ZQLI_CVPP)
  IF (ASSOCIATED(YL_ZTENT)) CALL FIELD_DELETE(YL_ZTENT)
  IF (ASSOCIATED(YL_ZKQLROV)) CALL FIELD_DELETE(YL_ZKQLROV)
  IF (ASSOCIATED(YL_ZSC_FEVN)) CALL FIELD_DELETE(YL_ZSC_FEVN)
  IF (ASSOCIATED(YL_ZCONDCVPPL)) CALL FIELD_DELETE(YL_ZCONDCVPPL)
  IF (ASSOCIATED(YL_ZDECRD)) CALL FIELD_DELETE(YL_ZDECRD)
  IF (ASSOCIATED(YL_ZSGROUPEL)) CALL FIELD_DELETE(YL_ZSGROUPEL)
  IF (ASSOCIATED(YL_ZTSN)) CALL FIELD_DELETE(YL_ZTSN)
  IF (ASSOCIATED(YL_ZSSNOW)) CALL FIELD_DELETE(YL_ZSSNOW)
  IF (ASSOCIATED(YL_ZSRAIN)) CALL FIELD_DELETE(YL_ZSRAIN)
  IF (ASSOCIATED(YL_ZCEROV)) CALL FIELD_DELETE(YL_ZCEROV)
  IF (ASSOCIATED(YL_ZCOEFN)) CALL FIELD_DELETE(YL_ZCOEFN)
  IF (ASSOCIATED(YL_ZALBDIR)) CALL FIELD_DELETE(YL_ZALBDIR)
  IF (ASSOCIATED(YL_ZCLCT_RAD)) CALL FIELD_DELETE(YL_ZCLCT_RAD)
  IF (ASSOCIATED(YL_ZWLMX)) CALL FIELD_DELETE(YL_ZWLMX)
  IF (ASSOCIATED(YL_ZHTR)) CALL FIELD_DELETE(YL_ZHTR)
  IF (ASSOCIATED(YL_ZDIFEXT)) CALL FIELD_DELETE(YL_ZDIFEXT)
  IF (LHOOK) CALL DR_HOOK('DELETE_TEMPORARIES', 1, ZHOOK_HANDLE_FIELD_API)
PRINT *,"apl_arpege_parallel_dbg ::: 669"
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE', 1, ZHOOK_HANDLE)
PRINT *,"apl_arpege_parallel_dbg ::: 670"
  
END SUBROUTINE APL_ARPEGE_PARALLEL
