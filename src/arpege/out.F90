...
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
  ...
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:CPPHINP')) THEN
    ...
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CPPHINP:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:CPPHINP:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, YDGEOMETRY, ZRDG_MU0LU, ZRDG_CVGT, ZRDG_CVGQ, ZRDG_SOLO,  &
!$ACC & ZRDG_MU0N, ZRDG_MU0M, Z_YDVARS_Q_T0, Z_YDVARS_GEOMETRY_GEMU_T0, Z_YDVARS_U_T0, Z_YDCPG_DYN0_CTY_EVEL, Z_YDVARS_Q_DL,  &
!$ACC & Z_YDVARS_GEOMETRY_GELAM_T0, Z_YDVARS_CVGQ_DL, Z_YDVARS_CVGQ_DM, Z_YDVARS_T_DM, Z_YDVARS_V_T0, Z_YDVARS_Q_DM,  &
!$ACC & Z_YDVARS_T_DL, Z_YDVARS_CVGQ_T0, Z_YDCPG_PHY0_XYB_RDELP ) PRIVATE( JBLK ) VECTOR_LENGTH( YDCPG_OPTS%KLON )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
...
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
  ...
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ZRDG_LCVQ')) THEN
    ...
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZRDG_LCVQ:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZRDG_LCVQ:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZRDG_LCVQ, ZFLU_QSATS, ZQCO2, ZRDG_CVGQ ) PRIVATE( JBLK )  &
...
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
  ...
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:APLPAR_INIT')) THEN
    ...
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:APLPAR_INIT:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:APLPAR_INIT:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZFLU_CDN, ZFLU_CH, ZFCHQ, ZDSA_C1, ZPFL_FEFB1, ZFLU_CHN,  &
!$ACC & ZDSA_RS, ZFLU_VEG, ZMSC_QW, ZMSC_LH, ZDSA_CPS, ZFLU_QSATS, ZPFL_FEFB2, ZFLU_EMIS, ZMSC_LSCPE, IMOC_CLPH, ZDSA_LHS,  &
!$ACC & ZPFL_FTKEI, ZMSC_TW, ZFLU_NEIJ, ZMSC_FRMQ, ZPFL_FEFB3, ZDSA_C2, ZFLU_FEVI, Z_YDMF_PHYS_SURF_GSD_VF_PVEG ) PRIVATE(  &
...
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
  ...
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ACTQSAT')) THEN
    ...
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACTQSAT:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACTQSAT:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZMSC_QW, ZMSC_LH, ZMSC_LSCPE, ZQV, ZFLU_QSAT, ZMSC_TW,  &
!$ACC & ZGEOSLC, Z_YDCPG_MISC_RH, Z_YDMF_PHYS_BASE_STATE_T, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP,  &
...
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK, LLHMT )
    ...
    ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ACSOL')) THEN
      ...
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACSOL:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACSOL:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZDSA_C1, ZWFC, ZNEIJV, ZCG, ZWSMX, ZWWILT, ZWSEQ, ZDSA_C2,  &
!$ACC & ZCN, ZWPMX, ZNEIJG, ZC3, Z_YDMF_PHYS_BASE_STATE_YGSP_SB_Q, Z_YDMF_PHYS_SURF_GSD_VV_PARG,  &
!$ACC & Z_YDMF_PHYS_SURF_GSD_VF_PLSM, Z_YDMF_PHYS_OUT_CT, Z_YDMF_PHYS_BASE_STATE_YGSP_RR_W, Z_YDMF_PHYS_SURF_GSD_VF_PVEG,  &
!$ACC & Z_YDMF_PHYS_SURF_GSD_VV_PSAB, Z_YDMF_PHYS_SURF_GSD_VV_PD2, Z_YDMF_PHYS_SURF_GSD_VF_PZ0F,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T, Z_YDMF_PHYS_BASE_STATE_YGSP_RR_IC, Z_YDMF_PHYS_BASE_STATE_YGSP_SB_TL,  &
!$ACC & Z_YDMF_PHYS_SURF_GSD_VV_PZ0H, Z_YDMF_PHYS_SURF_GSD_VV_PIVEG, Z_YDMF_PHYS_SURF_GSD_VF_PZ0RLF,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YGSP_SG_A, Z_YDMF_PHYS_BASE_STATE_YGSP_SG_F, Z_YDMF_PHYS_BASE_STATE_YGSP_SG_R,  &
!$ACC & Z_YDMF_PHYS_SURF_GSD_VV_PLAI ) PRIVATE( JBLK ) VECTOR_LENGTH( YDCPG_OPTS%KLON )
      DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
...
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
    ...
    ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ACHMTLS')) THEN
      ...
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMTLS:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMTLS:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZFLU_CDN, ZPRS, ZPCLS, ZDPHIT, ZNBVNO, ZMRIPP, ZDSA_CPS,  &
!$ACC & ZDSA_LHS, ZGWDCS, ZRTI, Z_YDMF_PHYS_BASE_STATE_U, Z_YDCPG_MISC_QS, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD, Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_T, Z_YDMF_PHYS_OUT_GZ0, Z_YDMF_PHYS_OUT_GZ0H, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_V, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R ) PRIVATE( JBLK ) VECTOR_LENGTH( YDCPG_OPTS%KLON )
      DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK )
        DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
          YLCPG_BNDS%KIDIA = JLON
...
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
    ...
    ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ACHMT')) THEN
      ...
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMT:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACHMT:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZFLU_CDN, ZXHROV, ZDQSTS, ZWFC, ZHU, ZNBVNO, ZCDROV, ZCHROV,  &
!$ACC & ZGWDCS, ZDPHIV, ZFLU_NEIJ, ZNEIJG, ZFLU_CH, ZPCLS, ZDPHIT, ZPFL_FPLSH, ZDSA_RS, ZFLU_VEG, ZNEIJV, ZDSA_CPS, ZXDROV,  &
!$ACC & ZQV, ZMRIPP, ZFLU_QSATS, ZRTI, ZPFL_FPLCH, ZSTAB, ZHQ, Z_YDMF_PHYS_OUT_TCLS, Z_YDMF_PHYS_OUT_VGST,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD, Z_YDMF_PHYS_SURF_GSD_VV_PHV, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_V, Z_YDMF_PHYS_OUT_UGST, Z_YDMF_PHYS_BASE_STATE_YGSP_SG_F, Z_YDMF_PHYS_OUT_NVCLS,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_U, Z_YDCPG_MISC_QS, Z_YDMF_PHYS_SURF_GSD_VF_PVEG, Z_YDMF_PHYS_OUT_GZ0H,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R, Z_YDMF_PHYS_OUT_UCLS, Z_YDMF_PHYS_SURF_GSD_VF_PZ0RLF,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T, Z_YDMF_PHYS_OUT_VCLS, Z_YDMF_PHYS_SURF_GSD_VF_PLSM,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YGSP_RR_W, Z_YDMF_PHYS_OUT_NUCLS, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI,  &
!$ACC & Z_YDMF_PHYS_SURF_GSD_VF_PZ0F, Z_YDMF_PHYS_BASE_STATE_T, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF,  &
!$ACC & Z_YDMF_PHYS_SURF_GSD_VV_PZ0H, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF, Z_YDMF_PHYS_OUT_QCLS,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YGSP_RR_IC, Z_YDMF_PHYS_OUT_RHCLS ) PRIVATE( JBLK ) VECTOR_LENGTH( YDCPG_OPTS%KLON )
      DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK )
        DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
          YLCPG_BNDS%KIDIA = JLON
...
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
  ...
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ACCLPH')) THEN
    ...
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACCLPH:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACCLPH:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZTHETAVS, ZVGST, IMOC_CLPH, ZQV, ZCP, ZBLH, ZUGST,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_U, Z_YDCPG_MISC_QS, Z_YDMF_PHYS_OUT_VEIN, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI, Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T, Z_YDMF_PHYS_BASE_STATE_V,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF, Z_YDMF_PHYS_BASE_STATE_T,  &
!$ACC & Z_YDMF_PHYS_OUT_CLPH, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF ) PRIVATE( JBLK ) VECTOR_LENGTH( YDCPG_OPTS%KLON )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK )
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
...
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
  ...
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:DIFTQ')) THEN
    ...
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:DIFTQ:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:DIFTQ:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZDIFCVPPV, ZEDMFS, ZEDMFV, ZDIFCVPPS, ZDIFCVPPU, ZDIFCVPPQ,  &
!$ACC & ZEDMFU, ZEDMFQ, Z_YDMF_PHYS_OUT_STRTV, Z_YDMF_PHYS_OUT_DIFTQ, Z_YDMF_PHYS_OUT_STRTU, Z_YDMF_PHYS_OUT_DIFTS )  &
...
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
  ...
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:FRSOPT')) THEN
    ...
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:FRSOPT:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:FRSOPT:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZFLU_EMIS, ZRDG_MU0, Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T,  &
!$ACC & Z_YDMF_PHYS_OUT_FRTH, Z_YDMF_PHYS_OUT_FRSOPT, Z_YDMF_PHYS_OUT_FRTHDS ) PRIVATE( JBLK ) VECTOR_LENGTH(  &
!$ACC & YDCPG_OPTS%KLON )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK )
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
...
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
  ...
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ACDRAG')) THEN
    ...
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRAG:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRAG:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZNEBS, ZQLIS, ZTRAJGWD, ZDQSTS, ZNBVNO, ZMSC_QW, ZCHROV,  &
!$ACC & ZFLU_EMIS, ZGWDCS, ZQV, ZFLU_NEIJ, Z_YDMF_PHYS_BASE_STATE_U, Z_YDCPG_MISC_QS, Z_YDMF_PHYS_OUT_FPEVPSL,  &
!$ACC & Z_YDMF_PHYS_SURF_GSD_VF_PLSM, Z_YDMF_PHYS_OUT_FPEVPSN, Z_YDMF_PHYS_SURF_GSD_VF_PVRLDI,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP, Z_YDMF_PHYS_OUT_FCSQL,  &
!$ACC & Z_YDMF_PHYS_OUT_DRNSHF, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP, Z_YDMF_PHYS_BASE_STATE_T, Z_YDMF_PHYS_OUT_FCSQN,  &
!$ACC & Z_YDMF_PHYS_OUT_FPFPSL, Z_YDMF_PHYS_OUT_FPFPSN, Z_YDMF_PHYS_OUT_STRDV, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_V, Z_YDVARS_GEOMETRY_RCORI_T0, Z_YDMF_PHYS_SURF_GSD_VF_PVRLAN, Z_YDMF_PHYS_OUT_STRDU,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R, Z_YDMF_PHYS_OUT_FPLSN, Z_YDMF_PHYS_SURF_GSD_VF_PGETRL,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T, Z_YDMF_PHYS_OUT_FPLSL ) PRIVATE( JBLK ) VECTOR_LENGTH( YDCPG_OPTS%KLON )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK )
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
...
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
    ...
    ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:TURBULENCE_PART2')) THEN
      ...
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:TURBULENCE_PART2:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:TURBULENCE_PART2:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZKUROV2, ZUSLE, ZPRODTH, ZECTCLS, ZKCLS, ZTKE1, ZPFL_FTKE,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_U, Z_YDMF_PHYS_OUT_EDR, Z_YDMF_PHYS_OUT_STRCU, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD,  &
!$ACC & Z_YDMF_PHYS_OUT_STRCV, Z_YDMF_PHYS_OUT_DIFCS, Z_YDMF_PHYS_BASE_STATE_TKE, Z_YDMF_PHYS_BASE_STATE_Q,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI, Z_YDMF_PHYS_OUT_DIFCQ, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF, Z_YDMF_PHYS_BASE_STATE_V,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R ) PRIVATE( JBLK ) VECTOR_LENGTH(  &
!$ACC & YDCPG_OPTS%KLON )
      DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK )
        DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
...
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
  ...
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:QNGCOR')) THEN
    ...
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:QNGCOR:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:QNGCOR:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZPFL_FPLCH, ZFPLSN, ZFLX_LOTT_GWU, ZQL, ZSEDIQL, ZFPCOR, ZQI,  &
!$ACC & ZFLX_LOTT_GWV, ZPFL_FPLSH, ZFPLSL, ZQV, ZSEDIQI, Z_YDMF_PHYS_OUT_FPLSN, Z_YDMF_PHYS_OUT_DIFCQN,  &
!$ACC & Z_YDMF_PHYS_OUT_FPEVPSL, Z_YDMF_PHYS_OUT_FPEVPSN, Z_YDMF_PHYS_OUT_DIFTQL, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_RDELP,  &
!$ACC & Z_YDMF_PHYS_OUT_FPEVPCN, Z_YDMF_PHYS_OUT_FCSQL, Z_YDMF_PHYS_OUT_FPFPCL, Z_YDMF_PHYS_OUT_FPFPSL,  &
!$ACC & Z_YDMF_PHYS_OUT_FCSQN, Z_YDMF_PHYS_OUT_FPFPSN, Z_YDMF_PHYS_OUT_FPEVPCL, Z_YDMF_PHYS_OUT_FPLSL,  &
!$ACC & Z_YDMF_PHYS_OUT_DIFTQN, Z_YDMF_PHYS_OUT_FPLSG, Z_YDMF_PHYS_OUT_FCCQN, Z_YDMF_PHYS_OUT_DIFCQL,  &
!$ACC & Z_YDMF_PHYS_OUT_FPFPCN, Z_YDMF_PHYS_OUT_FCCQL, Z_YDMF_PHYS_OUT_FCQNG ) PRIVATE( JBLK ) VECTOR_LENGTH(  &
!$ACC & YDCPG_OPTS%KLON )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK )
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
...
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
  ...
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ACDRME')) THEN
    ...
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRME:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACDRME:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, YDGEOMETRY, ZQV, ZMSC_FRMQ, Z_YDMF_PHYS_BASE_STATE_U,  &
!$ACC & Z_YDMF_PHYS_OUT_FRMH, Z_YDCPG_MISC_QLI, Z_YDCPG_MISC_NEB, Z_YDMF_PHYS_OUT_STRMV, Z_YDCPG_MISC_QICE,  &
!$ACC & Z_YDVARS_LRAD_T1, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP, Z_YDMF_PHYS_BASE_STATE_T,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP, Z_YDMF_PHYS_BASE_STATE_V, Z_YDMF_PHYS_OUT_STRMU, Z_YDVARS_A_T1,  &
!$ACC & Z_YDVARS_IRAD_T1 ) PRIVATE( JBLK ) VECTOR_LENGTH( YDCPG_OPTS%KLON )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK )
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
...
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
    ...
    ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ACEVADCAPE')) THEN
      ...
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACEVADCAPE:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACEVADCAPE:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZDCAPE, ZBLH, IMOC_CLPH, ZTKE1, Z_YDMF_PHYS_BASE_STATE_U,  &
!$ACC & Z_YDMF_PHYS_OUT_VCLS, Z_YDMF_PHYS_OUT_VGST, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD, Z_YDMF_PHYS_OUT_CAPE,  &
!$ACC & Z_YDVARS_GEOMETRY_OROG_T0, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI, Z_YDMF_PHYS_BASE_STATE_T, Z_YDMF_PHYS_OUT_FPLCL,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_CP, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF, Z_YDMF_PHYS_BASE_STATE_V,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF, Z_YDMF_PHYS_OUT_UGST, Z_YDMF_PHYS_OUT_FPLCN, Z_YDMF_PHYS_OUT_UCLS,  &
!$ACC & Z_YDMF_PHYS_OUT_FPLSN, Z_YDMF_PHYS_OUT_CLPH, Z_YDMF_PHYS_OUT_FPLSL ) PRIVATE( JBLK ) VECTOR_LENGTH( YDCPG_OPTS%KLON )
      DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK )
        DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
          YLCPG_BNDS%KIDIA = JLON
...
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
  ...
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ACVISIH')) THEN
    ...
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACVISIH:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ACVISIH:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZQS, ZQGM, ZQR, Z_YDMF_PHYS_OUT_VISIHYD,  &
!$ACC & Z_YDMF_PHYS_OUT_MXCLWC, Z_YDCPG_MISC_QICE, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI, Z_YDMF_PHYS_BASE_STATE_T,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R, Z_YDCPG_MISC_QLI, Z_YDMF_PHYS_OUT_VISICLD ) PRIVATE( JBLK ) VECTOR_LENGTH(  &
!$ACC & YDCPG_OPTS%KLON )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK )
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
...
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
  ...
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ZBAY_QRCONV')) THEN
    ...
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZBAY_QRCONV:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZBAY_QRCONV:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZBAY_QRCONV, ZBAY_QSCONV, Z_YDMF_PHYS_OUT_FPLCL,  &
...
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
  ...
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:ZDE2MR')) THEN
    ...
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDE2MR:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:ZDE2MR:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZBAY_QRCONV, ZDE2MR, ZBAY_QSCONV, Z_YDMF_PHYS_BASE_STATE_T,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF ) PRIVATE( JBLK ) VECTOR_LENGTH( YDCPG_OPTS%KLON )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK )
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
...
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
  ...
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:PPWETPOINT')) THEN
    ...
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PPWETPOINT:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PPWETPOINT:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZPCLS, Z_YDMF_PHYS_BASE_STATE_L, Z_YDMF_PHYS_OUT_TCLS,  &
!$ACC & Z_YDMF_PHYS_BASE_STATE_I, Z_YDMF_PHYS_OUT_TPWCLS, Z_YDMF_PHYS_BASE_STATE_Q ) PRIVATE( JBLK ) VECTOR_LENGTH(  &
!$ACC & YDCPG_OPTS%KLON )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK )
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
...
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK )
  ...
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_PARALLEL:PRECIPS')) THEN
    ...
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PRECIPS:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API)
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_PARALLEL:PRECIPS:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YDMODEL, YSTACK, ZPRC_DPRECIPS2, Z_YDMF_PHYS_SURF_GSD_XP_PPRECIP,  &
!$ACC & Z_YDMF_PHYS_SURF_GSD_XP2_PPRECIP2 ) PRIVATE( JBLK ) VECTOR_LENGTH( YDCPG_OPTS%KLON )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK )
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        YLCPG_BNDS%KIDIA = JLON
