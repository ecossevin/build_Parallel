SUBROUTINE APL_ARPEGE_SHALLOW_CONVECTION_AND_TURBULENCE_PARALLEL (YDMF_PHYS_BASE_STATE, YDCPG_BNDS, YDCPG_OPTS, YDCPG_MISC,  &
& YDMF_PHYS, YDMF_PHYS_SURF, YDMODEL, YDDDH, YD_KNLAB_CVPP, YD_PCDROV, YD_PCHROV, YD_PCOEFN, YD_PCONDCVPPI, YD_PCONDCVPPL,  &
& YD_PDIFCVPPQ, YD_PDIFCVPPS, PEPS, YD_PFLU_CD, YD_PFLU_CH, YD_PKQLROV, YD_PKQROV, YD_PKTROV, YD_PKUROV, YD_PKUROV2,  &
& YD_PMSC_LSCPE, YD_PNBVNO, YD_PNEBS, YD_PNEBS0, YD_PNEB_CVPP, YD_PPFL_FPLCH, YD_PPFL_FTKE, YD_PPFL_FTKEI, YD_PPRODTH_CVPP,  &
& YD_PQI, YD_PQIC, YD_PQL, YD_PQLC, YD_PQLIS, YD_PQLIS0, YD_PQLI_CVPP, YD_PQV, YD_PQR, YD_PQS, YD_PTKE1, YD_PXTROV,  &
& YD_PXUROV, YD_PUSLE, YD_PKCLS, YD_PECTCLS, YD_PPRODTH, YD_PEDMFQ, YD_PEDMFS, YD_PEDMFU, YD_PEDMFV, YD_PMF_UP)
...
#include "arp_shallow_mf_parallel.intfb.h"
  ...
  CLASS(FIELD_3RB), POINTER :: YL_ZNLAB => NULL()
  REAL(KIND=JPRB), POINTER :: ZNLAB(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZNLABCVP => NULL()
  REAL(KIND=JPRB), POINTER :: ZNLABCVP(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZTPRDY => NULL()
  REAL(KIND=JPRB), POINTER :: ZTPRDY(:, :, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZCDROV_SAVE => NULL()
  REAL(KIND=JPRB), POINTER :: ZCDROV_SAVE(:, :) => NULL()
  CLASS(FIELD_2RB), POINTER :: YL_ZCHROV_SAVE => NULL()
  REAL(KIND=JPRB), POINTER :: ZCHROV_SAVE(:, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZKTROV_SAVE => NULL()
  REAL(KIND=JPRB), POINTER :: ZKTROV_SAVE(:, :, :) => NULL()
  CLASS(FIELD_3RB), POINTER :: YL_ZKUROV_SAVE => NULL()
  REAL(KIND=JPRB), POINTER :: ZKUROV_SAVE(:, :, :) => NULL()
  CLASS(FIELD_2IM), POINTER :: YL_INND => NULL()
  INTEGER(KIND=JPIM), POINTER :: INND(:, :) => NULL()
  ...
  CALL FIELD_NEW(YL_INND, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZCDROV_SAVE, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZCHROV_SAVE, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%JBLKMAX /), LBOUNDS=(/ 1, YDCPG_OPTS%JBLKMIN /),  &
  & PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZKTROV_SAVE, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZKUROV_SAVE, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 0, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZNLAB, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZNLABCVP, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  CALL FIELD_NEW(YL_ZTPRDY, UBOUNDS=(/ YDCPG_OPTS%KLON, YDCPG_OPTS%KFLEVG, YDCPG_OPTS%JBLKMAX /),  &
  & LBOUNDS=(/ 1, 1, YDCPG_OPTS%JBLKMIN /), PERSISTENT=.true.)
  IF (LHOOK) CALL DR_HOOK('CREATE_TEMPORARIES', 1, ZHOOK_HANDLE_FIELD_API)
  
  
  IF (YDMODEL%YRML_PHY_MF%YRPHY%LCVPPKF) THEN
    CALL ACVPPKF_PARALLEL(YDMODEL%YRCST, YDMODEL%YRML_PHY_MF, YDCPG_BNDS, YDCPG_OPTS, YDMODEL%YRML_PHY_MF%YRTOPH%NTCVIM,  &
    & YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF, YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF,  &
    & YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_DELP, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_R, YDMF_PHYS_BASE_STATE%F_T, YD_PQV,  &
    & YD_PQL, YD_PQI, YDMF_PHYS_BASE_STATE%F_U, YDMF_PHYS_BASE_STATE%F_V, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP,  &
    & YDMF_PHYS_BASE_STATE%F_TKE, YD_PDIFCVPPQ, YD_PDIFCVPPS, YD_PCONDCVPPL, YD_PCONDCVPPI, YD_PPRODTH_CVPP, YD_KNLAB_CVPP,  &
    & YD_PQLI_CVPP, YD_PNEB_CVPP, YL_INND)
  END IF
  ...
  IF (YDMODEL%YRML_PHY_MF%YRPHY%LEDKF) THEN
    ...
    & YDMODEL%YRML_PHY_MF%YRPHY2%TSPHY, YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHI, YDMF_PHYS_BASE_STATE%YCPG_DYN%F_PHIF,  &
    & YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_R, YDMF_PHYS_BASE_STATE%YCPG_DYN%RCP%F_CP, YDMF_PHYS_BASE_STATE%F_U,  &
    & YDMF_PHYS_BASE_STATE%F_V, YDMF_PHYS_BASE_STATE%F_T, YD_PQV, YD_PQL, YD_PQI, YD_PQR, YD_PQS,  &
    & YDMF_PHYS_BASE_STATE%F_TKE, YDMF_PHYS_BASE_STATE%YCPG_PHY%F_PREHYDF, YDMF_PHYS_BASE_STATE%YCPG_PHY%XYB%F_DELP,  &
    & YD_PEDMFQ, YD_PEDMFS, YD_PEDMFU, YD_PEDMFV, YDMF_PHYS_SURF, YD_PPRODTH_CVPP, YD_PQLI_CVPP, YD_PNEB_CVPP,  &
    & YD_KNLAB_CVPP, YD_PMF_UP)
    
  END IF
  
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_SHALLOW_CONVECTION_AND_TURBULENCE_PARALLEL:ACTKE', 0, ZHOOK_HANDLE_PARALLEL)
  IF (LPARALLELMETHOD('OPENMP', 'APL_ARPEGE_SHALLOW_CONVECTION_AND_TURBULENCE_PARALLEL:ACTKE')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_SHALLOW_CONVECTION_AND_TURBULENCE_PARALLEL:ACTKE:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API &
    & )
    ...
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_SHALLOW_CONVECTION_AND_TURBULENCE_PARALLEL:ACTKE:GET_DATA', 1, ZHOOK_HANDLE_FIELD_API &
    & )
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_SHALLOW_CONVECTION_AND_TURBULENCE_PARALLEL:ACTKE:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
...
!$OMP PARALLEL DO PRIVATE( JBLK, JLEV, JLON ) FIRSTPRIVATE( YLCPG_BNDS )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      ...
      IF (YDMODEL%YRML_PHY_MF%YRPHY%LNEBN .or. YDMODEL%YRML_PHY_MF%YRPHY%LRRGUST) THEN
        DO JLEV=YDCPG_OPTS%KTDIA,YDCPG_OPTS%KFLEVG
          DO JLON=YLCPG_BNDS%KIDIA,YLCPG_BNDS%KFDIA
            ZNLABCVP(JLON, JLEV, JBLK) = ZNLABCVP(JLON, JLEV, JBLK)*MAX(0.0_JPRB, SIGN(1.0_JPRB, PPFL_FPLCH(JLON, JLEV, JBLK) &
            &  - PPFL_FPLCH(JLON, JLEV - 1, JBLK) - PEPS))
            ZNLAB(JLON, JLEV, JBLK) = REAL(KNLAB_CVPP(JLON, JLEV, JBLK), kind=JPRB)
          END DO
        END DO
      END IF
      DO JLEV=YDCPG_OPTS%KTDIA,YDCPG_OPTS%KFLEVG
      ...
      IF (YDMODEL%YRML_PHY_MF%YRPHY%LECTPRODCVP) THEN
        ...
        & YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDMODEL%YRML_PHY_MF%YRTOPH%NTCOEF,  &
        & YDMODEL%YRML_PHY_MF%YRTOPH%NTCOET, YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI(:, :, JBLK),  &
        ...
        & Z_YDMF_PHYS_BASE_STATE_U(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_V(:, :, JBLK), PQV(:, :, JBLK),  &
        & PMSC_LSCPE(:, :, JBLK), PFLU_CD(:, JBLK), PFLU_CH(:, JBLK), Z_YDMF_PHYS_OUT_GZ0(:, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T(:, JBLK), Z_YDCPG_MISC_QS(:, JBLK), Z_YDCPG_MISC_QICE(:, :, JBLK),  &
        & Z_YDCPG_MISC_QLI(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_TKE(:, :, JBLK), PPRODTH_CVPP(:, :, JBLK), ZNLAB(:, :, JBLK),  &
        & ZNLABCVP(:, :, JBLK), PKTROV(:, :, JBLK), PKQROV(:, :, JBLK), PKQLROV(:, :, JBLK), PKUROV(:, :, JBLK),  &
        & PXTROV(:, :, JBLK), PXUROV(:, :, JBLK), PNBVNO(:, :, JBLK), PNEBS(:, :, JBLK), PQLIS(:, :, JBLK),  &
        & PNEBS0(:, :, JBLK), PQLIS0(:, :, JBLK), PCOEFN(:, :, JBLK), PUSLE(:, :, JBLK), PKCLS(:, JBLK), PECTCLS(:, JBLK),  &
        & PPRODTH(:, :, JBLK))
        PKUROV2(:, :, JBLK) = PKUROV(:, :, JBLK)
      ELSE
        CALL ACTKE(YDMODEL%YRCST, YDMODEL%YRML_DIAG%YRLDDH, YDMODEL%YRML_DIAG%YRMDDH, YDMODEL%YRML_PHY_MF, YLCPG_BNDS%KIDIA,  &
        & YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDMODEL%YRML_PHY_MF%YRTOPH%NTCOEF, YDMODEL%YRML_PHY_MF%YRTOPH%NTCOET,  &
        & YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_RCP_R(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_T(:, :, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_U(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_V(:, :, JBLK), PQV(:, :, JBLK), PQIC(:, :, JBLK),  &
        & PQLC(:, :, JBLK), PMSC_LSCPE(:, :, JBLK), PFLU_CD(:, JBLK), PFLU_CH(:, JBLK), Z_YDMF_PHYS_OUT_GZ0(:, JBLK),  &
        & Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T(:, JBLK), Z_YDCPG_MISC_QS(:, JBLK), Z_YDCPG_MISC_QICE(:, :, JBLK),  &
        & Z_YDCPG_MISC_QLI(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_TKE(:, :, JBLK), PPRODTH_CVPP(:, :, JBLK), ZNLAB(:, :, JBLK),  &
        & ZNLABCVP(:, :, JBLK), PKTROV(:, :, JBLK), PKQROV(:, :, JBLK), PKQLROV(:, :, JBLK), PKUROV(:, :, JBLK),  &
        & PXTROV(:, :, JBLK), PXUROV(:, :, JBLK), PNBVNO(:, :, JBLK), PNEBS(:, :, JBLK), PQLIS(:, :, JBLK),  &
        & PNEBS0(:, :, JBLK), PQLIS0(:, :, JBLK), PCOEFN(:, :, JBLK), PPFL_FTKE(:, :, JBLK), PPFL_FTKEI(:, :, JBLK),  &
        & PTKE1(:, :, JBLK), ZTPRDY(:, :, JBLK), Z_YDMF_PHYS_OUT_EDR(:, :, JBLK), YDDDH)
      END IF
      DO JLON=YLCPG_BNDS%KIDIA,YLCPG_BNDS%KFDIA
        Z_YDMF_PHYS_OUT_CLPH(JLON, JBLK) =  &
        & MIN(YDMODEL%YRML_PHY_MF%YRPHY0%XMAXLM, MAX(YDMODEL%YRML_PHY_MF%YRPHY0%XMINLM, Z_YDMF_PHYS_OUT_CLPH(JLON, JBLK)))
      END DO
    ...
    END DO
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_SHALLOW_CONVECTION_AND_TURBULENCE_PARALLEL:ACTKE:COMPUTE', 1, ZHOOK_HANDLE_COMPUTE)
    IF (LSYNCHOST('APL_ARPEGE_SHALLOW_CONVECTION_AND_TURBULENCE_PARALLEL:ACTKE')) THEN
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_SHALLOW_CONVECTION_AND_TURBULENCE_PARALLEL:ACTKE:SYNCHOST', 0,  &
      & ZHOOK_HANDLE_FIELD_API)
      KNLAB_CVPP => GET_HOST_DATA_RDWR(YD_KNLAB_CVPP)
      PCDROV => GET_HOST_DATA_RDWR(YD_PCDROV)
      PCHROV => GET_HOST_DATA_RDWR(YD_PCHROV)
      PCOEFN => GET_HOST_DATA_RDWR(YD_PCOEFN)
      ...
      Z_YDMF_PHYS_OUT_EDR => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_EDR)
      Z_YDMF_PHYS_OUT_GZ0 => GET_HOST_DATA_RDWR(YDMF_PHYS%OUT%F_GZ0)
      IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_SHALLOW_CONVECTION_AND_TURBULENCE_PARALLEL:ACTKE:SYNCHOST', 1,  &
      & ZHOOK_HANDLE_FIELD_API)
    END IF
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_SHALLOW_CONVECTION_AND_TURBULENCE_PARALLEL:ACTKE:NULLIFY', 0, ZHOOK_HANDLE_FIELD_API)
    KNLAB_CVPP => NULL()
    PCDROV => NULL()
    PCHROV => NULL()
    PCOEFN => NULL()
    ...
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_SHALLOW_CONVECTION_AND_TURBULENCE_PARALLEL:ACTKE:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
  ELSE IF (LPARALLELMETHOD('OPENMPSINGLECOLUMN', 'APL_ARPEGE_SHALLOW_CONVECTION_AND_TURBULENCE_PARALLEL:ACTKE')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_SHALLOW_CONVECTION_AND_TURBULENCE_PARALLEL:ACTKE:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API &
    & )
    KNLAB_CVPP => GET_HOST_DATA_RDWR(YD_KNLAB_CVPP)
    PCDROV => GET_HOST_DATA_RDWR(YD_PCDROV)
...
!$OMP PARALLEL DO PRIVATE( JBLK, JLON, YLCPG_BNDS, YLSTACK, JLEV )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
      DO JLON=1,MIN(YDCPG_OPTS%KLON, YDCPG_OPTS%KGPCOMP - (JBLK - 1)*YDCPG_OPTS%KLON)
        ...
        IF (YDMODEL%YRML_PHY_MF%YRPHY%LECTPRODCVP) THEN
          CALL ACTKE1_OPENACC(YDMODEL%YRCST, YDMODEL%YRML_DIAG%YRLDDH, YDMODEL%YRML_DIAG%YRMDDH, YDMODEL%YRML_PHY_MF,  &
          & YLCPG_BNDS%KIDIA, YLCPG_BNDS%KFDIA, YDCPG_OPTS%KLON, YDMODEL%YRML_PHY_MF%YRTOPH%NTCOEF,  &
          & YDMODEL%YRML_PHY_MF%YRTOPH%NTCOET, YDCPG_OPTS%KFLEVG, Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHI(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_DYN_PHIF(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYD(:, :, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP(:, :, JBLK),  &
          ...
          & PMSC_LSCPE(:, :, JBLK), PFLU_CD(:, JBLK), PFLU_CH(:, JBLK), Z_YDMF_PHYS_OUT_GZ0(:, JBLK),  &
          & Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T(:, JBLK), Z_YDCPG_MISC_QS(:, JBLK), Z_YDCPG_MISC_QICE(:, :, JBLK),  &
          & Z_YDCPG_MISC_QLI(:, :, JBLK), Z_YDMF_PHYS_BASE_STATE_TKE(:, :, JBLK), PPRODTH_CVPP(:, :, JBLK),  &
          & ZNLAB(:, :, JBLK), ZNLABCVP(:, :, JBLK), PKTROV(:, :, JBLK), PKQROV(:, :, JBLK), PKQLROV(:, :, JBLK),  &
          & PKUROV(:, :, JBLK), PXTROV(:, :, JBLK), PXUROV(:, :, JBLK), PNBVNO(:, :, JBLK), PNEBS(:, :, JBLK),  &
          & PQLIS(:, :, JBLK), PNEBS0(:, :, JBLK), PQLIS0(:, :, JBLK), PCOEFN(:, :, JBLK), PUSLE(:, :, JBLK),  &
          & PKCLS(:, JBLK), PECTCLS(:, JBLK), PPRODTH(:, :, JBLK), YDSTACK=YLSTACK)
        ...
        ELSE
          ...
          & ZNLAB(:, :, JBLK), ZNLABCVP(:, :, JBLK), PKTROV(:, :, JBLK), PKQROV(:, :, JBLK), PKQLROV(:, :, JBLK),  &
          & PKUROV(:, :, JBLK), PXTROV(:, :, JBLK), PXUROV(:, :, JBLK), PNBVNO(:, :, JBLK), PNEBS(:, :, JBLK),  &
          & PQLIS(:, :, JBLK), PNEBS0(:, :, JBLK), PQLIS0(:, :, JBLK), PCOEFN(:, :, JBLK), PPFL_FTKE(:, :, JBLK),  &
          & PPFL_FTKEI(:, :, JBLK), PTKE1(:, :, JBLK), ZTPRDY(:, :, JBLK), Z_YDMF_PHYS_OUT_EDR(:, :, JBLK), YDDDH,  &
          & YDSTACK=YLSTACK)
        END IF
        Z_YDMF_PHYS_OUT_CLPH(JLON, JBLK) =  &
        & MIN(YDMODEL%YRML_PHY_MF%YRPHY0%XMAXLM, MAX(YDMODEL%YRML_PHY_MF%YRPHY0%XMINLM, Z_YDMF_PHYS_OUT_CLPH(JLON, JBLK)))
    ...
    Z_YDMF_PHYS_OUT_GZ0 => NULL()
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_SHALLOW_CONVECTION_AND_TURBULENCE_PARALLEL:ACTKE:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
  ELSE IF (LPARALLELMETHOD('OPENACCSINGLECOLUMN', 'APL_ARPEGE_SHALLOW_CONVECTION_AND_TURBULENCE_PARALLEL:ACTKE')) THEN
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_SHALLOW_CONVECTION_AND_TURBULENCE_PARALLEL:ACTKE:GET_DATA', 0, ZHOOK_HANDLE_FIELD_API &
    & )
    ...
    PMSC_LSCPE => GET_DEVICE_DATA_RDWR(YD_PMSC_LSCPE)
    ...
    & )
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_SHALLOW_CONVECTION_AND_TURBULENCE_PARALLEL:ACTKE:COMPUTE', 0, ZHOOK_HANDLE_COMPUTE)
!$ACC PARALLEL LOOP GANG PRESENT( YDCPG_OPTS, YSTACK,YDDDH, YDMODEL, KNLAB_CVPP, PCDROV, PCHROV, PCOEFN, PECTCLS, PFLU_CD,  &
!$ACC & PFLU_CH, PKCLS, PKQLROV, PKQROV, PKTROV, PKUROV, PKUROV2, PMSC_LSCPE, PNBVNO, PNEBS, PNEBS0, PPFL_FPLCH, PPFL_FTKE,  &
!$ACC & PPFL_FTKEI, PPRODTH, PPRODTH_CVPP, PQI, PQIC, PQL, PQLC, PQLIS, PQLIS0, PQV, PTKE1, PUSLE, PXTROV, PXUROV,  &
!$ACC & ZCDROV_SAVE, ZCHROV_SAVE, ZKTROV_SAVE, ZKUROV_SAVE, ZNLAB, ZNLABCVP, ZTPRDY, Z_YDCPG_MISC_QICE, Z_YDCPG_MISC_QLI,  &
!$ACC & Z_YDCPG_MISC_QS, Z_YDMF_PHYS_BASE_STATE_T, Z_YDMF_PHYS_BASE_STATE_TKE, Z_YDMF_PHYS_BASE_STATE_U,  &
...
!$ACC & Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_PREHYDF, Z_YDMF_PHYS_BASE_STATE_YCPG_PHY_XYB_DELP, Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T,  &
!$ACC & Z_YDMF_PHYS_OUT_CLPH, Z_YDMF_PHYS_OUT_EDR, Z_YDMF_PHYS_OUT_GZ0 ) PRIVATE( JBLK ) VECTOR_LENGTH( YDCPG_OPTS%KLON )
    DO JBLK=YDCPG_OPTS%JBLKMIN,YDCPG_OPTS%JBLKMAX
!$ACC LOOP VECTOR PRIVATE( JLON, YLCPG_BNDS, YLSTACK, JLEV )
    ...
    IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_SHALLOW_CONVECTION_AND_TURBULENCE_PARALLEL:ACTKE:NULLIFY', 1, ZHOOK_HANDLE_FIELD_API)
  END IF
  IF (LHOOK) CALL DR_HOOK('APL_ARPEGE_SHALLOW_CONVECTION_AND_TURBULENCE_PARALLEL:ACTKE', 1, ZHOOK_HANDLE_PARALLEL)
