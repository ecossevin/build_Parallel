    ...
    DO JBLK=1,YDCPG_OPTS%KGPBLKS
      ...
      DO JLEV=1,YDCPG_OPTS%KFLEVG
        DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
          YL_ZRDG_LCVQ(JLON, JLEV, JBLK) = YL_ZRDG_CVGQ(JLON, JLEV, JBLK)
        END DO
      END DO
      
      DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
        YL_ZFLU_QSATS(JLON, JBLK) = 0.0_JPRB
      END DO
    END DO
...
!$OMP PARALLEL DO PRIVATE( JBLK ) FIRSTPRIVATE( YLCPG_BNDS )
    DO JBLK=1,YDCPG_OPTS%KGPBLKS
      ...
      DO JLEV=YDCPG_OPTS%KTDIA,YDCPG_OPTS%KFLEVG
        DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
          Z_YDMF_PHYS_OUT_DIFTQ(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_DIFTQ(JLON, JLEV, JBLK) + YL_ZDIFCVPPQ(JLON, JLEV, JBLK)
          Z_YDMF_PHYS_OUT_DIFTS(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_DIFTS(JLON, JLEV, JBLK) + YL_ZDIFCVPPS(JLON, JLEV, JBLK)
          Z_YDMF_PHYS_OUT_STRTU(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_STRTU(JLON, JLEV, JBLK) + YL_ZDIFCVPPU(JLON, JLEV, JBLK)
          Z_YDMF_PHYS_OUT_STRTV(JLON, JLEV, JBLK) = Z_YDMF_PHYS_OUT_STRTV(JLON, JLEV, JBLK) + YL_ZDIFCVPPV(JLON, JLEV, JBLK)
        END DO
      END DO
    END DO
...
!$ACC PARALLEL LOOP GANG
    ...
    !$ACC&PRESENT(ZDIFCVPPQ,ZDIFCVPPV,Z_YDMF_PHYS_OUT_DIFTQ,Z_YDMF_PHYS_OUT_DIFTS,ZDIFCVPPU,Z_YDMF_PHYS_OUT_STRTV,Z_YDMF_PHYS_OUT_STRTU,ZDIFCVPPS) &
...
!$OMP PARALLEL DO PRIVATE( JBLK ) FIRSTPRIVATE( YLCPG_BNDS )
    DO JBLK=1,YDCPG_OPTS%KGPBLKS
      ...
      !DEC$ IVDEP
      DO JLON=YDCPG_BNDS%KIDIA,YDCPG_BNDS%KFDIA
        IF (.not.YDMODEL%YRML_PHY_MF%YRARPHY%LMSE) Z_YDMF_PHYS_OUT_FRTHDS(JLON, JBLK) = Z_YDMF_PHYS_OUT_FRTH(JLON, YDCPG_OPTS%KFLEVG, 1, JBLK) / YL_ZFLU_EMIS(JLON, JBLK) +  &
        & YDMODEL%YRCST%RSIGMA*Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T(JLON, JBLK)**4
        IF (YL_ZRDG_MU0(JLON, JBLK) <= 0.0_JPRB) THEN
          Z_YDMF_PHYS_OUT_FRSOPT(JLON, JBLK) = 0.0_JPRB
        ELSE
          Z_YDMF_PHYS_OUT_FRSOPT(JLON, JBLK) = YDMODEL%YRML_PHY_MF%YRPHY3%RII0*YL_ZRDG_MU0(JLON, JBLK)
        END IF
      END DO
    END DO
...
!$ACC PARALLEL LOOP GANG
    ...
    !$ACC&PRESENT(Z_YDMF_PHYS_OUT_FRTHDS,Z_YDMF_PHYS_OUT_FRSOPT,ZFLU_EMIS,Z_YDMF_PHYS_BASE_STATE_YGSP_RR_T,Z_YDMF_PHYS_OUT_FRTH,ZRDG_MU0) &
