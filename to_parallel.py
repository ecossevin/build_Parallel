...
def change_arrays(routine, dcls, lst_horizontal_size, map_dim):
    ...
    for dcl in dcls:
        ...
        if var_routine.type.kind:
            ...
            str_node2 = (
                f"{var_routine.type.dtype.name}, POINTER :: {var_routine.name} ({dd})"
            )
...
def generate_parallelregion(
    ...
    if parallelmethod == "OPENACCSINGLECOLUMN":
        str_data = generate_get_data(
            region_arrays, "DEVICE", "GET_DATA", subname, name, map_region_intent
        )
    else:
        str_data = generate_get_data(
            region_arrays, "HOST", "GET_DATA", subname, name, map_region_intent
        )
    ...
    if parallelmethod == "OPENMP":
        str_compute = str_compute + generate_compute_openmp(
            calls, region, args, map_dim, region_scalar, region_arrays, lst_derive_type
        )
    elif parallelmethod == "OPENMPSINGLECOLUMN" or "OPENACCSINGLECOLUMN":
        str_compute = str_compute + generate_compute_scc(
            calls,
            region,
            args,
            map_dim,
    ...
    strsynchost = generate_get_data(
        region_arrays, "HOST", "SYNCHOST", subname, name, map_region_intent
    )
...
def generate_variables(args, str_body_, map_dim):
    ...
    for arg in args:
        if not (
            isinstance(arg, symbols.LogicalOr) or isinstance(arg, symbols.LogicalAnd)
        ):
            ...
            if key == "array" or key == "derived":
                ...
                new_arg_name = re.match(
                    "[a-zA-Z0-9_%]+", new_arg
                )  # can use region_arrays to have directly name without dimensions...
                iter_match = find_arg(
                    str_body_, arg.name, regex
                )  # find all the places where the var appears
                ...
                if not iter_match:
                    ...
                    for match in reversed(list(iter_match)):
                        ...
                        if match.group(
                            2
                        ):  # if array already has some shapes: keep them and add JBLK at the end.
                            new_arg_dim = match.group(2)[:-1] + ",JBLK)"
                            new_arg = new_arg_name.group(0) + new_arg_dim
                            if verbose:
                            ...
                            #                            new_arg=map_dim[1][arg.name].replace(":)","JBLK)")
                            new_arg = map_variables[key][arg.name]["array_dim"].replace(
                                ":)", "JBLK)"
                            )
                            #   str_body_=re.sub(regex, new_arg, str_body_)
                            str_body_ = str_body_[:begin] + new_arg + str_body_[end:]
...
def generate_compute_scc(
    ...
    def generate_pragma_openaccscc(region_arrays, region_scalar):
        ...
        present = "!$ACC&PRESENT("
        count_present = (
            0  # having to long lines, bug with !$acc as comments and lines to long
        )
    ...
    str_region = generate_variables(
        args, str_region, map_dim
    )  # change variables (YL->Z_YL; +dim + JBLK)
...
def generate_call(calls, str_region):
    ...
    for call in calls:
        ...
        str_region = re.sub(
            regex, str_call, str_region, flags=re.MULTILINE
        )  # insert in str_region, at the place of the call
        return str_region
        ...
        if not (
            ...
            if "%" in arg_name:
                ...
                if var_routine.type.dtype.name == "MF_PHYS_SURF_TYPE":  # remove P
                    arg_name__ = (
                        "%".join(arg_name.split("%")[:-1])
                        + "%F_"
                        + arg_name.split("%")[-1:][0][1:]
                    )  # remove extra P
                else:
                    arg_name__ = (
                        "%".join(arg_name.split("%")[:-1])
                ...
                # ==========================================================
                    ...
                    new_var_name = (
                        "Z_" + "_".join(arg_name.split("%")[:-1]) + "_" + d
                    )  # A%B%C => A%B%C(:,:,:)
                    ...
                    new_var = irgrip.slurp_any_code(
                        f"{new_var_type}, POINTER :: {new_var_name}"
                    )
                ...
                # ==========================================================
                    ...
                    if verbose:
                        to_print = (
                            "Argument = "
                            + arg_name
                            + "not in field api index+ neither CPG_OPTS_TYPE!!!"
                        )
                        bprint(to_print)
            # ==========================================================
            # ==========================================================
            ...
            # ==========================================================
                ...
                if arg.name in dcls:
                    arg_name = arg.name
                    region_arrays[arg_name] = [
                        "YL_" + arg_name,
                        arg.name,
                    ]  # region_arrays[Z_A]=YL_ZA
                    if verbose:
...
def generate_get_data(region_arrays, machine, area, subname, name, map_region_intent):
    ...
    for var in region_arrays:  # lhs => get_data(rhs) : region_arrays[lhs]=rhs
        lhs = var
        if verbose:
            print("region_arrays[lhs]  = ", region_arrays[lhs])
        if intent:
            if region_arrays[lhs][1] in map_region_intent:
                access_type = map_intent_access[
                    map_region_intent[region_arrays[lhs][1]]
                ]
            ...
            datacode = (
                f"{lhs} => GET_{machine}_DATA_{access_type} ({region_arrays[lhs][0]})\n"
            )
    ...
    for decl in FindNodes(VariableDeclaration).visit(routine.spec):
        for s in decl.symbols:
            if s.name == "ZHOOK_HANDLE":
                ...
                routine.spec = irgrip.insert_after_node(
                    decl, node, rootnode=routine.spec
                )
                stop = True
            break
        if stop:
...
def parallel_trans(pathpack, pathview, pathfile, horizontal_opt, inlined):
    ...
    change_arrays(
        routine, dcls, lst_horizontal_size, map_dim
    )  # creation of field api objetcs associated with each array
    ...
    for i in range(len(regions)):
        ...
        for target in targets:
            ...
            if able:
                str_code = generate_parallelregion(
                    region,
                    calls,
                    map_dim,
                    region_arrays,
